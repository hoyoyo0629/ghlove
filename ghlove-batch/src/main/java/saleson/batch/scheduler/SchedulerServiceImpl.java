...
import saleson.batch.job.JobPsintService;
import saleson.batch.job.JobService;
import saleson.batch.support.BatchJobParam;


public class SchedulerServiceImpl extends EgovAbstractServiceImpl implements SchedulerService, InitializingBean {
	...
	@Autowired
	Scheduler schedulerPsint;

	@Autowired
	JobService jobService;

	@Autowired
	JobPsintService jobPsintService;
	...
	public void invokeJobChecker() {
		...
		try {
			...
			for (BatchJob batchJob : jobs) {
				...
				//statisticsYearReportPsintBatch:거주지, statisticsYearReportGoodsBatch:답례품순위
				if("statisticsYearReportPsintBatch".equals(batchJob.getJobMethod()) || "statisticsYearReportGoodsBatch".equals(batchJob.getJobMethod())) {
					MethodInvokingJobDetailFactoryBean jobPsintDetail = null;
					try {
						// Job Information
						jobPsintDetail = new MethodInvokingJobDetailFactoryBean();
						jobPsintDetail.setTargetObject(jobPsintService);
						jobPsintDetail.setTargetMethod(batchJob.getJobMethod());
						jobPsintDetail.setName(batchJob.getBatchJobId());
						jobPsintDetail.setConcurrent(false);
						jobPsintDetail.afterPropertiesSet();
					} catch (ClassNotFoundException | NoSuchMethodException e) {
						//log.error("[ERROR] jobDetail: {}", e.getMessage(), e);
						//log.error("[ERROR] jobDetail: {}", "ERROR-01: jobDetail error : " + batchJob.getJobMethod());

						continue;
					}

					//jdbc 전용: 거주지, 답례품순위
					CronTriggerFactoryBean cronPsintTrigger = null;
					if ("2".equals(batchJob.getTriggerType())) {  // cron
						cronPsintTrigger = new CronTriggerFactoryBean();
						cronPsintTrigger.setBeanName("Trigger" + batchJob.getBatchJobId());
						cronPsintTrigger.setJobDetail((JobDetail) jobPsintDetail.getObject());
						cronPsintTrigger.setCronExpression(batchJob.getTriggerCronExpression());
						cronPsintTrigger.afterPropertiesSet();


						if (cronPsintTrigger.getObject() != null) {
							// 트리거 Key.
							TriggerKey triggerKey = cronPsintTrigger.getObject().getKey();

							// 스케줄러 중지..
							if ("2".equals(batchJob.getBatchStatus())) {

								schedulerPsint.unscheduleJob(triggerKey);
								schedulerPsint.deleteJob(((JobDetail) jobPsintDetail.getObject()).getKey());
							...
							// 시작 /재시작
							try {
								schedulerPsint.scheduleJob((JobDetail) jobPsintDetail.getObject(), (CronTrigger) cronPsintTrigger.getObject());

							} catch (ObjectAlreadyExistsException ex) {
								log.error("ERROR: {}", ex);
								schedulerPsint.rescheduleJob(triggerKey, (CronTrigger) cronPsintTrigger.getObject());
							}
							// 스케줄러 적용 정보 업데이트
							batchJobMapper.updateBatchApplyFlagInBatchJob(Integer.parseInt(batchJob.getBatchJobId()));
						}
				...
				if ("1".equals(batchJob.getTriggerType())) {
					...
					long repeatInterval = 99999999999L;
					try {
						repeatInterval = Long.parseLong(batchJob.getTriggerRepeatSeconds());
					} catch (NumberFormatException ignore) {
						//log.error("[BATCH] simpleTrigger.setRepeatInterval error. repeatInterval: {} ({})", batchJob.getTriggerRepeatSeconds(), ignore.getMessage());
						log.error("[BATCH] simpleTrigger.setRepeatInterval error. repeatInterval: {} ({})", batchJob.getTriggerRepeatSeconds(), "ERROR-02: setRepeatInterval error");
					}
			...
			log.error("invokeJobChecker() : {}", "ERROR-03: invokeJobChecker error", e);
		}
	}
