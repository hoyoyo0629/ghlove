<template>
    <div>
        ...
        <div class="loc_search" id="locSearch">
            <div class="loc_search_area">
                <span class="close_area" @click="close_area()"><button type="button"><img class="icon-img"
                            src="/static/images/icon/tabbar_close_arrow.png" alt="지자체 검색 닫기"></button>
                </span>
                <span class="toggle_tit">{{ upperLocSearchTxt }} {{ locSearchTxt }}</span>
                <!-- <span class="s-txt">지역을 선택하시면 해당 지역의 지자체몰로 이동합니다.</span> -->
                <span class="locSelectWrap">
                    <span class="goods_loc_search" @click.stop="">
                        <select name="upperLocgovCode" id="upperLocgovCode" v-model="sidoCode" @change="changeSidoEvent">
                            <option v-for="(sido, index) in sidoList" :value="sido.sidoCode" :key="index">
                                {{ sido.sidoName }}
                            </option>
                        </select>
                        <select name="locgovCodeSet" id="locgovCodeSet" v-model="sigunguCode" @change="changeSigunguEvent">
                            <option v-for="(sigungu, index) in sigunguList" :value="sigungu.cityCode" :key="index">
                                {{ sigungu.cityName }}
                            </option>
                        </select>
                        <span class="btn-box">
                            <!-- <button type="button" class="formBtn" @click="this.$s.redirect('/goods/index.html')">답례품몰 가기</button> -->
                            <button type="button" class="formBtn loc" @click="searchLocgov">지자체몰
                                가기</button>
                        </span>
