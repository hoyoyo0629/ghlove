<template>
      <header id="header_g" class="header2">
            <nav class="gnb">
                  <div class="center">
                        <div class="gnb_slider_m">
                              <button type="button" class="total_menu" @click="allMenu()" v-if="!agencyMode">
                                    <span class="total_menu_h">
                                          <img :src="menuImageSrc" alt="전체메뉴열기">
                                    </span>
                                    <span>전체 카테고리</span>
                              </button>
                              <span class="v-line" v-if="!agencyMode & !exportMode"></span>
                              <ul class="top_menu" v-if="!agencyMode">
                                    <li v-for="(goods,i) in goodsMenu1" :key="i">
                                          <a :href="goods.pinUrl" :class="{ 'on': goods.pinUrl == menuUrl || isChildMenu(goods.key) }" v-if="!exportMode">{{ goods.menuName }}</a>
                                    </li>
                              </ul>
                              <span class="goods_search">
                                  <input type="text" placeholder="답례품 전체 검색" @keydown='keydown' v-model="searchKeyword" title="답례품 전체 검색" v-if="!agencyMode">
                                  <input type="text" placeholder="답례품 검색" @keydown='keydown' v-model="searchKeyword" title="답례품 검색" v-if="agencyMode">
                                  <button type="button" @click.prevent="search">
                                      <img class="icon-img" src="/static/images/icon/search-white.png" alt="검색하기">
                                  </button>
                              </span>
                              <span class="local_shop" @click="selectMenuOpen($event)" v-if="!agencyMode">
            ...
            <nav class="main_lnb" id="allMenu" v-if="!agencyMode">
                  <div class="center">
...
<script type="text/babel">
    module.exports = {
          ...
          data: function () {
                return {
                      ...
                      goodsMenu1: [
                            ...
                            {menuName:"마을기업관", pinUrl: '/community-business/communityList-main.html  ', key: '3' }, 
                      ],
                      goodsMenuChild: [
          ...
          methods: {
                ...
                search: function () {
                    if (!this.searchKeyword && !this.agencyMode) {
                ...
                categoryLink: function (value, valueName) {
                	if (this.exportMode) {
                    	$s.redirect(`/goods/searchGoods.html?category=${value}&type=C&exportMode=true`);
                	} else {
                ...
                categoryGroupLink: function (value, valueName) {
                	if (this.exportMode) {
                    	$s.redirect(`/goods/searchGoods.html?group=${value}&type=C&exportMode=true`);
                	} else {
                ...
                searchLocgov: async function () {
                    ...
                    $goods.goSearchGoods({ type: 'L', locgov: locgovCode }, this.exportMode);
                },
                ...
                searchLocgovMap: function (lclgvUpperCd, lclgvCd, lclgvUpperNm, lclgvNm) {
                    if (lclgvUpperCd) {
                        ...
                        $s.redirect('/goods/index-main.html');
                    }
                },
          },
          mounted: function () {
