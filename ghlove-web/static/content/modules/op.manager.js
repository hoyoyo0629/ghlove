...
//주문 상세 정보 팝업.
Manager.orderDetails = function(pageType, orderSequence, orderCode, isPopup, sellerYn) {
	var prefixUrl = '/opmanager';
	if (sellerYn != undefined && sellerYn == 'Y') {
		prefixUrl = '/seller';
	}
    if (isPopup == '1') {
        Common.popup(prefixUrl + '/order/'+pageType+'/order-detail/' + orderSequence + '/' + orderCode, "order-details", 1280, 800, 1, 0, 0);
    } else {
        location.href = prefixUrl + '/order/'+pageType+'/order-detail/' + orderSequence + '/' + orderCode;
    }
};
...
Manager.sellerItemLog = function(itemId) {
    Common.popup('/seller/item/popup/log/' + itemId, "item-log", 1528, 750, 1, 0, 0);
};
...
// 데모사이트 메뉴 설명 노출
Manager.displayDemoInfo = function() {
    var DEMO_URL = 'https://saleson.onlinepowers.com';

    if (location.href.indexOf(DEMO_URL) > -1) {
        $('.op-menu-info').show();
    }
}
...
$(function() {
    ...
    // 데모 사이트인 경우 메뉴 설명이 노출됨.
    Manager.displayDemoInfo();
