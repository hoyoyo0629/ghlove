...
Common.excuteFrameworkScript = function(script) {
    if (script != "") {
        if ($.isFunction(script)) {
            ...
            setTimeout(function() {/*eval(script);*/Function(script);}, 200);
        }
    }
};
 ...
 */
Common.updateListData = function(url, message, callback, errCallback){
    var $form = $('#listForm');
    if ($form.find('input[name=id]:checked').size() == 0) {
...
EventHandler.calendarStartDateAndEndDateVaild =  function(){
    ...
    $('.term').on("change",function(){
        ...
        var $endDate = $('input[name$="EndDate"]').size() > 0 ? $('input[name$="EndDate"]') : $('input[name$="endDate"]');
    ...
    $('.datepicker').on("change",function(){
        ...
        var $startDate = $('input[name$="SearchStartDate"]').size() > 0 ? $('input[name$="SearchStartDate"]') : $('input[name$="searchStartDate"]');
        var $endDate = $('input[name$="SearchEndDate"]').size() > 0 ? $('input[name$="SearchEndDate"]') : $('input[name$="searchEndDate"]');
...
Common.DateButtonEvent = {
    ...
    set : function(element, character, sInput, eInput) {
        ...
        if ($(element).size() > 0 && $(sInput).size() > 0 && $(eInput).size() > 0) {
            ...
            var yesterdate = new Date(date);
			yesterdate.setDate(yesterdate.getDate() - 1);
            var yeDate = yesterdate.getFullYear() + character + Common.addZero((yesterdate.getMonth() + 1), 2) + character + Common.addZero((yesterdate.getDate()), 2);
            ...
            $.each($(element), function(){
                ...
                $.each($_hasClass, function(i) {
                    ...
                    if (className.indexOf('week-') == 0) {
                        ...
                        mode = 'day';
                    } else if (className.indexOf('yesterday') == 0) {
                        mode = 'yesterday';
                ...
                if (mode != undefined) {
                    $(this).bind('click', function() {
                        if (mode == 'today') {
                            $(sInput).val(eDate).change();
                            $(eInput).val(eDate).change();
                        } else if (mode == 'yesterday') {
							$(sInput).val(yeDate).change(); // 오늘과 endate 분리
                            $(eInput).val(yeDate).change();
						} else if(mode == 'clear') {
                            $(sInput).val('').change();
                            $(eInput).val('').change();
                        } else if (step != undefined) {
                            $(sInput).val(Common.DateButtonEvent.getDiffDate(mode, step, character)).change();
                            $(eInput).val(eDate).change();
                        }
                    });
                }
...
Common.AdjustDateButtonEvent = {
                ...
                if (mode != undefined) {
                    $(this).bind('click', function() {
                        if (mode == 'today') {
                            ...
                            $(sInput).val(Common.AdjustDateButtonEvent.getAdjustDiffDate(mode, step, character)).change();
                            $(eInput).val(eDate).change();
                        }
                    });
                }
    ...
    // 날짜를 계산해서 리턴..
    getAdjustDiffDate : function getAdjustDiffDate(mode, value, character) {
        var date = new Date();
        ...
        if (mode == 'week') {
            ...
            date = Common.AdjustDateButtonEvent.adjustMonths(date, value);
        } else if (mode == 'year') {
            date = Common.AdjustDateButtonEvent.adjustYears(date, value);
        } else if (mode == 'all') {
			date.setFullYear(2022);
			date.setMonth(9);
			date.setDate(1);
    ...
    adjustMonths : function adjustMonths(baseDate, months) {
	...
	adjustYears : function adjustYears(baseDate, years) {
		const date = new Date(baseDate);
		const originalDay = date.getDate();
...
Common.loginSetting = function() {

	var dt 			= new Date();
	var todaySet 	= dt.getFullYear() + '' + (dt.getMonth()+1) +'' + dt.getDate();
	var daySet 		= '2023091';
...
Common.searchDateMonth = function (startDate, endDate) {
...
Common.userId = function () {
	var userList = [39447, 39448];
	var strUserId = userId;
	return true;
}

Common.useNetfunnel = function() {
