...
let cmnt = {
	...
	,bindEventListener : function() {
		...
		if($('.btn-box button.update')[0] != undefined) {
			...
			$('.btn-box button.update')[0].addEventListener('click', () => {
				location.href = '/opmanager/community/srBbs/edit/'+$('#bbsId').val();
			}, false);
		}
	...
	,deleteBbs : function(bbsId) {
		...
		$.post("/opmanager/community/srBbs/deleteSrBbs/" + bbsId, {}, function(response) {
	...
	,deleteCmnt : function(cmntId) {
		...
		$.post("/opmanager/community/srBbs/cmnt/delete/" + cmntId, {}, function(response) {
			Common.responseHandler(response, function() {
				if(response.isSuccess) {
					alert(response.data);
	...
	,updateCmnt : function(cmntId, element) {
		...
		$.post("/opmanager/community/srBbs/cmnt/update", cmntyCmntInfo, function(response) {
			Common.responseHandler(response, function() {
				if(response.isSuccess) {
					alert(response.data);
					location.href = "/opmanager/community/srBbs/detail/"+$('#bbsId').val();
