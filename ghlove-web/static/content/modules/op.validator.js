...
(function ($) {
	...
	var VALIDATOR_MESSAGE = {
		'ko' : {
			...
			_password: '비밀번호는 9~20 자리의 영문, 숫자, 특수 문자로 되어야 합니다.',
			_duplicated : '{0} 항목에 동일한 문자를 반복해서 사용할 수 없습니다. ({1}자 이상 사용 불가)',
			_continuous : '{0} 항목에 연속된 문자를 {1}자 이상 사용할 수 없습니다.',
...
//				}
		...
		validate: function (selector, options) {
			...
			let selectors = selector.find(targetSelector);
			let selectorsLength = selectors.length;
			for (let idx = 0 ; idx < selectorsLength ; idx++) {
				let selector = selectors[idx];
				...
				if (tagName == 'input') {
					try {
					tagType = $(selector).attr("type").toLowerCase();
					} catch(e) {
				...
				if ($.validator.currentClass != '') {
					...
					for (var i = 0; i < currentClassNames.length; i++) {
						...
						if (currentClass != '' && $(selector).hasClass(currentClass)) {
							isRequired = true;
							break;
						}
					}
				...
				if ($(selector).hasClass('_duplicated')) {
					isDuplicated = true;
				}
				...
				if (isDuplicated) {
					...
					if (value != '') {
						...
						for (var i=0; i<length; i++) {
							...
							checkedChar[char] = pattern;
						}
						*/
						// 특수문자 2개 연속 들어갈 경우 문제생겨서 수정해봄
						for (var i=0; i<length; i++) {
						...
						var regExp;
						for (var key in checkedChar) {
							var pattern = checkedChar[key];
						...
						if (isCharDuplicated) {
							...
							$.validator.validatorAlert($.validator.messages['_duplicated'].format(title, duplicateCount), $(selector));
							$(selector).focus();
						...
						for(let i = 0 ; i < length ; i++) {
							...
							if ((unicode >= 48 && unicode <= 57)				// 숫자
								|| (unicode >= 65 && unicode <= 90)				// 영문 소문자
								|| (unicode >= 97 && unicode <= 122)			// 영문 대문자
								) {
								if ((unicode - preStr) == 1 && plus >= 0) {			// 오름차순은 오름차순만 체크
									plus++;
									minus = 0;
								} else if ((unicode - preStr) == -1 && minus >= 0) {		// 내림차순은 내림차순만 체크
									plus = 0;
									minus++;
								} else {
									plus = 0;
							...
							if (minus >= (duplicateCount - 1) || plus >= (duplicateCount - 1)) {
								errors++;

								$.validator.validatorAlert($.validator.messages['_continuous'].format(title, duplicateCount), $(selector));
								$(selector).focus();
					...
					if (tagName == 'input') {
						...
						if (tagType == 'text' || tagType == "password" || tagType == "email"  || tagType == "tel" || tagType == "number") {
							...
							if (value == '') {
								...
								$.validator.validatorAlert($.validator.messages['text'].format(title), $(selector));
								$(selector).focus();
								return false;
							}
							...
							if (value == '') {
								...
								// 포커스 타겟 지정
								var $focusTarget = $(selector).find('> label');
						...
						if (value == '') {
							errors++;
							$.validator.validatorAlert($.validator.messages['select'].format(title), $(selector));
							$(selector).focus();
							return false;
						}
				...
				for (var i = 0; i < keys.length; i++) {
					...
					if (isRequired
							|| ($(selector).hasClass('optional') && $.trim($(selector).val()) != '')
							|| ($.validator.currentOptionalClass != '' && $(selector).hasClass($.validator.currentOptionalClass) && $.trim($(selector).val()) != '') ) {
					...
					if (keys[i] == "_date") {
						...
						for (var j = 0; j < classes.length; j++) {
							...
							if (cls == keys[i] + '_') {
								...
								} catch(e) {let errBln = true;}
							}
						}

					} else {

						if ($(selector).hasClass(keys[i]) && value != "") {
