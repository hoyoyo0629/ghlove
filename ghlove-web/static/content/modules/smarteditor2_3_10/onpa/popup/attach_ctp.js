	...
	var welUploadInputBox = jindo.$Element("uploadInputBox");
	var oNavigator = jindo.$Agent().navigator();
	
	//마크업-공통 
	var welBtnConfirm = jindo.$Element("btn_confirm");				//확인 버튼
	var welBtnCancel= jindo.$Element("btn_cancel");				//취소 버튼
	
	//진도로 랩핑된 element
	var welTextGuide = jindo.$Element(elTextGuide);
	var welDropArea = jindo.$Element(elDropArea);
	var welDropAreaUL = jindo.$Element(elDropAreaUL); 
	...
	function checkDragAndDropAPI(){
		...
		// 에디터는 이미지 첨부는 File API를 사용하지 않는다 - skc_2013-07-30
		return false;
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		...
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/smarteditor/upload',	//샘플 URL입니다.location.href.replace(/\/[^\/]*$/, '') + '/smarteditor/upload'
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	    	...
 	    	error : function(oCustomEvent) {
 	    		...
 	    		// 	}
 	    		//}
 	    		//var wel = jindo.jindo.$Element("info");
	...
	window.onload = function(){
  		...
  		/*if(bSupportDragAndDropAPI){
  			jindo.$Element("pop_container2").hide();
  			jindo.$Element("pop_container").show();
  			...
  			jindo.$Element("pop_container").hide();
  			jindo.$Element("pop_container2").show();
  			callFileUploader();
  		}*/
  		//jindo.$Element("pop_container2").show();
		callFileUploader();
