...
//			log.debug("섬네일 생성오류1 :  {}", e.getMessage());
			log.debug("섬네일 생성오류1 :  {}", getClass().getName() + " :: index IOException1 =========");
			try {
				String redirectUrl = ShopUtils.getNoImagePath();
				
				if (!ObjectUtils.isEmpty(thumbnail.getSrc())) {
...
//				log.debug("섬네일 생성 오류 시 Redirect 오류 :  {}", e1.getMessage());
				log.debug("섬네일 생성 오류 시 Redirect 오류 :  {}", getClass().getName() + " :: index IOException2 =========");
			}
		} 
	}
