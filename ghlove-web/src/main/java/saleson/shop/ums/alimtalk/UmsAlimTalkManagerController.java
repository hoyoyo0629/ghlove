...
//            log.error("SalesOn 템플릿 목록 조회 에러 : {}", e.getMessage(), e);
            log.error("SalesOn 템플릿 목록 조회 에러 : {}", "============ getAlimtalk RuntimeException ============");
//            throw new KakaoAlimTalkException(e.getMessage());
//            throw new KakaoAlimTalkException(MessageUtils.getMessage("문제가 발생했습니다."));		// "문제가 발생했습니다."
            throw new KakaoAlimTalkException("문제가 발생했습니다.");		// "문제가 발생했습니다."
        }
    ...
    public String getCreate(AlimTalkTemplate dto, Model model) {
        ...
        model.addAttribute("templateMessageTypeList", enumMapper.get("TemplateMessageType"));
        model.addAttribute("templateEmphasizeTypeList", enumMapper.get("TemplateEmphasizeType"));
...
//            log.error("Vendor&SalesOn 템플릿 등록 에러 : {}", e.getMessage(), e);
//            return ViewUtils.redirect("/opmanager/ums/alimtalk/" + vendor.getTemplateCode() + "/create", e.getMessage());
            log.error("Vendor&SalesOn 템플릿 등록 에러 : {}", "=========== postCreate RuntimeException =============", e);
//            return ViewUtils.redirect("/opmanager/ums/alimtalk/" + vendor.getTemplateCode() + "/create", MessageUtils.getMessage("등록에 실패했습니다."));		// 등록에 실패했습니다.
            return ViewUtils.redirect("/opmanager/ums/alimtalk/" + vendor.getTemplateCode() + "/create", "등록에 실패했습니다.");		// 등록에 실패했습니다.
        }
    ...
    public String getEdit(AlimTalkTemplate dto, Model model) {
        ...
        try {
			...
			log.error("Vendor&SalesOn 템플릿 조회 에러 : {}", /* e.getMessage(), */ e);
//            throw new KakaoAlimTalkException(e.getMessage());
            throw new KakaoAlimTalkException("템플릿 조회에 실패했습니다.");
        }
...
//            log.error("Vendor&SalesOn 템플릿 수정 에러 : {}", e.getMessage(), e);
//            return ViewUtils.redirect("/opmanager/ums/alimtalk/" + vendor.getTemplateCode() + "/edit/" + vendor.getApplyCode(), e.getMessage());
            log.error("Vendor&SalesOn 템플릿 수정 에러 : {}", "============ postEdit RuntimeException =============");
//            return ViewUtils.redirect("/opmanager/ums/alimtalk/" + vendor.getTemplateCode() + "/edit/" + vendor.getApplyCode(), MessageUtils.getMessage("수정에 실패했습니다."));		// 수정에 실패했습니다.
            return ViewUtils.redirect("/opmanager/ums/alimtalk/" + vendor.getTemplateCode() + "/edit/" + vendor.getApplyCode(), "수정에 실패했습니다.");		// 수정에 실패했습니다.
        }
...
//            return JsonViewUtils.failure(e.getMessage());
            log.error("Vendor 템플릿 검수 요청 에러 : {}", "=========== verify RuntimeException ==========", e);
...
//            log.error("Vendor 템플릿 검수 요청 에러 : {}", e.getMessage(), e);
//            return JsonViewUtils.failure(e.getMessage());
            log.error("Vendor 템플릿 검수 요청 에러 : {}", "=========== synchronization RuntimeExceptoin ============");
//            return JsonViewUtils.failure(MessageUtils.getMessage("실패했습니다."));				// 실패했습니다.
            return JsonViewUtils.failure("실패했습니다.");				// 실패했습니다.
        }
...
//            return ViewUtils.redirect("/opmanager/ums/alimtalk/" + dto.getTemplateCode() + "/list", e.getMessage());
            log.error("알림톡 -> UMS 템플릿 적용 에러 : {}", "========= select RuntimeException =============");
//            return ViewUtils.redirect("/opmanager/ums/alimtalk/" + dto.getTemplateCode() + "/list", MessageUtils.getMessage("실패했습니다."));			// 실패했습니다.
            return ViewUtils.redirect("/opmanager/ums/alimtalk/" + dto.getTemplateCode() + "/list", "실패했습니다.");			// 실패했습니다.
        }
...
//            log.error("Vendor 템플릿 검수 요청 에러 : {}", e.getMessage(), e);
//            return JsonViewUtils.failure(e.getMessage());
            log.error("Vendor 템플릿 검수 요청 에러 : {}", "========= delete RuntimeException ===========");
//            return JsonViewUtils.failure(MessageUtils.getMessage("실패했습니다."));				// 실패했습니다.
            return JsonViewUtils.failure("실패했습니다.");				// 실패했습니다.
        }
...
//                        throw new KakaoAlimTalkException(e.getMessage(), e);
                        log.error("Vendor 데이터 삭제 실패 : {}", "========== batchTest deleteTemplate RuntimeException ==========");
//                        throw new KakaoAlimTalkException(MessageUtils.getMessage("실패했습니다."));			// 실패했습니다.
                        throw new KakaoAlimTalkException("실패했습니다.");			// 실패했습니다.
                    }
                });
            }
...
//            log.error("[updateKakaoAlimTalkBatch] 카카오 알림톡 업데이트 실패 : {}", e.getMessage(), e);
            log.error("[updateKakaoAlimTalkBatch] 카카오 알림톡 업데이트 실패 : {}", "========== batchTest RuntimeException ==========", e);
        }
