...
public class EventController {
	...
	public String newItemList(DisplayItemParam displayItemParam, Model model) {
		...
		// 기본 정렬 조건
		if (ObjectUtils.isEmpty(displayItemParam.getOrderBy())) {
			displayItemParam.setOrderBy("ORDERING");
			displayItemParam.setSort("ASC");
		}

		// 리스트 타입.
		if (ObjectUtils.isEmpty(displayItemParam.getListType())
	...
	// 19_11_22 SKI's test
	// inc_header_middle.jsp  <li><a href="/event/choice" title = "choice">초이스</a></li>
	// header_middle.jsp 에 '베스트' 와 내용 동일, 페이지 렌더링 동일 -> 수정중  Customizing Test
	/**
	 * CHOICE 상품(== best)
	 * @param model
	 * @return
	 */
	@GetMapping("choice")
	public String choiceItemList(Model model) {
		...
		return ViewUtils.getView("/event/choice");
	}
