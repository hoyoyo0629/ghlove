...
//			throw new PageNotFoundException(e.getMessage());
			...
			throw new PageNotFoundException("상품정보가 없습니다.");			// 상품정보가 없습니다.
		}
		...
		if (!isPreview) {
			...
			if (item == null || !"Y".equals(item.getDisplayFlag())) {
				...
				return new ModelAndView(new RedirectView("/m"));
			}
			...
			if (todayItems.indexOf(newTodayItems) == -1) {
				...
				cookie.setMaxAge(60*60);				// 쿠키 유지 기간 - 1일 -> 보안코딩 1시간으로 줄임
	...
	public String createReviewAction(
		...
		try {
			...
			return ViewUtils.redirect("/m/item/create-review/"+ orderCode +"/"+ itemUserCode, e.getErrorMessage(), JavaScript.CLOSE);
		}
...
//			log.warn(e.getMessage());
			log.warn(getClass().getName() + " :: searchResultList RuntimeException ============", e);
		}
	...
	public String createQnaAction(@PathVariable("itemUserCode") String itemUserCode, @ModelAttribute("itemQna") Qna itemQna) {
		...
		try {
			...
			return ViewUtils.getView("/item/create-qna", "insertQna UserException");
		}
