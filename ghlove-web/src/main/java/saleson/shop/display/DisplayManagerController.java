...
public class DisplayManagerController {
	...
	public String form(Model model, DisplayItemParam displayItemParam, @PathVariable("code") String code) {
		...
		if ("best".equals(code)) {
			...
			List<saleson.shop.categories.domain.Group> shopCategoryGroups = new ArrayList<saleson.shop.categories.domain.Group>();
			...
			if (ObjectUtils.isEmpty(displayItemParam.getDisplaySubCode())){
	...
	public String form2(Model model, DisplayItemParam displayItemParam,
		...
		if (ObjectUtils.isEmpty(subCode) || !subCode.matches("\\w+")) {
	...
	public String formAction(Model model, DisplayItemParam displayItemParam,
		@PathVariable("code") String code) {
	...
	public String formAction2(Model model, DisplayItemParam displayItemParam,
		@PathVariable("code") String code,
		@PathVariable("subCode") String subCode) {
	...
	public String spotEdit(@PathVariable("itemId") String strItemId, Model model) {
		if (ObjectUtils.isEmpty(strItemId)) {
			throw new PageNotFoundException();
		}
		int itemId = 0;
		try {
	...
	public String displayTemplateAction(@PathVariable(name="groupCode") String groupCode,
		...
		String resultMessage = "수정되었습니다.";
		try {
			displayService.updateDisplayDataByGroupCode(sendData);
		}catch (RuntimeException e){
			resultMessage = "수정이 정상적으로 처리되지 않았습니다.";
		}

		return ViewUtils.redirect(url, resultMessage);
	}
	...
	public String snsList(DisplaySnsParam displaySnsParam, Model model) {
		...
		if (ObjectUtils.isEmpty(displaySnsParam.getOrderBy())) {
	...
	public String snsCheckedDelete(DisplaySnsParam displaySnsParam) {
		...
		return ViewUtils.redirect("/opmanager/display/sns/list", MessageUtils.getMessage("M00205")); // 삭제되었습니다.
