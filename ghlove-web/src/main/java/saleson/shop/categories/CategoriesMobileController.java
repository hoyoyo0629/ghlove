...
public class CategoriesMobileController {
	...
	private final CategoriesTeamGroupService categoriesTeamGroupService;

	private final ItemValidator itemValidator;

	private final CategoriesFilterService categoriesFilterService;
	...
	public ModelAndView index(@PathVariable("categoryCode") String categoryCode,
		...
		try {
			...
			// 2-3. 카테고리 메인.
			if ("category".equals(categoryType) && categoryCode != null) {
				invokeCategoryIndex(categoryCode, itemParam, model);

				ShopUtils.setCookieWhenServerLoad(request, response);
			}
	...
	public String options(@ModelAttribute CategoriesSearchParam categoriesSearchParam, Model model) {
		...
		if (!categoriesSearchParam.getCategoryClass().equals("")) {
			model.addAttribute("list", categoriesService.getChildCategoriesFromParentCategoryClass(categoriesSearchParam.getCategoryClass()));
		}
	...
	private void invokeCategoryIndex(String categoryCode, ItemParam itemParam, Model model) {
		...
		if (parentCategory != null) {
			category.setParentCategory(ShopUtils.getParentCategory(parentCategory.getUrl())); // 부모의 부모
		...
		// 카테고리 SEO 설정.
		Seo seo = category.getCategoriesSeo();
		if (seo != null && !seo.isSeoNull()) {
			ShopUtils.setSeo(seo);
		}
		...
		model.addAttribute("categoryPath", categoriesService.getCategoryPathByBreadcrumbs(breadcrumbs, false));
