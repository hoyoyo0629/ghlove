...
//			map.put("errorMessage", MessageUtils.getMessage("실패했습니다."));			// 실패했습니다.
			map.put("errorMessage", "실패했습니다.");			// 실패했습니다.
			map.put("isSuccess", false);
		}
		...
		if ((Boolean) map.get("isSuccess")) {
			try {
				googleAnalyticsService.allRefund(requestContext.getRequest(), orderParam.getOrderCode(), orderParam.getOrderSequence());
...
//			model.addAttribute("cashReceiptId", cashReceiptId);
	...
	private String getReturnApply(ReturnApply returnApply, Model model, OrderParam orderParam) {
		...
		orderService.setOrderItemOther(orderItem);
...
//			model.addAttribute("errorMessage", "ERROR - " +e.getMessage());
			log.error("ERROR: {}", getClass().getName() + " :: getReturnApply RuntimeException =========", e);

//			model.addAttribute("errorMessage", MessageUtils.getMessage("실패했습니다."));			// 실패했습니다.
			model.addAttribute("errorMessage", "실패했습니다.");			// 실패했습니다.
...
//
//			model.addAttribute("errorMessage", "ERROR - " +e.getMessage());
			log.error("ERROR: {}", getClass().getName() + " :: getExchangeApply RuntimeException =============", e);
...
//
//			model.addAttribute("errorMessage", "ERROR - " +e.getMessage());
			log.error("ERROR: {}", getClass().getName() + " :: getCancelApply RuntimeException ==========", e);
...
//			model.addAttribute("errorMessage", MessageUtils.getMessage("실패했습니다."));			// 실패했습니다.
	...
	public JsonView cancelApplyProcess(RequestContext requestContext, ClaimApply claimApply, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			return JsonViewUtils.exception(ex.getErrorMessage());
		}
