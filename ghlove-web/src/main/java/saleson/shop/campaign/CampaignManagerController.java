...
import saleson.shop.campaign.infra.CampaignBatchDtoEncryptor;
...
public class CampaignManagerController {
    ...
    @Autowired
    CampaignBatchDtoEncryptor campaignBatchDtoEncryptor;
    ...
    public String getUmsList(Model model, CampaignBatchDto campaignBatchDto,
        ...
        campaignBatchDto.encrypt(campaignBatchDtoEncryptor);
        Page<CampaignBatch> pageContent = campaignService.getCampaignBatchList(campaignBatchDto.getPredicate(), pageable);
        campaignBatchDto.decrypt(campaignBatchDtoEncryptor, ShopUtils.needMasking());
    ...
    public String postCampaignCreate(@PathVariable(value = "campaignId", required = false) Long campaignId,
        ...
        try {
            ...
            String utmSource = "";
            String utmMedium = !ObjectUtils.isEmpty(campaign.getUtmMedium()) ? "&utm_medium=" + campaign.getUtmMedium() : "";
            String utmCampaign = !ObjectUtils.isEmpty(campaign.getUtmCampaign()) ? "&utm_campaign=" + campaign.getUtmCampaign() : "";
            String utmItem = !ObjectUtils.isEmpty(campaign.getUtmItem()) ? "&utm_item=" + campaign.getUtmItem() : "";
            String utmContent =!ObjectUtils.isEmpty(campaign.getUtmContent()) ? "&utm_content=" + campaign.getUtmContent() : "";

            if (!ObjectUtils.isEmpty(utmMedium) || !ObjectUtils.isEmpty(utmCampaign) || !ObjectUtils.isEmpty(utmItem) || !ObjectUtils.isEmpty(utmContent)) {

                if ("0".equals(sendType)) {
                    utmSource = "sms";

                    if (contentLength > SMS_BYTE_LENGTH || !ObjectUtils.isEmpty(imageUrl)) {
                        utmSource = "mms";
                    }
            ...
            if (campaignId != null) {
                ...
                if (!ObjectUtils.isEmpty(campaign.getImageUrl()) && "1".equals(campaign.getSendType())) {
            ...
            log.error("캠페인 등록 에러 : {}", "OpRuntimeException");
        }

        return "redirect:/opmanager/campaign/list";
    }

    private void setEventCodes(Campaign campaign, EventCode eventCode) {
    ...
    private void setEventCodesForRegular(CampaignRegular campaignRegular, CampaignRegularUrl campaignRegularUrl) {
    ...
    public JsonView sendTest(Campaign campaign, CampaignUser campaignUser) {
        try {
            ...
            String callbackNumber = environment.getProperty("ums.callback");
            ...
            if ("2".equals(campaign.getSendType()) && ObjectUtils.isEmpty(campaignUser.getPushToken())) {
            ...
            //log.error("테스트 발송 에러 : {}", e.getErrorMessage(), e);
            log.error("테스트 발송 에러 : {}", "---------sendTest OpRuntimeException------------");
    ...
    public String getCampaignSendListDetail(Model model, CampaignDto campaignDto,
        ...
        } else if (!ObjectUtils.isEmpty(campaignDto.getGroupCode())) {
    ...
    public JsonView getMmsImageUrl(@RequestParam(name = "encodingFile", defaultValue = "") String encodingFile) {
        ...
        try {
            ...
            if (responseImage == null) {
                throw new UserException("responseImage null 통신 실패");
            }
            ...
            log.error("MMS Image Upload Api Error >>> {}");
    ...
    public JsonView edit(@PathVariable("campaignId") Long campaignId) throws Exception {
        ...
        try {
          ...
          //log.error("캠페인 취소 에러 : {}", e.getErrorMessage(), e);
          log.error("캠페인 취소 에러 : {}", "---------edit UserException---------");
          //return JsonViewUtils.failure(e.getErrorMessage());
          return JsonViewUtils.failure("캠페인의 정보가 없습니다.");
        } catch (OpRuntimeException e) {
          //log.error("캠페인 취소 에러 : {}", e.getErrorMessage(), e);
            log.error("캠페인 취소 에러 : {}","--------edit OpRuntimeException----------");
          //return JsonViewUtils.failure(e.getErrorMessage());
            return JsonViewUtils.failure("실패했습니다.");
        }
    ...
    public JsonView delete(@PathVariable("campaignId") Long campaignId) throws Exception {
        ...
        try {
            ...
            log.error("캠페인 삭제 에러 : {}", "---------delete UserException---------");
           // return JsonViewUtils.failure(e.getErrorMessage());
            return JsonViewUtils.failure("캠페인 정보가 없습니다.");
        }
    ...
    public JsonView getCalculateList(CalculateParam calculateParam) {
        ...
        try {
            ...
            log.error("MMS Api Error >>> {}", "UserException");
    ...
    public String postRegularCreate(@PathVariable(value = "id", required = false) Long id,
        ...
        try {
            ...
            String utmSource = "";
            String utmMedium = !ObjectUtils.isEmpty(campaignRegular.getUtmMedium()) ? "&utm_medium=" + campaignRegular.getUtmMedium() : "";
            String utmCampaign = !ObjectUtils.isEmpty(campaignRegular.getUtmCampaign()) ? "&utm_campaign=" + campaignRegular.getUtmCampaign() : "";
            String utmItem = !ObjectUtils.isEmpty(campaignRegular.getUtmItem()) ? "&utm_item=" + campaignRegular.getUtmItem() : "";
            String utmContent =!ObjectUtils.isEmpty(campaignRegular.getUtmContent()) ? "&utm_content=" + campaignRegular.getUtmContent() : "";
            ...
            log.error("정기발송 등록 에러 : {}", "OpRuntimeException");
        }
    ...
    public JsonView regularEdit(@PathVariable("id") Long id) {
        ...
        try {
            ...
            log.error("정기발송 취소 에러 : {}", "OpRuntimeException");
            return JsonViewUtils.failure(e.getErrorMessage());
        }
    ...
    public JsonView regularDelete(@PathVariable("id") Long id) {
        ...
        try {
            ...
            log.error("정기발송 삭제 에러 : {}","OpRuntimeException");
            return JsonViewUtils.failure(e.getErrorMessage());
        }
    ...
    private void setGroupLevelName(CampaignBatchDto campaignBatchDto, Model model) {
        ...
        } else if (!ObjectUtils.isEmpty(campaignBatchDto.getGroupCode())) {
