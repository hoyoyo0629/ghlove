...
@Controller
@RequestMapping("/opmanager/event-statistics")
...
public class EventStatisticsManagerController {
    ...
    @GetMapping("/list")
    public String getEventStatisticsList(Model model, EventStatisticsParam param, @PageableDefault Pageable pageable) {

        String today = LocalDateUtils.localDateTimeToString(LocalDateTime.now(), Const.DATE_FORMAT);

        if (ObjectUtils.isEmpty(param.getBeginDate()) || ObjectUtils.isEmpty(param.getEndDate())) {
    ...
    public String getEventItemStatisticsList(Model model, EventStatisticsParam param, @PageableDefault Pageable pageable) {
    ...
    private List<EventStatistics> getEventStatisticsList(Page<Map<String, Object>> pageContent) {
        ...
        if (pageContent != null) {
            ...
            if (content != null && !content.isEmpty()) {
                ...
                content.forEach(c-> {
                    ...
                    list.add(s);
                    if (!ObjectUtils.isEmpty(s.getEventCode())) {
        ...
        if (contents != null && !contents.isEmpty()) {
            ...
            for (EventStatistics statistics : list) {
                ...
                if (ObjectUtils.isEmpty(statistics.getEventCode())) {
                    continue;
                }
