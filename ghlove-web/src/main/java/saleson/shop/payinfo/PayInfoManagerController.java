...
public class PayInfoManagerController {
     ...
     */
    @PostMapping("/list")
    public String searchList(Model model, PayInfoParam payInfoParam) {
    	return list(model, payInfoParam);
    }

    @PostMapping("/cancel-fail-info/list")
    public String searchCancelFaillist(OrderCancelFailParam orderCancelFailParam
            , @PageableDefault @SortDefault(sort = "id", direction = Sort.Direction.DESC) Pageable pageable
            , Model model) {
    	return cancelFaillist(orderCancelFailParam ,pageable,  model);
    }
