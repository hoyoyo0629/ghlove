...
import saleson.shop.point.PointValidator;
...
import saleson.shop.ums.support.UserSms;
...
public class UserManagerController {
	...
	private final CategoriesTeamGroupService categoryCategoriesTeamGroupService;
	 ...
	 * @return
	 */
	@GetMapping("customer/list-old")
	public String customerList(UserSearchParam searchParam, Model model) {
	 ...
	 * @return
	 */
	@PostMapping("customer/list-old")
	public String searchCustomerList(UserSearchParam searchParam, Model model) {
		searchParam.setAuthority("ROLE_USER");
	...
	public ModelAndView downloadExcelProcessByUserList(UserSearchParam searchParam) {
...
//			throw new UserException(e.getMessage());
			//throw new UserException(MessageUtils.getMessage("실패했습니다."));			// 실패했습니다.
			throw new UserException("실패했습니다.");			// 실패했습니다.
		}

		return ViewUtils.redirect("/opmanager/user/customer/point", "처리되었습니다.", "self.close()");
	}
...
//			throw new UserException(e.getMessage());
		...
		return ViewUtils.redirect("/opmanager/user/customer/point-by-excel", "처리되었습니다.", "self.close()");
	}
	...
	public String managerListPost(
			@ModelAttribute("searchParam") UserSearchParam searchParam,
			Model model) {
		return managerLsit(searchParam, model);
	}
	...
	public String managerCreateAction(HttpServletRequest request,
		...
		if (userService.getUserCountByManagerPhoneNumber(user.getPhoneNumber()) > 0) {
			return ViewUtils.redirect("/opmanager/user/manager/list", MessageUtils.getMessage("M01690"));	// 이미 등록된 핸드폰 입니다.
		}

		userService.insertManager(user);
		changeLogService.insertManagerChangeLog(request, user, ChangeTypeEnum.CREATE);
	...
	public String managerEditAction(RequestContext requestContext, Model model,
		...
		/* SKC - 휴대폰 번호 중복은 우선 주석.
		User managerInfo = userService.getManagerByUserId(user.getUserId());
		if (!managerInfo.getPhoneNumber().equals(user.getPhoneNumber())) {
			if (userService.getUserCountByManagerPhoneNumber(user.getPhoneNumber()) > 0) {
				return ViewUtils.redirect("/opmanager/user/manager/list", "이미 등록된 핸드폰 번호입니다.");	// 이미 등록된 핸드폰 입니다.
			}
		}
		*/
	...
	public JsonView managerDeleteList(HttpServletRequest request,
									  ListParam listParam) {

		try {
			userService.deleteUserByListParam(listParam);
			changeLogService.insertManagerChangeLog(request, new User(), ChangeTypeEnum.DELETE);

			return JsonViewUtils.success();
		} catch (RuntimeException e) {
//			return JsonViewUtils.failure(e.getMessage());
			//return JsonViewUtils.failure(MessageUtils.getMessage("삭제에 실패했습니다."));		// 삭제에 실패했습니다.
			return JsonViewUtils.failure("삭제에 실패했습니다.");		// 삭제에 실패했습니다.
		}
	}
	...
	@PostMapping("manager-role/list")
	public String managerRoleListPost(
			@ModelAttribute("roleParam") SearchParam roleParam,
			Model model) {
		return managerRoleList(roleParam, model);
	}
	...
	public String userDetails(@PathVariable("userId") long userId, Model model) {
		...
		if (user == null) {
			...
			return ViewUtils.getView("/user/popup/details", "회원 정보가 없습니다.", "self.close()");
		}
...
//			return ViewUtils.redirect("/opmanager/user/popup/password/" + userId, e.getMessage());
			//return ViewUtils.redirect("/opmanager/user/popup/password/" + userId, MessageUtils.getMessage("비밀번호 변경에 실패했습니다."));			// 비밀번호 변경에 실패했습니다.
			return ViewUtils.redirect("/opmanager/user/popup/password/" + userId, "비밀번호 변경에 실패했습니다.");			// 비밀번호 변경에 실패했습니다.
		}
	}
...
//			throw new UserException(e.getMessage());
	...
	public String writeAction(Model model, UserDelivery userDelivery) {
		...
		return ViewUtils.redirect("/opmanager/user/popup/delivery-list/" + userDelivery.getUserId(), MessageUtils.getMessage("M00288")); // 등록되었습니다.
	}
	 ...
	 * @return
	 */
	@GetMapping("secede-user/list-old")
	 ...
	 * @return
	 */
	@PostMapping("secede-user/list-old")
	public String searchSecedeUserList(
			@ModelAttribute("searchParam") UserSearchParam searchParam,
			Model model) {
	...
	public String sendSmsList(@PathVariable("userId") long userId, SendSmsLogParam sendSmsLogParam, Model model) {
		...
		HashMap<String, String> codes = new UmsTemplate(cryptor).getTemplateCodes();
		codes.remove("site_url");
		model.addAttribute("smsTemplateCodeList",codes);
...
//			return ViewUtils.redirect("/opmanager/user/popup/group", e.getMessage(), "opener.location.reload();self.close()");
			return ViewUtils.redirect("/opmanager/user/popup/group", MessageUtils.getMessage("실패했습니다."), "opener.location.reload();self.close()");		// 실패했습니다.
		}
	}
	...
	@RequestProperty(layout="base")
	public String sendSms(Model model, @PathVariable("userId") long userId) {
		...
		Ums ums = umsService.getUms("user_sms");
		UmsDetail umsDetail = null;
		if (umsService.isValidUms(ums)) {
			List<UmsDetail> umsDetails = ums.getDetailList();

			if (umsDetails != null && !umsDetails.isEmpty()) {
				umsDetail = umsDetails.stream()
						.filter(ud -> UmsType.MESSAGE == ud.getUmsType())
						.findFirst().orElse(null);
			}
		}

		model.addAttribute("user", user);
		model.addAttribute("umsDetail", umsDetail);
		return "view:/user/popup/send-sms";
	}
	
	@PostMapping("/popup/send-sms/{userId}")
	public String sendSmsAction(UmsDetail umsDetail, @PathVariable("userId") long userId) {
		...
		if (umsService.isValidUms(ums)) {
			...
			if (!ObjectUtils.isEmpty(phoneNumber)) {
				...
				ums.getDetailList().stream()
						.filter(ud -> UmsType.MESSAGE == ud.getUmsType())
						.forEach(ud-> {
							ud.setSendType(umsDetail.getSendType());
							ud.setTitle(umsDetail.getTitle());
							ud.setMessage(umsDetail.getMessage());
						});

				unifiedMessagingService.sendMessage(new UserSms(user, ums, phoneNumber, cryptor, dataMasking));
		...
		if (smsConfig != null) {
			if (ObjectUtils.isEmpty(userDetail.getPhoneNumber()) == false) {
...
//			throw new UserException(e.getMessage());
		...
		return ViewUtils.redirect("/opmanager/user/customer/point-pay", "처리되었습니다.", "self.close()");
	}
