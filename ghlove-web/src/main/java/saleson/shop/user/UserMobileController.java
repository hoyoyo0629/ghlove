...
public class UserMobileController {
	...
	public String occupied(@ModelAttribute Customer customer, RequestContext requestContext,Model model,
			@RequestParam(value="redirect", required=false, defaultValue="") String redirect, UserDetail userDetail){

		return getJoinEntryForm(customer, model, redirect, userDetail);
	}
	...
	private String getJoinEntryForm(@ModelAttribute Customer customer, Model model, @RequestParam(value = "redirect", required = false, defaultValue = "") String redirect, UserDetail userDetail) {
	...
	public String joinAction(HttpServletRequest request, Model model,
		...
		if (StringUtils.isNotEmpty(checkResult)) {
			if(checkResult.equals("isOccupiedId")) {
				return ViewUtils.redirect("/m/users/entryForm?terms=1&privacy=1", "사용할수 없는 Email 입니다");
			}
		}
