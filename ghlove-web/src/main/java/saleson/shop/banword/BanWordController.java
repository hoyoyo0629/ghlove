...
public class BanWordController {
	...
	public String create(@Valid BanWord banWord, BindingResult bindingResult) {
		...
		try {
			...
			//return ViewUtils.redirect("/opmanager/ban-word/list", e.getErrorMessage());
			return ViewUtils.redirect("/opmanager/ban-word/list", "실패했습니다.");
		}
	...
	public JsonView delete(RequestContext requestContext, @PathVariable int banWordId) {
		...
		try {
			...
			return JsonViewUtils.exception("OpRuntimeException");
		}
	...
	private boolean hasPageAccessAuthority() {
