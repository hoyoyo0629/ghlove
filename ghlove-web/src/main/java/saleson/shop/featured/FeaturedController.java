...
public class FeaturedController {
	...
	@GetMapping("/pages/mypage/my_catalog_info")
	@RequestProperty(layout="mypage")
	public String myCatalogInfo() {
	
		return ViewUtils.getView("/featured/my_page/my_catalog_info");
	}
	...
	public String rating(Model model) {
		...
		if (UserUtils.isUserLogin()) {
			...
			if (ObjectUtils.isEmpty(userDetail.getGroupCode()) == false) {
	...
	public String fearturedList(FeaturedParam featuredParam, Model model) {
	 ...
	 * @return
	 */
	@GetMapping("/pages/tokusyuichiran")
	public String list(Model model, FeaturedParam featuredParam, CategoriesEditParam categoriesEditParam) {
		featuredParam.setFeaturedType("1"); 	// PC
		featuredParam.setFeaturedFlag("Y");
		featuredParam.setDisplayListFlag("Y");
		...
		model.addAttribute("promotionList", categoriesEditService.getCategoryPromotionListByParam(categoriesEditParam));
		
		int count = featuredService.getFeaturedCountByParam(featuredParam);
		Pagination pagination = Pagination.getInstance(count, 40);
		featuredParam.setPagination(pagination);
		
		model.addAttribute("pagination",pagination);
		model.addAttribute("featuredList",featuredService.getFeaturedListByParam(featuredParam));
		model.addAttribute(featuredParam.getFeaturedCodeChecked(), "1");
		model.addAttribute("featuredParam", featuredParam);
	 ...
	 * @return
	 */
	@GetMapping("/pages/catalog_download")
	public ModelAndView catalog_download(Model model, FeaturedParam featuredParam, CategoriesEditParam categoriesEditParam) {
		final RedirectView rv = new RedirectView("/digital-catalog/list");
	...
	public String details(@PathVariable("featuredUrl") String featuredUrl, FeaturedParam featuredParam, Model model,
		...
		/*if (!"0".equals(featured.getAccessAuth())) {
			Boolean authCheck = false;
			List<String> pdExGubnCodes = UserUtils.getPdExGubnCodes();
			for (String code : pdExGubnCodes) {
				if (code.equals(featured.getAccessAuth())) {
					authCheck = true;
				}
			}
			if (!authCheck) {
		...
		if (!ObjectUtils.isEmpty(featured.getEventCode())) {
