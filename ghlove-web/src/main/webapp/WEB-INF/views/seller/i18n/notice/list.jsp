...
<%@ taglib prefix="op" 		uri="/WEB-INF/tlds/functions" %>
			...
			<div class="board_write" >
				...
				<div class="btn_all">
					<div class="btn_left">
						<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/seller/notice/list'"><span class="glyphicon glyphicon-repeat"></span> <c:out value="${op:message('M00047')}"/></button> <!-- 초기화 -->
					</div>
					<div class="btn_right">
			...
			<div class="count_title mt20">
				<h5>
					<c:out value="${op:message('M00039')}"/> : <c:out value="${op:numberFormat(pagination.totalItems)}"/> <c:out value="${op:message('M00743')}"/>
				</h5>	 <!-- 전체 -->   <!-- 건 조회 -->
				<span>
				...
				<table class="board_list_table" summary="${op:message('M00273')}">
					...
					<thead>
						<tr>
							...
							<th scope="col"><c:out value="${op:message('M00275')}"/></th>
							<th scope="col">조회수</th>
							<th scope="col"><c:out value="${op:message('M00276')}"/></th>
						</tr>
					</thead>
					<tbody>
						...
						<!-- 2017.04.24 Jun-Eu Son 상단공지 표시 변경 -->
							<tr>
								<c:choose>
									...
									<c:otherwise><td><c:out value="${pagination.itemNumber - i.count}"/></td></c:otherwise>
								</c:choose>
								<td>
									<div class="tex_l">
										<%-- <a href="<c:url value="/seller/notice/view/${notice.noticeId}" />"><c:out value="${notice.subject}"/></a> --%>
