...
<div class="popup_wrap">
	<form:form modelAttribute="orderExchangeApply" name="orderExchangeApply" method="post" onsubmit="return Manager.Order.orderExchangeApplyAction('${fn:escapeXml(pageType)}')">
		<form:hidden path="orderItemId" />
		<div class="popup_contents">

			<div class="board_list">
				<h3><c:out value="${orderExchangeApply.orderItem.orderCode}"/> - 교환 신청 상품 정보</h3>	 	
		 			...
		 			<tbody>
		 				<tr>	
							<td class="left none_left">	 						 
		 						<div class="product_img_min"><img src="${fn:escapeXml(orderExchangeApply.orderItem.imageSrc)}" alt="" width="88" /></div>
		 						<div class="arrival3">
		 							<p class="code">[<c:out value="${orderExchangeApply.orderItem.itemUserCode}"/>]</p>
		 							<p class="name"><c:out value="${orderExchangeApply.orderItem.itemName}"/></p>
									<c:if test="${!empty orderExchangeApply.orderItem.options}">
										<p><c:out value="${shop:viewItemOptions(orderExchangeApply.orderItem.setItemFlag, orderExchangeApply.orderItem.options)}"/></p>
									</c:if>
		 						</div>
		 					</td>
		 					<td><c:out value="${op:numberFormat(orderExchangeApply.orderItem.quantity)}"/>개</td>
		 					<td><span class="price"><c:out value="${ op:numberFormat(orderExchangeApply.orderItem.saleAmount) }"/></span>원</td>
		 					<td><div class="sum"><span class="price"><c:out value="${ op:numberFormat(orderExchangeApply.orderItem.saleAmount) }"/></span>원</div></td>
		 				</tr>
		 			</tbody>
		 		</table><!--//esthe-table E-->	  		 
				...
				<table cellpadding="0" cellspacing="0" class="board_write_table">
		 			...
		 			<tbody>
		 						...
		 						<div>
			 						<form:select path="exchangeReason">
		 								<c:forEach var="code" items="${exchangeReasons}" varStatus="i">
		 									<form:option value="${fn:escapeXml(code.detail)}"><c:out value="${code.label}"/></form:option>
		 								</c:forEach>
		 							</form:select>
		 						...
		 						<div>
			 						<p class="mb3">
			 							...
			 							<form:select path="exchangeShippingDeliveryCompanyId">
			 								<form:options items="${op:nullToEmptyArr(orderExchangeApply.deliveryCompanys)}" itemValue="deliveryCompanyId" itemLabel="deliveryCompanyName" />
			 							</form:select>
			 						</p>
		 				...
		 				<tr>
		 					...
		 					<td class="left">
		 						<div>
		 							<c:out value="${orderExchangeApply.exchangeReceiveName }"/>
		 				...
		 				<tr>
		 					...
		 					<td class="left">
		 						<div>
		 							<c:out value="${orderExchangeApply.exchangeReceivePhone }"/> / <c:out value="${orderExchangeApply.exchangeReceiveMobile }"/>
		 				...
		 				<tr>
		 					...
		 					<td class="left">
		 						<div>
		 							(<c:out value="${orderExchangeApply.exchangeReceiveZipcode1 }"/>-<c:out value="${orderExchangeApply.exchangeReceiveZipcode2 }"/>)
			 						<form:hidden path="exchangeReceiveZipcode1" />
			 						<form:hidden path="exchangeReceiveZipcode2" />
			 						
			 						<c:out value="${orderExchangeApply.exchangeReceiveAddress }"/>
									...
									<c:out value="${orderExchangeApply.exchangeReceiveAddress2 }"/>
		 						...
		 						<div>
			 						...
			 						<c:if test="${orderExchangeApply.orderItem.orderItemStatus == '40' || orderExchangeApply.orderItem.orderItemStatus == '41'}">
			 							<span class="small_text">
			 								해당 주문건은 <strong><c:out value="${orderExchangeApply.orderItem.orderItemStatusLabel}"/></strong>건 입니다.
			 						...
			 						<div class="board_guide">
										...
										<p class="tip">
											...
											<c:out value="${orderExchangeApply.orderItem.orderShippingInfo.receiveAddress}"/> <c:out value="${orderExchangeApply.orderItem.orderShippingInfo.receiveAddressDetail}"/> 
										...
										<p class="mt3 tip">
											...
											고객 부담 : <c:out value="${op:numberFormat(orderExchangeApply.collectionShipping + orderExchangeApply.resendShipping)}"/> 원 / 판매자 부담 : 0원
										</p>
										<p class="mt3 tip">
										...
										<p class="mt3 tip">
											기본 반송배송비 : <c:out value="${op:numberFormat(orderExchangeApply.orderItem.shippingReturn)}"/> 원 /
				 						 	제주 : <c:out value="${op:numberFormat(orderExchangeApply.orderItem.shippingExtraCharge1)}"/> 원 /
				 						 	도서산간 : <c:out value="${op:numberFormat(orderExchangeApply.orderItem.shippingExtraCharge2)}"/> 원
										</p>
									</div> 
								</div>
		 					</td>
