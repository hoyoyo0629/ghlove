		...
		<form id="listForm">
			<table class="board_list_table" summary="전체상품리스트">
				...
				<c:forEach items="${list}" var="item" varStatus="i">
					...
					<tr>
						<td><input type="checkbox" name="id" value="${fn:escapeXml(item.itemSaleEditId)}" class="${fn:escapeXml(item.itemCode)}" title="" /></td>
						<td>
							<c:choose>
							...
							<p style="padding-top: 5px;">
								<c:choose>
									<c:when test='${op:property("saleson.view.type") eq "api"}'>
										<a href="${op:property('saleson.url.frontend'))}/items/details.html?code=${fn:escapeXml(item.itemCode)}" target="_blank"><img src="/content/opmanager/images/icon/icon_preview_pc.gif" alt="" /></a>
									</c:when>
									<c:otherwise>
										<a href="/products/preview/${fn:escapeXml(item.itemCode)}" target="_blank"><img src="/content/opmanager/images/icon/icon_preview_pc.gif" alt="" /></a>
										<a href="/m/products/preview/${fn:escapeXml(item.itemCode)}" target="_blank"><img src="/content/opmanager/images/icon/icon_preview_mobile.gif" alt="" /></a>
									</c:otherwise>
								</c:choose>
							</p>
						</td>
						<td>
							<a href="/seller/item/sale-edit/view/${fn:escapeXml(item.itemSaleEditId)}">
								<c:choose>
						...
						<td class="left">
							<c:choose>
							<c:when test="${item.status == '0'}">
							<a href="/seller/item/sale-edit/update/${fn:escapeXml(item.itemSaleEditId)}">[${fn:escapeXml(item.itemCode)}]<br/> ${fn:escapeXml(item.itemName)}</a></td>
						</c:when>
						<c:otherwise>
						...
						<td>
							<c:if test="${item.status == '0'}">
								<a href="/seller/item/sale-edit/update/${fn:escapeXml(item.itemSaleEditId)}" class="btn btn-gradient btn-xs">수정</a>
							</c:if>
