...
<%@ taglib prefix="daum"	tagdir="/WEB-INF/tags/daum" %>
				...
				<c:forEach items="${list}" var="shipment">
      				<tr class="listCount">
      					...
      					<td>
      						...
      						<span class="display-inline-block" style="width:50px;">제주도</span> : <c:out value="${op:numberFormat(shipment.shippingExtraCharge1)}"/>원<br />
      						<span class="display-inline-block" style="width:50px;">도서산간</span> : <c:out value="${op:numberFormat(shipment.shippingExtraCharge2)}"/>원
      					</td>
      		...
      		<form:form modelAttribute="shipment" id="shipment" method="post" enctype="multipart/form-data">
				...
				<p class="popup_btns">
					<button type="submit" class="btn btn-active btn-create"><c:out value="${op:message('M00088')}"/></button> <!-- 등록 -->
					<button type="submit" class="btn btn-active btn-edit hide"><c:out value="${op:message('M00087')}"/></button> <!-- 수정 -->
					<a href="javascript:shipmentDelete();" class="btn btn-default btn-delete hide"><c:out value="${op:message('M00074')}"/></a> <!-- 삭제 -->
					
					<c:if test="${isPopup}">
						<a href="javascript:closePopup();" class="btn btn-default"><c:out value="${op:message('M00037')}" /></a> <!-- 취소 -->
					</c:if>
				</p>
			</form:form>
