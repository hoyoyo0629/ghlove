...
<div class="popup_wrap">
	<h3 class="popup_title"><span>${fn:escapeXml(orderItem.orderCode)} - 교환 요청 정보</span></h3>
				...
				<table class="board_write_table">
					...
					<tbody id="order_items">
						...
						<tr>
							...
							<td class="tleft">
								<div>
									${fn:escapeXml(orderItem.orderExchangeApply.exchangeReceiveName)}
								</div>
							</td>
						</tr>
						...
						<tr>
							...
							<td class="tleft">
								<div>
									전화번호 : ${fn:escapeXml(orderItem.orderExchangeApply.exchangeReceivePhone)}<br/>
									휴대전화 : ${fn:escapeXml(orderItem.orderExchangeApply.exchangeReceiveMobile)}
								</div>
							</td>
						</tr>
						...
						<tr>
							...
							<td class="tleft">
								<div>
									(${fn:escapeXml(orderItem.orderExchangeApply.exchangeReceiveZipcode)})<br/>
									${fn:escapeXml(orderItem.orderExchangeApply.exchangeReceiveAddress)}<br/>
									${fn:escapeXml(orderItem.orderExchangeApply.exchangeReceiveAddress2)}
								</div>
							</td>
						</tr>
						...
						<tr>
							...
							<td colspan="2" class="tleft">
								<div>
									${fn:escapeXml(orderItem.orderExchangeApply.exchangeReasonText)}
								</div>
							</td>
						</tr>
						<tr>
								...
								<div>
									<c:choose>
										<c:when test="${orderItem.orderExchangeApply.exchangeShippingStartFlag == 'Y'}">
											택배사 : ${fn:escapeXml(orderItem.orderExchangeApply.exchangeShippingCompanyName)}, 송장번호 : ${fn:escapeXml(orderItem.orderExchangeApply.exchangeShippingNumber)}<br/>
											(해당 배송사로 고객이 <strong>${op:date(orderItem.orderExchangeApply.exchangeShippingStartDate)}일</strong>에 발송 하였습니다.)
										</c:when>
										<c:otherwise>
											지정택배사 <strong>${fn:escapeXml(orderItem.deliveryCompanyName)}</strong> 으로 회수신청 하였습니다.
										</c:otherwise>
									</c:choose>
								</div>
							</td>
