...
<div class="popup_wrap">
	<div class="popup_contents">
		<form:form modelAttribute="orderReturnApply" name="orderReturnApply" method="post" onsubmit="return Manager.Order.orderReturnApplyAction('${fn:escapeXml(pageType)}')">
		
			<form:hidden path="orderItemId" />

			<div class="board_list"> 	
				<h3><span>${fn:escapeXml(orderReturnApply.orderItem.orderCode)} - 환불 신청 상품 정보</span></h3>
		 			...
		 			<tbody>
		 				<tr>	
							<td class="left none_left">	 						 
		 						<div class="product_img_min"><img src="${fn:escapeXml(orderReturnApply.orderItem.imageSrc)}" alt="" width="88" /></div>
		 						<div class="arrival3">
		 							<p class="code">[${fn:escapeXml(orderReturnApply.orderItem.itemUserCode)}]</p>
		 							<p class="name">${fn:escapeXml(orderReturnApply.orderItem.itemName)}</p>
									<c:if test="${!empty orderReturnApply.orderItem.options}">
										<p>${shop:viewItemOptions(orderReturnApply.orderItem.setItemFlag, orderReturnApply.orderItem.options)}</p>
									</c:if>
		 						</div>
		 					</td>
		 					<td>
		 						...
		 						<div>
			 						<form:select path="returnReason">
		 								<c:forEach var="code" items="${returnReasons}" varStatus="i">
		 									<form:option value="${fn:escapeXml(code.detail)}">${fn:escapeXml(code.label)}</form:option>
		 								</c:forEach>
		 							</form:select>
		 						...
		 						<div>
			 						<p class="mb3">
			 							...
			 							<form:select path="returnShippingDeliveryCompanyId">
			 								<form:options items="${op:nullToEmptyArr(orderReturnApply.deliveryCompanys)}" itemValue="deliveryCompanyId" itemLabel="deliveryCompanyName" />
			 							</form:select>
			 						</p>
		 						...
		 						<div>
			 						...
			 						<c:if test="${orderReturnApply.orderItem.orderItemStatus == '30' || orderReturnApply.orderItem.orderItemStatus == '31'}">
			 							<span class="small_text">
			 								해당 주문건은 <strong>${fn:escapeXml(orderReturnApply.orderItem.orderItemStatusLabel)}</strong>건 입니다.
			 						...
			 						<div class="board_guide">
										...
										<p class="tip">
											...
											${fn:escapeXml(orderReturnApply.orderItem.orderShippingInfo.receiveAddress)} ${fn:escapeXml(orderReturnApply.orderItem.orderShippingInfo.receiveAddressDetail)} 
