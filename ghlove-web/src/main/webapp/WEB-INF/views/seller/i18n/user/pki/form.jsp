<%@ page language="java"	contentType="text/html; charset=utf-8" pageEncoding="utf-8"%>
<%@ taglib prefix="spring"	uri="http://www.springframework.org/tags" %>
<%@ taglib prefix="c"		uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="fn"		uri="http://java.sun.com/jsp/jstl/functions" %>
<%@ taglib prefix="page"	tagdir="/WEB-INF/tags/page"%>
<%@ taglib prefix="op"		uri="/WEB-INF/tlds/functions" %>
<%@ taglib prefix="fmt" 	uri="http://java.sun.com/jsp/jstl/fmt"%>

<script type="text/javascript" src="/content/MagicLine4Web/ML4Web/js/ext/jquery-1.10.2.js"></script>
<script type="text/javascript" src="/content/MagicLine4Web/ML4Web/js/ext/jquery-ui.min.js"></script>
<script type="text/javascript" src="/content/MagicLine4Web/ML4Web/js/ext/jquery.blockUI.js"></script>
<!-- ML4WEB JS -->
<script type="text/javascript" src="/content/MagicLine4Web/ML4Web/js/ext/ML_Config.js"></script>
<!-- 간편인증 JS -->
<script type="text/javascript" src="/content/MagicLine4Web/ML4Web/js/ML4Web_Config.js"></script>
<script type="text/javascript" src="/content/MagicLine4Web/ML4Web/js/ext/magicline-js-api.min.js"></script>

<script type="text/javascript" src="/content/modules/op.fincert.js"></script>
<script type="text/javascript" src="/content/modules/jsencrypt.min.js"></script>

<div class="admin_wrap">
	<!-- Header -->
	<%@ include file="/WEB-INF/tags/seller/login-header.tag" %>
	<!--// Header -->

	<!-- Container -->
	<div id="container" class="login">
		<div class="login_cont">
			<div class="login_tit mt-70">인증서 등록<br>
				<span style="color:#d40000;font-weight:bold;font-size: 20px;">법인 인증서는 등록/사용이 불가능합니다.</span></div>
			<div class="form-wrap">
				<div class="board_write">
					<table class="board_write_table">
						<colgroup>
							<col style="width: 220px;" />
						</colgroup>
						<tr>
							<td class="label">인증서 종류</td>
							<td>
								<div class="flex_box gap-08">
									<!-- TODO 운영 테스트시 처리필요 -->
									<input type="radio" id="fin" name="cert-type" value="F" />
									<label for="fin"><p class="txt">금융인증서</p></label>
									<input type="radio" id="pki" name="cert-type" value="P" />
									<label for="pki"><p class="txt">공동인증서(구 공인인증서)</p></label>
								</div>
							</td>
						</tr>
						<tr>
							<td class="label"><span class="required_mark">*</span>로그인 아이디<br>(등록된 답례품 제공자 정보)</td>
							<td>
								<div>
									<input id="loginId" class="optional sixsix" title="로그인 아이디" type="text" placeholder="로그인 아이디" maxlength="100">
								</div>
							</td>
						</tr>
						<tr>
							<td class="label"><span class="required_mark">*</span>휴대폰번호<br>(등록된 답례품 제공자 정보)</td>
							<td>
								<div>
									<input id="phoneNumber1" class="optional" title="휴대폰 전화번호 첫번째 자리" type="text" placeholder="번호1" maxlength="4" style="width:20%;" autocomplete="off" onkeyup="javascript:inputCheck(event);">
									-
									<input id="phoneNumber2" class="optional" title="휴대폰 전화번호 두번째 자리" type="text" placeholder="번호2" maxlength="4" style="width:20%;" autocomplete="off" onkeyup="javascript:inputCheck(event);">
									-
									<input id="phoneNumber3" class="optional" title="휴대폰 전화번호 세번째 자리" type="text" placeholder="번호3" maxlength="4" style="width:20%;" autocomplete="off" onkeyup="javascript:inputCheck(event);">
								</div>
							</td>
						</tr>
						<tr>
							<td class="label"><span class="required_mark">*</span>사업자번호<br>(등록된 답례품 제공자 정보)</td>
							<td>
								<div>
									<input id="businessNumber1" class="optional" title="사업자번호 첫번째 자리" type="text" placeholder="번호1" maxlength="3" style="width:20%;" autocomplete="off" onkeyup="javascript:inputCheck(event);">
									-
									<input id="businessNumber2" class="optional" title="사업자번호 두번째 자리" type="text" placeholder="번호2" maxlength="2" style="width:20%;" autocomplete="off" onkeyup="javascript:inputCheck(event);">
									-
									<input id="businessNumber3" class="optional" title="사업자번호 세번째 자리" type="text" placeholder="번호3" maxlength="5" style="width:20%;" autocomplete="off" onkeyup="javascript:inputCheck(event);">
								</div>
							</td>
						</tr>
					</table>
				</div>
			</div>
			<div class="button_wrap">
				<button type="button" class="btn btn-dark-gray btn-mid h-large" onclick="pkiModify('D');"><span>삭제</span></button>
				<button type="button" class="btn btn-dark-gray btn-mid h-large" onclick="pkiModify('C');"><span>등록</span></button>
				<button type="button" class="btn btn-default btn-mid h-large" onclick="moveLoginPage();"><span>취소</span></button>
			</div>
		</div>
	</div>
	<!--// Container -->

	<!-- footer -->
	<!-- 하단  -->
    <%@ include file="/WEB-INF/tags/seller/login-footer.tag" %>
	<!--// footer -->
</div>

<!-- 결과 수신 메시지  -->
<form id="reqForm" name="reqForm" method="post" action="/seller/magicline/signedFormRGhlove" target="magiclineIframe">
	<input type="hidden"	id="signOrigin"		name="signOrigin" /> <!-- 180701 서명 원문 폼 추가 -->
	<input type="hidden"	id="sign"			name="sign"/>
	<input type="hidden"	id="csCheckType"	name="csCheckType"	value="1"/>
	<input type="hidden"	id="signData"		name="signData"		value="LOGIN" />
</form>
<!--// 결과 수신 메시지  -->

<!-- magic line 영역 -->
<iframe name="magiclineIframe" style="display:none;" width="1200px" height="500px"></iframe>
<!--// magic line 영역 -->

<script type="text/javascript">

	let publicKeyStr = "${publicKeyStr}";
	let crypt = new JSEncrypt();

	let signParam;


	$(function() {
		// 날짜 변경 이벤트
		datepickerChange();

		// 휴대폰번호 변경 이벤트
		phoneNumberChange();

		// enter Key Event
		$('#loginId, #phoneNumber, #businessNumber').keypress(function(e) {
			if (e.keyCode === 13) {
				e.preventDefault();
				pkiModify("C");
			}
		});

		// 금융인증서 초기화
		//fincert.init("${op:property('outconn.fincert.orgCode')}", "${op:property('outconn.fincert.apiKey')}");

		crypt.setPrivateKey(publicKeyStr);

	});

	/**
	 *	함 수 명 : moveLoginPage
	 *	기	능  : 로그인 페이지 이동
	 */
	function moveLoginPage() {
		location.href = "/seller/login";
	}

	/**
	 *	함 수 명 : pkiModify
	 *	기	능  : 인증서 수정 (등록/삭제)
	 */
	function pkiModify(type) {
		if(validator()){
			var validParam = {
				"loginId": $("#loginId").val(),
				"phoneNumber": $("#phoneNumber1").val() + '-' + $("#phoneNumber2").val() + '-' + $("#phoneNumber3").val(),
				"businessNumber": $("#businessNumber1").val() + '-' + $("#businessNumber2").val() + '-' + $("#businessNumber3").val(),
				"mode" : type
			};

			const param = {
				"loginId": $("#loginId").val(),
				"phoneNumber": $("#phoneNumber1").val() + '-' + $("#phoneNumber2").val() + '-' + $("#phoneNumber3").val(),
				"businessNumber": $("#businessNumber1").val() + '-' + $("#businessNumber2").val() + '-' + $("#businessNumber3").val(),
				"mode" : type
			};

			var pkiType = $("input[name=cert-type]:checked").val();

			$.post("/seller/user/pki/form/valid", validParam, function(response){
				console.log(response);
				if (response) {
					if (response.isSuccess) {
						if (response.data.code == "SUCC") {
							if (pkiType == 'P') {
								// 인증서 등록
								if(type == "C") {
									// 공동인증서 존재여부
									if(response.data.mberDn != 'Y') {
										doSignData(validParam);
									} else {
										alert("등록한 공동인증서가 존재합니다. 삭제후 등록해주세요.");
										return;
									}

								// 인증서 삭제
								} else if(type == "D") {
									pkiDelete(response.data.mberDn, param, pkiType, type);
								}
							}

							// 금융인증서
							if (pkiType == 'F') {
								// 인증서 등록
								if(type == "C") {

									// 금융인증서 존재여부
									if(response.data.mberFinDn != 'Y') {
										fincertSignData(response.data.mberCi, validParam);
									} else {
										alert("등록한 금융인증서가 존재합니다. 삭제후 등록해주세요.");
										return;
									}
								// 인증서 삭제
								} else if(type == "D") {
									pkiDelete(response.data.mberFinDn, param, pkiType, type);
								}
							}
						} else {
							if (response.data.errMsg) {
								alert(response.data.errMsg);
							} else {
								let message = (type == "C") ? "등록을" : "삭제를";
								alert("회원정보가 존재하지 않아 인증서 "+ message +" 할 수 없습니다.");
							}
							moveLoginPage();
						}
					} else {
						alert(response.errorMessage);
					}
					return;
				}
				alert("오류가 발생했습니다.");
			}, "json");
		}
	}

	/**
	 *	함 수 명 : validator
	 *	기	능  : 유효성 검사
	 */
	function validator() {

		if (!$("input[name=cert-type]").is(':checked')) {
			alert("인증서 종류를 선택해 주세요.");
			$("input[name=cert-type]").focus();
			return false;
		}

		if($.trim($("#loginId").val()) == "") {
			alert("아이디를 입력해 주세요.");
			$("#loginId").focus();
			return false;
		}

		if($.trim($("#phoneNumber1").val()) == "") {
			alert("휴대폰번호 첫번째 자리를 입력해 주세요.");
			$("#phoneNumber1").focus();
			return false;
		}

		if($.trim($("#phoneNumber2").val()) == "") {
			alert("휴대폰번호 두번째 자리를 입력해 주세요.");
			$("#phoneNumber2").focus();
			return false;
		}

		if($.trim($("#phoneNumber3").val()) == "") {
			alert("휴대폰번호 세번째 자리를 입력해 주세요.");
			$("#phoneNumber3").focus();
			return false;
		}

		if($.trim($("#businessNumber1").val()) == "") {
			alert("사업자번호 첫번째 자리를 입력해 주세요.");
			$("#businessNumber1").focus();
			return false;
		}

		if($.trim($("#businessNumber2").val()) == "") {
			alert("사업자번호 두번째 자리를 입력해 주세요.");
			$("#businessNumber2").focus();
			return false;
		}

		if($.trim($("#businessNumber3").val()) == "") {
			alert("사업자번호 세번째 자리를 입력해 주세요.");
			$("#businessNumber3").focus();
			return false;
		}

		return true;
	}

	/**
	 *	함 수 명 : doSignData
	 *	기	능  : 인증서 활성화
	 */
	function doSignData(param) {
		signParam = param;
		var signData = $("#signData").val();
		document.reqForm.signOrigin.value = document.reqForm.signData.value;
		magicline.uiapi.MakeSignData(signData, null, mlCallBack);
	}

	/**
	 *	함 수 명 : doSignData
	 *	기	능  : MagicLine 결과값 수신 CallBack
	 *	파라미터  : code - 전자서명 결과값, message - 전자서명 메시지
	 */
	function mlCallBack(code, message) {
		if(code==0){
			document.reqForm.sign.value = encodeURIComponent( message.encMsg );
			document.reqForm.submit();
		}else{
			alert("결과값 수신에 실패하였습니다.");
			return;
		}
	}

	/**
	 *	함 수 명 : signedResponse
	 *	기	능  : 인증서 검증 결과 callback
	 *	파라미터  : result - DN, message, result 정보 리턴
	 */
	function signedResponse(response, param) {

		/* var param = {
			"loginId": crypt.encrypt($("#loginId").val()),
			"mberDn" : ""
		}; */

		if (!param) {
			param = signParam;
			signParam = '';
		}

		if ($("input[name=cert-type]:checked").val() == 'P') {
			if(response && response.result && response.result == "SUCCESS" && response.DN) {
				try {
					param.mberDn = response.DN;
				} catch (e) {
					console.log = () => {};
				}
			}
		} else if ($("input[name=cert-type]:checked").val() == 'F') {
			if (response && response.dn) {
				try {
					param.mberFinDn = response.dn;
				} catch (e) {
					console.log = () => {};
				}
			}
		}

		if (!param.mberDn && !param.mberFinDn) {
			alert('인증서 정보가 없습니다.');
			return false;
		}

		pkiModifyProccess(param, function(data) {
			if (data) {
				if(data.code == "SUCC") {
					if (param.mode == 'D') {
						alert("인증서가 삭제되었습니다.");
					} else {
						alert("인증서 등록이 완료되었습니다.");
					}
					moveLoginPage();
				} else {
					alert(data.errMsg);
				}
			} else {
				alert("오류가 발생했습니다.");
			}
		});

	}

	/**
	 *	함 수 명 : pkiDelete
	 *	기	능  : 인증서 삭제
	 */
	function pkiDelete(mberDn, param, pkiType ,type) {
		// var mberDn = $("#mberDn").val();

		if(mberDn != "Y") {
			alert("등록된 인증서가 없습니다.");
			return false;
		}

		if(pkiType == 'P') {
			param.mberDn = mberDn;
			param.mberFinDn = '';
		}

		if(pkiType == 'F') {
			param.mberFinDn = mberDn;
			param.mberDn = '';
		}

		param.mode = type;

		if(confirm("등록된 인증서를 삭제 하시겠습니까?")) {
			console.log(param);
			/* let param = {
				"loginId": $("#loginId").val(),
				"phoneNumber": $("#phoneNumber1").val() + '-' + $("#phoneNumber2").val() + '-' + $("#phoneNumber3").val(),
				"businessNumber": $("#businessNumber1").val() + '-' + $("#businessNumber2").val() + '-' + $("#businessNumber3").val(),
				"mode" : "D"
			}; */

			pkiModifyProccess(param, function(data) {
				if (data) {
					if(data.code == "SUCC") {
						alert("인증서가 삭제되었습니다.");
						location.reload();
					} else {
						alert(data.errMsg);
					}
				} else {
					alert("오류가 발생했습니다.");
				}
			});
			/* if (pkiType == 'P') {
				doSignData(param);
			} else if (pkiType == 'F'){
				fincertSignData('', param);
			} */
		}
	}

	/**
	 *	함 수 명 : pkiModifyProccess
	 *	기	능  : 인증서 수정 처리
	 */
	function pkiModifyProccess(param, callBack) {
		$.post('/seller/user/pki/update', param, function(response) {
			if(response.isSuccess && response.data) {
				callBack(response.data);
			} else {
				if (response.errorMessage) {
					alert(response.errorMessage);
				} else {
					alert("문제가 발생했습니다.");
				}
			}
		});
	}

	/**
	 *	함 수 명 : datepickerChange
	 *	기	능  : 날짜 변경 이벤트
	 */
	function datepickerChange() {
		if($("input.datepicker").length) {
			$("input.datepicker").keyup(function(e) {
				if(e && e.target && e.target.value) {
					var value = e.target.value.replace(/[^0-9]/g, "");
					if(value.length > 8) { value = value.slice(0, 8); }
					e.target.value = value;
				}
			});
		}
	}

	/**
	 *	함 수 명 : phoneNumberChange
	 *	기	능  : 휴대폰번호 변경 이벤트
	 */
	function phoneNumberChange() {
		if($("#phoneNumber").length) {
			$("#phoneNumber").keyup(function(e) {
				if(e && e.target && e.target.value) {
					var value = e.target.value.replace(/[^0-9]/g, "");
					if(value.length > 11) { value = value.slice(0, 11); }
					e.target.value = value;
				}
			});
		}
	}

	function inputCheck(e) {
		if(e && e.target && e.target.value) {
			var value = e.target.value.replace(/[^0-9]/g, "");
			//if(value.length > e.target.maxLength) { value = value.slice(0, e.target.maxLength); }
			e.target.value = value;
		}
	}

	function fincertSignData(mberCi, param) {
		fincert.getUserDn(function (response) {
			/* if (mberCi != response.ci) {
				alert('회원정보와 인증서정보가 일치하지 않습니다.');
				return false;
			} */
			param.ci = response.ci;
			signedResponse(response, param);
		}, function (err) {
			alert('금융인증서 처리 중 오류가 발생하였습니다.\n관리자에게 문의 부탁드립니다.');
			return false;
		});
	}

</script>