...
<%@ page import = "saleson.common.security.crypto.RsaCryptor" %>
...
<%
    ...
    if (request.getRequestURL().indexOf("localhost") > -1) {
    	...
    	srvNo    = "001001";
    } else if (request.getRequestURL().indexOf("152.99.104.8") > -1) {			// apm 추가
    	srvNo    = "001001";
    } else {
    	srvNo    = "002003";
    }

    String privateKeyStr = request.getSession().getAttribute("privateKeyStr").toString();

    String reqNum   = "123456789"; // request.getParameter("reqNum");                       // 실명확인 요청번호  (sample 페이지와 result 페이지가  동일하지 않으면 결과페이지 복호화 시 에러)

    if (request.getParameter("sci_jumin1") == null
    	|| request.getParameter("sci_jumin2") == null
    	|| request.getParameter("sci_name") == null) {
    	throw new NullPointerException("인증 정보가 없습니다.");
    }

	/* String jumin1   = RsaCryptor.decrypt(request.getParameter("sci_jumin1").toString(), privateKeyStr);                           // 실명확인 주민번호1
	String jumin2   = RsaCryptor.decrypt(request.getParameter("sci_jumin2").toString(), privateKeyStr);     */                       // 실명확인 주민번호2
	String jumin1   = request.getParameter("sci_jumin1").toString();                           // 실명확인 주민번호1
	String jumin2   = request.getParameter("sci_jumin2").toString();

	/* String name     = RsaCryptor.decrypt(request.getParameter("sci_name").toString(), privateKeyStr);                             // 이름(siren서버는 euc-kr이기 때문에 같은환경이면 인코딩을 안해도 상관없음) */
	String name     = request.getParameter("sci_name").toString();                             // 이름(siren서버는 euc-kr이기 때문에 같은환경이면 인코딩을 안해도 상관없음)
    ...
    String retUrl   = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort()+"/seller/user/sci-result"; //request.getParameter("retUrl");                           // 실명확인 결과수신 URL(추가 파라미터셋팅은 http://도메인?param=value1&param2=value2 식으로 하면 됨)
