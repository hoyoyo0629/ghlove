...
<%@ taglib prefix="shop"	uri="/WEB-INF/tlds/shop" %>
		...
		<!-- 버튼시작 -->
			<div class="btn_all">
				<div class="btn_left">
					<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/seller/qna/list'"><span><c:out value="${op:message('M00047')}"/></span></button> <!-- 초기화 -->
				</div>
				<div class="btn_right">
		...
		<!-- 버튼 끝-->
			<div class="count_title mt20">
				<h5>전체 : <c:out value="${qnaCount}"/> 건 조회</h5>
				<span>출력수
					...
					<table class="board_list_table" summary="${op:message('M00273')}"> <!-- 주문내역 리스트 -->
						...
						<thead>
							<tr>
								...
								<th scope="col" colspan="3">작성자(아이디)</th>
								<th scope="col">작성일</th>
						...
						<tbody>
							<c:forEach items="${qnaList}" var="list" varStatus="i">
								<tr>
									...
									<td class="tex_l">
										<a href="/seller/qna/view/${fn:escapeXml(list.qnaId)}"><c:out value="${list.subject}"/></a> <!-- 제목 -->
									</td>

									<td>
										<div>
											<c:out value="${list.userId > 0 ? op:message('M00465') : op:message('M00466')}"/>
										</div>
									</td> <!-- 회원 --> <!-- 비회원 -->
									...
									<td colspan="3">
										<c:choose>
											<c:when test="${list.userId > 0 && !empty list.loginId}">
												<a href="javascript:Common.popup('/seller/user/popup/details/${fn:escapeXml(list.userId)}', '/user/popup/details', 1100, 800 ,1, 0, 0)"><c:out value="${list.userName}"/> (<c:out value="${list.loginId}"/>)</a>
											</c:when>
...
<iframe id="downloadFrame" name="downloadFrame" style="display: none;" title="다운로드"></iframe>
