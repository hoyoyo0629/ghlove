	...
	<form:form modelAttribute="itemParam" method="get">
				...
				<tr>
					...
					<td>
						<div class="flex_box gap-12">
							...
							<div class="input-form">
								<form:radiobutton path="saleStatus" value="mig" label="판매대기" /> <!-- 임시등록 -->
							</div>
						</div>
					</td>
		...
		<div class="btn_all btn_right">
			<div class="flex_box gap-08">
				<button type="button" class="btn btn-dark-gray btn-mini" onclick="location.href='/seller/item/list';"> ${op:message('M00047')}</button> <!-- 초기화 -->
		...
		<form id="listForm">
			<table class="board_list_table" summary="전체상품리스트">
				...
				<colgroup>
	                ...
	                <col style="width:40px;"> <!-- 20260325 필수 추가정보_ isb 수정 -->
	                <col style="width:60px;">  <!-- 20260325 필수 추가정보_ isb 수정 -->
	                <col style="width:100px;">
	                <col style="width:100px;">
	                <col style="width:180px;"><!-- 20260325 필수 추가정보_ isb 수정 -->
	                <col style="width:100px;" class="hidden">
	                <col style="width:120px;"><!-- 20260325 필수 추가정보_ isb 수정 -->
				</colgroup>
				<thead>
					<tr>
						...
						<th>${op:message('M00018')}</th> <!-- 상품명 --> <!-- 상품코드 -->
					...
					<c:forEach items="${list}" var="item" varStatus="i">
						...
						<c:choose>
							...
							<c:when test="${item.dataStatusCode == '0'}">	<!-- 1차 DB 이관 답례품 -->
								<c:set var="itemSaleStatusText">판매대기</c:set>
							</c:when>
						...
						<tr>
							...
							<td>
								${fn:escapeXml(item.itemTextOptionFlag)}  <!-- 20260325 필수 추가정보_ isb 수정 -->
							</td>
							<td>
								<c:choose>
...
<%-- 								<a href="/seller/item/copy/${fn:escapeXml(item.itemId)}" class="btn btn-default btn-xs">${op:message('M00788')}</a><br /> --%>
		...
		<div class="flex_box juc-sbt mt15">
			<div>
				<div class="flex_box gap-08">
					<button type="button" id="update_list_data_sold_out" onclick="updateListDataLabel('1')" class="btn btn-dark-gray btn-mini">품절</button>
					<button type="button" id="update_list_data_sold_out" onclick="updateListDataLabel('0')" class="btn btn-dark-gray btn-mini">품절해제</button>
					<button type="button" id="update_list_data_display" onclick="updateListDataDisplay('Y')" class="btn btn-dark-gray btn-mini">공개</button>
					<button type="button" id="update_list_data_display" onclick="updateListDataDisplay('N')" class="btn btn-dark-gray btn-mini">비공개</button>
					<!-- <button type="button" id="update_list_data_sale_off" onclick="updateListDataLabel('90')" class="btn btn-dark-gray btn-mini">판매종료처리</button> -->
                </div>
            </div>
				<!--
                <button type="button" id="update_list_data_resale" class="btn btn-dark-gray btn-sm">품절해제</button>
                -->
				<button type="button" id="update_list_data_add_category" class="btn btn-default btn-sm hidden"><span class="glyphicon glyphicon-plus"></span> ${op:message('M00789')}</button> <!-- 카테고리추가 -->

				<c:if test="${!empty itemParam.categoryId}">
					<button type="button" id="change_ordering" class="btn btn-dark-gray btn-sm hidden">${op:message('M00791')}</button> <!-- 정렬순서변경 -->
				</c:if>

			<div>
				<div class="flex_box gap-08">
					<a href="/seller/item/create" class="btn btn-dark-gray btn-mini"> ${op:message('M00773')}</a> <!-- 상품등록 -->
					<a href="javascript:downloadExcel()" class="btn btn-dark-gray btn-mini"> ${op:message('M00254')}</a> <!-- 엑셀 다운로드 -->
					<!-- <a href="javascript:uploadExcel()" class="btn btn-dark-gray btn-mini"> ${op:message('M00793')}</a> <!-- 엑셀 업로드 -->
	...
	<script type="text/javascript">
        $(function() {
            ...
            $('.delete_item').on('click', function(e) {
                ...
                Common.updateListData("/seller/item/list/delete", Message.get("M00306") , function(response) { // 선택된 데이터를 삭제하시겠습니까?
                	if(response.isSuccess == true) {
                		alert("답례품 삭제가 신청되었습니다.\n관리자의 승인 후 해당 답례품이 삭제처리됩니다.");
                		location.href = "/seller/item/pending/list";
                	}
                });
            });
        ...
        function downloadExcel() {
			...
			Common.popup("/seller/item/download-excel?" + $("#itemParam").serialize(), "download-excel", 600, 550, 0);
        }
        ...
        function updateListDataDisplay(flag) {
            ...
            if ($('#listForm').find('input[name=id]:checked').size() == 0) {
                ...
                Common.updateListData("/seller/item/list/update-display/" + flag, "정보를 수정 하시겠습니까?");
            }
        }
        ...
        function updateListDataLabel(flag) {
            ...
            if ($('#listForm').find('input[name=id]:checked').size() == 0) {
                ...
                Common.updateListData("/seller/item/list/update-label/" + flag, "정보를 수정 하시겠습니까?");
            }
        }
