...
<form:form modelAttribute="orderParam" action="" method="post">
	...
	<div class="board_write">
		...
		<div class="btn_all">
			<div class="btn_left"> 
				<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/seller/order/shipping'"><span><c:out value="${op:message('M00047')}"/><!-- 초기화 --></span></button>
			</div> 
			<div class="btn_right">
				<button type="submit" class="btn btn-dark-gray btn-sm"><span><c:out value="${op:message('M00048')}"/><!-- 검색 --></span></button>
			</div>
		</div>
	</div>
	...
	<div class="count_title mt20">
		<h5>
			<c:out value="${op:message('M00039')}"/> : <c:out value="${op:numberFormat(totalCount)}"/> <c:out value="${op:message('M00743')}"/>
		</h5>
		<span>
			...
			<tbody>
				<c:forEach items="${ list }" var="orderReturnApply" varStatus="index">
					...
					<tr>
						...
						<td>
							<c:out value="${op:datetime(orderItem.returnRequestDate)}"/>
						...
						<td>
							<a href="javascript:Common.popup('/seller/order/return-request/order-detail/${orderItem.orderCode}', 'returnRequestDetail', 1000, 700, 1);"><c:out value="${orderItem.orderCode}"/></a>
							<c:if test="${fn:escapeXml(orderItem.orderItemStatus) != '45'}">
								<button type="button" class="btn btn-gradient btn-xs" onclick="Common.popup('/seller/order/return-request/process/${fn:escapeXml(orderItem.orderItemId)}', 'returnRequestProcess', 700, 500, 1)">환불처리</button>
								<button type="button" class="btn btn-gradient btn-xs" onclick="Manager.Order.orderExchangeApplyView('return-request', ${fn:escapeXml(orderItem.orderItemId)})">교환으로변경</button>
							</c:if>
						</td>
						<td class="left">
						...
						<td><c:out value="${op:numberFormat(orderItem.price)}"/>원</td>
						<td><c:out value="${op:numberFormat(orderItem.optionPrice)}"/>원</td>
						<td><strong><c:out value="${orderReturnApply.applyQuantity}"/>개</strong></td>
						<td><c:out value="${op:numberFormat(orderItem.saleAmount)}"/>원</td>
						<td><c:out value="${orderItem.orderItemStatusLabel}"/></td>
