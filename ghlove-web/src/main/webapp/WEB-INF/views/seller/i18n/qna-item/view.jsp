...
<%@ taglib prefix="shop"	uri="/WEB-INF/tlds/shop" %>
		...
		<form id="qnaParam" method="post" action="/seller/qna-item/list">
			...
			<input type="hidden" name="qnaId" value="${fn:escapeXml(qnaId)}" />
		</form>

		<form:form modelAttribute="qna" method="post" action="/seller/qna-item/delete/${fn:escapeXml(qnaId)}">
								...
								<div>
									<c:choose>
										<c:when test='${op:property("saleson.view.type") eq "api"}'>
											<a href="${op:property('saleson.url.frontend')}/items/details.html?code=${fn:escapeXml(qna.itemUserCode)}" target="_blank">${fn:escapeXml(qna.itemName)}</a>
										</c:when>
										<c:otherwise>
											<a href="/products/preview/${fn:escapeXml(qna.itemUserCode)}" target="_blank">${fn:escapeXml(qna.itemName)}</a>
										</c:otherwise>
									</c:choose>
		...
		<div class="board_write">
			<c:choose>
				<c:when test="${qnaAnswer.qnaAnswerId > 0}">
					<form:form modelAttribute="qnaAnswer" action="/seller/qna-item/answer/${fn:escapeXml(qna.qnaId)}" method="post">
						<tbody>
					...
					<div class="btn_center">
						...
						<a href="/seller/qna-item/delete/${fn:escapeXml(qnaId)}" id="delete_data" class="btn btn-default">삭제</a>
						<%-- <a href="/seller/qna-item/answer/${qnaId}" class="btn btn-active">수정</a> --%>
						<a href="javascript:qnaAnswer(${fn:escapeXml(qnaId)});" class="btn btn-active">수정</a>
					</div>
				</c:when>
				<c:otherwise>
					<!-- 버튼시작 -->
					<div class="btn_center">
						...
						<a href="javascript:qnaAnswer(${fn:escapeXml(qnaId)});" class="btn btn-active">답변등록</a>
					</div>
				</c:otherwise>
			</c:choose>
		</div>
