													...
													<c:if test="${categoriesTeam.categoryTeamFlag == 'Y'}">
														...
														<c:forEach items="${categoriesTeam.categoriesGroupList}" var="categoriesGroup">
															<c:if test="${categoriesGroup.categoryGroupFlag == 'Y'}">
																<form:option value="${fn:escapeXml(categoriesGroup.categoryGroupId)}" label="${categoriesGroup.groupName}" />
															</c:if>
														</c:forEach>
														</optgroup>
													</c:if>
						...
						<table class="board_list_table" summary="전체상품리스트">
							...
							<c:forEach items="${list}" var="item" varStatus="i">
								...
								<tr>
									...
									<td>
										<div>
											<a href="javascript:Link.view('/seller/item/edit/${fn:escapeXml(item.itemId)}')"><img src="${ shop:loadImage(item.itemUserCode, item.itemImage, 'XS') }" class="item_image" alt="상품이미지" /></a>
										</div>
									</td>
									<td class="left break-word">

										<a href="javascript:Link.view('/seller/item/edit/${fn:escapeXml(item.itemUserCode)}')" class="break-word">${fn:escapeXml(item.itemName)}</a>
									...
									<td class="text-right">
										${op:numberFormat(item.displaySupplyPrice)}원
									</td>
									<td class="text-right">
										${op:numberFormat(item.displayCommissionRate)}%
									</td>
									<td>
										${fn:escapeXml(item.shippingTypeText)}
									</td>
									<td class="text-right">
									...
									<td>
										<c:if test="${itemSaleStatusText == '판매중'}">
											<a href="javascript:Link.view('/seller/item/sale/edit/${fn:escapeXml(item.itemId)}')" class="btn btn-gradient btn-xs">가격변경</a><br />
										</c:if>
										<a href="/seller/item/copy/${fn:escapeXml(item.itemId)}" class="btn btn-gradient btn-xs">${op:message('M00788')}</a><br />
