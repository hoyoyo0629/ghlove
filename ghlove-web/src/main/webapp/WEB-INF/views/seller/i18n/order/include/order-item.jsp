...
<c:if test="${not empty orderItem}">
	...
	<table class="board_write_table">
		<caption><c:out value="${op:message('M00059')}"/></caption> <!-- 주문정보 --> 
		<colgroup> 						
			<col style="width:15%;" />
			<col />
			<col style="width:15%;" />
		...
		<tbody id="order_items">
			<tr>
				...
				<td class="tleft">
					<div> 
						<c:out value="${orderItem.orderItemStatusLabel}"/>
					</div>
				</td>
				<th class="label">상품번호</th>
				<td class="tleft">
					<div> 
						<c:out value="${orderItem.itemCode}"/>
					</div>
				</td>
				<th class="label">판매가</th>
				<td class="tleft">
					<div>
						<c:out value="${op:numberFormat(orderItem.price)}"/>원
					</div>
				</td>
			</tr>
			<tr>
				...
				<td class="tleft">
					<div>
						<c:out value="${op:numberFormat(orderItem.optionPrice)}"/>원
					</div>
				</td>
			...
			<tr>
				...
				<td>
					<div>
						...
						<c:out value="${op:numberFormat(orderItem.sellerDiscountAmount)}"/>원
					</div>
				</td>
				<th class="label">판매자 할인 상세</th>
				<td colspan="3" class="tleft">
					<div><c:out value="${orderItem.sellerDiscountDetail}"/></div>
				</td>
			</tr>
			<tr>
				...
				<td>
					<div>
						...
						<c:out value="${op:numberFormat(orderItem.adminDiscountAmount)}"/>원
					</div>
				</td>
				<th class="label">운영사 할인 상세</th>
				<td colspan="3" class="tleft">
					<div><c:out value="${orderItem.adminDiscountDetail}"/></div>
				</td>
			</tr>
			<tr>   
				<th class="label">판매 수수료</th>
				<td>
					<div><c:out value="${op:numberFormat(orderItem.commissionPrice)}"/>원</div>
				</td>
				<th class="label">정산예정 금액</th>
				<td colspan="3">
					<div>
						<c:out value="${op:numberFormat(orderItem.supplyPrice)}"/>원
						<c:if test="${orderItem.payShipping > 0}">
							(배송료 : +<c:out value="${op:numberFormat(orderItem.payShipping)}"/>원)
						</c:if>
					</div>
				</td>
			</tr> 
			<tr>   
				<th class="label"><c:out value="${op:message('M00018')}"/></th> <!-- 상품명 -->
			...
			<tr>   
				<th class="label"><c:out value="${op:message('M01624')}"/></th> <!-- 사은품정보 -->
				<td colspan="5" class="tleft">
					<div>
						<c:out value="${orderItem.freeGiftName}"/>
					</div>
				</td>
			</tr>
		</tbody>
