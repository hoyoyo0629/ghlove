...
<%
	...
	propPath = request.getAttribute("prop-path").toString();

	sResult = sResult+"- propPath ["+propPath+"]<br>\n";

	// 서명 데이타가 있을때 서명 검증
	if (sSignData != null && sSignData.length() > 0){
		try{
			sResult = sResult+" 서명 데이타가 있을때 서명 검증 <br>\n";
			// 내부망 인증서 검증하려면 아래 주석 처리합니다. (1) (1~6번)
			JCaosCheckCert jcaosCheck = new JCaosCheckCert(propPath);
			sResult = sResult+" JCaosCheckCert <br>\n";
			...
			*/
			...
			returnResult = "서명 검증에 실패 하였습니다.";
// 			e.printStackTrace();
// 			sResult = "서명 검증에 실패 하였습니다.\n [" + e.getMessage() +"]\");";
// 			returnResult = "서명 검증에 실패 하였습니다. [" + e.getMessage()+"]\");";
		}
	}else{
		sResult=" - 서명 데이타가 존재하지 않습니다..<br>\n";
		returnResult=" - 서명 데이타가 존재하지 않습니다.";

	}
// 	out.print(sResult);
	////System.out.println(sResult);

	request.setAttribute("iResult", iResult);
	request.setAttribute("returnResult", returnResult);
	request.setAttribute("signerDN", signerDN);
	request.setAttribute("sResult", sResult);
	request.setAttribute("sSourceText", sSourceText);
	request.setAttribute("signOrigin", signOrigin);
			...
			<tbody>
				<tr>
					...
					<table class="normal" cellspacing="0">
						<tr>
							<td>서명원문</td>
							<td id="signOrigin">${fn:escapeXml(signOrigin) }</td>
						</tr>
						<tr>
							<td>서명결과</td>
							<td id="signResult">${fn:escapeXml(sSourceText) }</td>
						</tr>
						<tr>
							<td>일치여부</td>
							<td id="resultArea"></td>
...
$(window).load(function(){
	...
	if (("${fn:escapeXml(iResult) }" == "0" || "${fn:escapeXml(iResult) }" == "3000") && "${fn:escapeXml(returnResult.replace("\"", ""))}" == "") {
		result = {
            "result" : "SUCCESS",
            "message" : "",
            "DN" : "${fn:escapeXml(signerDN)}"
        };
	} else {
		result = {
            "result" : "FAIL",
