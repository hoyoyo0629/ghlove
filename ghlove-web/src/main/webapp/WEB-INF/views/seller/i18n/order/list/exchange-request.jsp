...
<form:form modelAttribute="orderParam" action="" method="post">
	...
	<div class="board_write">
		...
		<div class="btn_all">
			<div class="btn_left">
				<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/seller/order/exchange-request'"><span><c:out value="${op:message('M00047')}"/><!-- 초기화 --></span></button>
			</div>
			<div class="btn_right">
				...
				<c:forEach items="${ list }" var="orderShipping" varStatus="index">
					<c:forEach items="${orderShipping.orderItems}" var="orderItem">
						...
						<tr>
							...
							<td>
								<c:out value="${op:datetime(orderItem.exchangeRequestDate)}"/>
							...
							<td>
								<a href="javascript:Common.popup('/seller/order/exchange-request/order-detail/${fn:escapeXml(orderItem.orderCode)}', 'exchangeRequestDetail', 1000, 700, 1);"><c:out value="${fn:escapeXml(orderItem.orderCode)}"/></a>
								<button type="button" class="btn btn-gradient btn-xs" onclick="Common.popup('/seller/order/exchange-request/process/${fn:escapeXml(orderItem.orderItemId)}', 'exchangeRequestProcess', 700, 500, 1)">교환처리</button>
								<button type="button" class="btn btn-gradient btn-xs" onclick="Manager.Order.orderReturnApplyView('exchange-request', ${fn:escapeXml(orderItem.orderItemId)})">환불로변경</button>
							</td>
							<td class="left">
							...
							<td><strong><c:out value="${orderItem.quantity}"/>개</strong></td>
