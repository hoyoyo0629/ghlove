					...
					<div>
						<c:choose>
							<c:when test="${qna.userId > 0 && !empty qna.loginId}">
								<a href="javascript:Common.popup('/opmanager/user/popup/details/${fn:escapeXml(qna.userId)}', '/user/popup/details', 1100, 800 ,1, 0, 0)"><c:out value="${qna.userName}"/> (<c:out value="${qna.loginId}"/>)</a>
							</c:when>
...
<div class="board_write">
	...
	<form:form modelAttribute="qnaAnswer" action="/opmanager/qna/answer/${fn:escapeXml(qna.qnaId)}" method="post">
		...
		<table class="board_write_table">
			...
			<tbody>
				<tr>
					<td class="label"><c:out value="${op:message('M00475')}"/></td> <!-- 답변자 -->
					<td>
						<div>
							<c:out value="${qnaAnswer.userId}"/>
					    </div>
					</td>
					<td class="label"><c:out value="${op:message('M00476')}"/></td> <!-- 답변일 -->
					<td>
						<div>
							<c:out value="${op:datetime(qnaAnswer.createdDate)}"/>
					    </div>
					</td>
				</tr>
				<tr>
		...
		<div class="btn_center">
			<div>
				...
				<button type="button" class="btn btn-default" onclick="location.href='/opmanager/qna/view/${fn:escapeXml(qnaId)}';"><span>취소</span></button>
			</div>
		</div>
	</form:form>
</div>
