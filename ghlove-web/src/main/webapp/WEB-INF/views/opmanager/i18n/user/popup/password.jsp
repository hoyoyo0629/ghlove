					...
					<c:otherwise>
						<c:choose>
							<c:when test="${mailConfig.buyerSendFlag == 'Y'}">
								<p class="mb5 text-left">※Email : <c:out value="${user.email}"/></p>
							</c:when>
							<c:otherwise>
					...
					<c:otherwise>
						<c:choose>
							<c:when test="${smsConfig.buyerSendFlag == 'Y'}">
								<p class="mb5 text-left">※SMS : <c:out value="${user.userDetail.phoneNumber}"/></p>
							</c:when>
							<c:otherwise>
	...
	$(function() {
		...
		$(".email").on("click",function(){
			Common.confirm("비밀번호를 재발급 하시겠습니까?\n임시 비밀번호가 회원의 이메일주소로 발송됩니다.", function() {
				location.href = '/opmanager/user/popup/password/${fn:escapeXml(user.userId)}/email';
			});
		});
		
		$(".sms").on("click",function(){
			Common.confirm("비밀번호를 재발급 하시겠습니까?\n임시 비밀번호가 회원의 SMS로 발송됩니다.", function() {
				location.href = '/opmanager/user/popup/password/${fn:escapeXml(user.userId)}/sms';
			});
		});
	});
</script>