...
<form:form modelAttribute="inquiry" action="/opmanager/inquiry/answer" method="post">
    <form:hidden path="inquiryId" value="${fn:escapeXml(inquiry.inquiryId) }"/>
    <form:hidden path="telNumber" value="${fn:escapeXml(inquiry.telNumber) }"/>
    <form:hidden path="inquiryType" value="${fn:escapeXml(inquiry.inquiryType) }"/>
    ...
    <table cellpadding="0" cellspacing="0" summary="" class="board_list_table inquiry_view" style="width:80%;">
            ...
            <td class="tleft">
                <c:choose>
                    <c:when test="${inquiry.userEmail != '' && inquiry.userEmail != null }">
                        회원(<c:out value="${inquiry.userEmail }"/>)
                    </c:when>
                    <c:otherwise>
                        비회원
                    </c:otherwise>
        ...
        <tr>
            <th scope="row">작성일</th>
            <td class="tleft">
                   <c:out value="${op:date(inquiry.createdDate) }"/> 
            </td>
        </tr>
        <tr>
            <th scope="row">상품명</th>
            <td class="tleft">
                <c:choose>
                    <c:when test="${inquiry.itemCode != null && inquiry.itemCode != '' }">
                        <a href="javascript:window.open('/products/view/${fn:escapeXml(inquiry.itemCode)} ', 'newWindow');"><c:out value="${inquiry.itemName }"/> (<c:out value="${inquiry.itemCode }"/>)</a>
                    </c:when>
                    <c:otherwise>
                        -
                    </c:otherwise>
        ...
        <tr>
            ...
            <td class="tleft">
                <a type="application/octet-stream" href="#" download="${fn:escapeXml(inquiry.inquiryImgName)}" onclick="downloadFile('${fn:escapeXml(inquiry.inquiryImgName)}');">
                    <p><c:out value="${inquiry.inquiryImgName}"/></p>
                </a>
            </td>
        </tr>
                ...
                <td class="tleft">
                    <table class="inner-table">
                        ...
                        <c:forEach items="${answerList}" var="answer">
                            <tr>
                                <td class="tleft"><c:out value="${op:nl2br(answer.content)}"/></td>
                                <td><c:out value="${op:datetime(answer.createdDate)}"/></td>
                            </tr>
                        </c:forEach>
                        </tbody>
                    </table>
