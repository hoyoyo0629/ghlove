						...
						<table class="board_list_table" summary="전체상품리스트">
							...
							<c:forEach items="${list}" var="item" varStatus="i">
								...
								<tr>
									...
									<td>
										<div>
											<a href="javascript:Link.view('/opmanager/item/seller/edit/${fn:escapeXml(item.itemId)}')"><img src="${fn:escapeXml(item.imageSrc)}" class="item_image" alt="상품이미지" /></a>
										</div>
									</td>
									<td class="left break-word">
										
										<a href="javascript:Link.view('/opmanager/item/seller/edit/${fn:escapeXml(item.itemId)}')" class="break-word">${fn:escapeXml(item.itemName)}</a>
										<a href="javascript:Link.view('/opmanager/item/seller/edit/${fn:escapeXml(item.itemId)}', 1)" class="break-word" style="color:#517BAB">[새탭]</a>
										<c:if test="${item.itemLabel == '2'}">
											<img src="/content/opmanager/images/icon/icon_new2.gif" alt="new" />
										</c:if>
										
										<br/>(${fn:escapeXml(item.itemUserCode)})
									</td>
									...
									<td>
										<a href="/opmanager/item/copy/${fn:escapeXml(item.itemId)}" class="table_btn">${op:message('M00788')}</a> <!-- 복사 -->
