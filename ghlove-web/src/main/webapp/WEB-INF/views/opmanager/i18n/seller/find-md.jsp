...
span.pagination {width: 100%; text-align: center;}
	...
	<div class="popup_wrap">
		...
		<form:form modelAttribute="userSearchParam" method="post" enctype="multipart/form-data">
					...
					<table class="board_list_table">
						...
						<tbody>
							...
							<tr
								...
								data-user-id="${fn:escapeXml(user.userId)}"
								data-login-id="${fn:escapeXml(user.loginId)}"
								data-user-name="${fn:escapeXml(user.userName)}"
								data-email="${fn:escapeXml(user.email)}">
								<td><div>${fn:escapeXml(user.loginId)}</div></td>
								<td><div>${fn:escapeXml(user.userName)}</div></td>
								<td><div>${fn:escapeXml(user.email)}</div></td>
