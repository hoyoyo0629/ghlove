				...
				<tr>
					...
					<td>
						<c:choose>
							...
							<c:otherwise>
								일반 거래처<br/>[<c:out value="${user.customerCode}"/>]
							</c:otherwise>
						</c:choose>
					</td>
					...
					<td style="text-align:left;">
						[<c:out value="${user.zipcode}"/>] <c:out value="${user.address}"/><br/>
						<c:out value="${user.addressDetail}"/>
					</td>
					<td>
						<c:choose>
							<c:when test="${user.userId > 0}">
								<a href="javascript:newUserOrder('${fn:escapeXml(user.userId)}')">[주문하기]</a>
							</c:when>
							<c:otherwise>
								<a href="javascript:newGuestOrder('${fn:escapeXml(user.customerCode)}')">[주문하기]</a>
							</c:otherwise>
						</c:choose>
					</td>
				</tr>
