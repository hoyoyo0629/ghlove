...
<div class="board_list">
			...
			<tbody>
                <c:forEach items="${list}" var="orderItem" varStatus="index">
                        ...
                        <tr style="background:${fn:escapeXml(tdColor)};">
                            ...
                            <td>
                                <a href="${requestContext.sellerPage ? '/seller' : '/opmanager'}/order/new-order-mobile/order-detail/${fn:escapeXml(orderItem.orderSequence)}/${fn:escapeXml(orderItem.orderCode)}">${fn:escapeXml(orderItem.orderCode)}</a>
                            </td>
                            <td>
                            ...
                            <td>
                                <input type="hidden" name="shippingReadys[${fn:escapeXml(index.index)}].key" value="${fn:escapeXml(orderItem.itemKey)}" />
                                <select name="shippingReadys[${fn:escapeXml(index.index)}].quantity" class="hidden">
	...
	<div class="flex_box juc-sbt">
		...
		<div class="btn_all">
			<div class="flex_box gap-08">
				<!-- <button type="button" class="btn btn-dark-gray btn-mini shipping-ready">발송지시</button> -->
...
<div>
    <ul class="list-bullet point">
        <li>
            <strong>"배송지시"</strong> 하시면 해당 주문은 <strong>"발송준비중(모바일)"</strong>로 이동하며 <strong>배송정보는 저장되지 않습니다.</strong>
        </li>
        <li>
            <strong>"임시저장"</strong> 하시면 해당 주문의 <strong>배송정보를 저장</strong>합니다.
        </li>
        <li>
...
<script type="text/javascript">
	$(function(){
		...
		$('.shipping-ready').on('click', function() {
			...
			//if (confirm("선택하신 주문상품을 발송지시 처리 하시겠습니까?")) {
		...
		$('.shipping-cancel').on('click', function() {
			...
			if (confirm("선택하신 답례품을 배송지시 취소 하시겠습니까?\n해당 주문은 신규주문 목록으로 이동 합니다.")) {
		...
		$('.shipping-start-send-message').on('click', function() {
			...
			if (confirm("선택하신 답례품을 배송처리 하시겠습니까?")) {
