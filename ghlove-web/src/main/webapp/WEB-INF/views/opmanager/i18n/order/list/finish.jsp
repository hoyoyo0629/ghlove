	...
	<div class="board_write">
		...
		<table class="board_write_table" summary="${fn:escapeXml(title)}">
			...
			<tbody>
				...
				<tr>
				 	<td class="label">배송완료일</td>
				 	<td colspan="3">
				 		<div>
				 			<form:select path="searchDateType" class="hidden">
				 				<form:option value="OI.SHIPPING_FINISH_DATE" label="배송완료일" />
                    ...
                    <td>
				 	...
				 	<td colspan="${(requestContext.sellerPage == false && adminRole == 'SYS') ? '0' : '3'}">
				 		<div class="flex_box gap-08">
							...
							<div class="checkbox">
								<form:checkbox path="shippingStatus" value="58" label=" 교환완료" />
							</div>
							<%-- <div class="checkbox">
								<form:checkbox path="shippingStatus" value="35" label=" 발송완료" />
							</div> --%>
						</div>
				 	</td>
				 </tr>
		...
		<div class="btn_all btn_right">
			<div class="flex_box gap-08">
				<button type="button" class="btn btn-dark-gray btn-mini" onclick="location.href='${requestContext.sellerPage ? '/seller' : '/opmanager'}/order/finish'"> <c:out value="${op:message('M00047')}"/><!-- 초기화 --></button>
...
<div class="board_list">
			...
			<tbody>
				<c:forEach items="${list}" var="orderItem" varStatus="index">
					...
					<tr>
						...
						<td>
							<c:out value="${op:datetime(orderItem.shippingFinishDate)}"/>
						...
						<td>
							<a href="${fn:escapeXml(requestContext.sellerPage) ? '/seller' : '/opmanager'}/order/finish/order-detail/${fn:escapeXml(orderItem.orderSequence)}/${fn:escapeXml(orderItem.orderCode)}"><c:out value="${orderItem.orderCode}"/></a><br />
						</td>
						<td>
						...
						<td>
							<div>
								<%-- <c:choose>
									<c:when test="${!empty orderItem.mobileNumber}">
										발송완료 (<c:out value="${orderItem.mobileNumber}"/>)
									</c:when>
									<c:otherwise> --%>
							...
							<div>
								<c:choose>
									<c:when test="${empty orderItem.deliveryNumber && empty orderItem.mobileNumber}">
										<c:out value="${orderItem.deliveryCompanyName == null ? '택배사 정보 없음' : orderItem.deliveryCompanyName == '' ? '택배사 정보 없음' : orderItem.deliveryCompanyName}"/><br /> (<c:out value="송장번호 없음"/>)
									</c:when>
									<c:when test="${!empty orderItem.deliveryNumber}">
										<c:out value="${orderItem.deliveryCompanyName == null ? '택배사 정보 없음' : orderItem.deliveryCompanyName == '' ? '택배사 정보 없음' : orderItem.deliveryCompanyName}"/><br /> (<c:out value="${orderItem.deliveryNumber}"/>)
									</c:when>
								</c:choose>
							</div>
						</td>
	...
	<div class="flex_box juc-sbt">
		<div class="btn_all">
			<div class="flex_box gap-08">
				<button type="button" class="btn btn-dark-gray btn-mini" onclick="downloadOrderExcel( ${totalCount})">엑셀</button>
			</div>
		</div>
	</div>
	--%>
