	...
	<tbody>
		...
		<c:forEach items="${orderPayments}" var="payment">
			...
			<c:if test="${payment.paymentType == '1' && payment.remainingAmount > 0}">
				...
				<tr>
					<td class="text-center" rowspan="${fn:escapeXml(rowspan)}">${fn:escapeXml(payment.approvalTypeLabel)}
					<c:if test="${payment.escrowStatus ne 'N' }">(에스크로)</c:if>
					</td>
					<td class="text-right" rowspan="${fn:escapeXml(rowspan)}">
						${op:numberFormat(payment.amount)}${fn:escapeXml(unit)}
						<c:if test="${payment.amount != payment.remainingAmount && payment.remainingAmount > 0}">
							<br/><strong>(잔여액 : ${op:numberFormat(payment.remainingAmount)}${fn:escapeXml(unit)})</strong>
						</c:if>
					</td>
					<td class="text-right" rowspan="${fn:escapeXml(rowspan)}">

						취소 가능 금액 : ${op:numberFormat(payment.remainingAmount)}${fn:escapeXml(unit)}

						<input type="hidden" name="changePayments[${fn:escapeXml(index)}].paymentSequence" value="${fn:escapeXml(payment.paymentSequence)}" />
                        <input type="text" name="changePayments[${fn:escapeXml(index)}].cancelAmount" data-remaining-amount="${fn:escapeXml(payment.remainingAmount)}" maxlength="9" class="_number form-amount op-pay-amount payAmounts" value="0"> <span class="price-unit">${fn:escapeXml(unit)}</span>
					</td>
								...
								<tbody>
									<tr>
                                        ...
                                        <td style="border-right: 0px; padding-right: 0;">
                                            ...
                                            <select name="changePayments[${fn:escapeXml(index)}].returnBankName" title="은행명" class="form-block required">
                                                <c:forEach items="${bankListByKey}" var="bankList">
                                                    <option value="${fn:escapeXml(bankList.key.id)}" ${op:selected(bankList.label, returnBankName)}>${fn:escapeXml(bankList.label)}</option>	<!-- 2017.04.28 Jun-Eu Son 은행명선택 수정	 -->
                                                </c:forEach>
                                            </select>
                                        </td>
                                        <td>
                                            <input type="text" name="changePayments[${fn:escapeXml(index)}].returnBankVirtualNo" title="계좌번호" value="${fn:escapeXml(payment.returnBankVirtualNo)}" class="required _number form-block" placeholder="계좌번호" />
                                            </div>
										</td>

										<th>예금주</th>
										<td>
											<input type="text" name="changePayments[${fn:escapeXml(index)}].returnBankInName" title="예금주" value="${fn:escapeXml(payment.returnBankInName)}" class="required form-block" />
										</td>
									</tr>
             ...
             <td colspan="2" style="background: #fcf8e8; padding: 10px">
                <table class="inner-table">
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" name="refundAccountNumber" value="${fn:escapeXml(refund.returnVirtualNo)}" title="계좌번호" class="_number form-block" placeholder="계좌번호" />
                            </div>
                        </td>

                        <th>예금주</th>
                        <td>
                            <input type="text" name="refundAccountName" value="${fn:escapeXml(refund.returnBankInName)}" title="예금주" class="form-block" />
                        </td>
                    </tr>
		...
		<table class="inner-table">
			...
			<tbody>
				...
				<c:forEach items="${orderPayments}" var="payment" >
					<c:if test="${empty payment.payDate}">
						<tr>
							<td><label><input type="checkbox" name="deletePaymentIds" value="${fn:escapeXml(payment.paymentSequence)}" /> ${fn:escapeXml(payment.approvalTypeLabel)} 취소</label></td>
							<td class="text-right" data-wait-amount="${fn:escapeXml(payment.amount)}">${op:numberFormat(payment.amount)}원</td>
						</tr>
						<c:set var="index">${index + 1}</c:set>
