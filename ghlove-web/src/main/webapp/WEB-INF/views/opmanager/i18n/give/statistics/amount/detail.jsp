...
<form:form modelAttribute="searchParam" method="post">
    ...
    <div class="btn_all btn_right">
        <div class="flex_box juc-sbt">
            ...
            <c:if test="${adminRole ne 'LOC'}">
                <div>
                    <button type="button" class="btn btn-default btn-mini" onClick="location.href='/opmanager/give/statistics/amount'">목록</button>
                </div>
            </c:if>
        </div>
    </div>
  ...
  <h3 class="mt50 fs24"><span>월별 기부금액통계</span></h3>
            ...
            <li>
                ...
                <div class="bar_wrap">
                    <p class="bar yellow" style="width: ${fn:escapeXml(list.percent)}%">
                        <span class="number" title=""><c:out value="${op:numberFormat(list.giveAmt)}" /></span>
...
<h3 id="day_stats_title" class="mt50 fs24"><span>2022년 1월 일별 기부금액통계</span></h3>
<div id="day_stats">
...
<script type="text/javascript">
    ...
    function detail(locgovCode, cntrMonth) {
        ...
        $.post('/opmanager/give/statistics/amount/' + locgovCode, params, function(resp) {
            ...
            if (resp.isSuccess) {
                ...
                for (var i in resp.data) {
                    ...
                    html += '               <span class="number">' + Common.numberFormat(ele.giveAmt) + '</span>';
                    html += '           </p>';
                    html += '       </div>';
                    html += '   </div>';
                    html += '</td>';
    ...
    function downloadExcel(locgovCode) {
        Shop.downloadExcelOrder("/opmanager/give/statistics/amount/" + locgovCode + "/excel", $('#searchParam').serialize(), true);
    }
