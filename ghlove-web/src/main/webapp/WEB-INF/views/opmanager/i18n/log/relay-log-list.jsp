...
<h3><span>연계로그관리</span></h3>

<form:form modelAttribute="relayLogParam" cssClass="opmanager-search-form clear" method="post" id="relayLogParam">
	...
	<div class="board_write">
		<table class="board_write_table" summary="연계로그관리">
			<colgroup>
				<col style="width:15%;">
				<col style="width:35%;">
				<col style="width:15%;">
			...
			<tbody>
				<tr>
					<td class="label">연계구분</td>
					<td>
						<div class="flex_box gap-08">
							<form:select path="srchRelayType" title="아이디" class="wd-150">
								<form:option value="">전체</form:option>
								<c:forEach items="${cmmnCodeList}" var="item" varStatus="i">
									<form:option value="${fn:escapeXml(item.id)}">${fn:escapeXml(item.label)}</form:option>
								</c:forEach>
							</form:select>
						</div>
					</td>
					...
					<td>
						<div class="flex_box gap-08">
							<form:select path="srchRelayResultCode" title="아이디" class="wd-150">
								<form:option value="">전체</form:option>
								<form:option value="100">성공</form:option>
								<form:option value="999">실패</form:option>
							</form:select>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td colspan="3">
						<div class="flex_box gap-08">
							<form:select path="srchType" title="검색구분" class="wd-150">
								<form:option value="">전체</form:option>
								<form:option value="LOGIN_ID">로그인아이디</form:option>
								<form:option value="USER_NAME">사용자명</form:option>
							</form:select>
							<form:input path="srchTxt" title="검색구분" class="input_txt required _filter wd-500" type="text" value="${fn:escapeXml(relayLogParam.srchTxt)}"/>
						</div>
					</td>
				</tr>
				<tr>
		...
		<div class="btn_all btn_right">
			<div class="flex_box gap-08">
				<button type="button" class="btn btn-dark-gray btn-mini" onclick="location.href='/opmanager/log/relay-log'">초기화</button>
				...
				<c:when test="${not empty list}">
					<c:forEach items="${list}" var="item" varStatus="i">
						<tr style="background:#fff;">
							...
							<td><div><c:out value="${item.userId}"/></div></td>
							<td><div><c:out value="${item.loginId}"/></div></td>
							<td><div><c:out value="${item.userName}"/></div></td>
							<td><div><c:out value="${item.relayTypeName}"/></div></td>
							<td><div><c:out value="${item.cntrLocgovCodeName}"/></div></td>
							<td><div><c:out value="${item.elctrnPayNo}"/></div></td>
							<td><div><c:out value="${item.relayResultCode}"/></div></td>
