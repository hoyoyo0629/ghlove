...
<c:if test="${requestContext.sellerPage == false}">
	<h3 class="mt50 fs24"><span>▫ [${fn:escapeXml(seller.companyName)}] ${op:date(remittanceParam.startDate)}</span></h3>
</c:if>
...
<div class="board_write">
		...
		<table class="board_write_table">
			...
			<tbody>
				<tr>
					...
					<td>
						<div>${fn:escapeXml(seller.representativeName)}</div>
					</td>
				...
				<tr>
					...
					<td>
						<div>${fn:escapeXml(seller.phoneNumber)}</div>
					</td>
					<td class="label">연락처</td>
					<td>
						<div>${fn:escapeXml(seller.telephoneNumber)}</div>
					</td>
				</tr>
			</tbody>
		</table>
				...
				<c:if test="${list != null && !list.isEmpty()}">
					<c:forEach items="${list}" var="item" varStatus="index">
						<tr ${item.remittanceStatusCode == '2' ? 'style="background-color:#fff;"' : ''}>
							...
							<td>${op:date(item.payDate.substring(0, 8))}</td>
							<td>${fn:escapeXml(item.orderCode)}</td>
							<td>${fn:escapeXml(item.itemUserCode)}</td>
							<td>${fn:escapeXml(item.itemName)}</td>
							<td>${shop:viewItemOptions(item.setItemFlag, item.options)}</td>
							<td>${fn:escapeXml(item.buyerName)}</td>
							<td>
								<c:choose>
									<c:when test="${item.commissionBasePrice != null && item.commissionBasePrice > 0}">
										${op:numberFormat(item.commissionBasePrice * item.quantity + item.etcAmt)}
									</c:when>
									<c:otherwise><!-- 리얼커머스 데이터용 -->
										${op:numberFormat(item.salePrice * item.quantity + item.etcAmt)}
									</c:otherwise>
								</c:choose>
							</td>
							<td>${op:numberFormat(item.quantity)}개</td>
							<td>
								${op:numberFormat(item.remittancePrice * item.quantity + item.etcAmt)}
							</td>
						</tr>
					</c:forEach>
				</c:if>
				...
				<c:if test="${list != null && !list.isEmpty()}">
					<c:forEach items="${list}" var="item" varStatus="index">
						<tr ${item.remittanceStatusCode == '2' ? 'style="background-color:#fff;"' : ''}>
							<td>${op:numberFormat(pagination.itemNumber - index.count)}</td>
							<td>${op:date(item.payDate.substring(0, 8))}</td>
							<td>${op:date(item.confirmDate.substring(0, 8))}</td>
							<td>${fn:escapeXml(item.orderCode)}</td>
							<td>${fn:escapeXml(item.itemUserCode)}</td>
							<td>${fn:escapeXml(item.itemName)}</td>
							<td>
								${shop:viewItemOptions(item.setItemFlag, item.options)}
							</td>
							<td>${fn:escapeXml(item.buyerName)}</td>
	...
	<div class="flex_box juc-sbt">
		...
		<div class="btn_all btn_right">
			<div class="flex_box gap-08">
				<a href="${requestContext.sellerPage ? '/seller' : '/opmanager'}/temp-process/remittance/list" class="btn btn-dark-gray btn-mini">목록</a>
			</div>
		</div>
	</div>
...
<script type="text/javascript">
	...
	function excelDownload() {
		...
		Shop.downloadExcelOrder("${fn:escapeXml(requestContext.sellerPage) ? '/seller' : '/opmanager'}/temp-process/remittance/view-excel/${fn:escapeXml(remittanceParam.remittanceId)}", param, true);
	}
</script>