    ...
    <form:form modelAttribute="configPg" method="post">
        ...
        <table class="board_write_table pg-info" summary="PG 정보">
            ...
            <tbody>
                        ...
                        <div>
                            <form:select path="pgType" title="결제타입">
                                ...
                                <c:forEach var="list" items="${pgTypes}">
                                    <form:option value="${fn:escapeXml(list.code)}" disabled="${fn:escapeXml(list.code) == 'NICEPAY' ? 'false' : 'true'}"><c:out value="${list.title }"/></form:option>
                                </c:forEach>
                            </form:select>
                        </div>
                    </td>
                        ...
                        <div>
                            <form:select path="cashbillServiceType" title="결제타입">
                                ...
                                <c:forEach var="list" items="${cashbillServiceTypes}">
                                    <form:option value="${fn:escapeXml(list.code)}" disabled="${fn:escapeXml(list.code) == 'NICEPAY' ? 'false' : 'true'}"><c:out value="${fn:escapeXml(list.title)}"/> </form:option>
                                </c:forEach>
                            </form:select>
                        </div>
                    </td>
                ...
                <tr class="nicepay">
                    <td class="label">특정 카드사 선택 정보</td>
                    <td>
                        <div>
                            <form:input path="cardCode" class="form-block" title="특정 카드사 선택 정보" />
                        </div>
                    </td>
                </tr>
                <tr class="all">
...
<script type="text/javascript">
    ...
    function displayEvent() {
        $('#pgType').on('change', function(){
            initDisplay($(this).val(), true);
        });
    }

    function initDisplay(pgType, initValueFlag) {
        ...
        if (typeof pgType != 'undefined' && pgType != '') {
            ...
            if (initValueFlag) {
                $('.pg-info').find('input').val('');
                $('.pg-info').find('input:radio').eq(0).trigger('click');
            }
