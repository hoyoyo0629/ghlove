...
<form:form modelAttribute="gifSeoulParam" cssClass="opmanager-search-form clear" method="post" id="gifSeoulParam">
	...
	<div class="board_write">
		<table class="board_write_table" summary="서울 수납 로그">
			<colgroup>
				<col style="width:15%;">
				<col style="width:35%;">
				<col style="width:15%;">
				...
				<tr>
					...
					<td>
						<div class="flex_box gap-08">
							<form:select path="srchRstCd" title="아이디" class="wd-150">
...
<div class="board_list mt-40">
	...
	<table class="board_list_table" summary="서울 수납 로그">
		<caption>서울 수납 로그</caption>
		<colgroup>
			<col style="width:13%;">
			<col style="width:10%;">
			<col style="width:10%;">
				...
				<c:when test="${not empty list}">
					<c:forEach items="${list}" var="item" varStatus="i">
						<tr style="background:#fff;">
							...
							<td><div><c:out value="${item.comReqMeche}"/></div></td>
							<td><div><c:out value="${item.orgC}"/></div></td>
							<td><div><c:out value="${item.sunapYn}"/></div></td>
							<td style="text-align: right;"><div><fmt:formatNumber value="${fn:escapeXml(item.sunapAmt)}" pattern="#,###"/></div></td>
							<td><div><c:out value="${item.sunapDt}"/></div></td>
							<td><div><c:out value="${item.rstCd}"/></div></td>
							<td><div><c:out value="${item.rstMsg}"/></div></td>
