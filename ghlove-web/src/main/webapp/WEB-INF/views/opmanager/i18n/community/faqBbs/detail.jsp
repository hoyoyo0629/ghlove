		...
		<div class="center">
			<div class="s-contents list-page-wrapper">
				...
				<div>
					...
					<div style="padding: 10px 0px; margin: 10px 0px; font-size: 20px; border-bottom: 1px solid #000">댓글<span style="color: red; margin-left: 10px;">[<c:out value="${faqBbsCmntListCnt}" />]</span></div>
					...
					<div id="cmntList" style="border-top: 1px solid #000">
						<c:forEach items="${detail.cmntyFaqBbsCmntList}" var="cmnt" varStatus="i">
							...
							<form:form class="cmntFileUploadForm" id="cmntFileUploadForm${cmnt.cmntId}" method="post" action='/opmanager/community/faqBbs/cmnt/upload' enctype="multipart/form-data">
			...
			<div class="btn-box">
				...
				<c:if test="${role eq 'ROLE_ADMIN_1' }">				<%-- 시스템주관리자만 FAQ 수정삭제 가능 --%>
					<button type="button" class="btn btn-dark-gray btn-small"
					onClick="location.href='/opmanager/community/faqBbs/edit/${detail.bbsId}'">
...
<script type="text/javascript" src="/content/modules/cmnty/faqBbsCmnt.js?<spring:eval expression="@environment.getProperty('jsVersion')" />"></script>
...
function downloadItemImage(fileId) {
	location.href = '/opmanager/community/faqBbs/file-download/'+fileId
}

function deleteItem(bbsId) {
	var url = "/opmanager/community/faqBbs/deleteFaqBbs/" + bbsId
	if(confirm("등록된 게시글을 삭제합니다. 글 삭제 시 댓글도 함께 삭제됩니다.")){
	$.post(url, {}, function(response) {
		Common.responseHandler(response, function() {
			alert(response.data);
			location.href = "/opmanager/community/faqBbs/list";
		});
	});
	}
}
...
function addCmnt() {
	...
	let cmntyFaqBbsCmntDto = {bbsId : $('#bbsId').val(), cmntCn : $('#cmntCn').val()};
	$.post("/opmanager/community/faqBbs/cmnt/add/", cmntyFaqBbsCmntDto, function(response) {
		Common.responseHandler(response, function() {
			if(response.isSuccess) {
				alert(response.data);
				location.href = "/opmanager/community/faqBbs/detail/"+$('#bbsId').val();
			}
		});
	});
}
...
function deleteItemImage(fileId) {
	...
	$.post('/opmanager/community/faqBbs/cmnt/deleteFile', param, function(response){
...
function downloadCmntItemImage(fileId) {
	location.href = '/opmanager/community/faqBbs/cmnt/file-download/'+fileId
}
</script>
