...
<form:form modelAttribute="searchParam" cssClass="opmanager-search-form clear" method="post" id="searchForm">
	...
	<div class="board_write">
		...
		<div class="btn_all btn_right">
			<div class="flex_box gap-08">
				<button type="button" class="btn btn-dark-gray btn-mini" onclick="location.href='/opmanager/designated-donation/request/list';">초기화</button>
...
<div class="count_title mt-40">
	<h5>총 <fmt:formatNumber value="${fn:escapeXml(searchParam.pagination.totalItems)}" pattern="#,###"/>건</h5>
	<span>
...
<script type="text/javascript">
	...
	function detailsPopup(userId, reqstSn) {
		var url = '/opmanager/designated-donation/request/popup/details/'+userId+'/'+reqstSn;
		var popupName = '/opmanager/designated-donation/request/popup/details';
	...
	function histroyPopup(userId) {
		var url = '/opmanager/designated-donation/request/popup/history/'+userId;
		var popupName = '/opmanager/designated-donation/request/popup/history';
			...
			$.post('/opmanager/user/locgov-charger/delete', {"userIdList": [userId]}, function(response) {
				if(response.isSuccess && response.data) {
					if(response.data.code == "SUCC") {
						alert("삭제되었습니다.");
						location.reload();
					} else if (response.data.code == "ERR_NOT_ALLOW") {
						alert("권한을 확인해주세요.\n(시스템관리자, 행안부담당자, 지자체 주 관리자만 가능)");
					} else {
						alert("문제가 발생했습니다.");
					}
				}
