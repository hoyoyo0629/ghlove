...
<div class="popup_wrap">
	...
	<h1 class="popup_title"><c:out value="${op:message('M00709')}"/></h1> <!-- 배송비 수정 -->
			...
			<table class="board_write_table" id="addWriteTable">
				...
				<tbody>
					...
					<tr>
						...
						<td>
					 		<div>
					 			<form:input type="text" path="deliveryCharge" class="freeText required one _number" title="${op:message('M00067')}"/> <c:out value="${op:message('M00049')}"/> <!-- 원 -->
				 			 	<span class="amountText">
				 			 		<form:input type="text" path="deliveryFreeAmount" class="freeAmountText required one _number" title="배송비"/> <c:out value="${op:message('M00641')}"/> <!-- 원 이상 구매시 배송비 무료 -->
				 			 	</span>
							</div>
						</td>
					</tr>
					<form:input type="hidden" path="deliveryChargeId" value="${fn:escapeXml(deliveryCharge.deliveryChargeId)}" />
					<form:input type="hidden" path="deliveryId" value="${fn:escapeXml(deliveryCharge.deliveryId)}" />
				</tbody>
			</table>
			<p class="btn_center">
				<button type="submit" class="btn btn-active"><span><c:out value="${op:message('M00101')}"/></span></button> &nbsp; <!-- 저장 -->
				<a href="javascript:self.close();" class="btn btn-default"><span><c:out value="${op:message('M00037')}"/></span></a> <!-- 취소 -->
			</p>
		</form:form>
	</div>
