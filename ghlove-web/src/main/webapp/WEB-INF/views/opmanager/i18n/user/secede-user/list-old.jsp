...
<%@ taglib prefix="op" 		uri="/WEB-INF/tlds/functions" %>
	...
	<form:form modelAttribute="searchParam" cssClass="opmanager-search-form clear" method="post">
		<fieldset>
			...
			<div class="btn_all">
				<div class="btn_left">
					<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/opmanager/user/secede-user/list';"><span class="glyphicon glyphicon-repeat"></span> <c:out value="${op:message('M00047')}"/></button> <!-- 초기화 -->
				</div>
				<div class="btn_right">
	...
	<div class="count_title mt20">
		<h5>
			<c:out value="${op:message('M00238')}"/> : <c:out value="${op:numberFormat(pagination.totalItems)}"/><c:out value="${op:message('M01220')}"/><!-- 명 -->
		</h5>
		<span>
				...
				<tbody>
					<c:forEach items="${list}" var="user" varStatus="i">
						<tr>
							 ...
							 <td><c:out value="${user.userDetail.leaveReason}"/></td>
							 <%-- <td>${user.userName}</td> --%>
							 <td>
								<c:out value="${op:date(user.leaveDate)}"/>
							 </td>
						</tr>
					</c:forEach>
				</tbody>
