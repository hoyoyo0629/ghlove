...
<div class="statistics_web">
	...
	<div class="board_list">
				...
				<c:forEach items="${sellerStatsList}" var="list" varStatus="i">
					<tr class="${i.count % 2 == 0 ? 'even' : '' }">
						...
						<td rowspan="${fn:length(list.groupStats) }" style="text-align: left;">
							<a href="javascript:Common.popup('/opmanager/shop-statistics/sales/day/detail?sd=${fn:escapeXml(statisticsParam.startDate)}&ed=${fn:escapeXml(statisticsParam.endDate)}&sellerId=${fn:escapeXml(list.sellerId)}&where=${fn:escapeXml(statisticsParam.where)}&query=${fn:escapeXml(statisticsParam.query)}&type=detail', 'order-popup-seller', '1600', '800', 1);">
							    <c:out value="${list.sellerName}"/>
							</a>
						</td>
		...
		<sec:authorize access="hasRole('ROLE_EXCEL')">
			<div class="btn_all">
				<div class="right">
					<a href="/opmanager/shop-statistics/sales/seller/excel-download?${fn:escapeXml(queryString)}"  class="btn btn-success btn-sm"><span class="glyphicon glyphicon-save"></span><c:out value="${op:message('M00254')}"/></a> <!-- 엑셀 다운로드 -->
				</div>
			</div>
		</sec:authorize>
