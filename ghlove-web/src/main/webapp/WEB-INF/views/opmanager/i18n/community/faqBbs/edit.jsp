...
<form:form modelAttribute="cmntyFaqBbsDto" method="post" action='${fn:escapeXml(requestContext.requestUri)}' enctype="multipart/form-data">
	...
	<div class="board_write">
		<table class="board_write_table" summary="${op:message('M00269')}">
			...
			<tbody>
				...
				<tr>
					<td class="label"><span class="required_mark">*</span><c:out value="${op:message('M00676')}"/></td>
					<td>
						<div>
							<form:select path="faqType" title="${op:message('M00677')}"> <!-- 질문유형 선택 -->
								<c:forEach var="list" items="${faqTypes}">
									<form:option value="${fn:escapeXml(list.code)}"><c:out value="${fn:escapeXml(list.title)}"/></form:option>
								</c:forEach>
							</form:select>
						</div>
					</td>
...
$(function() {
	try{
		$('#cmntyFaqBbsDto').validator(function() {
...
$(function(){
	$("#cmntyFaqBbsDto").on("submit", function(e){
...
function deleteItemImage(fileId) {
	...
	$.post('/opmanager/community/faqBbs/delete-item-image', param, function(response){
