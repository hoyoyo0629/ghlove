						 	...
						 	<td colspan="3">
						 		<div>
								    ...
								    <input type="text" id="pointDefault" name="pointDefault" title="${op:message('M00426')}" class="form-sm text-center _number required" maxlength="${pointDefaultType == 1 ? 3 : 5}"
								           value="${fn:escapeXml(config.pointDefaultText)}"/>
								    <%--
				...
				<table id="item_point_config" class="board_list_table" summary="${op:message('M00503')}"> <!-- 포인트 추가 -->
					...
					<tbody id="item_point_area">
						<c:forEach items="${pointConfig}" var="pointConfig" varStatus="i">
							<tr>
								<td>
									<input type="text" name="point" id="point_${fn:escapeXml(i.count)}" value="${fn:escapeXml(pointConfig.pointText)}" maxlength="${fn:escapeXml(pointConfig.pointType) == 1 ? 3 : 5}" class="three required _number" title="${op:message('M00246')}" />
									<input type="hidden" name="pointType" value="${fn:escapeXml(pointConfig.pointType)}" />

									<input type="radio" name="R${fn:escapeXml(pointConfig.pointConfigId)}" id="R${fn:escapeXml(pointConfig.pointConfigId)}1" ${op:checked('1', pointConfig.pointType)} class="lengthChange" value="1"> <label for="R${fn:escapeXml(ointConfig.pointConfigId)}1">%</label>
									<input type="radio" name="R${fn:escapeXml(pointConfig.pointConfigId)}" id="R${fn:escapeXml(pointConfig.pointConfigId)}2" ${op:checked('2', pointConfig.pointType)} class="lengthChange" value="2"> <label for="R${fn:escapeXml(ointConfig.pointConfigId)}2">P</label>
								</td>
								<td>
									<span class="datepicker"><input type="text" name="startDate" class="term required _date" maxlength="8" value="${fn:escapeXml(pointConfig.startDate)}" title="${op:message('M00507')}" /></span> <!-- 시작일 -->
									...
									<span class="datepicker"><input type="text" name="endDate" class="term required _date" maxlength="8" value="${fn:escapeXml(pointConfig.endDate)}" title="${op:message('M00509')}" /></span> <!-- 종료일 -->
