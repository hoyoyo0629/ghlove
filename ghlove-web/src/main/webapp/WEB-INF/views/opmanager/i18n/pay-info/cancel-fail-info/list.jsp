...
<!-- 2022.06.23 -->
<h3>결제취소 실패 현황</h3>
<form:form modelAttribute="orderCancelFailParam" method="post">
        ...
        <table class="board_write_table">
            ...
            <tr>
                ...
                <td>
                    <div>
                        ...
                        <%-- 해당화면 사용할 경우 'op-pagination' class 추가필요 --%>
                ...
                <tr>
                    ...
                    <td>
                        <div>
                            ...
                            <form:radiobutton path="pgServiceType" value="${fn:escapeXml(configPg.pgType)}" label="${fn:escapeXml(configPg.pgType)}" />
            ...
            <tr>
                <div>
                    ...
                    <td>
                        <div>
                            <span class="datepicker"><form:input type="text" path="searchStartDate" class="datepicker _number" title="${op:message('M00507')}" value="${fn:escapeXml(today)}" /></span> <!-- 시작일 -->
                            <span class="wave">~</span>
                            <span class="datepicker"><form:input type="text" path="searchEndDate" class="datepicker _number" title="${op:message('M00509')}" value="${fn:escapeXml(today)}" /></span> <!-- 종료일 -->
                            <span class="day_btns">
    ...
    <div class="btn_all">
        <div class="btn_left">
            <button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/opmanager/pay-info/cancel-fail-info/list'"><span><c:out value="${op:message('M00047')}"/></span></button> <!-- 초기화 -->
        </div>
        <div class="btn_right">
    ...
    <div class="count_title mt20">
        <h5>전체 : <c:out value="${op:numberFormat(pageContent.totalElements)}"/> 건 조회</h5>
        <span>출력수
...
<form action="/opmanager/pay-info/cancel-fail-info/list" method="post" id="listForm">
    ...
    <div class="board_write">
        <table class="board_list_table" summary="결제취소 실패 현황">
            ...
            <c:forEach items="${pageContent.content}" var="content" varStatus="i">
                <tr>
                    <td><c:out value="${op:numbering(pageContent, i.index)}"/></td>
                    <td><c:out value="${content.orderCode}"/></td>
                    <td><c:out value="${content.pgKey}"/></td>
                    <td><c:out value="${content.approvalType}"/></td>
                    <td><c:out value="${content.payDate}"/></td>
                    <td><c:out value="${op:numberFormat(content.cancelAmount) }"/>원</td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
