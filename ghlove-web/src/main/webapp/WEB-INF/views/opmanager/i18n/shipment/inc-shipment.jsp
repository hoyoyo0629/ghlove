	      	...
	      	<div id="sm-display-area1">
	      		<table class="inner-table th-center">
					...
					<tr>
						...
						<th>주소</th>    		
						<th>연락처</th>    		
						<th class="hidden">배송비</th>    		
						<th class="hidden">도서산간<br />추가배송비</th>    		
						<th class="hidden">통합배송비</th>   
					</tr>
					<c:forEach items="${list}" var="shipment">
	      				<tr class="listCount">
	      					<td class="text-center">
	      						<input type="radio" name="shipmentId" value="${fn:escapeXml(shipment.shipmentId)}" />
	      					...
	      					<td class="text-center">
	      						...
	      						<span class="text-info ${shipment.defaultAddressFlag == 'Y' ? '' : 'hide'}">(기본)</span>
	      					</td>
	      					...
	      					<td>
	      						...
	      						<span class="zipcode">(<c:out value="${shipment.zipcode}"/>)</span>
	      					...
	      					<td class="text-center hidden">
	      						<span class="shipping hide"><c:out value="${shipment.shipping}"/></span>
	      						<span class="shippingFreeAmount hide"><c:out value="${shipment.shippingFreeAmount}"/></span>
	      						
	      						<c:out value="${op:numberFormat(shipment.shipping)}"/>원<br />
	      						(<c:out value="${op:numberFormat(shipment.shippingFreeAmount)}"/> 이상 무료)	
	      					</td>
	      					<td class="text-center hidden">
	      						<span class="shippingExtraCharge1 hide"><c:out value="${shipment.shippingExtraCharge1}"/></span>
	      						<span class="shippingExtraCharge2 hide"><c:out value="${shipment.shippingExtraCharge2}"/></span>
	      						
	      						<span class="display-inline-block">제주</span> : <c:out value="${op:numberFormat(shipment.shippingExtraCharge1)}"/>원<br />
	      						<span class="display-inline-block">도서</span> : <c:out value="${op:numberFormat(shipment.shippingExtraCharge2)}"/>원
	      					</td>
	      					<td class="text-center hidden">
	      						<span class="shipmentGroupCode hide"><c:out value="${shipment.shipmentGroupCode}"/></span>
	      						<c:choose>
	      			...
	      			<div class="btn_all flex_box juc-sbt">
		      			...
		      			<c:if test="${hasModifyPermission}">
			      			<div class="btn_left gap-08">
			      				<button type="button" class="btn btn-active btn-create" id="btnCreate">출고지 <c:out value="${op:message('M00088')}"/></button> <!-- 등록 -->
								<button type="button" class="btn btn-active btn-edit hide" id="btnUpdate"><c:out value="${op:message('M00087')}"/></button> <!-- 수정 -->
      	...
      	<c:if test="${hasModifyPermission}">
      		<div id="sm-display-area2" style="display: none">
	      		<form:form modelAttribute="shipment" method="post" enctype="multipart/form-data">
	      			...
	      			<input type="hidden" name="sellerId" value="${fn:escapeXml(shipment.sellerId)}"/>
		      		...
		      		<table class="board_write_table">
		      			...
		      			<tr>
		      				...
		      				<td>
		      					<div>
		      						<form:input path="name" class="required" title="상호명" />
		      					</div>
		      				</td>
		      			</tr>
		      			<tr>
		      			...
		      			<tr>
		      				...
		      				<td>
		      					<div class="flex_box gap-08">
		      						<input type="text" id="zipcode" name="zipcode" maxlength="7" class="form-sm text-center required" readonly="readonly" style="width: 70px;" title="주소"/>
			      					<a href="#" class="btn btn-default btn-mini" id="find-address">주소찾기</a>
		      					</div>
		      					<div class="flex_box gap-08">
		      			...
		      			<tr class="hidden">
		      				...
		      				<td>
		      					<form:input path="shipping" maxlength="8" class="amount _number" title="배송비" /> 원 | 
		      					<form:input path="shippingFreeAmount" maxlength="8" class="amount _number" title="배송비 무료 금액" /> 원 이상 구매 시 무료배송. 
		      				</td>
		      			</tr>
		      			<tr class="hidden">
		      				<td class="label">도서산간 추가 배송비</td>
		      				<td>
		      					<span class="display-inline-block" style="width:50px;">제주도</span> : 
		      					<form:input path="shippingExtraCharge1" maxlength="8" class="amount _number" title="제주도 추가 배송비" /> 원 <br />
		      					
		      					<div class="line-group">
		      						<span class="display-inline-block" style="width:50px;">도서산간</span> : 
		      						<form:input path="shippingExtraCharge2" maxlength="8" class="amount _number" title="도서산간 추가 배송비" /> 원
		      					</div>
		      				</td>
		      			</tr>
		      			<tr class="hidden">
		      				<td class="label">통합배송비 부과 설정</td>
		      				<td>
		      					...
		      					<form:select path="shipmentGroupCode">
		      						...
		      						<c:forEach begin="1" end="10" var="i">
		      							<form:option value="${shipmentGroupCodeValue}_${i}"> 통합배송정책<c:out value="${i}"/></form:option>
		      						</c:forEach>
...
function shipmentEvent() {
	...
	$('.btn-create').on('click', function() {
		...
		$('#shipping').val('0');
		$('#shippingFreeAmount').val('0');
		message = "저장하시겠습니까?";
		$('#shipment input[name=shipmentId]').val('0');
		$('#shipment').attr('action', '${fn:escapeXml(requestContext.managerUri)}/shipment/create');
	...
	$('.btn-edit').on('click', function() {
		...
		var $radidElement = $('input[name=shipmentId]:checked');
		changeShipmentId($radidElement);
	...
	$('.btn-edit1').on('click', function() {
		...
		$('#shipment').attr('action', '${fn:escapeXml(requestContext.managerUri)}/shipment/edit');
	});
...
function shipmentDelete() {
	...
	if (confirm("배송정보를 삭제하시겠습니까?")) {
		$('#shipment').attr('action', '${fn:escapeXml(requestContext.managerUri)}/shipment/delete');
		$('#shipment').submit();
	}
}
...
// 출고지 라디오 버튼 클릭시 주소 값 세팅
function changeShipmentId($radidElement){
	...
	shipment.shipmentId = $radidElement.val();
