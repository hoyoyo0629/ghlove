...
<h3>1:1 문의</h3>
<form:form modelAttribute="inquiryParam" method="post">
    ...
    <div class="count_title mt20">
        <h5>전체 : ${fn:escapeXml(inquiryCount)} 건 조회</h5>
        <span>출력수
...
<!-- 2015.1.6 -->
<form action="/opmanager/inquiry/list" method="post" id="listForm">
    ...
    <div class="board_write">
        <table class="board_list_table" summary="${op:message('M00273')}"> <!-- 주문내역 리스트 -->
            ...
            <c:forEach items="${inquiryList}" var="list" varStatus="i">
                <tr>
                    <td>
                        <input type="checkbox" id="inquiryIds" name="inquiryIds" value="${fn:escapeXml(list.inquiryId)}" title=${op:message('M00169')} />
                    </td>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${list.itemCode != null && list.itemCode != ''}">
                                <a href="javascript:window.open('/products/view/${fn:escapeXml(list.itemCode)} ', 'newWindow');">${fn:escapeXml(list.itemName) } (${fn:escapeXml(list.itemCode)})</a>
                            </c:when>
                            <c:otherwise> - </c:otherwise>
                        </c:choose>
                    </td>
                    <td style="text-align:center;">
                        <a href="view/${fn:escapeXml(list.inquiryId) }">${op:strcut(list.inquiryContent, 20) }</a> <!-- 내용 -->
                    </td>
                    <td>
                            ${fn:escapeXml(list.telNumber) }
                    </td>
