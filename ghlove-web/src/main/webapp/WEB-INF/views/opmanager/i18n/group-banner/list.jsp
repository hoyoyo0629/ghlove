			...
			<tbody>
				...
				<c:forEach items="${categoriesTeamGroupList}" var="team" varStatus="i">
					<c:if test="${team.code != 'sale_outlets'}">
						...
						<tr class="team_tr">
							<td rowspan="${fn:escapeXml(rowspan)}"><c:out value="${team.name}"/></td>
							<c:forEach items="${team.categoriesGroupList}" var="group" varStatus="j">
								<c:if test="${j.index > 0}"></tr><tr></c:if>
									<td class="tex_l" style="border-left: 1px solid #ececec;"><c:out value="${group.groupName}"/></td>
									<td>
										<c:set var="count" value="0" />
										<c:if test="${not empty group.groupBanners}">
											<c:forEach items="${group.groupBanners}" var="banner" varStatus="i">
												<img src="${fn:escapeXml(banner.imageSrc)}" title="${fn:escapeXml(banner.title)}" width="${fn:escapeXml(banner.thumbnailWidth / 3)}" height="${fn:escapeXml(banner.thumbnailHeight / 3)}" />
												<c:if test="${ i.first == false }">
													&nbsp;&nbsp;&nbsp;
												</c:if>

												<c:set var="count"><c:out value="${count + 1}"/></c:set>
											</c:forEach>
										</c:if>
										...
										<c:if test="${not empty group.itemList}">
											...
											<c:out value="${ group.itemList }"/> 상품 출력<br/>(배너가 등록되어 있을경우 배너 3개를 우선적으로 출력후 노출됨)
											<c:set var="count"><c:out value="${count + 1}"/></c:set>
										</c:if>
									...
									<td>
										<a href="/opmanager/group-banner/form/${fn:escapeXml(group.categoryGroupId)}" class="btn btn-gradient btn-xs" title="수정"><c:out value="${op:message('M00087')}"/></a>  <!-- 수정 -->
									</td>
							</c:forEach>
						</tr>
					</c:if>
