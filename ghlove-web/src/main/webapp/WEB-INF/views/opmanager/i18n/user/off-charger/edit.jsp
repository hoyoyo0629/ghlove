...
<div class="btn_all btn_right mb15">
	<div class="flex_box gap-08">
		<c:choose>
			<c:when test="${adminRoleType eq 'SYS' or (adminRoleType eq 'OFF' and adminRoleGrade eq 'M')}">
			...
			<c:otherwise>
				<button type="button" class="btn btn-dark-gray btn-mini" onclick="chargerEdit();">저장</button>
			</c:otherwise>
		</c:choose>
	</div>
</div>
	...
	<table class="board_write_table" summary="">
		...
		<tbody>
						...
						<c:choose>
							<c:when test="${adminRoleType eq 'SYS' or (adminRoleType eq 'OFF' and adminRoleGrade eq 'M')}">
								<div class="input-form">
									<input id="authorityM" name="authority" type="radio" value="ROLE_ADMIN_7"
										 ${details.authority eq 'ROLE_ADMIN_7' ? 'checked' : ''}>
									<label for="authorityM">오프라인 주담당자</label>
								</div>
								<div class="input-form">
									<input id="authorityS" name="authority" type="radio" value="ROLE_ADMIN_8"
										 ${details.authority eq 'ROLE_ADMIN_8' ? 'checked' : ''}>
									<label for="authorityS">오프라인 부담당자</label>
								</div>
							</c:when>
							<c:otherwise>
								<input name="authority" type="radio" value="${fn:escapeXml(details.authority)}" style="display:none;" checked>
								<c:out value="${details.authority eq 'ROLE_ADMIN_7' ? '오프라인 주담당자' : '오프라인 부담당자'}"/>
							</c:otherwise>
						</c:choose>
					</div>
				</td>
			...
			<tr>
				...
				<td>
					<div>
						<input id="userName" title="이름" class="input_txt required _filter wd-200" type="text" value="${fn:escapeXml(details.userName)}">
					</div>
				</td>
			</tr>
				...
				<td>
					<div class="flex_box gap-08 item-center">
						<c:choose>
							...
							<c:otherwise>
								<input type="hidden" id="bankCode" value="${fn:escapeXml(details.bankCode)}" />
								<input title="" class="input_txt required _filter wd-100" type="text" value="${fn:escapeXml(details.bankNm)}" disabled>
							</c:otherwise>
						</c:choose>
						<input id="psitnNm" title="" class="input_txt required _filter wd-150" type="text" value="${details.psitnNm}">
					</div>
				</td>
			</tr>
			<tr>
				...
				<td>
					<div class="flex_box gap-08 item-center">
						<input id="empId" title="개인번호" class="input_txt required _filter wd-200" type="text" value="${fn:escapeXml(details.empId)}" maxlength="20">
					</div>
				</td>
			</tr>
			<tr>
				...
				<td>
					<div>
						<input id="email" title="EMAIL" class="input_txt required _filter wd-200" type="text" value="${fn:escapeXml(details.email.split('@')[0])}">&nbsp;@
			...
			<c:if test="${adminRoleType eq 'SYS' or (adminRoleType eq 'OFF' and adminRoleGrade eq 'M')}">
				<tr>
...
<c:if test="${adminRoleType eq 'SYS' or (adminRoleType eq 'OFF' and adminRoleGrade eq 'M')}">
	<div class="btn_all btn_right">
		<div class="flex_box gap-08">
			<button type="button" class="btn btn-dark-gray btn-small" onclick="passwordInit();">비밀번호 초기화 및 신규 비밀번호 발급</button>
		</div>
	</div>
</c:if>
...
<script type="text/javascript">
	$(function() {
		...
		if("${fn:escapeXml(details.userId)}" == "") {
			...
			let adminRoleType = '${fn:escapeXml(adminRoleType)}';
			if(adminRoleType == "SYS" || (adminRoleType == "OFF" && adminRoleType == "M")) {
				moveListPage();
			} else {
	...
	function chargerEdit() {
		...
		var resultMsg = "저장되었습니다.";

		if("${fn:escapeXml(adminRoleType)}" == "SYS" || "${fn:escapeXml(adminRoleType)}" == "OFF" && "${fn:escapeXml(adminRoleGrade)}" == "M") {
			confirmMsg = "정보를 수정 하시겠습니까?";
			resultMsg = "수정되었습니다.";
		}
...
// 				"telNumber": $("#telNumber1").val()+"-"+ $("#telNumber2").val()+"-"+$("#telNumber3").val(),
			...
			$.post("/opmanager/user/off-charger/edit", param, function(response) {
				if(response.isSuccess && response.data) {
					...
					} else if(response.data.code == "NEED_AUTH_SWAP") {
						if(confirm("현재 주관리자가 2명이므로 권한이 승계됩니다. \n권한 승계 후 현재 로그인한 계정은 계정 중지 처리됩니다.\n승계하시겠습니까?")) {
							// 승계 확인한 경우 권한 이관 처리
							$.post("/opmanager/user/off-charger/auth-swap", param, function(response) {
								if(response.data.code == "AUTH_CHANGED_LOGOUT") {
									alert("수정되었습니다. 권한이 변경되어 로그아웃됩니다.");
...
// 		}
	...
	function chargerDelete() {
		if(confirm("삭제하시겠습니까?")) {
			$.post("/opmanager/user/off-charger/delete", {"userIdList": ["${fn:escapeXml(details.userId)}"]}, function(response) {
				if(response.isSuccess && response.data) {
					if(response.data.code == "SUCC") {
						alert("삭제되었습니다.");
	...
	/**
	 *	함 수 명 : passwordInit
	 *	기	능  : 비밀번호 초기화 및 신규 비밀번호 발급
	 */
	function passwordInit() {
		var url = "/opmanager/user/off-charger/popup/password-init/${fn:escapeXml(details.userId)}";
		var popupName = "/opmanager/user/off-charger/popup/password-init";
		Common.popup(url, popupName, 500, 320 ,1, 0, 0);
	}
</script>