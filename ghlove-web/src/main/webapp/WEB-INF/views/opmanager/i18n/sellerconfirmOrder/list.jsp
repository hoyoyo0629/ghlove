...
<!--전체주문 내역 시작-->
<h3><span>${ fn:escapeXml(title) }<!-- 전체주문 내역 --></span></h3>
<form:form modelAttribute="orderSearchParam" action="" method="post">
			...
			<tbody>
				 	...
				 	<td>
				 		<div>
				 			<form:select path="searchDateType">
				 				...
				 				<form:option value="OI.CREATED_DATE" label="${op:message('M00023')}" /> <!-- 주문 일자 -->
				 ...
				 <c:if test="${ not empty searchOrderStatusList }">
					 <tr>
					 	...
					 	<td>
					 		<div>
					 			<form:checkboxes items="${ op:nullToEmptyArr(searchOrderStatusList) }" path="searchOrderStatus" itemLabel="label" itemValue="value" />
							</div>
					 	</td>
					 </tr>
				 </c:if>
	...
	<div class="btn_all">
		<div class="btn_left">
			<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/opmanager/sellerconfirm/list<c:if test="${ !empty searchOrderStatus }">/${fn:escapeXml(searchOrderStatus)}</c:if>'"><span class="glyphicon glyphicon-repeat"></span> ${op:message('M00047')}<!-- 초기화 --></button>
		</div>
		<div class="btn_right">
 	...
 	<div class="board_guide" style="border:1px solid #d5d5d5; padding: 15px; margin-top:15px;">
		...
		<p class="tip">
			<c:forEach items="${brandList}" var="brand">
				<a href="javascript:downloadBrandExcel('${fn:escapeXml(brand)}')" class="btn_write gray_small"><img src="/content/opmanager/images/icon/icon_excel.png" alt=""><span>${fn:escapeXml(brand)} 주문서 다운로드</span> </a>
			</c:forEach>
...
<c:if test="${searchOrderStatus eq '4' }">
	<div class="board_guide" style="border:1px solid #d5d5d5; padding: 15px; margin-top:15px;">
		...
		<p class="tip">
			<a href="javascript:;" class="btn_write gray_small" onclick="downloadShippingExcelByOpenMarket('11st')"><img src="/content/opmanager/images/icon/icon_excel.png" alt=""><span>11st 엑셀 다운로드</span> </a> <!-- 엑셀 다운로드 -->
			<a href="javascript:;" class="btn_write gray_small" onclick="downloadShippingExcelByOpenMarket('gmarket')"><img src="/content/opmanager/images/icon/icon_excel.png" alt=""><span>지마켓&옥션 엑셀 다운로드</span> </a> <!-- 엑셀 다운로드 -->
			<a href="javascript:;" class="btn_write gray_small" onclick="downloadShippingExcelByOpenMarket('storefarm')"><img src="/content/opmanager/images/icon/icon_excel.png" alt=""><span>스토어팜 엑셀 다운로드</span> </a> <!-- 엑셀 다운로드 -->
			<a href="javascript:;" class="btn_write gray_small"onclick="downloadShippingExcelByOpenMarket('beautycall')"><img src="/content/opmanager/images/icon/icon_excel.png" alt=""><span>뷰티콜 엑셀 다운로드</span> </a> <!-- 엑셀 다운로드 -->
			<a href="javascript:;" class="btn_write gray_small" onclick="downloadShippingExcelByOpenMarket('allthat')"><img src="/content/opmanager/images/icon/icon_excel.png" alt=""><span>올댓 쇼핑 엑셀 다운로드</span> </a> <!-- 엑셀 다운로드 -->
			<a href="javascript:;" class="btn_write gray_small" onclick="downloadShippingExcelByOpenMarket('naver')"><img src="/content/opmanager/images/icon/icon_excel.png" alt=""><span>네이버 페이 엑셀 다운로드</span> </a> <!-- 엑셀 다운로드 -->
		</p>
		<p class="tip">
			<br/>선택한 항목을 다운로드 합니다.
...
<div class="board_guide" style="border:1px solid #d5d5d5; padding: 15px; margin-top:15px;">
	<p class="tip">
		<a href="javascript:;" class="btn_write gray_small" onclick="downloadOrderExcel('${fn:escapeXml(searchOrderStatus)}', ${fn:escapeXml(totalCount)})"><img src="/content/opmanager/images/icon/icon_excel.png" alt=""><span>주문내역 다운로드</span> </a>
	</p>
...
<form id="listForm" name="listForm">
	...
	<div class="sort_area mt15">
		<div>
			...
			<c:if test="${searchOrderStatus eq '3' }">
				...
				<a href="#" class="btn_write gray_small" onclick="downloadShippingTargetExcel()"><img src="/content/opmanager/images/icon/icon_excel.png" alt=""><span>송장 미등록 주문 엑셀다운로드</span> </a> <!-- 엑셀 다운로드 -->
				...
				<a href="#" class="btn_write gray_small" onclick="downloadForDeliveryCompanyExcel()"><img src="/content/opmanager/images/icon/icon_excel.png" alt=""><span>배송업체 전달용 엑셀 다운로드</span> </a> <!-- 엑셀 다운로드 -->
	...
	<div class="sort_area mt15">
		<div>
			...
			<c:if test="${searchOrderStatus eq '3' }">
				<a href="javascript:downloadShippingTargetExcel()" class="btn_write gray_small"><img src="/content/opmanager/images/icon/icon_excel.png" alt=""><span>송장 미등록 주문 엑셀다운로드</span> </a> <!-- 엑셀 다운로드 -->
				<a href="javascript:uploadExcel('shipping')" class="btn_write gray_small"><img src="/content/opmanager/images/icon/icon_excel.png" alt=""><span>송장번호 엑셀업로드</span> </a>
				<a href="#" class="btn_write gray_small" onclick="downloadForDeliveryCompanyExcel()"><img src="/content/opmanager/images/icon/icon_excel.png" alt=""><span>배송업체 전달용 엑셀 다운로드</span> </a> <!-- 엑셀 다운로드 -->
			</c:if>
		</div>
...
function downloadErpExcel(){
	Shop.downloadExcelOrder("/opmanager/order/erp-excel-download", "searchStartDate=" + $('#erpDownDate').val(), false);
}

function downloadErpExcel2(){
	Shop.downloadExcelOrder("/opmanager/order/erp-excel-download2", "searchStartDate=" + $('#erpDownDate').val(), false);
}

function downloadShippingTargetExcel(){
	var $id = $('input[name="id"]:checked');
	if ($id.size() == 0) {
		alert(Message.get("M00308"));	// 처리할 항목을 선택해 주세요.
		return;
	}
	Shop.downloadExcelOrder("/opmanager/order/shipping-target/download-excel", $id.serialize(), false);
}
...
function downloadBrandExcel(brandName) {
	...
	Shop.downloadExcelOrder("/opmanager/order/brand/download-excel", param, false);
}
...
function downloadOrderExcel(searchOrderStatus, totalCount) {
	...
	Shop.downloadExcelOrder("/opmanager/order/excel-download/" + searchOrderStatus, param, true);
}

function downloadShippingExcelByOpenMarket(market){
	var $id = $('input[name="id"]:checked');
	if ($id.size() == 0) {
		alert(Message.get("M00308"));	// 처리할 항목을 선택해 주세요.
		return;
	}
	Shop.downloadExcelOrder("/opmanager/order/shipping/"+market+"/download-excel", $id.serialize(), false);
}

function downloadForDeliveryCompanyExcel(){
	var $id = $('input[name="id"]:checked');
	if ($id.size() == 0) {
		alert(Message.get("M00308"));	// 처리할 항목을 선택해 주세요.
		return;
	}
	Shop.downloadExcelOrder("/3/download-excel", $id.serialize(), false);
}

function showOrderStatusSelector(){
	var showSelectorOrderStatus = ['1','2','3','4','5','9','6'];
	var searchOrderStatus = '${fn:escapeXml(searchOrderStatus)}';
