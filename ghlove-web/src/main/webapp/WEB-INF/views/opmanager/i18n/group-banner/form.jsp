...
<form id="groupBanner" method="post" enctype="multipart/form-data">
	<input type="hidden" name="categoryGroupId" value="${fn:escapeXml(categoryGroupId)}" />
	<input type="hidden" name="uploadFileMaxSize" value="${fn:escapeXml(groupBanner.uploadFileMaxSize)}" />
	<input type="hidden" name="uploadFileMinSize" value="${fn:escapeXml(groupBanner.uploadFileMinSize)}" />
			...
			<tbody>
				...
				<c:forEach items="${list}" var="banner" varStatus="i">
					<tr>
						...
						<td>
							<input type="hidden" name="writeDatas[${fn:escapeXml(i.index)}].categoryGroupBannerId" class="categoryGroupBannerId" value="${fn:escapeXml(banner.categoryGroupBannerId)}" />
							<div>
								<input type="text" name="writeDatas[${fn:escapeXml(i.index)}].title" value="${fn:escapeXml(banner.title)}" class="full input_title" title="배너 타이틀" maxlength="255" />
							</div>
						</td>
					</tr>
					<tr>
						...
						<td>
							<div>
								<input type="text" name="writeDatas[${fn:escapeXml(i.index)}].linkUrl" value="${fn:escapeXml(banner.linkUrl)}" class="full input_linkUrl" title="연결 URL" maxlength="255" />
							</div>
						</td>
					</tr>
					<tr>
						...
						<td>
							<div>
								<input type="hidden" name="writeDatas[${fn:escapeXml(i.index)}].fileName" value="${fn:escapeXml(banner.fileName)}" id="imgFileName${fn:escapeXml(i.index)}" />
								<input type="hidden" name="writeDatas[${fn:escapeXml(i.index)}].deleteFlag" value="${fn:escapeXml(banner.fileName)}" id="deleteFlag${fn:escapeXml(i.index)}" />
								<img src="${fn:escapeXml(banner.imageSrc)}" title="${fn:escapeXml(banner.title)}" width="${fn:escapeXml(banner.thumbnailWidth)}" height="${fn:escapeXml(banner.thumbnailHeight)}" id="imgFile${fn:escapeXml(i.index)}" /> <a href="javascript:deleteItem('${fn:escapeXml(i.index)}')" id="deleteBannerButton${fn:escapeXml(i.index)}">[삭제]</a> <br />
								파일명 : ${fn:escapeXml(banner.fileName)} <br />
								파일 수정 : <input type="file" name="writeDatas[${fn:escapeXml(i.index)}].uploadFile" class="full input_file" title="이미지" accept="image/png, image/jpeg, image/gif" /> (${fn:escapeXml(banner.thumbnailWidth)} X ${fn:escapeXml(banner.thumbnailHeight)})
							</div>
						</td>
					</tr>
				...
				<c:forEach begin="${startIndex}" end="${groupBanner.uploadFileMaxSize - 1}" step="1" varStatus="i">
					...
					<tr>
						...
						<td>
							<div>
								<input type="file" name="writeDatas[${i.index}].uploadFile" class="full input_file" title="이미지" accept="image/png, image/jpeg, image/gif" /> (${groupBanner.thumbnailWidth} X ${groupBanner.thumbnailHeight})
							</div>
						</td>
					</tr>
				</c:forEach>
...
<%-- 								<input type="text" name="itemList" id="itemList" class="full input_itemList" title="상품코드" maxlength="250" value="${categoryGroup.itemList}" readonly="readonly" /> --%>
								<ul id="group-banner" class="sortable_item_relation">
									<c:forEach items="${list2}" var="item" varStatus="i">
										<li id="group-banner_item_${fn:escapeXml(item.itemId)}">
											<input type="hidden" name="itemList" value="${fn:escapeXml(item.itemId)}" />
											<p class="image"><img src="${shop:loadImage(item.itemUserCode, item.itemImage, 'XS')}" class="item_image size-100 none" alt="상품이미지" /></p>
											<p class="title">[${fn:escapeXml(item.itemUserCode)}]<br />${fn:escapeXml(item.itemName)}</p>

											<span class="ordering">${fn:escapeXml(i.count)}</span>
											<a href="javascript:Shop.deleteRelationItem('group-banner_item_${fn:escapeXml(item.itemId)}');" class="delete_item_image"><img src="/content/opmanager/images/icon/icon_x.gif" alt="" /></a>
										</li>
									</c:forEach>
								</ul>
							</div>
