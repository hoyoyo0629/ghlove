...
<div class="statistics_web">
							...
							<div>
								<form:select path="sellerId">
									...
									<c:forEach items="${sellerList}" var="list" varStatus="i">
										<form:option value="${fn:escapeXml(list.sellerId)}">[<c:out value="${fn:escapeXml(list.loginId)}"/>] <c:out value="${fn:escapeXml(list.sellerName)}"/></form:option>
									</c:forEach>
								</form:select>
	...
	<div class="board_list">
		...
		<table class="board_list_table stats ${statisticsParam.displaySubtotal == 'N' ? 'odd-even' : ''}">
			...
			<thead>
				<tr>
					...
					<th rowspan="2" ><c:out value="${op:message('M00018')}"/></th> <!-- 상품명 -->
					<th rowspan="2" class=""><c:out value="${op:message('M01368')}"/></th> <!-- 주문방법 -->
				...
				<c:forEach items="${itemStatsList}" var="list" varStatus="i">
					<tr class="${i.count % 2 == 0 ? 'even' : '' }">
						...
						<td rowspan="${fn:length(list.groupStats) }" style="text-align: left;">
							<a href="javascript:;" onclick="Common.popup('/opmanager/shop-statistics/sales/item/${fn:escapeXml(list.itemId)}?startDate=${fn:escapeXml(statisticsParam.startDate)}&endDate=${fn:escapeXml(statisticsParam.endDate)}', '${fn:escapeXml(list.itemId)}', '1000', '800', 1);">
								
								<c:out value="${list.itemName }"/> (<c:out value="${list.itemUserCode }"/>)
							</a>
						</td>
		...
		<sec:authorize access="hasRole('ROLE_EXCEL')">
			<div class="btn_all">
				<div class="right">
					<a href="/opmanager/shop-statistics/sales/item/excel-download?${fn:escapeXml(queryString)}" class="btn btn-success btn-sm"><span class="glyphicon glyphicon-save"></span><c:out value="${op:message('M00254')}"/></a> <!-- 엑셀 다운로드 -->
				</div>
			</div>
		</sec:authorize>
