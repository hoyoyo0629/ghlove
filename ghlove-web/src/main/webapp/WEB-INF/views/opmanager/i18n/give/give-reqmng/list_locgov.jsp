	                    ...
	                    <div class="flex_box gap-08">
	                        <form:select id="shCntrReqmngCode" path="shCntrReqmngCode" title="요청분류" class="wd-150">
								...
								<c:forEach items="${reqmngCodeList}" var="code" varStatus="status" begin="0" end="1">
									<form:option value="${fn:escapeXml(code.id)}"><c:out value="${fn:escapeXml(code.label)}"/></form:option>
							</c:forEach>
	                        </form:select>
	                    </div>
	                </td>
            ...
            <tr style="background:#fff;">
                <td>
                 ...
                 <td>
                    ...
                    <!-- <c:out value='${list.cntrReqmngCode == 100 ? "과오납" : "포인트생성"}'/> -->
                </td>
               <td>
                ...
                <td>
                    ...
                    <c:out value='${list.discription}' escapeXml="false" /></div>
                </td>
                 <td>
                ...
                <td>
                 	<c:choose>
                 		<c:when test="${list.reqStatusCode eq 100}">
	    					<button type="button" class="btn btn-dark-gray btn-mini" onClick="cancelReq('${fn:escapeXml(list.reqId)}','${fn:escapeXml(list.cntrReqmngCode)}','${fn:escapeXml(list.cntrSn)}')">요청취소</button>
                 		</c:when>
...
function searchEnterKey() {
	$("#shUserName, #shCntrDeStart, #shCntrDeEnd").on('keydown', function(e){
		if (e.keyCode == '13') {
			$("#searchForm").submit();
		}
	});
 ...
 *	기	능  : 변경신청 취소
 */
 function cancelReq(reqID,cntrReqmngCode,cntrSn){
