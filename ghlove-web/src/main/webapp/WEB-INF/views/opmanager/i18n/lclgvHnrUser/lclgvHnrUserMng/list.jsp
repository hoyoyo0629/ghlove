...
<div>
	...
	<div class="item_list">
		<h3><span>기부혜택증 기준목록</span></h3>

		<form:form modelAttribute="lclgvHnrUserMngParam" method="post" id="lclgvHnrUserMngParam">

				<%-- 검색조건 --%>
				<div class="board_write">
				<table class="board_write_table" summary="지자체 기부혜택증 기준목록">
					<caption>지자체 기부혜택증 기준목록</caption>
					<colgroup>
		...
		<div class="btn_all btn_left">
			<ul class="list-bullet point">
				...
				<div class="btn_all btn_right">
					<div class="flex_box gap-08">
						<button type="button" class="btn btn-dark-gray btn-mini" onclick="location.href='${requestContext.sellerPage ? '/seller' : '/opmanager'}/lclgvHnrUser/lclgvHnrUserMng/list'"> <c:out value="${op:message('M00047')}"/><!-- 초기화 --></button>
...
<%-- 	 				<input type="hidden" id="rowNumber" value="${lclgvHnrUserMngList.rowNumber}"> --%>
					...
					<c:choose>
						...
						<tr>
							...
							<td>
							    <c:out value="${lclgvHnrUser.upperLocgovNm}"/>
							</td>
							<td>
								<a href="javascript:lclgvHnrUserDetail('${fn:escapeXml(lclgvHnrUser.lclgvCd)}')">
								    <c:out value="${lclgvHnrUser.lclgvCdNm}"/>
								</a>
							</td>
							<%-- <td>${fn:escapeXml(lclgvHnrUser.gldGrdDntnAmt)}</td>
							<td>${fn:escapeXml(lclgvHnrUser.slvrGrdDntnAmt)}</td> --%>
							<td>${op:numberFormat(lclgvHnrUser.brnzGrdDntnAmt)}</td>
							<td>${fn:escapeXml(lclgvHnrUser.hnrUserSlctnSeCdNm)}</td>
							<td>${fn:escapeXml(lclgvHnrUser.useYn)}</td>
						</tr>
						</c:forEach>
						</c:when>
						<c:otherwise>
...
// 지자체 기부혜택증 상세 보기
function lclgvHnrUserDetail(lclgvCd) {
	location.href = '/opmanager/lclgvHnrUser/lclgvHnrUserMng/form/'+lclgvCd;
}
