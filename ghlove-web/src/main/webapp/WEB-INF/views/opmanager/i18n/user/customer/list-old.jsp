...
<%@ taglib prefix="sec" 	uri="http://www.springframework.org/security/tags"%>
	...
	<form:form modelAttribute="searchParam" cssClass="opmanager-search-form clear" method="post">
		<fieldset>
			...
			<div class="board_write">
				<table class="board_write_table">
					...
					<tbody>
							...
							<td colspan="3">
								<div>
									<form:select path="where" title="${op:message('M00211')} ">
										<form:option value="LOGIN_ID"><c:out value="${op:message('M00081')}"/></form:option>
										<form:option value="EMAIL">E-mail</form:option>
										<form:option value="USER_NAME">이름</form:option>
										<form:option value="PHONE_NUMBER">휴대폰 번호</form:option>
									</form:select>
						...
						<tr>
							<td class="label"><c:out value="${op:message('M00219')}"/></td>
							<td colspan="3">
								<div class="search-date">
									...
									<span class="day_btns">
										<a href="#" class="btn_date today"><c:out value="${op:message('M00026')}"/></a>
										<a href="#" class="btn_date week"><c:out value="${op:message('M00222')}"/></a>
										<a href="#" class="btn_date month1"><c:out value="${op:message('M00029')}"/></a>
										<a href="#" class="btn_date month2"><c:out value="${op:message('M00223')}"/></a>
										<a href="#" class="btn_date all"><c:out value="${op:message('M00039')}"/></a>
									</span>
								</div>
							</td>
						</tr>
						<tr>
							<td class="label"><c:out value="${op:message('M00224')}"/> </td>
							<td colspan="3">
								<div class="search-date">
						...
						<tr>
							<td class="label"><c:out value="${op:message('M00229')}"/></td>
							<td>
								<div>
									<form:input path="startLoginCount" cssClass="optional _number form-amount" title="${op:message('M00229')}" /> <c:out value="${op:message('M00851')}"/> <!-- 회 -->
									<span class="wave">~</span>
									<form:input path="endLoginCount" cssClass="optional  _number form-amount" title="${op:message('M00229')}" /> <c:out value="${op:message('M00851')}"/> <!-- 회 -->
							 	</div>
							</td>
							<td class="label"><c:out value="${op:message('M00230')}"/></td>
							<td>
								<div>
									<form:input path="loginCount" cssClass="optional  _number form-amount" title="${op:message('M00230')}" />
									<span><c:out value="${op:message('M00231')}"/> </span>
							 	</div>
							</td>
						</tr>
						<tr>
						...
						<tr>
							<td class="label"><c:out value="${op:message('M00235')}"/> </td>
							<td colspan="3">
								<div>
								...
								<div>
									<form:select path="groupCode" title="그룹명">
										...
										<c:forEach items="${groups}" var="group">
											<form:option value="${fn:escapeXml(group.groupCode)}"><c:out value="${group.groupName}"/></form:option>
										</c:forEach>
									</form:select>
								</div>
							</td>
								...
								<div>
									<form:select path="levelId" title="회원 Level">
										...
										<c:forEach items="${groups}" var="group">
											...
											<optgroup label="${fn:escapeXml(group.groupName)}">
												<c:forEach items="${group.userLevels}" var="userLevel">
													<form:option value="${fn:escapeXml(userLevel.levelId)}" ><c:out value="${userLevel.levelName}"/></form:option>
												</c:forEach>
											</optgroup>
										</c:forEach>
									</form:select>
			...
			<div class="btn_all">
				<div class="btn_left">
					<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/opmanager/user/customer/list';"><span class="glyphicon glyphicon-repeat"></span> <c:out value="${op:message('M00047')}"/></button> <!-- 초기화 -->
				</div>
				<div class="btn_right">
			...
			<table class="board_list_table">
				...
				<thead>
					<tr>
						...
						<th scope="col"><c:out value="${op:message('M00005')}"/></th> <!-- 이름 -->
						<%-- <th scope="col">${op:message('M00175')}</th> --%> <!-- 닉네임 -->
						<th scope="col">휴대폰 번호</th>
						<th scope="col"><c:out value="${op:message('M00246')}"/></th>
						<th scope="col"><c:out value="${op:message('M00229')}"/></th>
						...
						<th scope="col"><c:out value="${op:message('M00219')}"/></th> <!-- 가입일 -->
						<th scope="col">SMS 수신동의</th>
						<th scope="col">Email 수신동의</th>
						<th scope="col"><c:out value="${op:message('M00235')}"/> </th>
						<th scope="col">로그인</th>
					</tr>
				</thead>
				<tbody>
				<c:forEach items="${list}" var="user" varStatus="i">
					<tr>
						<td>
							<input type="checkbox" name="userIds" value="${fn:escapeXml(user.userId)}" title="${op:message('M00169')}"  />
						</td>
						<td><c:out value="${pagination.itemNumber - i.count}"/></td>
						
						<td><a href="javascript:Manager.userDetails(${fn:escapeXml(user.userId)})"><c:out value="${user.loginId}"/></a></td>
						<td>
							<a href="javascript:Manager.userDetails(${fn:escapeXml(user.userId)})"><c:out value="${user.userName}"/></a>
						</td>
						...
						<td class="text-right"><c:out value="${op:numberFormat(user.userDetail.point)}"/>P</td>
						<td><c:out value="${user.loginCount}"/></td>
						<%--<td>${op:numberFormat(user.userDetail.buyCount)}건</td>	 --%>
						<%--<td>${op:numberFormat(user.userDetail.buyPrice)}${op:message('M00049')}</td>--%>
						<td>
							<c:out value="${op:date(user.createdDate)}"/>
						</td>
						<td>
							<c:choose>
								<c:when test="${user.userDetail.receiveSms == '0'}"><c:out value="${op:message('M00233')}"/></c:when>
								<c:otherwise><c:out value="${op:message('M00234')}"/></c:otherwise>
							</c:choose>
						</td>
						<td>
							<c:choose>
								<c:when test="${user.userDetail.receiveEmail == '0'}"><c:out value="${op:message('M00233')}"/></c:when>
								<c:otherwise><c:out value="${op:message('M00234')}"/></c:otherwise>
							</c:choose>
						</td>
						<td>
						...
						<td><button type="button" class="btn btn-gradient btn-xs btn-shadow-login" data-login-id="${fn:escapeXml(user.loginId)}">로그인</button></td>
					</tr>
				</c:forEach>
				</tbody>
			</table>
	...
	<div class="btn_all">
		<div class="btn_left">
			...
			<button type="button" class="ctrl_btn">
			 ...
			 -->
			<a href="point_popup.html" class="btn btn-default btn-sm" target="_blank" onclick="Common.popup('/opmanager/user/customer/point', 'donation', 600, 550); return false;" title="새창"><c:out value="${op:message('M00710')}"/> <!-- 전체회원 포인트지급 --> </a>
			<button type="button" class="btn btn-default btn-sm" onclick="Common.popup('/opmanager/user/customer/point-by-excel', 'donation', 600, 550); return false;" title="새창"><c:out value="${op:message('M00246')}"/> 일괄지급(엑셀업로드)	</button>
			<button type="button" class="btn btn-default btn-sm" onclick="selectPoint()" title="새창">선택회원 <c:out value="${op:message('M00246')}"/> 지급</button>
		...
		<sec:authorize access="hasRole('ROLE_EXCEL')">
			<div class="btn_right">
				<button type="button" class="btn btn-success btn-sm" onclick="downloadExcel()"><span class="glyphicon glyphicon-save" aria-hidden="true"></span> 엑셀 다운로드</button>
			</div>
		</sec:authorize>
	...
	<jsp:include page="../../include/modal/privacy-access.jsp" />
...
<script type="text/javascript">
	...
	function downloadExcel() {
		...
		if ($ids.size() > 0) {
			...
			param = $('#searchParam').serialize();
		}

		Shop.downloadExcel("/opmanager/user/customer/list/download-excel", param, true);
	}

	function groupSetting() {

		if ($("input:checkbox[name='userIds']:checked").size() == 0) {
			alert('그룹을 설정하실 회원을 선택하세요.');
			return;
		}

		if ($("input:checkbox[name='userIds']:checked").size() > 100) {
			alert('선택회원 그룹설정의 경우 100명씩 변경 가능합니다.');
			return;
		}

		var userIds = [];
		$("input:checkbox[name='userIds']:checked").each(function(index){
