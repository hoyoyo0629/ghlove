...
.table_btn {position: absolute; top: 77px; right: 18px;}
		...
		<div class="popup_wrap">
			...
			<div class="popup_contents">
				...
				<form:form modelAttribute="itemParam" action="${fn:escapeXml(requestContext.managerUri)}/item/download-excel" method="post" target="downloadFrame">
					...
					<form:hidden path="shWdr" />
					<form:hidden path="shLocgovCode" />
						...
						<table class="board_write_table">
							...
							<tbody>
								 <tr>
								 	...
								 	<td>
								 		<div>
								 			<p style="padding-bottom: 10px"><label><input type="checkbox" id="check-all" checked/> ${op:message('M00039')}</label></p> <!-- 전체 -->
								 			<p><label><input type="checkbox" name="excelDownloadData" value="item_main" checked/> ${op:message('M01270')} <!-- (MAIN) --></label>	</p><!-- 상품정보 -->
								 			<%-- <p><label><input type="checkbox" name="excelDownloadData" value="item_seo" checked/> ${op:message('M01270')} (SEO)</label>	</p> --%><!-- 상품정보 -->
								 			<p><label><input type="checkbox" name="excelDownloadData" value="item_table" checked/> 상품 정보고시 <!-- (테이블) --></label>	</p><!-- 상품상세설명(테이블) -->
								 			<!-- <p><label><input type="checkbox" name="excelDownloadData" value="item_table_mobile" checked/> ${op:message('M01271')} [모바일]</label>	</p><!-- 상품상세설명(테이블) -->
								 			<p><label><input type="checkbox" name="excelDownloadData" value="item_option" checked/> ${op:message('M01272')}</label>	</p><!-- 옵션정보 -->
								 			<p><label><input type="checkbox" name="excelDownloadData" value="item_image" checked/> ${op:message('M01273')}</label>	</p><!-- 상품이미지정보 -->
								 			<%-- <p><label><input type="checkbox" name="excelDownloadData" value="item_relation" checked/> ${op:message('M01276')}</label>	</p> --%><!-- 관련상품정보 -->
								 			<p><label><input type="checkbox" name="excelDownloadData" value="item_category" checked/> ${op:message('M01274')}</label>	</p><!-- 상품카테고리정보 -->
								 			<!-- <p><label><input type="checkbox" name="excelDownloadData" value="item_point" checked/> ${op:message('M01275')}</label>	</p><!-- 상품포인트정보 -->
					...
					<p class="popup_btns">
						...
						<button type="button" class="table_btn hidden" onclick="downloadSample()"><span>${op:message('M01280')}</span></button> <!-- 샘플 다운로드 -->
					</p>
