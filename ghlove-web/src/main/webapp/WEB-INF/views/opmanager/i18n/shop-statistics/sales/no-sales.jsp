...
<%@ taglib prefix="op" 		uri="/WEB-INF/tlds/functions" %>
	...
	<div class="statistics_web">
		...
		<div class="board_list mt30">
			...
			<table class="board_list_table">
				...
				<tbody>
					<c:forEach items="${itemList}" var="list">
						<tr>
							<td style="text-align: left;"><a href="/categories/index/${fn:escapeXml(list.categoryCode )}" target="_blank"> ${fn:escapeXml(list.title )}</a></td>
							<td class="border_left" style="text-align: left;">
								<c:choose>
									<c:when test='${op:property("saleson.view.type") eq "api"}'>
										<a href="${op:property('saleson.url.frontend')}/items/details.html?code=${fn:escapeXml(item.itemUserCode)}" target="_blank"><img src="${fn:escapeXml(list.itemImage)}" class="item_image" alt="상품이미지"  /></a>
										<a href="${op:property('saleson.url.frontend')}/items/details.html?code=${fn:escapeXml(item.itemUserCode)}" target="_blank">${fn:escapeXml(list.itemName )} (${fn:escapeXml(list.itemUserCode )})</a>
									</c:when>
									<c:otherwise>
										<a href="/products/view/${fn:escapeXml(list.itemUserCode)}" target="_blank"><img src="${fn:escapeXml(list.itemImage)}" class="item_image" alt="상품이미지"  /></a>
										<a href="/products/view/${fn:escapeXml(list.itemUserCode)}" target="_blank">${fn:escapeXml(list.itemName )} (${fn:escapeXml(list.itemUserCode )})</a>
									</c:otherwise>
								</c:choose>

							</td>
							<td class="border_left number">${fn:escapeXml(list.payCount )}</td>
			...
			<div class="sort_area">
				<div class="right">
					<a href="/opmanager/shop-statistics/sales/no-sales/excel-download?${fn:escapeXml(queryString)}"  class="btn_write gray_small"><img src="/content/opmanager/images/icon/icon_excel.png" alt=""><span>${op:message('M00254')}</span> </a> <!-- 엑셀 다운로드 -->
				</div>
			</div>
