...
<div class="popup_wrap">
	...
	<div class="popup_contents">
		...
		<form:form modelAttribute="sellerSearchParam" method="post">
			<form:hidden path="defaultOpmanagerSellerId" />
			...
			<div class="btn_all">
				<div class="btn_left">
					<a href="/opmanager/seller/find?defaultOpmanagerSellerId=${fn:escapeXml(sellerSearchParam.defaultOpmanagerSellerId)}" class="btn btn-dark-gray btn-sm"><span class="glyphicon glyphicon-repeat"></span> <c:out value="${op:message('M00047')}"/></a> <!-- 초기화 -->
				</div>
				<div class="btn_right">
			...
			<table class="board_list_table" summary="상품리스트">
				...
				<tbody>
					<c:forEach items="${list}" var="seller">
						<tr onclick="selectSeller('${fn:escapeXml(seller.sellerId)}','${fn:escapeXml(seller.sellerName)}')" style="cursor: pointer">
							<td class="text-center"><c:out value="${seller.loginId}"/></td>
							<td class="text-left"><c:out value="${seller.sellerName}"/></td>
							<td class="text-center"><c:out value="${seller.userName}"/></td>
							<td class="text-center"><c:out value="${seller.businessNumber}"/></td>
							<td class="text-center"><c:out value="${seller.telephoneNumber}"/></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
...
<script type="text/javascript">
	function selectSeller(sellerId, sellerName) {
		var target = "${fn:escapeXml(targetId)}";
		if (target!="") {
