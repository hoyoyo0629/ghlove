				...
				<ul class="list">
					<c:forEach items="${ list }" var="list" varStatus="i">
						...
						<li>
							<a href="<c:url value="/opmanager/proposal/details/${ list.proposalId }?url=${requestContext.currentUrl}"/>">
								<c:forEach items="${ list.proposalFiles }" var="file" varStatus="i">
									<c:if test="${!empty file.fileName && file.fileName != null}">
										<img src="<c:url value="/upload/proposal/${ fn:escapeXml(list.proposalId) }/thumb/${ fn:escapeXml(list.thumNamilSize) }_${ fn:escapeXml(file.fileId) }.${ fn:escapeXml(file.fileType)}"/>" alt="${ fn:escapeXml(file.fileDescription) }" />
									</c:if>
								</c:forEach>
							...
							<p class="writer">${ fn:escapeXml(list.userName) }</p>
							<div class="tooltip_wrap reject_cause">
								<span class="icon image ${ fn:escapeXml(statusIcon) } tooltip" tabindex="0">${ fn:escapeXml(statusAlt) }</span>
								<c:if test="${list.statusCode == '7'}">
									<div class="tooltip_area">
										<div>
											<p>반송사유</p>
											<p>${fn:escapeXml(list.approvalRemark)}</p>
										</div>
									</div>
								</c:if>
							</div>
							<p class="period">${op:formatDate(list.startDate,'-') } ~ ${op:formatDate(list.endDate,'-')}</p>
							<p class="title"><a href="<c:url value="/opmanager/proposal/details/${ fn:escapeXml(list.proposalId) }?url=${fn:escapeXml(requestContext.currentUrl)}"/>">${ fn:escapeXml(list.title) }</a></p>
							<p class="goal">
								<span>목표 모금액</span>
								<span class="amount"><em><fmt:formatNumber value="${ fn:escapeXml(list.targetAmount) }" pattern="#,###" /></em>원</span>
							</p>
						</li>
					</c:forEach>
				</ul>
...
<script type="text/javascript">
	...
	function fn_donationExcelDownload(){
		Shop.downloadExcelOrder("/opmanager/proposal/proposal-download-excel", $('#proposalSearchParam').serialize(), false);
	}
	$(function(){
		EventHandler.calendarStartDateAndEndDateVaild();
	});
