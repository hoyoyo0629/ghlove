...
<h3 class="mt10">[<c:out value="${seller.sellerName}"/>] 추가금 정산 상세 - <c:out value="${op:date(remittanceParam.startDate)}"/> ~ <c:out value="${op:date(remittanceParam.endDate)}"/></h3>
...
<div class="board_write">
			...
			<table class="board_list_table" summary="정산내역 리스트">
				...
				<tbody>
					<c:forEach items="${ list }" var="item" varStatus="index">
						<c:set var="key"><c:out value="${item.addPaymentId}"/></c:set>
						<tr>
							...
							<td class="amount text-right">
								<c:if test="${item.addPaymentType == '2'}">-</c:if><c:out value="${op:numberFormat(item.remittanceAmount)}"/>원
							</td>
							<td class="amount text-right">
								<c:if test="${item.addPaymentType == '2'}">-</c:if><c:out value="${op:numberFormat(item.remittanceAmount)}"/>원
							</td>
							<td>
								<input type="text" name="editAddPaymentRemittanceMap[${key}].remittanceExpectedDate" class="required" value="${fn:escapeXml(item.remittanceExpectedDate)}" />
							</td>
						</tr>
					</c:forEach>
				</tbody>
	...
	<div class="btn_all">
		...
		<div class="btn_right mb0">
			<a href="/opmanager/remittance/expected/list?${op:numberFormat(queryString)}" class="btn btn-active btn-sm">목록으로</a>
		</div>
	</div>
