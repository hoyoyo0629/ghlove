			...
			<tbody>
				...
				<c:choose>
					<c:when test="${details.confmSttusCode eq 200}">
						...
						<!--// 상태 : 대기 -->
						<c:choose>
							<c:when test="${details.reqstSeCode eq 'ROLE_ADMIN_10'}">
								<tr id="trDsgncntrPart">
									<td class="label">특정사업 기부<br>사업 부서</td>
									<td colspan="3">
										<div class="flex_box gap-08">
											<c:choose>
												<c:when test="${details.confmSttusCode == '200'}">
													<select class="wd-150" value="${details.reqDsgncntrPartId}" id="dsgncntrPartId">
													<option value="0">-사업부서 미지정-</option>
														<c:forEach items="${partList}" var="part">
															<c:choose>
																<c:when test="${details.reqDsgncntrPartId == part.dsgncntrPartId}">
																	<option value="${fn:escapeXml(part.dsgncntrPartId)}" label="${fn:escapeXml(part.dsgncntrPartName)}" selected="selected"/>
																</c:when>
																<c:otherwise>
																	<option value="${fn:escapeXml(part.dsgncntrPartId)}" label="${fn:escapeXml(part.dsgncntrPartName)}" />
																</c:otherwise>
															</c:choose>
														</c:forEach>
													</select>
												...
												<c:otherwise>
													<select class="wd-150" value="${fn:escapeXml(details.dsgncntrPartId)}" id="dsgncntrPartId">
													<option value="0">-사업부서 미지정-</option>
														<c:forEach items="${partList}" var="part">
															<c:choose>
																<c:when test="${details.dsgncntrPartId == part.dsgncntrPartId}">
																	<option value="${fn:escapeXml(part.dsgncntrPartId)}" label="${fn:escapeXml(part.dsgncntrPartName)}" selected="selected"/>
																</c:when>
																<c:otherwise>
																	<option value="${fn:escapeXml(part.dsgncntrPartId)}" label="${fn:escapeXml(part.dsgncntrPartName)}" />
					...
					<c:when test="${details.confmSttusCode eq 100}">
						...
						<!--// 상태 : 승인 -->
						<c:choose>
							<c:when test="${details.reqstSeCode eq 'ROLE_ADMIN_10' && details.managerLocgovCode != null}">
							<tr>
								<td class="label">특정사업 기부<br>사업 부서</td>
								<td colspan="3">
									<div class="flex_box gap-08">
										<c:choose>
											<c:when test="${details.managerLocgovCode == details.locgovCode}">
												<select class="wd-150" value="${fn:escapeXml(details.dsgncntrPartId)}" id="dsgncntrPartId">
													<option value="0">-사업부서 미지정-</option>
													<c:forEach items="${partList}" var="part">
														<c:choose>
											...
											<c:when test="${details.managerLocgovCode == null}">
												<!-- 승인 삭제 상태 -->
											</c:when>
											<c:otherwise>
												<!-- 현재 타지자체 부서 -->
										...
										<c:if test="${details.managerLocgovCode == details.locgovCode}">
											<button type="button" class="btn" onclick="javascript:changePart();">변경</button>
										</c:if>
									</div>
								</td>
							</tr>
	...
	function managerConfirm() {
		...
		if(selectedObj && selectedObj.length > 0) {
			...
			if(confirm(confirmMsg)) {
				$.post("/opmanager/designated-donation/request/confirm", param, function(response) {
