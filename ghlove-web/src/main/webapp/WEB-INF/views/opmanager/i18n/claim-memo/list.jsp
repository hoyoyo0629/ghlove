...
<%@ taglib prefix="shop"	uri="/WEB-INF/tlds/shop" %>
	...
	<form:form modelAttribute="ClaimMemoParam" method="post" enctype="multipart/form-data">
		<!--자주 묻는 질문관리 시작-->
		<h3><span><c:out value="${op:message('M00674')}"/></span></h3> <!-- 자주 묻는 질문관리 --> 
		<div class="board_write">
			<table class="board_write_table" summary="${op:message('M00675')}"> <!-- 자주 묻는 질문 --> 
				<caption><c:out value="${op:message('M00675')}"/></caption>
				<colgroup>
				...
				<tbody>
					 <tr>
					 	<td class="label"><c:out value="${op:message('M00276')}"/></td>
					 	<td colspan="3">
					 ...
					 <tr>
					 	...
					 	<td colspan="3">
					 		<div>
								<form:select path="where" title="${op:message('M00468')}"> <!-- 키워드 선택 --> 
									<form:option value="USER_NAME"><c:out value="${op:message('M00005')}"/></form:option> <!-- 이름 --> 
									<form:option value="ORDER_CODE"><c:out value="${op:message('M00013')}"/></form:option> <!-- 주문번호 --> 
								</form:select> 
					  ...
					  <tr>
					 	...
					 	<td class="label"><c:out value="${op:message('M01665')}"/></td> <!-- 상담구분 --> 
					 	<td>
					 		<div>
		...
		<div class="btn_all">
			<div class="btn_left">
				<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/opmanager/claim-memo/list';"><span class="glyphicon glyphicon-repeat"></span> <c:out value="${op:message('M00047')}"/></button> <!-- 초기화 -->
			</div>
			<div class="btn_right">
		...
		<div class="count_title mt20">
			<h5>
				<c:out value="${op:message('M00039')}"/> : <c:out value="${totalCount}"/> <c:out value="${op:message('M00743')}"/>
			</h5>	 <!-- 전체 -->   <!-- 건 조회 --> 
			<span>
				<c:out value="${op:message('M00052')}"/> : 
	...
	<form id="listForm">
		<div class="board_write">
			<table class="board_list_table" summary="${op:message('M00273')}"> <!-- 주문내역 리스트 --> 
				...
				<thead>
					<tr>
						<th scope="col"><c:out value="${op:message('M00200')}"/></th> <!-- 순번 --> 
						<th scope="col"><c:out value="${op:message('M00276')}"/></th> <!-- 작성일 --> 
						<th scope="col"><c:out value="${op:message('M00005')}"/></th> <!-- 이름 --> 
						<th scope="col"><c:out value="${op:message('M00472')}"/></th> <!-- 작성자 --> 
						<th scope="col"><c:out value="${op:message('M00013')}"/></th> <!-- 주문번호 --> 
				...
				<tbody>
					<c:forEach items="${ClaimMemolist}" var="memo" varStatus="i">
						<tr>
							...
							<td><a href="javascript:Manager.userDetails(${fn:escapeXml(memo.userId)})"><c:out value="${memo.userName}"/></a></td>
							<td><c:out value="${memo.managerLoginId}"/></td>
							<td>
								<c:if test="${memo.orderCode != null}">
									<a href="/opmanager/order/new-order/order-detail/0/${fn:escapeXml(memo.orderCode)}"><span><c:out value="${memo.orderCode}"/></span></a>
		...
		<c:if test="${empty ClaimMemolist}">
			...
			<!-- div class="btn_all">
				<div class="btn_left mb0">
					<a id="delete_list_data" href="#" class="ctrl_btn"><span><c:out value="${op:message('M00074')}"/></span></a> <!-- 삭제 --> 
				<!-- /div>
				<div class="btn_right mb0">
					<a href="" class="ctrl_btn"><span><c:out value="${op:message('M00088')}"/></span></a> <!-- 등록 --> 
			<!-- /div>-->
		</div>
