					...
					<table class="board_write_table">
						...
						<tbody>
								...
								<td colspan="3">
									<div>
										...
										<c:forEach items="${adminMenuRoles}" var="adminMenuRole" varStatus="i">
											...
											<label><input type="checkbox" name="userRoles[${fn:escapeXml(i.index)}].authority" id="role${fn:escapeXml(i.index)}" value="${fn:escapeXml(adminMenuRole.authority)}" title="${op:message('M00147')} " class="authority" ${fn:escapeXml(checked)} /> ${op:message(adminMenuRole.roleName)}</label>
											<br />
										</c:forEach>

										<input type="hidden" name="adminMenu" id="adminMenu" value="${fn:escapeXml(userDetails.adminMenu)}" />
 									</div>
								</td>
							</tr>
							<!--  tr>
							...
							<tr>
								...
								<td>
									<div>
										<input type="password"  name="password" id="password" title="${op:message('M00150')} " class="form-block ${fn:escapeXml(required)} _password" minlength="4"  maxlength="20"/>
								    </div>
								</td>
								<td class="label">${op:message('M00151')} </td>
								<td>
									<div>
										<input type="password" name="password2" id="password_confirm" title="${op:message('M00151')} " class="form-block ${fn:escapeXml(required)} _password" minlength="4"  maxlength="20"/>
								    </div>
								</td>
							</tr>
							<tr>
								...
								<td>
									<div>
										<input type="text" name="userNameKatakana" id="userNameKatakana" value="${fn:escapeXml(userDetails.userNameKatakana)}" title="부서명" maxlength="50" class="form-block">
								    </div>
								</td>
							</tr>
							<tr>
								...
								<td>
									<div>
										...
										<input type="hidden" id="orgEmail" value="${fn:escapeXml(user.email)}" />
								...
								<td>
									<div>
										<input type="text" name="phoneNumber1" id="phoneNumber" value="${fn:escapeXml(userDetails.phoneNumber1)}" title="연락처" maxlength="4" class="form-phone _number"> -
										<input type="text" name="phoneNumber2" id="phoneNumber" value="${fn:escapeXml(userDetails.phoneNumber2)}" title="연락처" maxlength="4" class="form-phone _number"> -
										<input type="text" name="phoneNumber3" id="phoneNumber" value="${fn:escapeXml(userDetails.phoneNumber3)}" title="연락처" maxlength="4" class="form-phone _number">
									</div>
								</td>
							</tr>
						</tbody>
