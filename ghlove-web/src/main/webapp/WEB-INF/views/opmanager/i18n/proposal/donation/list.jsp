...
<%@ taglib prefix="page" 	tagdir="/WEB-INF/tags/page"%>
			...
			<div class="donation_state_wrap">
				<h4>정기기부 <span class="normal" id="regularDate"><c:out value="${op:formatDate(regularSearchParam.startDate,'-')}"/> ~ <c:out value="${op:formatDate(regularSearchParam.endDate,'-')}"/></span></h4>
				<div class="sort_area">
					<form:form modelAttribute="regularSearchParam" cssClass="opmanager-search-form clear" onsubmit="regularSubmit(); return false;"  method="post">
						<fieldset>
							...
							<legend class="hidden">검색</legend>
								<span class="datepicker"><input type="text" name="startDate" id="startDate1" title="검색 시작일" value="${fn:escapeXml(regularSearchParam.startDate)}" class="required _date term term1" maxlength="8"></span>
								<span class="wave">~</span>
								<span class="datepicker"><input type="text" name="endDate" id="endDate1" value="${ fn:escapeXml(regularSearchParam.endDate) }" title="검색 종료일" class="required _date term term1" maxlength="8"  /></span>
			...
			<div class="board_list">
				<h4>수시기부 <span class="normal" id="donationDate"><c:out value="${ op:formatDate(searchParam.startDate,'-') }"/> ~ <c:out value="${ op:formatDate(searchParam.endDate,'-') }"/></span></h4>
				...
				<div class="sort_area">
					<form:form modelAttribute="searchParam" cssClass="opmanager-search-form clear" method="post" onsubmit="pagination(); return false;">
						<fieldset>
							...
							<div class="right">
								<form:select path="where" items="${op:nullToEmptyArr(whereColumns)}" itemValue="value" itemLabel="label" />
...
<script type="text/javascript">
	...
	function fn_regularExcelDownload() {
		Shop.downloadExcelOrder("/opmanager/proposal/donation/regular-download-excel", $('#regularSearchParam').serialize(), false);
	}

	function fn_donationExcelDownload() {
		Shop.downloadExcelOrder("/opmanager/proposal/donation/donation-download-excel", $('#searchParam').serialize(), false);
	}
