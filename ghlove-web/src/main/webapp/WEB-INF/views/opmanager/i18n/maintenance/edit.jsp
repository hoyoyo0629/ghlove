						...
						<td>
							<div class="flex_box gap-12">
								<div class="input-form">
									<input id="ki_1" name="kiType" type="radio" value="1"
										${maintenanceDto.kiType eq '1' ? 'checked' : ''}>
									<label for="ki_1">홈페이지</label>
								</div>
								<div class="input-form">
									<input id="ki_2" name="kiType" type="radio" value="2"
										${maintenanceDto.kiType eq '2' ? 'checked' : ''}>
									<label for="ki_2">관리자시스템</label>
								</div>
								<div class="input-form">
									<input id="ki_3" name="kiType" type="radio" value="3"
										${maintenanceDto.kiType eq '3' ? 'checked' : ''}>
									<label for="ki_3">기타</label>
								</div>
							</div>
						</td>
						<td class="label"><span class="required_mark"></span>CSR번호</td>
						<td>
							<div class="flex_box gap-08 item-center">
								<%-- <input id="srNoCode" title="" class="input_txt required _filter wd-150" type="text" value="${fn:escapeXml(maintenanceDto.srNo)}"> --%>
								${maintenanceDto.srNo}
							</div>
						</td>
					</tr>
					<tr>
						<td class="label"><span class="required_mark"></span>등록일자</td>
						<td>
							<div class="flex_box gap-08 item-center">
								${maintenanceDto.reqCrtDate}
							</div>
						</td>
						<td class="label"><span class="required_mark"></span>작성자명</td>
						<td>
							<div class="flex_box gap-08 item-center">
								${maintenanceDto.reqManagerNm}
							</div>
						</td>
					</tr>
					<tr>
						...
						<td>
							<div class="flex_box gap-08 item-center">
								<select id="reqChannel" name="reqChannel" title="-선택-" class="wd-200">
						...
						<td>
							<%-- <div class="flex_box gap-08 item-center">
								<select id="reqTypeVal" title="-선택-" class="wd-200">
									<option value="">-선택-</option>
									<c:forEach items="${maintenCateList}" var="code" varStatus="status">
										<option value="${fn:escapeXml(code.id)}"
											<c:if test="${maintenanceDto.reqType eq code.id}">selected="selected"</c:if>>
											<c:out value="${fn:escapeXml(code.label)}"/>
										</option>
									</c:forEach>
								</select>
							...
							<div class="flex_box gap-12">
								<div class="input-form">
									<input id="req_1" name="reqType" type="radio" value="1"
										${maintenanceDto.reqType eq '1' ? 'checked' : ''}>
									<label for="req_1">기부</label>
								</div>
								<div class="input-form">
									<input id="req_2" name="reqType" type="radio" value="2"
										${maintenanceDto.reqType eq '2' ? 'checked' : ''}>
									<label for="req_2">답례품</label>
								</div>
								<div class="input-form">
									<input id="req_3" name="reqType" type="radio" value="3"
										${maintenanceDto.reqType eq '3' ? 'checked' : ''}>
							...
							<form:hidden path="reqUserPhoneNumber" />
								...
								<select id="chargerCttpc1" title="-선택-" class="wd-150">
									...
									<c:forEach items="${phoneCodeList}" var="code" varStatus="status">
										<option value="${fn:escapeXml(code.id)}" <c:if test="${code.id eq fn:split(maintenanceDto.reqUserPhoneNumber, '-')[0]}">selected</c:if>><c:out value="${code.label}"/></option>
									</c:forEach>
								...
								<input id="chargerCttpc2" class="basic wd-150" title="" class="input_txt required _filter half _number" type="text" maxlength="4" value="${fn:split(maintenanceDto.reqUserPhoneNumber, '-')[1]}">
								<span class="wave">-</span>
								<input id="chargerCttpc3" class="basic wd-150" title="" class="input_txt required _filter half _number" type="text" maxlength="4" value="${fn:split(maintenanceDto.reqUserPhoneNumber, '-')[2]}">
							</div>
						</td>
	...
	<%-- 								<textarea id="processCn" name="processCn" cols="30" rows="10" class="required _filter" title="조치결과" ${fn:escapeXml(role) ne 'ROLE_ADMIN_1' ? 'disabled="disabled"' : ''}>${op:removeIframe(maintenanceDto.processCn)}</textarea> --%>
			...
			<div class="btn_all btn_center">
				<div class="flex_box gap-08">
					<!-- <button type="button" class="btn btn-defualt btn-small list" onclick="location.href='/opmanager/maintenance/list'">목록</button> -->
...
// 		console.log(${fn:escapeXml(role) != 'ROLE_ADMIN_1'});
// 		console.log("${fn:escapeXml(role)}");
...
function downloadItemImage(fileId) {
	location.href = '/opmanager/maintenance/file-download/'+fileId
}
...
function deleteItemImage(fileId) {
	...
	$.post('/opmanager/maintenance/delete-item-image', param, function(response){
...
function deleteItem(bbsId) {
	var url = "/opmanager/maintenance/deleteMaintenance/" + bbsId
