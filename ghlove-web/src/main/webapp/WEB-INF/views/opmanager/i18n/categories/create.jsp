...
<%@ taglib prefix="module" 	tagdir="/WEB-INF/tags/modules"%>
	...
	<h5 style="margin-top: 0; border-left: 4px solid #000; padding-left: 8px;"> 2차 카테고리 등록</h5>
			 ...
			 <tr>
				...
				<td>
					<div class="flex_box gap-12 item-center">
						...
						<a href="javascript:;" id="categroyUrlSearch" class="btn btn-gradient btn-sm">${op:message('M01488')}</a>
				 	</div>
				</td>
			</tr>
			<tr>
					...
					<div>
						<p>
							<select name="categoryGroupId" class="category">
								<option value="0">${op:message('M00431')}</option>
									...
									<c:if test="${categoriesTeam.categoryTeamFlag == 'Y'}">
										...
										<c:forEach items="${categoriesTeam.categoriesGroupList}" var="categoriesGroup">
											<c:if test="${categoriesGroup.categoryGroupFlag == 'Y'}">
												<option value="${fn:escapeXml(categoriesGroup.categoryGroupId)}" label="${fn:escapeXml(categoriesGroup.groupName)}" />
											</c:if>
										</c:forEach>
										</optgroup>
									</c:if>
				...
				<td>
                    <div class="flex_box gap-12">
                        <div class="input-form">
                            <input type="radio" name="categoryFlag" value="Y" id="out_all" checked="checked" />
                            <label for="out_all">${op:message('M00096')}</label>
                        </div>
                        <div class="input-form">
                            <input type="radio" name="categoryFlag" value="N" id="out"  />
                            <label for="out">${op:message('M00097')}</label>
                        </div>
                        </div>
				</td>
			</tr>
