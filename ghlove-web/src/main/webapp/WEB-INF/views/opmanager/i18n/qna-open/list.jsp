...
<!-- 						</tr> -->
						 	...
						 	<td>
						 		<div class="flex_box gap-08">
									<form:select path="where" title="${op:message('M00468')}" class="wd-150"> <!-- 키워드선택 -->
										...
										<!-- 2014.12.28 -->
										<%-- <form:option value="ALL" label="전체"></form:option> <!-- 전체 --> --%>
										<form:option value="GROUP" label="문의유형"></form:option> <!-- 문의유형 -->
					 		...
					 		<td>
			                    <div class="flex_box gap-12">
			                        <div class="input-form">
			                            <form:radiobutton path="qnaOpenAnswerCode" value="" label="전체" checked="checked"/>
			                        </div>
			                        <div class="input-form">
			                            <form:radiobutton path="qnaOpenAnswerCode" value="0" label="답변완료"/>
			                        </div>
			                        <div class="input-form">
			                            <form:radiobutton path="qnaOpenAnswerCode" value="1" label="답변미완료"/>
			                        </div>
			                    </div>
			                </td>
						 </tr>
		...
		<!-- 버튼시작 -->
			<div class="btn_all btn_right">
				<div class="flex_box gap-08">
					<button type="button" class="btn btn-dark-gray btn-mini" onclick="location.href='/opmanager/qna-open/list'"><c:out value="${op:message('M00047')}"/></button> <!-- 초기화 -->
						...
						<tbody>
							<c:forEach items="${qnaList}" var="list" varStatus="i">
								<tr>
									...
									<td><c:out value="${list.qnaDetailType }"/></td>
									<td class="tex_l">
										<%-- <a href="/opmanager/qna/view/${list.qnaId}">${list.subject}</a> <!-- 제목 --> --%>
										<a href="javascript:qnaDetail(${fn:escapeXml(list.qnaId)});"><c:if test="${list.qnaDetailType == 'A' }"><span class="reply"></span></c:if>${op:nl2br(list.subject)}</a> <!-- 제목 -->
									</td>
<%-- 									<td>
										<div>
...
<%-- 												<a href="javascript:Common.popup('/opmanager/user/popup/details/${list.userId}', '/user/popup/details', 1100, 800, 1, 0, 0)">${list.userName} (${list.loginId})</a> --%>
									...
									<td><c:out value="${op:numberFormat(list.hits)}"/></td>
									<td><c:out value="${op:datetime(list.createdDate)}"/></td>
									<td><c:out value="${list.answerCnt > 0 ? '답변완료' : '답변미완료'}"/></td>
									<td>
										<c:choose>
											<c:when test="${list.answerUserId > 0 && !empty list.answerLoginId}">
												<c:out value="${list.answerUserName}"/> (<c:out value="${list.answerLoginId}"/>)
											</c:when>
											<c:when test="${!empty list.answerUserName}">
												<c:out value="${list.answerUserName}"/>
											</c:when>
											<c:otherwise>-</c:otherwise>
										</c:choose>
									</td>
									<td>
										<c:choose>
											<c:when test="${!empty list.answerDate}">
												<c:out value="${op:date(list.answerDate)}"/>
											</c:when>
											<c:otherwise>-</c:otherwise>
										</c:choose>
									</td>
...
$(function() {
	...
	$('#delete_list_data').on('click', function() {
		Common.updateListData("/opmanager/qna-open/delete", "정말로 삭제하시겠습니까?");// 선택된 데이터를 삭제하시겠습니까?
	});
...
function qnaDetail(qnaId) {
	$("#qna").attr("method", "get").attr("action", "/opmanager/qna-open/answer/"+qnaId).submit();
}
</script>
