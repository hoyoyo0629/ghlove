...
<%@ taglib prefix="op" 		uri="/WEB-INF/tlds/functions"%>
	...
	<div class="board_list">
					...
					<div class="set_objective">
						<c:forEach items="${ list }" var="list" varStatus="i">
							<c:if test="${ year eq list.targetYear }">
								<span class="label">${ fn:escapeXml(list.targetYear) }년도</span> 
								...
								<form:input path="targetYear" value="${ fn:escapeXml(year) }" cssClass="hidden" />
								<c:set var="targetCount" value="1" />
							</c:if>
						</c:forEach>
						
						<c:if test="${ empty list || targetCount == 0 }">
							<span class="label">${ fn:escapeXml(year) }년도</span>
		...
		<table class="board_list_table">
			...
			<tbody>
				<c:forEach items="${ list }" var="list" varStatus="i">
					<tr>
						<td>{ fn:escapeXml(list.targetYear) }</td>
						<td><fmt:formatNumber value="${ fn:escapeXml(list.targetAmount) }" pattern="#,###" /></td>
						<td><fmt:formatNumber value="${ fn:escapeXml(list.donationAmount) }" pattern="#,###" /></td>
						<td><fmt:formatNumber value="${ fn:escapeXml(list.donationParticipationRate) }" pattern="###,###.##" />%</td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
		<p class="btns">
			<a href="javascript:downloadExcel();" class="btn orange">엑셀파일 다운로드</a>
		</p>
	</div>
	...
	<script type="text/javascript">
		...
		function downloadExcel() {
			Shop.downloadExcelOrder("/opmanager/proposal/target-download-excel", "", false);
		}
	</script>
