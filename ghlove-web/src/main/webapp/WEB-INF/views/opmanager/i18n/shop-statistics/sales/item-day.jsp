...
<div class="statistics_web">
							...
							<div>
								...
								<form:select path="weekType" class="required" title="${op:message('M01348')} ${op:message('M01349')}">
									<c:forEach items="${weekList}" var="list">
										<option value="${fn:escapeXml(list.value)}" ${op:selected(list.value,statisticsParam.weekType) }>${fn:escapeXml(list.week)}</option>
									</c:forEach>
	...
	<div class="board_list" style="overflow-y: auto;">
				...
				<c:forEach items="${itemDateList}" var="list">
					<tr>
						<td style="text-align: left;">
							...
							${fn:escapeXml(list.itemName )} (${fn:escapeXml(list.itemUserCode )})
						</td>
						<td class="border_left">
							${fn:escapeXml(list.code)}
						</td>
						<td class="border_left">
							${fn:escapeXml(list.groupCode)}
						</td>
						<td class="border_left">
							${fn:escapeXml(list.groupName)}
						</td>
						<td class="border_left">
							${fn:escapeXml(list.categoryCode)}
						</td>
						<td class="border_left">
		...
		<div class="sort_area">
			<div class="right">
				<a href="/opmanager/shop-statistics/sales/item/day-excel-download?${fn:escapeXml(queryString)}" class="btn btn-success btn-sm"><span class="glyphicon glyphicon-save"></span> ${op:message('M00254')}</a> <!-- 엑셀 다운로드 -->
			</div>
		</div>
