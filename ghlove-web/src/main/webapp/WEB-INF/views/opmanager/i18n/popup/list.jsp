...
<%@ taglib prefix="page" 	tagdir="/WEB-INF/tags/page"%>
		...
		<div class="location">
			...
			<form:form modelAttribute="popupSearchParam" method="post" enctype="multipart/form-data">
						...
						<tbody>
							...
							<tr>
							 ...
							 <tr>
							 	...
							 	<td colspan="3">
							 		<div>
										<span class="datepicker"><form:input path="startDate" maxlength="8" class="datepicker" title="${op:message('M00507')}" /></span> <!-- 팝업시작일자 -->
										<span class="wave">~</span>
										<span class="datepicker"><form:input path="endDate" maxlength="8" class="datepicker" title="${op:message('M00509')}" /></span> <!-- 팝업종료일자 -->
									</div>
							 	</td>
							 </tr>
						</tbody>
				...
				<div class="btn_all btn_right">
					<div class="flex_box gap-08">
						<button type="button" class="btn btn-dark-gray btn-mini" onclick="location.href='/opmanager/popup/list';"><span class="glyphicon glyphicon-repeat"></span> ${op:message('M00047')}</button> <!-- 초기화 -->
						<button type="submit" class="btn btn-dark-gray btn-mini"><span class="glyphicon glyphicon-search"></span> ${op:message('M00048')}</button> <!-- 검색 -->
					</div>
				</div>
				<!-- 버튼 끝-->
			</form:form>
			...
			<div class="count_title mt20">
				<h5>
					${op:message('M00039')} : ${fn:escapeXml(popupCount)} ${op:message('M00743')}
				</h5>	 <!-- 전체 -->   <!-- 건 조회 -->
			</div>
			...
			<form id="listForm">
					...
					<table class="board_list_table" summary="${op:message('M00273')}"> <!-- 주문내역 리스트 -->
						...
						<tbody>
							<c:forEach items="${popupList}" var="list" varStatus="i" >
								<tr>
									<td><input type="checkbox" name="id" id="check" value="${fn:escapeXml(list.popupId)}" title=${op:message('M00169')} /></td>
									<td>${pagination.itemNumber - i.count}</td>
									<td class="tleft">
										<div>
											<a href="/opmanager/popup/edit/${fn:escapeXml(list.popupId)}">${fn:escapeXml(list.subject)}</a>
										</div>
									</td>
									<td>${op:date(list.startDate)} ~ ${op:date(list.endDate)}</td>
									<td>${fn:escapeXml(list.popupClose)}</td>
									<td>${fn:escapeXml(list.popupType)}</td>
									<td>${fn:escapeXml(list.popupStyle)}</td>
									<td class="red">
                                        <div class="btn_all btn_center">
                                            <div class="flex_box gap-08">
                                                <a href="/opmanager/popup/edit/${fn:escapeXml(list.popupId)}" class="btn btn-gradient btn-sm">${op:message('M00087')}</a>
                                                <a href="javascript:deleteCheck('${fn:escapeXml(list.popupId)}');" class="btn btn-gradient btn-sm">${op:message('M00074')}</a>
                                            </div>
                                        </div>
                                    </td>
								</tr>
				...
				<div class="btn_all btn_right">
					<div class="flex_box gap-08">
						<a id="delete_list_data" href="#" class="btn btn-default btn-mini">${op:message('M00576')}</a> <!-- 선택삭제 -->
						<a href="/opmanager/popup/write" class="btn btn-active btn-mini"><span class="glyphicon glyphicon-plus"></span> ${op:message('M00088')}</a> <!-- 등록 -->
					</div>
				</div>
