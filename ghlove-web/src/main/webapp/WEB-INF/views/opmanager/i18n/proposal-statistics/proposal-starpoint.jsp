			...
			<span>
				<form:select path="localHqCode" id="localHqCode2">
					...
					<c:forEach items="${kbGroupTypeList}" var="list">
						<form:option value="${fn:escapeXml(list.localHqCode)}">${fn:escapeXml(list.localHqName)}</form:option>
					</c:forEach>
				</form:select>
			</span>
			<span>부점</span>
			<span class="deptNum deptNum2">
				<form:select path="deptNum" id="deptNum2">
					...
					<c:forEach items="${kbGroupDetailByIdLocalHqCode}" var="list">
						<form:option value="${fn:escapeXml(list.deptNum)}">${fn:escapeXml(list.deptName)}</form:option>
					</c:forEach>
				</form:select>
			</span>
			<span>직원</span>
			...
			<c:forEach items="${proposalStatistics}" var="list" varStatus="i">
				<tr>
					...
					<c:forEach items="${list.kbGroups}" var="kbGroups" varStatus="j" >
						<td>${fn:escapeXml(kbGroups.localHqName)}</td>
						<td>${fn:escapeXml(kbGroups.deptName)}</td>
					</c:forEach>
					<td>${fn:escapeXml(list.userName)}</td>
					<td class="border_left"><fmt:formatNumber value="${ fn:escapeXml(list.proposalDonationCount)}" pattern="#,###" /></td>
					<td><fmt:formatNumber value="${ fn:escapeXml(list.totalDonationAmount)}" pattern="#,###" /></td>
					<td class="border_left"><fmt:formatNumber value="${fn:escapeXml(list.starPoint)}" pattern="#,###" /></td>
				</tr>
			</c:forEach>
		</c:if>
	</tbody>
...
$(function(){
	...
	var order2 = '${fn:escapeXml(proposalStarPointStatisticsSearchParam.order)}'.toLowerCase();
	var sort2 = '${fn:escapeXml(proposalStarPointStatisticsSearchParam.sort)}'.toLowerCase();
