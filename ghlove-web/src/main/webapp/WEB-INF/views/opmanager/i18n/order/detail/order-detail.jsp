	...
	<div class="board_list">
		...
		<table class="board_write_table">
			...
			<tr>
				...
				<td><div><c:out value="${op:datetime(order.createdDate)}"/></div></td>
			</tr>
...
<%-- 				<C:When test="${requestContext.sellerPage == false}"> --%>
					<tr>
						...
						<td>
							<div><c:out value="${order.buyerName}"/></div>
						</td>
					</tr>
<%-- 				</c:when> --%>
<%-- 				<c:otherwise> --%>
...
<%-- 			</c:choose> --%>
	...
	<c:if test="${requestContext.sellerPage == false}">
		<h3 class="mt30" hidden>고객상담 내역</h3>
		<div id="claim-memo-list" hidden></div>

		<div style="width:50%;float:left;padding:5px;" hidden>
			<div class="board_list">
				...
				<form id="orderAdminMemoForm">
					...
					<table class="board_write_table">
						...
						<tr>
							<td colspan="2">
								<div>
									<textarea name="orderAdminMemo" maxlength="1000" class="" title="관리자 메모"><c:out value="${order.orderAdminMemo}"/></textarea>
								</div>
							</td>
							<td>
								<button type="submit" class="btn btn-active">저장</button>
	...
	<c:if test='${mode != "popup"}'>
        <div class="btn_all btn_center">
            <div class="flex_box gap-08">
                <button type="button" onclick="Link.list('${requestContext.managerUri}/order/${pageType == 'all'?'list':pageType}')" class="btn btn-default btn-small">목록</button>
            </div>
        </div>
	</c:if>
...
<script type="text/javascript">
        ...
        $('body').on('click', 'input[name="claimType"]', function(){
            ...
            $.each(claimItems, function(){
                ...
                if (claimType == '1' && (orderStatus == '10' || orderStatus == '20')) {
                    ...
                    $('#reason_input_area').show();
                    $('#reason_btn_area').show();
    ...
    function rePayShippingView() {
        $.post('${requestContext.sellerPage ? "/seller" : "/opmanager"}/order/${pageType}/re-shipping-amount', $("#cancelForm").serialize(), function(response){
        ...
        $('#exchangeForm').validator(function() {
            ...
            $.each($('input[name="exchangeIds"]:checked'), function(){
	            ...
	            if (status == '03') {
					...
					let checkMobile = "";
					try {
						checkMobile = $('input[name="exchangeApplyMap['+ $(this).val() +'].exchangeShippingCompanyName"]').val();
					} catch (e) {
						$s.error(e);
					}

                    if ($('input[name="exchangeApplyMap['+ $(this).val() +'].exchangeDeliveryNumber"]').val() == '') {
                    	if (checkMobile == "모바일번호") {
                        	alert('재발송 모바일 번호를 입력해 주세요.');
                    	} else {
	            ...
	            if (status == '99') {
			        let returnRefusalReason = $('input[name="exchangeApplyMap['+ $(this).val() +'].exchangeRefusalReasonText"]').val();
			        if (!returnRefusalReason) {
				        alert('교환거절 사유를 입력해주세요.');
	                    statusList = [];
				        $('input[name="exchangeApplyMap['+ $(this).val() +'].exchangeRefusalReasonText"]').focus();
				        isSuccess = false;
				        return false;
			        }
	            }
    ...
    function returnFormValidator() {
        $('#returnForm').validator(function() {
	        ...
	        $.each($('input[name="returnIds"]:checked'), function(){
		        ...
		        if (status == '99') {
		        	/*
		        	let rejectShippingDeliveryCompanyId = $('select[name="returnApplyMap['+ key +'].rejectShippingDeliveryCompanyId"]').val();
		        	if (!rejectShippingDeliveryCompanyId) {
				        alert('반품거절 송장 택배사 정보를 입력해주세요.');
	                    statusList = [];
				        $('select[name="returnApplyMap['+ key +'].rejectShippingDeliveryCompanyId"]').focus();
				        isSuccess = false;
				        return false;
		        	}

		        	let rejectShippingDeliveryNumber = $('input[name="returnApplyMap['+ key +'].rejectShippingDeliveryNumber"]').val();
		        	if (!rejectShippingDeliveryNumber) {
				        alert('반품거절 송장번호를 입력해주세요.');
	                    statusList = [];
				        $('input[name="returnApplyMap['+ key +'].rejectShippingDeliveryNumber"]').focus();
				        isSuccess = false;
				        return false;
		        	}
		        	*/
			        let returnRefusalReason = $('input[name="returnApplyMap['+ key +'].returnRefusalReasonText"]').val();
			        if (!returnRefusalReason) {
				        alert('반품거절 사유를 입력해주세요.');
	                    statusList = [];
				        $('input[name="returnApplyMap['+ key +'].returnRefusalReasonText"]').focus();
				        isSuccess = false;
				        return false;
			        }
		        }
