...
<form:form modelAttribute="userDelivery" method="post" action="${fn:escapeXml(action)}">
	...
	<div class="board_write">
		<table class="board_write_table" summary="신규회원등록에 관련한 정보를 입력하는 칸입니다.">
 			...
 			<tbody>
 				<tr>
 					<td class="label">${op:message('M01584')} ${fn:escapeXml(requireMark)}</td> <!-- 배송지 이름 -->
 					<td>
 						<div>
 				...
 				<tr>
 					<td class="label">${op:message('M01583')} ${fn:escapeXml(requireMark)}</td><!-- 받으시는 분 -->
 					<td>
 						<div>
 				...
 				<tr>
 					<td class="label">전화번호 ${fn:escapeXml(requireMark)}</td>
 					<td>
 						<div>
	 						...
	 						<select title="전화번호 앞자리" id="phone1" name="phone1">
								...
								<c:forEach items="${op:getCodeInfoList('TEL')}" var="codes">
									<option value="${fn:escapeXml(codes.detail)}"  ${op:selected(userDelivery.phone1, codes.detail)}> ${fn:escapeXml(codes.label)}</option>
								</c:forEach>
							</select>
	 						-
 						...
 						<div>
	 						...
	 						<select class="required" title="전화번호 앞자리" id="mobile1" name="mobile1">
								...
								<c:forEach items="${op:getCodeInfoList('PHONE')}" var="codes">
									<option value="${fn:escapeXml(codes.detail)}" ${op:selected(userDelivery.mobile1, codes.label)}> ${fn:escapeXml(codes.label)}</option>
								</c:forEach>
							</select>
	 						-
 				...
 				<tr>
 					<td class="label last">주소 ${fn:escapeXml(requireMark)}</td>
 					<td class="last">
 						<div>
	...
	<div class="popup_btns">
		...
		<button type="button" class="btn btn-default" onclick="location.href='/opmanager/user/popup/delivery-list/${fn:escapeXml(userDelivery.userId)}'">${op:message('M00037')}</button> <!-- 취소 -->
	</div>
</form:form>
