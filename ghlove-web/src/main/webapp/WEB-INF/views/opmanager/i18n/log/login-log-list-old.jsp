...
<form:form modelAttribute="loginLogParam" method="get">
	...
	<div class="btn_all flex_box juc-sbt">
        <button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/opmanager/log/login-log'"><span class="glyphicon glyphicon-repeat"></span> <c:out value="${op:message('M00047')}"/></button> <!-- 초기화 -->
	...
	<div class="count_title mt20">
		...
		<span>
			<span><c:out value="${op:message('M00052')}"/> <!-- 출력수 --></span>
		...
		<tbody>
			<c:forEach items="${list}" var="list" varStatus="i" >
				<tr>
					<td>
						<div>
							<c:out value="${op:datetime(list.loginDate) }"/>
						</div>
					</td>
					<td>
						<div>
							<c:out value='${list.loginType == "manager" ? "관리자" : list.loginType == "seller" ? "판매자" : list.loginType }'/>
						</div>
					</td>
					<td>
						<div>
					...
					<td>
						<div>
							<c:out value='${list.successFlag == "Y" ? "성공" : "실패" }'/>
						</div>
					</td>
					<td>
						<div>
							<c:out value="${list.remoteAddr }"/>
						</div>
					</td>
					<td>
						<div>
