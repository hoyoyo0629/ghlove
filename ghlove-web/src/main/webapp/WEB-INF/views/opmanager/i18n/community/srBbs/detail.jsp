		...
		<div class="center">
			<div class="s-contents list-page-wrapper">
				...
				<div>
					...
					<div style="padding: 10px 0px; margin: 10px 0px; font-size: 20px; border-bottom: 1px solid #000">댓글<span style="color: red; margin-left: 10px;">[<c:out value="${srBbsCmntListCnt}" />]</span></div>
					...
					<div id="cmntList" style="border-top: 1px solid #000">
						<c:forEach items="${detail.cmntySrBbsCmntList}" var="cmnt" varStatus="i">
							<li style="border-bottom: 1px solid #ccc; padding: 15px 10px; margin-bottom: 10px; display:inline-block; width:100%">
								...
								<c:if test="${!empty cmnt.cmntySrBbsCmntFileList}">
			                     <div style="padding: 10px; display:flex;">
			                       ...
			                       <div class="file-list-title">
			             	         	...
			             	         	<c:forEach items="${cmnt.cmntySrBbsCmntFileList}" var="cmntFile" varStatus="i">
							...
							<form:form class="cmntFileUploadForm" id="cmntFileUploadForm${cmnt.cmntId}" method="post" action='/opmanager/community/srBbs/cmnt/upload' enctype="multipart/form-data">
			...
			<div class="btn-box">
				...
				<c:if test="${detail.frstCrtId eq userId }">
					...
					onClick="location.href='/opmanager/community/srBbs/edit/${detail.bbsId}'">
...
<script type="text/javascript" src="/content/modules/cmnty/srBbsCmnt.js?<spring:eval expression="@environment.getProperty('jsVersion')" />"></script>
...
function downloadItemImage(fileId) {
	location.href = '/opmanager/community/srBbs/file-download/'+fileId
}

function deleteItem(bbsId) {
	var url = "/opmanager/community/srBbs/deleteSrBbs/" + bbsId
...
function addCmnt() {
	...
	let cmntySrBbsCmntDto = {bbsId : $('#bbsId').val(), cmntCn : $('#cmntCn').val()};
	$.post("/opmanager/community/srBbs/cmnt/add/", cmntySrBbsCmntDto, function(response) {
		Common.responseHandler(response, function() {
			if(response.isSuccess) {
				alert(response.data);
				location.href = "/opmanager/community/srBbs/detail/"+$('#bbsId').val();
			}
		});
	});
}
...
function deleteItemImage(fileId) {
	...
	$.post('/opmanager/community/srBbs/cmnt/deleteFile', param, function(response){
...
function downloadCmntItemImage(fileId) {
	location.href = '/opmanager/community/srBbs/cmnt/file-download/'+fileId
}
</script>
