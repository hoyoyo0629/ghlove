			...
			<tbody>
				<c:forEach items="${mailConfigList}" var="list" varStatus="i">
					<tr>
						 ...
						 <td>${fn:escapeXml(list.templateId)}</td>
						 <td>${fn:escapeXml(list.title)}</td>
						 <td>${fn:escapeXml(list.buyerSendFlagText) }</td>
						 <td>${fn:escapeXml(list.adminSendFlagText) }</td>
						 <td><a href="/opmanager/mail-config/edit/${fn:escapeXml(list.mailConfigId)}" class="btn_write" title="수정">수정</a> <a href="javascript:;" class="btn_write deleteMail" title="삭제" rel="${fn:escapeXml(list.mailConfigId)}">삭제</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>	
