...
<form:form modelAttribute="searchParam" cssClass="opmanager-search-form clear" method="post" id="searchForm">
	...
	<div class="board_write">
				...
				<tr>
					...
					<td>
						<div class="flex_box gap-08">
							<form:select path="srchKey" title="지자체명" class="wd-150">
								<c:if test="${adminRoleType eq 'SYS'}"><form:option value="LOCGOV_NM">지자체명</form:option></c:if>
		...
		<div class="btn_all btn_right">
			<div class="flex_box gap-08">
				<button type="button" class="btn btn-dark-gray btn-mini" onclick="location.href='/opmanager/user/locgov-charger/list'">초기화</button>
...
<div class="board_list">
	<table class="board_list_table" summary="담당자관리">
		...
		<thead>
			<tr>
				...
				<th scope="col">No.</th>
				<c:if test="${adminRoleType eq 'SYS'}"><th scope="col">지자체명</th></c:if>
		...
		<tbody>
			<c:choose>
					...
					<c:forEach items="${list}" var="item" varStatus="i">
						<tr style="background:#fff;">
							...
							<c:if test="${adminRoleType eq 'SYS'}">
								<td>
									<a href="javascript:chargerEdit('${fn:escapeXml(item.userId)}');"><c:out value="${item.upperLocgovNm}"/>&nbsp;<c:out value="${item.locgovNm}"/></a>
								</td>
							</c:if>
							<td><c:out value="${item.authority eq 'ROLE_ADMIN_5' ? '주관리자' : '-'}"/></td>
							<c:choose>
								<c:when test="${adminRoleType eq 'LOC'}">
									<td><a href="javascript:chargerEdit('${item.userId}');"><c:out value="${item.loginId}"/></a></td>
									<td><a href="javascript:chargerEdit('${item.userId}');"><c:out value="${item.userName}"/></a></td>
								</c:when>
								<c:otherwise>
				...
				<c:otherwise>
					<tr style="background:#fff;">
						<td colspan="${adminRoleType eq 'SYS' ? 10 : 9}">담당자 정보가 존재하지 않습니다.</td>
					</tr>
				</c:otherwise>
			</c:choose>
		</tbody>
	</table>
	<c:if test="${adminRoleType eq 'SYS' or (adminRoleType eq 'LOC' and adminRoleGrade eq 'M')}">
		<div class="btn_all btn_right">
			<div class="flex_box gap-08">
				<button type="button" class="btn btn-defualt btn-mini" onclick="chargerDelete();" style="display:none;">삭제</button>
			</div>
		</div>
	</c:if>
...
<script type="text/javascript">
	...
	function chargerEdit(userId) {
		location.href = "/opmanager/user/locgov-charger/edit/"+userId+location.search;
	}
