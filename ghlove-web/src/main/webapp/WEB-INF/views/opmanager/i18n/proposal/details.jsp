	...
	<c:choose>
		...
		<div class="board_view">
			<div class="title_wrap">
				<h4 class="title"><c:out value="${ proposal.title }"/></h4>
				<p class="info info_tooltip">
					<span class="state">상태 : <span class="icon ${fn:escapeXml(statusIcon)}"><c:out value="${ statusAlt }"/></span></span>
				</p>
				<c:if test="${proposal.statusCode == '7'}">
					<div class="tooltip_wrap reject_cause">
						...
						<div class="tooltip_area">
							<div>
								<p>반송사유</p>
								<p><c:out value="${proposal.approvalRemark}"/></p>
			...
			<div class="info">
				<p class="detail">
					<span><c:out value="${ proposal.userName }"/></span>
					<span>목표금액 :  <fmt:formatNumber value="${fn:escapeXml(proposal.targetAmount)}" pattern="#,###" /> 원</span>
					<span>모금기간 : <c:out value="${ op:formatDate(proposal.startDate,'-') }"/> ~ <c:out value="${ op:formatDate(proposal.endDate,'-') }"/></span>
					<span>등록일 : <c:out value="${ fn:substring(proposal.creationDate, 0, 10)}"/></span>
				</p>
			</div>
			<div class="text">
				<c:forEach items="${ uploadFiles }" var="uploadFile" varStatus="i">
					<p class="img">
						<img src="/upload/proposal/${fn:escapeXml(proposal.proposalId)}/${fn:escapeXml(uploadFile.fileId)}.${fn:escapeXml(uploadFile.fileType)}">
					</p>
					<p class="hidden">
						<c:out value="${uploadFile.fileDescription}"/>
					</p>
				</c:forEach>
			</div>
			<p class="btns">
				...
				<span class="right">
					...
					<c:if test="${proposalApprovalInfo.approvalButtonView}">
						<a href="javascript:accept('${fn:escapeXml(proposal.proposalId)}')" class="btn green"><img src="/content/opmanager/images/icon/icon_check.png" alt="" /> <c:out value="${proposalApprovalInfo.approvalButtonText}"/></a>
						<a href="javascript:reject('${fn:escapeXml(proposal.proposalId)}')" class="btn purple"><img src="/content/opmanager/images/icon/icon_info2.png" alt="" /> 반송</a>
					</c:if>

					<a href="javascript:list('${fn:escapeXml(requestContext.prevPageUrl)}')" class="btn gray">목록</a>
...
function accept(proposalId) {
	var statusCode = "${fn:escapeXml(proposal.statusCode)}";
	var approvalStatusCode = "${fn:escapeXml(proposalApprovalInfo.approvalStatusCode)}";
	...
	if (statusCode == '1') {
		...
		approvalMessage = '${fn:escapeXml(proposal.updatedUsername)}님의 확인요청 건\n확인처리 하시겠습니까?';
	}
