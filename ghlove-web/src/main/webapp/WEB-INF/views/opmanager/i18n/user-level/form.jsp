...
<h3><span><c:out value="${op:message('M00267')}"/></span></h3>

<form:form modelAttribute="userLevel" action="${fn:escapeXml(requestContext.opmanagerUri)}/user-level/${fn:escapeXml(formAction)}" method="post" enctype="multipart/form-data">
	...
	<div class="board_write">
		<table class="board_write_table" summary="신규회원등록에 관련한 정보를 입력하는 칸입니다.">
			...
			<tbody>
				<tr>
					<th class="label"><c:out value="${op:message('M00262')}"/></th>
					<td>
						<div>
						 	<c:choose>
				 				<c:when test="${!empty userLevel.fileName}">
						 			<img alt="${fn:escapeXml(userLevel.fileName)}" src="/upload/user_level/${fn:escapeXml(userLevel.levelId)}/${fn:escapeXml(userLevel.fileName)}" style="width: 60px;" />
		 							<a href="javascript:fileDelete('${fn:escapeXml(userLevel.levelId)}');"><c:out value="${op:message('M00074')}"/></a>
				...
				<tr>
					<th class="label"><c:out value="${op:message('M00263')}"/></th>
					<td>
						<div>
	...
	<div class="tex_c mt20">
		<button type="submit" class="btn btn-active"><c:out value="${op:message('M00101')}"/></button>
		<a href="/opmanager/user-level/list/${fn:escapeXml(userLevel.groupCode)}" class="btn btn-default"><c:out value="${op:message('M00037')}"/></a>
	</div>
</form:form>
