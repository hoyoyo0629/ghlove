...
<%@ taglib prefix="shop"	uri="/WEB-INF/tlds/shop" %>
	...
	<h3><span>${fn:escapeXml(title)} ${op:message('M00392')} <!-- 이메일 설정 등록/수정 --></span></h3>

	<ul class="mail_list">
		<c:forEach items="${mailTemplateCodeList}" var="list">
			<li ><a href="<c:url value="/opmanager/mail-config/edit/${fn:escapeXml(list.key)}" />" <c:if test="${ mailConfig.templateId eq list.key}">style="color:red;"</c:if>>${fn:escapeXml(list.value)}</a></li>
		</c:forEach>
	</ul> <!-- // mail_list -->
	<br/>
	<span class="mail_code" >
		<a href="/opmanager/mail-config/change-code/${fn:escapeXml(mailConfig.templateId)}" class="btn_right btn gray02" onclick="Common.popup(this.href, 'orderView', 500, 500, 1); return false;" title="새창">${op:message('M00393')} <!-- 메일 대체코드 --></a>
	</span>
	<br/><br/>
	<form:form modelAttribute="mailConfig" method="post" enctype="multipart/form-data">
					...
					<td>
                        <div class="flex_box gap-12">
                            <div class="input-form">
                                <form:radiobutton path="buyerSendFlag" value="Y" class="required _filter" id="rdo2-1" name="rdo2" checked="checked"/>
                                <label for="rdo2-1">${op:message('M00397')}</label>
                            </div>
                            <div class="input-form">
                                <form:radiobutton path="buyerSendFlag" value="N" class="required _filter" id="rdo2-2" name="rdo2"/>
                                <label for="rdo2-2">${op:message('M00398')}</label>
                            </div>
                        </div>
					</td>
				</tr>
			...
			<div class="board_write">
				...
				<table class="board_write_table">
					...
					<tr>
						...
						<td>
							<div>
								<form:textarea path="buyerContent" cols="30" rows="40" class="editor-content" title="${op:message('M00400')}" />
							</div>
						</td>
					</tr>
				</table>
        ...
        <div class="btn_all btn_center">
            <div class="flex_box gap-08">
                <button type="submit" class="btn btn-active btn-small">${op:message('M00101')} <!-- 저장 --></button>
            </div>
        </div>
	</form:form>
