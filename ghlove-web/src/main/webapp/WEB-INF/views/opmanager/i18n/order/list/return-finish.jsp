...
<form:form modelAttribute="orderParam" action="" method="post">
	...
	<div class="board_write">
		...
		<div class="btn_all">
			<div class="btn_left">
				<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/opmanager/order/return-finish'"><span><c:out value="${op:message('M00047')}"/><!-- 초기화 --></span></button>
			</div>
			<div class="btn_right">
				...
				<c:forEach items="${ list }" var="orderShipping" varStatus="index">
					<c:forEach items="${orderShipping.orderItems}" var="orderItem">
						...
						<tr>
							...
							<td><c:out value="${op:datetime(orderItem.returnRequestFinishDate)}"/></td>
							<td>
								<a href="javascript:Common.popup('/opmanager/order/return-finish/order-detail/${fn:escapeXml(orderItem.orderCode)}', 'returnFinishDetail', 1000, 700, 1);"><c:out value="${fn:escapeXml(orderItem.orderCode)}"/></a>
							</td>
							<td>
								<c:choose>
