...
<%@ taglib prefix="op" 		uri="/WEB-INF/tlds/functions" %>
	...
	<div class="board_write">
		<form:form modelAttribute="groupParam" class="opmanager-search-form clear" method="post">
			...
			<div class="btn_all">
				<div class="btn_left">
					<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/opmanager/group/list';"><span class="glyphicon glyphicon-repeat"></span> <c:out value="${op:message('M00047')}"/></button> <!-- 초기화 -->
				</div>
				<div class="btn_right">
		...
		<table class="board_list_table">
			...
			<tbody>
				<c:forEach items="${list}" var="group" varStatus="i">
					<tr>
						<td><input type="checkbox" name="id" value="${fn:escapeXml(group.groupCode)}" title="" /></td>

						<td>
						 	<c:out value="${pagination.itemNumber - i.count}"/>
						</td>
						<td><a href="/opmanager/group/edit/${fn:escapeXml(group.groupCode)}"><c:out value="${group.groupName}"/></a></td>
						<td><c:out value="${group.groupExp}"/> </td>
						<td><c:out value="${group.userCount}"/></td>
						<td><c:out value="${op:date(group.createdDate)}"/></td>
						<td>
							<a href="/opmanager/user-level/list/${fn:escapeXml(group.groupCode)}" class="btn btn-gradient btn-xs">등급관리</a><br/>
					 		<a href="javascript:userGroupEdit('${fn:escapeXml(group.groupCode)}');" class="btn btn-gradient btn-xs">수정</a>
					 		<a href="javascript:userGroupDelete('${fn:escapeXml(group.groupCode)}');" class="btn btn-gradient btn-xs">삭제</a>
						</td>
					</tr>
				</c:forEach>
			</tbody>
