...
<form:form modelAttribute="remittanceParam" action="" method="post">
	...
	<div class="board_write">
		...
		<div class="btn_all btn_right">
			<div class="flex_box gap-08">
				<button type="button" class="btn btn-dark-gray btn-mini" onclick="location.href='${requestContext.sellerPage ? '/seller' : '/opmanager'}/remittance/confirm/list'"> <c:out value="${op:message('M00047')}"/><!-- 초기화 --></button>
...
<div class="board_list">
	...
	<form id="listForm" action="${fn:escapeXml(requestContext.managerUri)}/remittance/confirm/list/updateNew" method="post">
				...
				<c:forEach items="${ list }" var="item" varStatus="index">
					...
					<tr style="background:#fff;">
						<c:if test="${!requestContext.sellerPage}">
							...
							<c:if test="${!requestContext.sellerPage
										&& (op:hasRole('ROLE_ADMIN_1') || op:hasRole('ROLE_ADMIN_2') || op:hasRole('ROLE_ADMIN_3') || op:hasRole('ROLE_ADMIN_4'))}">
								<td><div><c:out value="${fn:escapeXml(item.locgovNm)}"/></div></td>
							</c:if>
							<td>
								...
								<a href="${requestContext.sellerPage ? '/seller' : '/opmanager'}/remittance/confirm/detailNew/view/${item.remittanceId}"><c:out value="${item.companyName}"/></a>
							</td>
							...
							<td><c:out value="${item.remiDetailCnt}"/></td>
							<td>
								<c:choose>
