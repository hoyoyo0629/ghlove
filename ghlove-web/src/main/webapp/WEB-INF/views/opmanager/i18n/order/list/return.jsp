	...
	<div class="board_write">
		...
		<table class="board_write_table" summary="${ fn:escapeXml(title) }">
			...
			<tbody>
				...
				<tr>
				 	<td class="label"><c:out value="${op:message('M01520')}"/></td>
				 	<td>
				 		<div>
				 ...
				 <tr>
				 	...
				 	<td>
				 		<div class="flex_box gap-08">
							...
							<div class="checkbox">
								<form:checkbox path="claimStatus" value="03" label=" 반품환불대기" />
							</div>
							<div class="checkbox">
								<form:checkbox path="claimStatus" value="04" label=" 반품완료" />
							</div>
				 		</div>
				 	</td>
				 </tr>
		...
		<div class="btn_all btn_right">
			<div class="flex_box gap-08">
				<button type="button" class="btn btn-dark-gray btn-mini" onclick="location.href='${requestContext.sellerPage ? '/seller' : '/opmanager'}/order/return/list'"> <c:out value="${op:message('M00047')}"/><!-- 초기화 --></button>
			...
			<tbody>
				...
				<c:forEach items="${ list }" var="apply" varStatus="index">
					...
					<tr style="background:${fn:escapeXml(tdColor)};">
						...
						<c:if test="${requestContext.sellerPage == false}">
							<c:if test="${(op:hasRole('ROLE_ADMIN_1') || op:hasRole('ROLE_ADMIN_2') || op:hasRole('ROLE_ADMIN_3') || op:hasRole('ROLE_ADMIN_4'))}">
								<td><div><c:out value="${apply.locgovNm}"/></div></td>
							</c:if>
						</c:if>
						<td><c:out value="${op:datetime(orderItem.createdDate)}"/></td>
						<td><c:out value="${op:datetime(apply.createdDate)}"/></td>
						<td>
							<a href="${fn:escapeXml(requestContext.sellerPage) ? '/seller' : '/opmanager'}/order/return/order-detail/${fn:escapeXml(orderItem.orderSequence)}/${fn:escapeXml(orderItem.orderCode)}?url=${fn:escapeXml(requestContext.currentUrl)}"><c:out value="${orderItem.orderCode}"/></a>
						</td>
