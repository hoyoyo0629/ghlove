...
<%@ taglib prefix="shop"	uri="/WEB-INF/tlds/shop" %>
    ...
    <div class="board_write">
                           ...
                           <div>
							<c:choose>
								<c:when test="${qna.userId > 0 && !empty qna.loginId}">
									<c:out value="${fn:escapeXml(qna.userName)}"/> <a href="/opmanager/user/customer/details/${qna.userId}" target="_blank" rel="noopener noreferrer">(<c:out value="${fn:escapeXml(qna.loginId)}"/>)</a>
									<!-- a href="javascript:Common.popup('/opmanager/user/popup/details/${qna.userId}', '/user/popup/details', 1100, 800, 1, 0, 0)">${qna.userName} (${qna.loginId})</a-->
								</c:when>
       ...
       <form id="qnaParam" method="post" action="/opmanager/qna-open/list">
	...
	<form:form modelAttribute="qnaAnswer" action="/opmanager/qna-open/answer/${fn:escapeXml(qna.qnaId)}" method="post" enctype="multipart/form-data">
		...
		<input type="hidden" name="qnaDetailType" value="qna-open" />
	...
	<div class="btn_all btn_right">
	  ...
	  <form:input  type="hidden" path="title" title="답변제목" class="input_txt required _filter full" value="답변입니다." maxlength="100" />
	</form:form>
...
function qnaList() {
	$("#qnaParam").attr("method", "get").attr("action", "/opmanager/qna-open/list").submit();
}

function deleteQna(id) {
	Common.confirm("삭제하시겠습니까?", function() {
		$.get(url("/opmanager/qna-open/delete/" + id), {}, function(response) {
			Common.responseHandler(response, function() {
				alert("${op:message('M00205')}");
				location.href = "/opmanager/qna-open/list";
// 				location.reload();
			});
		});
	});
...
function downloadItemImage(id, qnaDetailType) {
	location.href = '/opmanager/qna-open/file-download/'+encodeURIComponent(id)+'/'+qnaDetailType;
}
...
function deleteItemImage(id, index) {
	...
	$.post('/opmanager/qna-open/delete-item-image', param, function(response){
