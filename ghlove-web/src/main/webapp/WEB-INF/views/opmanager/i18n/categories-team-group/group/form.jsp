...
<style type="text/css">
	...
	<h3><span>${op:message('MENU_2104')} <!-- 카테고리 그룹 관리 --> </span></h3>
    ...
    <div class="btn_all btn_right mb15">
        <div class="flex_box gap-08">
			<c:if test="${categoriesGroup.categoryGroupId > 0}">
				<button type="submit" class="btn btn-dark-gray btn-mini">${op:message('M00087')} <!-- 수정 --> </button>
				<button type="button" onclick="deleteCategoryGroup(${fn:escapeXml(categoriesGroup.categoryGroupId)})" class="btn btn-dark-gray btn-mini">${op:message('M00074')} <!-- 삭제 --> </button>
				<button type="button" class="btn btn-default btn-mini" onclick="location.href='/opmanager/categories-team-group/list';">목록</button>
			</c:if>
        </div>
    </div>

	<div class="board_write">
		<table class="board_write_table" summary="${op:message('MENU_2104')} <!-- 카테고리 그룹 관리 --> ">
			<caption>${op:message('MENU_2104')} <!-- 카테고리 그룹 관리 --> </caption>
			<colgroup>
			...
			<tbody>
			 	 <tr>
					<td class="label">${op:message('M00865')} <!-- 대그룹 --> </td>
					<td>
						<div>
							<form:select path="categoryTeamId" items="${op:nullToEmptyMap(categoriesTeamSelectBox)}" class="required full" title="${op:message('M00865')}" readonly="true"></form:select>
					 	</div>
					</td>
				</tr>
				<tr>
					<td class="label">${op:message('M00866')} <!-- 카테고리(메뉴)명 --> </td>
					<td>
						<div>
							<form:input path="name" class="input_txt required _filter full" title="${op:message('M00866')}" /><!-- 카테고리(메뉴)명 -->
					 	</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						<div>
							<form:input path="code" class="input_txt required _filter full" title="CODE" />
					 	</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						<div class="flex_box gap-12">
							<div class="input-form">
								<form:radiobutton path="accessType" value="1" checked="checked" />
								<label for="accessType1">${op:message('M00497')}</label><!-- 제한없음 -->
							</div>
							<div class="input-form hidden">
								<form:radiobutton path="accessType" value="2" />
								<label for="accessType2">${op:message('M01679')}</label><!-- 성인만 -->
							</div>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						<div class="flex_box gap-12">
							<div class="input-form">
								<form:radiobutton path="categoryGroupFlag" value="Y" /> <!-- 공개 -->
								<label for="categoryGroupFlag1">${op:message('M00096')}</label><!-- 공개 -->
							</div>
							<div class="input-form">
								<form:radiobutton path="categoryGroupFlag" value="N" /> <!-- 비공개 -->
								<label for="categoryGroupFlag2">${op:message('M00097')}</label><!-- 비공개 -->
							</div>
						</div>
					</td>
				</tr>
	...
	<div class="email_tb" style="display:none;">
					...
					<div class="bundle_two">
						...
						<c:if test="${list.categoryGroupId == 0}">
							...
							<c:forEach items="${list.categoryList}" var="list2">
								<li class="categories_go" id="${list2.categoryCode}"><a href="javascript:;">- ${fn:escapeXml(list2.categoryName)}</a></li>
							</c:forEach>
							</ul>
						</c:forEach>
					</div>
						...
						<ul id="addCategory">
							<c:forEach items="${categoriesList}" var="list">
								<li class="categories_value">
									...
									<input type="hidden" name="categoryCodes" value="${fn:escapeXml(list.categoryCode)}" />
								</li>
							</c:forEach>
						</ul>
					</div> <!-- // bundle_two -->
	...
	<div class="btn_all btn_center">
		<div class="flex_box gap-08">
			<c:if test="${categoriesGroup.categoryGroupId == 0}">
				<button type="submit" class="btn btn-dark-gray btn-small">${op:message('M00088')} <!-- 등록 --> </button>
				<button type="button" class="btn btn-default btn-small" onclick="location.href='/opmanager/categories-team-group/list';">${op:message('M00037')}</button>
			</c:if>
			<c:if test="${categoriesGroup.categoryGroupId > 0}">
...
var categoryGroupId = '${fn:escapeXml(categoriesGroup.categoryGroupId)}';
