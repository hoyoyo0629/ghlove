								...
								<c:forEach items="${list.deliveryChargeList}" var="chargeList" varStatus="j">
									...
									<c:choose>
										<c:when test="${j.count == 1}">
											<tr>
												<td ${fn:escapeXml(rowspan)}><c:out value="${i.count}" /></td>
												<td ${fn:escapeXml(rowspan)} class="text-left" style="border-left: 1px solid #ececec"><c:out value="${list.title}" /></td>
												<td ${fn:escapeXml(rowspan)} style="border-left: 1px solid #ececec">${fn:escapeXml(list.deliveryType) == "1" ? op:message('M00637') : op:message('M00638')}</td> <!-- 묶음배송 -->  <!-- 개별배송 -->
												...
												<td style="border-left: 1px solid #ececec" ${fn:escapeXml(rowspan)}>
													<div>
														<a href="/opmanager/delivery/edit/${fn:escapeXml(list.deliveryId)}" class="table_btn">${op:message('M00087')}</a> <!-- 수정 --> 
														<a href="javascript:deleteCheck('${fn:escapeXml(list.deliveryId)}');" class="table_btn">${op:message('M00074')}</a> <!-- 삭제 --> 
													</div>
												</td>
											</tr>
										</c:when>
