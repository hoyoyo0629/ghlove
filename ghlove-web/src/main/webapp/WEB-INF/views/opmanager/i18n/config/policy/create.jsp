...
<h3><span>${fn:escapeXml(typeString)}</span></h3>
<ul class="mail_list">
	...
	<form:form modelAttribute="policy" method="post">
		<table class="board_write_table" cellpadding="0" cellspacing="0" summary="${op:message('M00206')}">
			...
			<tr>
					...
					<div>
						<select id="policyList">
							<c:forEach var="p" items="${policyList}">
								<option value="${fn:escapeXml(p.policyId)}" ${op:selected(p.policyId, policy.policyId)}>${op:datetime(p.createdDate)}</option>
							</c:forEach>
						</select>
					</div>
				</td>
				<td>
					<div>
						<form:textarea path="content" cols="100" rows="20" class="editor-content editor-lg" title="내용" />
					</div>
				</td>
			</tr>
			</tbody>
...
<script type="text/javascript">
    $(function() {
        ...
        $('#policyList').on('change', function() {
            location.href = '/opmanager/config/policy/edit/${fn:escapeXml(type)}?id='+$(this).val();
        });
    });
