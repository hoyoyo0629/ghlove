...
<h3 class="mt10">[<c:out value="${seller.sellerName}"/>] 배송비 정산 상세 - <c:out value="${op:date(remittanceParam.startDate)}"/> ~ <c:out value="${op:date(remittanceParam.endDate)}"/></h3>
...
<div class="board_write">
		...
		<table class="board_write_table">
			...
			<tbody>
				<tr>
					...
					<td>
						<div><c:out value="${seller.userName}"/></div>
					</td>
					<th class="label" rowspan="3">주문 담당자</th>
					<th class="label">이름</th>
					<td>
						<div><c:out value="${seller.secondUserName}"/></div>
					</td>
				</tr>
				<tr>
					...
					<th class="label">전화번호</th>
					<td>
						<div><c:out value="${seller.secondTelephoneNumber}"/></div>
					</td>
				</tr>
				<tr>
					...
					<th class="label">휴대전화</th>
					<td>
						<div><c:out value="${seller.secondPhoneNumber}"/></div>
					</td>
				</tr>
			</tbody>
		</table>
							...
							<td>
								<c:choose>
									...
									<c:otherwise><strong>확정</strong>
										<p>(<c:out value="${op:date(item.remittanceDate)}"/>)</p>
									</c:otherwise>
								</c:choose>
							</td>
							<td class="amount text-right"><c:out value="${op:numberFormat(item.payShipping)}"/>원</td>
							<td class="amount text-right"><c:out value="${op:numberFormat(item.discountShipping)}"/>원</td>
							<td class="amount text-right"><c:if test="${item.returnFlag == 'Y'}">-</c:if><c:out value="${op:numberFormat(item.remittanceAmount)}"/>원</td>
							<td>
								<input type="text" name="editShippingRemittanceMap[${key}].remittanceExpectedDate" class="required" value="${fn:escapeXml(item.remittanceExpectedDate)}" />
							</td>
						</tr>
					</c:forEach>
				</tbody>
	...
	<div class="btn_all">
		...
		<div class="btn_right mb0">
			<a href="/opmanager/remittance/expected/list?${fn:escapeXml(queryString)}" class="btn btn-active btn-sm">목록으로</a>
		</div>
	</div>
