...
<form:form modelAttribute="batchJobParam" method="post">
	...
	<div class="btn_all btn_right">
		<div class="flex_box gap-08">
            <button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/opmanager/batch-job/list';"><span class="glyphicon glyphicon-repeat"></span> <c:out value="${op:message('M00047')}"/></button> <!-- 초기화 -->
            <button type="submit" class="btn btn-dark-gray btn-sm"><span class="glyphicon glyphicon-search"></span> 검색</button> <!-- 검색 -->
            <button type="button" class="btn btn-dark-gray btn-sm" onclick="testForNh()"> 농협배치테스트</button> <!-- 검색 -->
    ...
    <div class="count_title mt20">
        <h5>
        <c:out value="${op:message('M00039')}"/>: <c:out value='${batchJobList.size()}'/> <c:out value="${op:message('M00743')}"/>
        </h5>	 <!-- 전체 -->   <!-- 건 조회 -->
    </div>
</form:form>
...
<form id="listForm">
        ...
        <table class="board_list_table" summary="배치 작업">
            ...
            <c:forEach items="${batchJobList}" var="list" varStatus="i">
                <tr>
                    ...
                    <td><input type="hidden" name="batchJobId" id="batchJobId" value="${fn:escapeXml(list.batchJobId)}" /></td>
                    <td><c:out value='${list.jobName}'/></td>
                    <td><c:out value='${list.jobMethod}'/></td>
                    <td><c:out value='${list.triggerType}'/></td>
                    <td><c:out value='${list.triggerRepeatSeconds}'/></td>
                    <td><c:out value='${list.triggerCronExpression}'/></td>
                    <td><c:out value='${list.batchStatus}'/></td>
                    <td><c:out value='${list.batchExcuteDate}'/></td>
                    <td><c:out value='${list.batchApplyFlag}'/></td>
                    <td>
                        <button type="button" class="btn btn-gradient btn-xs" onclick="fnBatchJobUpdate('${fn:escapeXml(list.batchJobId)}')"><c:out value="${op:message('M00087')}"/></button> <!-- 수정 -->
                    </td>
                </tr>
            </c:forEach>
            </tbody>
	...
	<div class="btn_all btn_center">
		<div class="flex_box gap-08">
            ...
            <a href="javascript:fnBatchJobCreate();" class="btn btn-active btn-sm"><span class="glyphicon glyphicon-plus"></span> <c:out value="${op:message('M00088')}"/></a> <!-- 신규등록 -->
        </div>
    </div>
...
<script type="text/javascript">
    ...
    function testForNh() {
    	$.post("/opmanager/batch-job/batchForNh", {}, function(response) {
            Common.responseHandler(response, function(res){
