		...
		<tbody>
			<tr>
				<td class="f11">
					...
					${op:message('M00034')}<%-- ${op:message('M00603')} --%> <!-- 발송준비 --> : <span>${ op:numberFormat(userOrderCountInfo.orderStatus1) }</span>${op:message('M00272')} <!-- 건 --> |
					${op:message('M00382')}<%-- ${op:message('M00035')} --%> <!-- 발송완료 --> : <span>${ op:numberFormat(userOrderCountInfo.orderStatus3) }</span>${op:message('M00272')} <!-- 건 -->  |
...
<form:form modelAttribute="orderParam" action="" method="post">
				 	...
				 	<td>
				 		<div>
							...
							<c:forEach items="${ payTypeList }" var="item">
								...
								<form:radiobutton path="approvalType" label="${fn:escapeXml(payTypeLabel)}" value="${ fn:escapeXml(item.value) }" />
							</c:forEach>
						</div>
				 	</td>
				 </tr>
				 	...
				 	<td>
				 		<div>
							...
							<c:forEach items="${categoryCategoriesTeamList}" var="item" varStatus="i">
								<form:checkbox path="searchTeam" label="${ fn:escapeXml(item.name) }" value="${ fn:escapeXml(item.code) }" /> &nbsp;
							</c:forEach>
						</div>
				 	</td>
				 </tr>
	...
	<div class="btn_all">
		<div class="btn_left">
			<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/opmanager/user/popup/order/${fn:escapeXml(user.userId)}'"><span>${op:message('M00047')}<!-- 초기화 --></span></button>
		</div>
		<div class="btn_right">
		...
		<div class="left">
			...
			<c:if test="${ searchOrderStatus != '7' }">
				<select title="주문상태" id="orderStatus1">
					...
					<option value="1">${op:message('M00034')}<%-- ${op:message('M00603')} --%></option> <!-- 발송준비 -->
					<option value="3">${op:message('M00382')}<%-- ${op:message('M00035')} --%></option> <!-- 발송완료 -->
					...
					<tr>
						<td>
							<c:if test="${ !(vendor.orderStatus eq '99' || vendor.orderStatus eq '10' || vendor.orderStatus eq '16') }">
								<input type="checkbox" name="id" value="${ fn:escapeXml(order.orderId) }" />
							</c:if>
						</td>
						...
						<td>
							<div><a href="${ fn:escapeXml(href) }" target="_blank" onclick="Common.popup(this.href, 'orderView', 1200, 1000, 1); return false;">
								${ fn:escapeXml(order.orderCode) } <br/>
								<c:choose>
						...
						<td>${ fn:escapeXml(order.approvalTypeLabel) }</td>
						<td>${ fn:escapeXml(order.deliveryPrearrangedDate) }</td>
						...
						<td>${ fn:escapeXml(vendor.orderStatusLabel) }</td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
