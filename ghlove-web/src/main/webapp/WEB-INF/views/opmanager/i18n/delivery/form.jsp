...
<%@ taglib prefix="shop"	uri="/WEB-INF/tlds/shop" %>
		...
		<form:form modelAttribute="delivery" method="post" enctype="multipart/form-data">
			<h3><span><c:out value="${op:message('M00643')}"/></span></h3> <!-- 배송비 설정 -->
			<div class="board_write">
				<table class="board_write_table" summary="${op:message('M00643')}">
					<caption><c:out value="${op:message('M00643')}"/></caption>
					<colgroup>
					...
					<tbody>
						 <tr>
							<td class="label"><c:out value="${op:message('M00635')}"/></td> <!-- 배송비 정책 제목 -->
						 	<td>
						 		<div>
						 ...
						 <tr>
						 	<td class="label"><c:out value="${op:message('M00636')}"/></td> <!-- 배송 구분 -->
						 	<td>
						 		<div>
									<c:choose>
										...
										<c:otherwise>
											<c:out value="${delivery.deliveryType == 1 ? op:message('M00637') : op:message('M00638')}"/>
											<input type="hidden" name="deliveryType" id="editDeliveryType" value="${fn:escapeXml(delivery.deliveryType)}" />
										</c:otherwise>
									</c:choose>
								</div>
						 	</td>
						 ...
						 <tr>
							<td class="label">제주도</td>
						 	<td>
						 		<div>
						 			배송비 : <form:input type="text" path="deliveryExtraCharge1" class="required one _number" maxlength="6" title="제주도 배송비"/> <c:out value="${op:message('M00049')}"/> /
			...
			<h3 class="title"><span><c:out value="${op:message('M00067')}"/></span> <!-- 배송비 -->

			<!-- <div class="addDelivery btn_right">
				<button type="button" onclick="javascript:addDeliveryKind();" class="btn btn-dark-gray btn-sm"><span><c:out value="${op:message('M00655')}"/></span></button> <!-- 배송비 추가 -->
			<!-- </div> -->
			</h3>
			<div class="board_write">
				<table class="board_write_table" id="writeTable" summary="${op:message('M00067')}">
					<caption><c:out value="${op:message('M00067')}"/></caption>
					<colgroup>
						<col style="width:150px;">
						<col style="width:*">
					</colgroup>
						<c:forEach items="${delivery.deliveryChargeList}" var="list" varStatus="i" >
							...
							<tbody style="border-top : 1px solid #d5d5d5;">
								 		...
								 		<div>
								 			<c:choose>
												<c:when test="${list.deliveryChargeType == 1}">
									 				<c:out value="${op:message('M00653')}"/> <!-- 무료 (판매자 부담) -->
									 			</c:when>
									 			<c:when test="${list.deliveryChargeType == 2}">
									 				<c:out value="${op:message('M00639')}"/> <!-- 유료 -->
									 			</c:when>
									 			<c:when test="${list.deliveryChargeType == 3}">
									 				<c:out value="${op:message('M00640')}"/> <!-- 조건부 무료 -->
												</c:when>
									 		</c:choose>
										</div>
								 	</td>
								...
								<tr>
									<td class="label"><c:out value="${op:message('M00654')}"/></td> <!-- 금액 -->
									<td>
								 		<div>
								 			<c:out value="${list.deliveryCharge}"/><c:out value="${op:message('M00049')}"/>
								 			<c:if test="${list.deliveryChargeType == 3}">
								 				(<c:out value="${list.deliveryFreeAmount}"/><c:out value="${op:message('M00641')}"/>) <!-- 원 이상 구매시 배송비 무료 -->
								 			</c:if>
										</div>
									</td>
								</tr>
								<tr>
									...
									<td>
										<div class="btn_right">
											<a href="javascript:deliveryEditPopup('${fn:escapeXml(list.deliveryChargeId)}');" class="table_btn"><c:out value="${op:message('M00087')}"/></a> <!-- 수정 -->
											<!-- <a href="javascript:deleteCheck('${fn:escapeXml(list.deliveryId)}', '${fn:escapeXml(list.deliveryChargeId)}')" class="table_btn"><c:out value="${op:message('M00074')}"/></a> --> <!-- 삭제 -->
										</div>
									</td>
								</tr>
							</tbody>
			...
			<div class="btn_center">
				...
				<a href="/opmanager/delivery/list" class="btn btn-default"><span><c:out value="${op:message('M00037')}"/></span></a> <!-- 취소 -->
			</div>
		</form:form>
...
<table class="board_write_table" id="addWriteTable" style="display: none;">
	<tbody>
		...
		<tr>
			...
			<td>
		 		<div>
		 			 배송비 : <input type="text" name="deliveryChargeArray" class="freeText required one _number" value="0" maxlength="6" readonly="readonly" title="${op:message('M00067')}"/> <c:out value="${op:message('M00049')}"/>  <!-- 원 -->
		 			 <span class="amountText" style="display: none;">
		 			 	/ <input type="text" name="deliveryFreeAmountArray" class="freeAmountText required one _number" value="0" maxlength="6" title="${op:message('M00067')}"/> <c:out value="${op:message('M00641')}"/> <!-- 원 이상 구매시 배송비 무료 -->
		 			 </span>
				</div>
			</td>
		</tr>
...
function addDeliveryKind() {
	...
	//var randomKey =()=>crypto.getRandomValues(newUint32Array(1))[0]/2**32;
