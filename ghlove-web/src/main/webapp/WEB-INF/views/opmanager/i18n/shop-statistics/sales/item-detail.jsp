...
<h1 class="popup_title"><c:out value="${item.itemName}"/> <c:out value="${op:message('M01382')}"/> <c:out value="${op:message('M01296')}"/></h1><!-- 상품별 매출 상세 -->  <!-- 팝업 -->
<div class="popup_contents">
	 <h3><span><c:out value="${item.itemName}"/> <c:out value="${op:message('M01382')}"/></span></h3>			
	<form:form modelAttribute="statisticsParam" method="post">
		...
		<div class="btn_all">
			...
			<div class="btn_right">
				...
				<a href="/opmanager/shop-statistics/sales/item/${fn:escapeXml(item.itemId)}/excel-download?${fn:escapeXml(queryString)}"  class="btn btn-success btn-sm"><span class="glyphicon glyphicon-save"></span><c:out value="${op:message('M00254')}"/></a> <!-- 엑셀 다운로드 -->
			</div>
		</div>
	</form:form>
	...
	<div class="board_list" style="overflow: hidden; float: left; width: 100%;">
			...
			<tbody>
				...
				<c:forEach items="${itemStatisticsList }" var="list" varStatus="i">
					<c:if test="${list.orderType == 'PAY' }">
						...
						<c:set var="z"><c:out value="${z+1 }"/></c:set> 
					</c:if>
				</c:forEach>
			</tbody>
		</table>
		...
		<table class="board_list_table" style="width: 50%; float: left;">
			<thead>
				<tr>
					<th colspan="3" class="border_left"><c:out value="${op:message('M00037')}"/></th> <!-- 취소 -->
				</tr>
				<tr>
