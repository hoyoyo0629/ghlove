			...
			<table class="board_list_table">
				...
				<c:forEach items="${list}" var="item" varStatus="i">
					<tr>
						...
						<td><c:out value="${item.accessType eq '1'? '관리자':'판매자'}"/></td>
						<td class="tleft"><c:out value="${item.remoteAddr }"/></td>
						<td><c:out value="${op:date(item.createdDate)}"/></td>
						<td>
                            <div class="flex_box juc-center gap-08">
                                <c:if test="${item.allowIpId ne 1}">
                                    <a href="#" class="delete_item btn btn-default btn-sm" style="margin-top: 1px;" data-id="${fn:escapeXml(item.allowIpId)}">
                                        삭제
                                    </a>
                                </c:if>
                            </div>
