	...
	<div class="popup_contents">
		<div class="board_write">
			...
			<c:forEach items="${ list }" var="seller" varStatus="index">
				<h3 <c:if test="${fn:escapeXml(index.index) > 0}">class="mt30"</c:if>><span>${fn:escapeXml(seller.sellerInfo.companyName)}</span></h3>
				...
				<table class="board_write_table" >
					...
					<tbody>
						<tr>  
							...
							<td><div>${fn:escapeXml(seller.sellerInfo.sellerName)}</div></td>
							<td class="label">담당자명</td>
							<td><div>${fn:escapeXml(seller.sellerInfo.userName)}</div></td>
						</tr>
						<tr>  
							<td class="label">담당자 전화번호</td>
							<td><div>${fn:escapeXml(seller.sellerInfo.telephoneNumber)}</div></td>
							<td class="label">담당자 휴대폰번호</td>
							<td><div>${fn:escapeXml(seller.sellerInfo.phoneNumber)}</div></td>
						</tr>
					</tbody>
				</table>  
							...
							<c:forEach items="${ seller.orderItems }" var="orderItem" varStatus="itemIndex">
				 				<tr>	
									<td class="left none_left">	 						 
				 						<div class="arrival3">
				 							<p class="name">${fn:escapeXml(orderItem.itemName)} [${fn:escapeXml(orderItem.itemUserCode)}]</p>
				 					...
				 					<td>
				 						<div>
				 							${fn:escapeXml(orderItem.orderItemStatusLabel)}
				 						</div>
				 					</td>
				 				</tr>	
							</c:forEach>			 
