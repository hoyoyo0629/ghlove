...
<%@ taglib prefix="shop"	uri="/WEB-INF/tlds/shop" %>
		...
		<form:form modelAttribute="item" action="${fn:escapeXml(action)}" method="post" enctype="multipart/form-data">
					...
					<table class="board_write_table" summary="상품기본정보">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<div>
										<form:radiobuttons path="team" items="${op:nullToEmptyArr(categoryTeamList)}" itemValue="code" itemLabel="name" class="required" title="소속팀" />
									</div>
								</td>
							</tr>
								...
								<td>
									<div id="item_colors">
										...
										<c:forEach items="${colors}" var="code" varStatus="i">
											<span class="radio_wrap">
												<input type="radio" name="color" id="color_${fn:escapeXml(i.count)}" value="${fn:escapeXml(code.value)}" ${op:checked(code.value, item.color)} />
												<label for="color_${fn:escapeXml(i.count)}"><span class="item_color_box" style="background:${fn:escapeXml(code.value)};"></span> ${fn:escapeXml(code.label)}</label>
											</span>
										</c:forEach>
												...
												<c:forEach items="${delivery.deliveryChargeList}" var="deliveryCharge">
													<c:choose>
														<c:when test="${deliveryCharge.deliveryChargeType == '1'}">
															<option value="${fn:escapeXml(delivery.deliveryId)}_${fn:escapeXml(deliveryCharge.deliveryChargeId)}" ${op:selected(deliveryCharge.deliveryChargeId, item.deliveryChargeId)} class="mail_${fn:escapeXml(delivery.mailFlag)}">[${fn:escapeXml(deliveryTypeText)}] ${fn:escapeXml(delivery.title)} (무료)</option>

														</c:when>
														<c:when test="${deliveryCharge.deliveryChargeType == '2'}">
															<option value="${fn:escapeXml(delivery.deliveryId)}_${fn:escapeXml(deliveryCharge.deliveryChargeId)}" ${op:selected(deliveryCharge.deliveryChargeId, item.deliveryChargeId)} class="mail_${fn:escapeXml(delivery.mailFlag)}">[${fn:escapeXml(deliveryTypeText)}] ${fn:escapeXml(delivery.title)} (유료-${op:numberFormat(deliveryCharge.deliveryCharge)}원)</option>

														</c:when>
														<c:when test="${deliveryCharge.deliveryChargeType == '3'}">
															<option value="${fn:escapeXml(delivery.deliveryId)}_${fn:escapeXml(deliveryCharge.deliveryChargeId)}" ${op:selected(deliveryCharge.deliveryChargeId, item.deliveryChargeId)} class="mail_${fn:escapeXml(delivery.mailFlag)}">[${fn:escapeXml(deliveryTypeText)}] ${fn:escapeXml(delivery.title)} (조건-${op:numberFormat(deliveryCharge.deliveryCharge)}원 | ${op:numberFormat(deliveryCharge.deliveryFreeAmount)}원 이상 무료)</option>
											...
											<table id="item_point_config" class="board_list_table" summary="${op:message('M00246')} 추가">
												...
												<tbody id="item_point_area">
													...
													<c:forEach items="${pointConfigList}" var="pointConfig">
														<tr>
															<td>
																<input type="text" name="point" maxlength="4" value="${fn:escapeXml(pointConfig.point)}"  class="three required _number" title="${op:message('M00246')}" />
																<input type="hidden" name="pointType" value="${fn:escapeXml(pointConfig.pointType)}" />

																<input type="radio" name="R${fn:escapeXml(pointConfig.pointConfigId)}" id="R${fn:escapeXml(pointConfig.pointConfigId)}1" ${op:checked('1', pointConfig.pointType)}> <label for="R${fn:escapeXml(pointConfig.pointConfigId)}1">%</label>
																<input type="radio" name="R${fn:escapeXml(pointConfig.pointConfigId)}" id="R${fn:escapeXml(pointConfig.pointConfigId)}2" ${op:checked('2', pointConfig.pointType)}> <label for="R${fn:escapeXml(pointConfig.pointConfigId)}2">원</label>
															</td>
															<td>
																<span class="datepicker"><input type="text" name="pointStartDate" class="term  _date" maxlength="8" value="${fn:escapeXml(pointConfig.startDate)}" title="시작일"></span>
																<select name="pointStartTime" title="시간 선택">
																	<c:forEach items="${hours}" var="code">
																		<option value="${fn:escapeXml(code.value)}" ${op:selected(code.value, pointConfig.startTime)}>${fn:escapeXml(code.label)}</option>
																	</c:forEach>
																</select> 시 ~
																<span class="datepicker"><input type="text" name="pointEndDate" class="term required _date" maxlength="8" value="${fn:escapeXml(pointConfig.endDate)}" title="종료일"></span>
																<select name="pointEndTime" title="시간 선택">
																	<c:forEach items="${hours}" var="code">
																		<option value="${fn:escapeXml(code.value)}" ${op:selected(code.value, pointConfig.endTime)}>${fn:escapeXml(code.label)}</option>
																	</c:forEach>
																</select> 시
															</td>
															<td>
																<input type="text" name="pointRepeatDay" maxlength="2" class="optional _number" value="${fn:escapeXml(pointConfig.repeatDay)}" style="width: 25px;" title="일" /> 일
															</td>
						...
						<table class="board_list_table" summary="상품옵션 추가">
							...
							<tbody>
								...
								<c:forEach items="${item.itemOptionGroups}" var="itemOptionGroup" varStatus="i">
									...
									<tr>
										<td>
											<input type="text" name="option_title" class="full required_item_option" maxlength="100" value="${fn:escapeXml(itemOptionGroup.optionTitle)}" title="옵션명" /><br /><br />
										...
										<td class="item_sub_option_area">
											...
											<c:forEach items="${itemOptionGroup.itemOptions}" var="itemOption" varStatus="i">
												<div>
													<input type="hidden" name="optionName1" value="${fn:escapeXml(itemOption.optionName1)}" />
													<span class="option_display"><input type="checkbox" name="optionDisplayFlagCheck" ${op:checked('N', itemOption.optionDisplayFlag)}/><input type="hidden" name="optionDisplayFlag" value="${fn:escapeXml(itemOption.optionDisplayFlag)}" /></span>
													<span class="option_name"><input type="text" name="optionName2" class="required_item_option full" value="${fn:escapeXml(itemOption.optionName2)}" title="옵션항목명" /></span>
													<span class="option_code"><input type="text" name="optionCode" class="required_item_option full" value="${fn:escapeXml(itemOption.optionStockCode)}" title="상품코드" /></span>
													...
													<span class="option_stock_schedule_text"><input type="text" name="optionStockScheduleText" class="full" value="${fn:escapeXml(itemOption.stockScheduleText)}" title="입하예정일 텍스" /></span>
													<span class="option_stock_schedule_date"><input type="text" name="optionStockScheduleDate" class="full" value="${fn:escapeXml(itemOption.stockScheduleDate)}" title="입하예정일" /></span>
						...
						<tbody>
							<tr>
								...
								<td>
									<div>
										<p>
											<input type="file" name="imageFile" title="대표이미지" style="width: 550px;" accept="image/png, image/jpeg, image/gif" />
											<!--
										...
										<c:if test="${!empty item.itemImage}">
											<p class="item_image_main">
												<img src="${fn:escapeXml(item.imageSrc)}" class="item_image size-100" alt="" />
												<a href="javascript:deleteItemImage('main', ${fn:escapeXml(itemId)});" class="delete_item_image"><img src="/content/opmanager/images/icon/icon_x.gif" alt="" /></a>
											</p>
										</c:if>
											...
											<c:forEach items="${item.itemImages}" var="itemImage" varStatus="i">
												<c:if test="${itemImage.itemImageId != 0}">
													<li id="item_image_id_${fn:escapeXml(itemImage.itemImageId)}">
														<img src="${fn:escapeXml(itemImage.imageSrc)}" class="item_image size-100" alt="" />
														...
														<input type="hidden" name="itemImageIds" value="${fn:escapeXml(itemImage.itemImageId)}" />
													</li>
												</c:if>
											</c:forEach>
										</ul>
											...
											<c:forEach items="${item.itemRelations}" var="itemRelation" varStatus="i">
												<c:if test="${!empty itemRelation.item.itemId}">
													<li id="related_item_${fn:escapeXml(itemRelation.item.itemId)}">
														<input type="hidden" name="relatedItemIds" value="${fn:escapeXml(itemRelation.relatedItemId)}" />
														<p class="image"><img src="${fn:escapeXml(itemRelation.item.imageSrc)}" class="item_image size-100 none" alt="상품이미지" /></p>
			...
			<div class="item_list mt60">
				...
				<form:textarea path="detailContent" cols="30" rows="20" class="editor-content" style="width: 100%" class="" title="상품상세설명" />
			</div>
			...
			<div class="item_list mt30">
				...
				<form:textarea path="detailContentMobile" cols="30" rows="6" class="editor-content" style="width: 100%" title="모바일 상품상세설명" />
			</div>

			<div class="item_list mt30">
				<h3><span>사용방법</span></h3>

				<form:textarea path="useManual" cols="30" rows="3"  class="editor-content" style="width: 100%" title="사용방법" />
			</div>

			<div class="item_list mt30">
				<h3><span>조립방법</span></h3>

				<form:textarea path="makeManual" cols="30" rows="3" class="editor-content" style="width: 100%" title="조립방법" />
			</div>
