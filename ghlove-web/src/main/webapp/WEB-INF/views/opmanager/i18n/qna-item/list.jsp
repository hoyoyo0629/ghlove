    ...
    <form:form modelAttribute="qna" method="post" enctype="multipart/form-data">
	    <div class="board_write">
			                ...
			                <tr>
			                    ...
			                    <td>
			                        <div class="flex_box gap-08">
			                            <form:select path="sido" title="${op:message('지자체')}" class="wd-150" onchange="javascript:searchSigungu(this);"><!-- 지자체 -->
			                                <form:option value="">${op:message('-시,도-')}</form:option><!-- -시,도- -->
			                                <c:forEach items="${sido}" var="item">
		                                   		<form:option value="${fn:escapeXml(item.value)}">${fn:escapeXml(item.label)}</form:option><!-- 시,도 선택 -->
											</c:forEach>
			                            </form:select>
			                            <form:select path="sigungu" title="${op:message('지자체')}" class="wd-150"><!-- 지자체 -->
			                                <form:option value="">${op:message('-시,군,구-')}</form:option><!-- -시,군,구- -->
			                            </form:select>
			                        </div>
			                    </td>
	        ...
	        <div class="btn_all btn_right">
	            <div class="flex_box gap-08">
	                <button type="button" class="btn btn-dark-gray btn-mini" onclick="location.href='/opmanager/qna-item/list';">${op:message('M00047')}</button><!-- 초기화 -->
	        ...
	        <table class="board_list_table">
	            ...
	            <thead>
	                <tr>
	                    ...
	                    <th scope="col">${'비밀글여부'}</th><!-- 답변상태 -->
	                    <th scope="col">${op:message('M00276')}</th><!-- 작성일 -->
	                    <th scope="col">답변일</th><!-- 작성일 -->
	                </tr>
	            </thead>
	            <tbody>
	            	<c:forEach items="${itemQnaLists}" var="item" varStatus="i">
		                <tr style="background:#fff;">
		                    ...
		                    <td>
		                        <div>${fn:escapeXml(item.secretFlag)}</div>
		                    </td>
		                    <td>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        $('#delete_list_data').on('click', function() {
            Common.updateListData("/opmanager/qna-item/delete", "${op:message('M00196')}");// 삭제하시겠습니까?
...
--%>
    function qnaDetail(qnaId, answerId) {
    	var detailUrl = "/opmanager/qna-item/answer/"+qnaId;
    	if(answerId > 0 && answerId !== undefined){
    		detailUrl = "/opmanager/qna-item/answer/"+qnaId + "?answerId="+answerId;
    	}
    ...
    function searchSigungu(sidoElement) {
        ...
        $('#sigungu').append('<option value="">${op:message("-시,군,구-")}</option>');
        if (sidoValue) {
            $.post(
            		...
            		, function(response) {
            			Common.responseHandler(response, function(){
	    		            ...
	    		            for (var i = 0; i < data.length; i++) {
	    		                ...
	    		                $('#sigungu').append(options);
	    		            }

	    		            // 조회 된 값 유지
	    		            if ("${fn:escapeXml(param.sigungu)}" != "") {
	    		            	$("#sigungu").val("${fn:escapeXml(param.sigungu)}").prop("selected", true);

	    		            	if (!$("#sigungu").val()) {		// 선택된 값이 없으면 제일 첫 번째 값 기본 세팅
	...
	function downloadExcel() {
		...
		const savedSearchCondition = sessionStorage.getItem("qna");
		...
		Shop.downloadExcelOrder("/opmanager/qna-item/download-excel", savedSearchCondition, true);
	}
