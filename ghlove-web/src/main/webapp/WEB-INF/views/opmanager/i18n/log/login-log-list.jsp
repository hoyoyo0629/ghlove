...
<form:form modelAttribute="loginLogParam" cssClass="opmanager-search-form clear" method="post" id="searchForm">
	...
	<div class="board_write">
		<table class="board_write_table" summary="관리자 로그관리">
			<colgroup>
			...
			<tbody>
					...
					<td colspan="3">
						<div class="flex_box gap-08">
							<form:select path="srchKey" title="아이디" class="wd-150">
								<form:option value="LOGIN_ID">로그인ID</form:option>
				...
				<tr>
					...
					<td class="label">권한그룹</td>
					<td>
						<div class="flex_box gap-12">
							<form:select path="srcRole" title="권한그룹">
								<form:option value="">전체</form:option>
								<c:forEach items="${adminMenuRoles}" var="item" varStatus="i">
									<form:option value="${fn:escapeXml(item.authority)}"><c:out value="${item.roleName}"/></form:option>
								</c:forEach>
							</form:select>
						</div>
					</td>
		...
		<div class="btn_all btn_right">
			<div class="flex_box gap-08">
				<button type="button" class="btn btn-dark-gray btn-mini" onclick="location.href='/opmanager/log/login-log'">초기화</button>
...
<div class="board_list mt-40">
	...
	<table class="board_list_table" summary="관리자 로그관리">
		<caption>관리자 로그관리</caption>
		<colgroup>
		...
		<tbody>
			<c:choose>
					...
					<c:forEach items="${list}" var="item" varStatus="i">
						<tr style="background:#fff;">
							...
							<td>
								<div>
									<c:choose>
										<c:when test="${item.loginType eq 'seller'}">답례품관리자</c:when>
										<c:otherwise><c:out value="${item.roleName}"/></c:otherwise>
									</c:choose>
								</div>
							</td>
				...
				<c:otherwise>
					<tr style="background:#fff;">
						<td colspan="7">관리자 로그관리 정보가 없습니다.</td>
					</tr>
				</c:otherwise>
			</c:choose>
		</tbody>
...
<script type="text/javascript">
	...
	function details(loginLogId) {
		location.href = "/opmanager/log/login-log/details/"+loginLogId+location.search;
	}
