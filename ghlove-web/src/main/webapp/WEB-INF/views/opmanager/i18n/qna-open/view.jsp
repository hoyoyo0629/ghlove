...
<%@ taglib prefix="shop"	uri="/WEB-INF/tlds/shop" %>
		...
		<div class="board_write">
			<form:form modelAttribute="qna" method="post" action="/opmanager/qna/delete/${fn:escapeXml(qnaId)}">
				<h3>1:1 문의 </h3>
...
<%-- 							<div>
						...
						<tr>
							...
							<td>
								<div>
									<c:out value="${op:datetime(qna.createdDate)}"/>
								</div>
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td colspan="3">
								<div>
									<c:out value="${qna.displayFlag == 'Y' ? op:message('M00096') : op:message('M00097')}"/>
								</div>
							</td>
						</tr>
						 <tr>
							 ...
							 <td colspan="3">
								 <div>
									 <c:if test="${!empty qna.qnaImage}">
										 <img src="${fn:escapeXml(qna.imageSrc)}" alt="이미지" style="width: 300px;" />
									 </c:if>
								 </div>
							 </td>
						 </tr>
			...
			<div class="board_write">
				<c:choose>
					<c:when test="${qnaAnswer.answer > '1'}">
						<form:form modelAttribute="qnaAnswer" action="/opmanager/qna/view/${fn:escapeXml(qna.qnaId)}">
							<tbody>
								<table class="board_write_table">
									...
									<tr>
										...
										<td>
											<div>
												<c:out value="${op:nl2br(qnaAnswer.answer)}"/>
											</div>
										</td>
									</tr>
								</table>
						...
						<div class="btn_all btn_right">
							...
							<a href="javascript:qnaList();" class="btn btn-default"><span><c:out value="${op:message('M00480')}"/></span></a> <!-- 목록 -->
							<a href="/opmanager/qna/delete/${fn:escapeXml(qnaId)}" id="delete_data" class="btn btn-default">삭제</a>
							<%-- <a href="/opmanager/qna/edit/${qnaId}" class="btn btn-active">수정</a> --%>
							<a href="javascript:qnaModify(${fn:escapeXml(qnaId)});" class="btn btn-active">수정</a>
							</div>
						</div>
					</c:when>
					<c:otherwise>
						<div class="btn_all btn_right">
							...
							<a href="javascript:qnaModify(${fn:escapeXml(qnaId)});" class="btn btn-active">답변등록</a>
						</div>
						</div>
					</c:otherwise>
				</c:choose>
