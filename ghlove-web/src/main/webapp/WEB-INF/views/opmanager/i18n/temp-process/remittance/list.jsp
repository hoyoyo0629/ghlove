...
<h3>미완료 정산 내역(2023. 6. 9. 이전 데이터)</h3>
	...
	<div class="board_write">
		...
		<table class="board_write_table" summary="${ fn:escapeXml(title) }">
			...
			<tbody>
				...
				<tr>
				 	<td class="label">
				 		<form:select path="searchDateType" title="검색기간 구분" class="wd-150">
						 	<form:option value="EXPECTED" label="정산예정일" />
						 	<form:option value="CONFIRM" label="정산확정일" />
					 	</form:select>	
				 	</td>
				 	<td>
				 		<div>
		...
		<div class="btn_all btn_right">
			<div class="flex_box gap-08">
				<button type="button" class="btn btn-dark-gray btn-mini" onclick="location.href='${requestContext.sellerPage ? '/seller' : '/opmanager'}/temp-process/remittance/list'"> <c:out value="${op:message('M00047')}"/><!-- 초기화 --></button>
...
<div class="board_list">
	...
	<form id="listForm" action="${fn:escapeXml(requestContext.managerUri)}/temp-process/remittance/list-update" method="post">
				...
				<c:forEach items="${ list }" var="item" varStatus="index">
					...
					<tr style="background:#fff;">
						<c:if test="${!requestContext.sellerPage}">
							...
							<td><c:out value="${item.remittanceYmDate}"/></td>
							<td>
								<a href="${requestContext.sellerPage ? '/seller' : '/opmanager'}/temp-process/remittance/view/${fn:escapeXml(item.remittanceId)}"><c:out value="${item.companyName}"/></a>
							</td>
							...
							<td><c:out value="${item.remittanceStatusCode == 0 ? '미정' : ''}"/></td>
						</c:if>	
						<c:if test="${requestContext.sellerPage}">
							<td>
							...
							<td>
								...
								<a href="${requestContext.sellerPage ? '/seller' : '/opmanager'}/temp-process/remittance/view/${fn:escapeXml(item.remittanceId)}">
									상세보기
								</a>
							</td>
						</c:if>
			...
			<div class="flex_box gap-08">
				<c:if test="${!requestContext.sellerPage}">
					<c:if test="${(op:hasRole('ROLE_ADMIN_5') || op:hasRole('ROLE_ADMIN_6') || op:hasRole('ROLE_ADMIN_7') || op:hasRole('ROLE_ADMIN_8'))}">
						<button type="button" class="btn btn-dark-gray btn-mini finishing-remittance">정산 확정</button>
					</c:if>
				</c:if>			
			</div>
		</div>
...
<script type="text/javascript">
	$(function(){
		...
		$('.finishing-remittance').on('click', function() {
			...
			for(let i = 0 ; i < length ; i++) {
				...
				for(let j = 0 ; j < checkDataLength ; j++) {
					...
					if (checkData.remittanceId == remittanceId) {
						if (checkData.remittanceStatusCode != 0) {
							alert("정산 미정 상태만 처리 가능합니다.");
							return;
						}
					}
				}
			...
			if (confirm("선택한 정산정보를 확정처리 하시겠습니까?")) {
				$form.submit();
			}
	...
	function excelDownload() {
		...
		Shop.downloadExcelOrder("${requestContext.sellerPage ? '/seller' : '/opmanager'}/temp-process/remittance/list-excel", param, true);
	}--%>
</script>
