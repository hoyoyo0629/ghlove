			...
			<tbody>
				<c:forEach items="${list}" var="user" varStatus="i">
					<tr>
						...
						<td><a href="/opmanager/user/edit?userId=${fn:escapeXml(user.userId)}"><c:out value="${user.userName}"/></a></td>
						<td><a href="/opmanager/user/edit?userId=${fn:escapeXml(user.userId)}"><c:out value="${user.loginId}"/></a></td>
						<td><c:out value="${user.userRoles[0].authority}"/></td>
