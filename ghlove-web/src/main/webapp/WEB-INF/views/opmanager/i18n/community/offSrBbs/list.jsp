					...
					<td colspan="3">
						<div class="flex_box gap-08">
							<form:select path="searchRole" title="소속" class="wd-150"
								...
								<form:option value="ROLE_ADMIN_7">오프라인 담당자</form:option>
							</form:select>
							<!-- 지자체가 선택되면 2, 3차 카테고리(시도/군구) 선택 -->
							<form:select path="shBank" class="wd-150" style="display:none"
								onChange="bankChange(this.value)">
								<form:option value="">-은행 선택-</form:option>
								<c:forEach items="${offBanks}" var="offBank">
									<form:option value="${fn:escapeXml(offBank.id)}" label="${fn:escapeXml(offBank.label)}" />
								</c:forEach>
							</form:select>
						</div>
					</td>
					...
					<td colspan="3">
						<div class="flex_box gap-08">
							<form:select path="where" title="검색구분" class="wd-150">
								...
								<form:option value="PSITN">소속지점</form:option>
		...
		<div class="btn_all btn_right">
			<div class="flex_box gap-08">
				<button type="button" class="btn btn-default btn-mini"
					onclick="location.href='/opmanager/community/offSrBbs/list'">
...
<div class="board_list">
			...
			<c:forEach items="${list}" var="community" varStatus="i">
				<tr>
					...
					<c:choose>
						...
						<c:otherwise>
							<td>[<c:out value="${community.bankNm}" />] <c:out value="${community.psitnNm}" /></td>
						</c:otherwise>
					</c:choose>
					<td>
	...
	<div class="btn_all btn_right">
		<div class="flex_box gap-08">
			<a href="<c:url value="/opmanager/community/offSrBbs/form" />"
...
<script type="text/javascript">
	...
	function adminChange(value) {
		if ($("#searchRole").val() == "ROLE_ADMIN_7") {
			$("#shBank").css('display', 'block');
		} else {
			$("#shBank").css('display', 'none');
			$("#shBank").val('');
		}
	}
	...
	function communityDetail(id) {
		location.href = '/opmanager/community/offSrBbs/detail/' + id;
	}
