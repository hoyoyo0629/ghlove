...
<%@ taglib prefix="op" 		uri="/WEB-INF/tlds/functions" %>
	...
	<h2><span>¶ <c:out value="${op:message('M00830')}"/> <!-- 회원탈퇴 --></span></h2>
	<form:form modelAttribute="user" method="post">
		<div class="board_write">
			<table class="board_write_table" summary="회원탈퇴에 관련한 정보를 입력하는 칸입니다.">
				...
				<tbody>
					<tr>
						<td class="label"><c:out value="${op:message('M00868')}"/></td> <!-- 탈퇴사유 -->
						<td>
							<div>				<!-- 탈퇴사유 -->				 
								<textarea name="leaveReason" cols="30" rows="6" title="${op:message('M00868')}" class="required"><c:out value="${user.userDetail.leaveReason}"/></textarea>
						 	</div>
						</td>
					</tr>
				</tbody>
