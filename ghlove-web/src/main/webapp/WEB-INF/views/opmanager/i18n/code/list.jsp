...
<div class="container-fluid">
  ...
  <div class="row">
    ...
    <div class="col-sm-3" style="height:100%;border 1px;" id="dvCodeType" name="dvCodeType">
		<form id="listForm" style="height:100%;">
			<div style="height:600px;margin:0px 0px 5px 0px;">
				<div style="text-align:right;background-color:#f8f8f8;padding:10px;">
				  <input id="SimpleSearch" type="text" placeholder="CodeType Filter.." style="width:100%;">
				  <i><font >total: ${fn:escapeXml(codeTypeCount)}</font></i>
				</div>

				<ul class="list-group" id="simpleSearchList" style="height:600px;margin:30px 0px;overflow:auto">
					...
					<c:forEach items="${codeTypeList}" var="list">
					  ...
					  <li class="list-group-item" onclick="fnCodeSearch('${fn:escapeXml(list.codeType)}');" ${ fn:escapeXml(listStyle) } >${fn:escapeXml(list.codeType)} <span class="badge">${fn:escapeXml(list.codeTypeCnt)}</span></li>
					</c:forEach>
				</ul>
			</div>
		</form>
	...
	<!-- 조회 부 -------------------------------------------------------------------- -->
	<form:form modelAttribute="codeParam" method="post" enctype="multipart/form-data">
		<table class="board_write_table"  summary="${op:message('M01676')}"> <!-- 공통코드 -->
			<colgroup>
		...
		<div class="count_title mt20">
			<h5>
				${op:message('M00039')} : ${fn:escapeXml(pagination.totalItems)} ${op:message('M00743')}
			</h5>	 <!-- 전체 -->   <!-- 건 조회 -->
		</div>
				...
				<table class="board_list_table" summary="${op:message('M01676')}"> <!-- 공통코드 -->
					...
					<tbody>
						<c:forEach items="${codeList}" var="list" varStatus="i">
							<tr >
								<td>${fn:escapeXml(list.codeType)}</td>
								<td style="text-align:left;">${fn:escapeXml(list.id)}</td>
								<td style="text-align:left;">${fn:escapeXml(list.label)}</td>
								<td>${fn:escapeXml(list.detail)}</td>
								<td>${fn:escapeXml(list.ordering)}</td>
								<td>${fn:escapeXml(list.useYn)}</td>
								<td style="text-align:left;">${fn:escapeXml(list.upId)}</td>
								<td style="text-align:left;">${fn:escapeXml(list.codeValue)}</td>
								<td style="text-align:left;">${fn:escapeXml(list.extentionCode)}</td>
								<td style="text-align:left;">${fn:escapeXml(list.mappingCode)}</td>
								<td>
                                    <div class="flex_box juc-center gap-08">
                                        <a href='javascript:fnUpdateCode("${fn:escapeXml(list.codeType)}","${fn:escapeXml(list.id) }")' class="btn btn-gradient btn-xs">${op:message('M00087')}</a> <!-- 수정 -->
                                        <a href='javascript:fnDeleteCode("${fn:escapeXml(list.codeType)}","${fn:escapeXml(list.id) }")' class="delete_item btn btn-gradient btn-xs">${op:message('M00074')}</a> <!-- 삭제 -->
                                    </div>
								</td>
							</tr>
						</c:forEach>
...
<script type="text/javascript">
	...
	function fnSearch() {
		fnCodeSearch("${fn:escapeXml(codeParam.codeType)}");
	}
