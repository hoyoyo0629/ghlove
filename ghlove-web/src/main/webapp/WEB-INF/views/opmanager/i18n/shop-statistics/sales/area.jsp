...
<%@ taglib prefix="sec" 	uri="http://www.springframework.org/security/tags"%>
	...
	<div class="location">
		<a href="#">통계</a> &gt;  <a href="#">매출통계</a> &gt; <a href="#" class="on"><c:out value="${op:message('M01362')}"/></a>
	</div>
	<div class="statistics_web">
		<h3><span><c:out value="${op:message('M01362')}"/></span></h3> <!-- 지역별 매출 -->
		<form:form modelAttribute="statisticsParam" method="post" >
			...
			<div class="board_write">
				<table class="board_write_table" summary="${op:message('M01362')}">
					<caption><c:out value="${op:message('M01362')}"/></caption>
					<colgroup>
		...
		<div class="board_list">
			...
			<table class="board_list_table stats ${statisticsParam.displaySubtotal == 'N' ? 'odd-even' : ''}">
				<thead>
					<tr>
						<th rowspan="2" class="border_left"><c:out value="${op:message('M01367')}"/></th> <!-- 지역 -->
					...
					<tr>
						...
						<th class="border_left number"><c:out value="${op:message('M00452')}"/></th> <!-- 할인 -->
						<th class="border_left number"><c:out value="${op:message('M01369')}"/></th> <!-- 판매액 -->
					...
					<c:forEach items="${areaStatsList}" var="list" varStatus="i">
						<tr class="${i.count % 2 == 0 ? 'even' : '' }">
							<td rowspan="${fn:length(list.groupStats) }" class="under_line">
								<c:set var="action">/opmanager/shop-statistics/sales/area?conditionType=SIGUNGU&sidoMappingGroupKey=<c:out value="${list.sidoMappingGroupKey}"/>&startDate=<c:out value="${statisticsParam.startDate}"/>&endDate=<c:out value="${statisticsParam.endDate}"/>&sellerId=<c:out value="${statisticsParam.sellerId}"/></c:set>
								<c:if test="${ statisticsParam.conditionType eq 'SIGUNGU'}">
									<c:set var="action">javascript:Common.popup('/opmanager/shop-statistics/sales/area/detail?sidoMappingGroupKey=<c:out value="${list.sidoMappingGroupKey}"/>&dodobuhyun=<c:out value="${list.sigungu}"/>&startDate=<c:out value="${statisticsParam.startDate}"/>&endDate=<c:out value="${statisticsParam.endDate}"/>&sellerId=<c:out value="${statisticsParam.sellerId}"/>', '<c:out value="${list.sigungu}"/>' , '1000', '800', 1)</c:set>
									<c:if test="${empty list.sigungu}">

									</c:if>
								</c:if>
								<a href="${fn:escapeXml(action)}">
									<c:choose>
											...
											<c:choose>
												...
												<c:otherwise>
													<c:if test="${not empty list.sido}">
														<c:out value="${list.sido}"/> >
													</c:if>
													<c:out value="${list.sigungu}"/>
												</c:otherwise>
											</c:choose>
										</c:when>
										<c:otherwise>
											<c:choose>
												...
												<c:otherwise><c:out value="${list.sido}"/></c:otherwise>
											</c:choose>
										</c:otherwise>
									</c:choose>
								</a>
			...
			<sec:authorize access="hasRole('ROLE_EXCEL')">
				<div class="btn_all">
					<div class="right">
						<a href="/opmanager/shop-statistics/sales/area/excel-download?${fn:escapeXml(queryString)}"  class="btn btn-success btn-sm"><span class="glyphicon glyphicon-save"></span><c:out value="${op:message('M00254')}"/></a> <!-- 엑셀 다운로드 -->
					</div>
				</div>
			</sec:authorize>
			<!--
