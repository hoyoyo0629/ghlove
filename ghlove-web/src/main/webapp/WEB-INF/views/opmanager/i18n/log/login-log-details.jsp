		...
		<tbody>
			<tr>
				...
				<td>
					<div>
						<c:choose>
							<c:when test="${details.loginType eq 'seller'}">답례품관리자</c:when>
							<c:otherwise><c:out value="${details.roleName}"/></c:otherwise>
						</c:choose>
					</div>
				</td>
			</tr>
...
<script type="text/javascript">
	...
	function getActionLogList(page, itemPerPage) {
		...
		$.get("/opmanager/log/login-log/details/${fn:escapeXml(loginLogId)}/action-log-list?"+searchValue, {}, function(result) {
	...
	function moveListPage() {
		location.href = "/opmanager/log/login-log"+$("#listSearchParam").val();
	}
</script>