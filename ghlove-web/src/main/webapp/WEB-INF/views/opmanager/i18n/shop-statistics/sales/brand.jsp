...
<div class="statistics_web">
		...
		<div class="board_write">
			<table class="board_write_table" summary="상품별 판매율">
				...
				<tbody>
					...
					<tr>
						<td class="label"><c:out value="${op:message('M01353')}"/></td> <!-- 카테고리선택 -->
						<td colspan="3">
							<div>
					...
					<tr>
						<td class="label"><c:out value="${op:message('M00001')}"/></td> <!-- 상품 -->
						<td colspan="3">
							<div>
					...
					<tr>
						<td class="label"><c:out value="${op:message('M00052')}"/> <!-- 출력수 --></td>
						<td colspan="3">
							<div>
								...
								<form:select path="brand">
									...
									<c:forEach items="${brandList}" var="brand">
										<c:if test="${brand.displayFlag == 'Y'}">
											<form:option value="${fn:escapeXml(brand.brandName)}" label="${fn:escapeXml(brand.brandName)}" />
										</c:if>
									</c:forEach>
								</form:select>
							</div>
	...
	<div class="sort_area mt30">
		<div class="left">
			<span><c:out value="${op:message('M01388')}"/> : <span class="font_b"><c:out value="${op:numberFormat(total.totalPayCount + total.totalCancelCount)}"/></span><c:out value="${op:message('M01004')}"/> (<c:out value="${op:message('M01355')}"/> : <c:out value="${op:numberFormat(total.totalPayCount)}"/><c:out value="${op:message('M01004')}"/>, <c:out value="${op:message('M01365')}"/> : <c:out value="${op:numberFormat(total.totalCancelCount)}"/><c:out value="${op:message('M01004')}"/>)</span> | <span><c:out value="${op:message('M01389')}"/> : <span class="font_b"><c:out value="${op:numberFormat(total.totalRevenueAmount) }"/></span><c:out value="${op:message('M00814')}"/></span>
		</div>
	</div>
	...
	<div class="board_list">
		...
		<table class="board_list_table">
			...
			<thead>
				<tr>
					...
					<th colspan="4" class="border_left"><c:out value="${op:message('M01355')}"/></th> <!-- 결제 -->
					<th colspan="4" class="border_left"><c:out value="${op:message('M00037')}"/></th> <!-- 취소 -->
					<th colspan="4" class="border_left"><c:out value="${op:message('M00064')}"/></th> <!-- 소계 -->
				</tr>
				<tr>
					<th class="border_left"><c:out value="${op:message('M01375')}"/></th> <!-- 갯수 -->
					<th class="border_left"><c:out value="${op:message('M00627')}"/></th> <!-- 상품금액 -->
					<th class="border_left"><c:out value="${op:message('M00452')}"/></th> <!-- 할인액 -->
			...
			<tbody>
				<c:forEach items="${brandStatisticsList}" var="list" varStatus="i">
					<tr>
						...
						<td rowspan="${fn:length(list.groupList) }" style="text-align: left;">
							...
							<a href="javascript:;" onclick="openBrandDetail('${fn:escapeXml(brandName)}')">
								<c:out value="${brandTitle}"/>
							</a>
						...
						<c:forEach items="${ list.groupList }" var="item" varStatus="groupIndex">
							...
							<td class="border_left number"><c:out value="${op:numberFormat(item.cancelItemCouponDiscountAmount)}"/></td>
							<td class="border_left number"><c:out value="${op:numberFormat(item.cancelTotal)}"/></td>

							<td class="border_left number"><c:out value="${op:numberFormat(item.payCount + item.cancelCount)}"/></td>
							<td class="border_left number"><c:out value="${op:numberFormat(item.sumItemPrice)}"/></td>
							<td class="border_left number"><c:out value="${op:numberFormat(item.sumItemCouponDiscountAmount)}"/></td>
					...
					<tr style="background: #f2f2f2;">
						...
						<td class="border_left number"><c:out value="${op:numberFormat(list.totalPayItemCouponDiscountAmount)}"/></td>
						...
						<td class="border_left number"><c:out value="${op:numberFormat(list.totalCancelItemCouponDiscountAmount)}"/></td>
						<td class="border_left number"><c:out value="${op:numberFormat(list.totalCancel)}"/></td>

						<td class="border_left number"><c:out value="${op:numberFormat(list.totalPayCount + list.totalCancelCount)}"/></td>
						<td class="border_left number"><c:out value="${op:numberFormat(list.totalPayItemPrice + list.totalCancelItemPrice)}"/></td>
						<td class="border_left number"><c:out value="${op:numberFormat(list.totalPayItemCouponDiscountAmount + list.totalCancelItemCouponDiscountAmount)}"/></td>
				...
				<c:if test="${not empty brandStatisticsList }">
					<tr style="background: #CAC6C6;">
						<td colspan="3"><c:out value="${op:message('M00358')"/>}</td> <!-- 소계 -->
						<td class="border_left number"><c:out value="${op:numberFormat(total.totalPayCount)}"/></td>
						<td class="border_left number"><c:out value="${op:numberFormat(total.totalItemPrice)}"/></td>
						<td class="border_left number"><c:out value="${op:numberFormat(total.totalItemCouponDiscountAmount)}"/></td>
						<td class="border_left number"><c:out value="${op:numberFormat(total.totalPayAmount)}"/></td>
						<td class="border_left number"><c:out value="${op:numberFormat(total.totalCancelCount)}"/></td>
						<td class="border_left number"><c:out value="${op:numberFormat(total.totalCancelItemPrice) }"/></td>
						<td class="border_left number"><c:out value="${op:numberFormat(total.totalCancelItemCouponDiscountAmount) }"/></td>
						<td class="border_left number"><c:out value="${op:numberFormat(total.totalCancelAmount)}"/></td>
						<td class="border_left number"><c:out value="${op:numberFormat(total.totalRevenueItemPrice)}"/></td>
						<td class="border_left number"><c:out value="${op:numberFormat(total.totalRevenueItemCouponDiscountAmount)}"/></td>
						<td class="border_left number"><c:out value="${op:numberFormat(total.totalRevenueAmount)}"/></td>
					</tr>
				</c:if>
			</tbody>
		</table>
		...
		<div class="sort_area">
			<div class="right">
				<a href="/opmanager/shop-statistics/sales/brand/excel-download?${fn:escapeXml(queryString)}" class="btn_write gray_small"><img src="/content/opmanager/images/icon/icon_excel.png" alt=""><span><c:out value="${op:message('M00254')}"/></span> </a> <!-- 엑셀 다운로드 -->
			</div>
		</div>
