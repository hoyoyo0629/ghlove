	...
	<div class="email_tb">				
		...
		<p class="c_icon">
			<span class="mr5">${op:message('M00557')} <!-- 카테고리 검색 -->  : 
				...
				<select name="categoryGroupId" title="메인 구분" > 
					...
					<c:forEach items="${categoryTeamGroupList}" var="list">
						<optgroup label="${fn:escapeXml(list.name)}">
							<c:forEach items="${list.categoriesGroupList}" var="list2">
								<option value="${fn:escapeXml(list2.categoryGroupId)}" ${op:selected(searchParam.categoryGroupId, list2.categoryGroupId)} >-${fn:escapeXml(list2.groupName)}</option>	
							</c:forEach>
						</optgroup>
					</c:forEach>
				</select>	
			...
			<span style="display: inline-block !important;">
				<button type="button" class="btn btn-dark-gray btn-sm" id="categoriesCreate"><span>2${op:message('M00075')}${op:message('M00192')}</span></button> <!-- 차 카테고리 --> <!-- 추가 -->
			</span> 
		</p>	
		...
		<form id="categoryForm" method="post" action="/opmanager/categories/create">
			<input type="hidden" name="cateGroupId" value="${fn:escapeXml(searchParam.categoryGroupId)}" />
...
var node = "";
var categoryGroupId = '${fn:escapeXml(searchParam.categoryGroupId)}'; 
...
$(function(){
		...
		$("#categoryForm").submit(function(){
			...
			if($("input[name='categoryLevel']").val() == '1'){
				...
				if ($categoryGroupId.val() == '0') {
					alert("1" + Message.get("M00075") + "를 선택하십시오."); // 1차 카테고리
					$categoryGroupId.focus();
					return false;
				}
			}

			if($("#categoryForm").attr("action") == '/opmanager/categories/create')	{
				// 시스템/행안부 관리자만 권한있음
				if('${fn:escapeXml(adminRole)}' != 'SYS') {
					alert("카테고리 등록 권한이 없습니다.");
					return false;
				} 			
        	...
        	} else if($("#categoryForm").attr("action") == '/opmanager/categories/edit') {
				// 시스템/행안부 관리자만 권한있음
				if('${fn:escapeXml(adminRole)}' != 'SYS') {
					alert("카테고리 수정 권한이 없습니다.");
					return false;
				}       
	...
	$("body").on("click","#categroyLowAdd",function() {
		...
		if ($("input[name='categoryLowUrlCheck']").val() != '1') {
			$("input[name='categoryLowUrl']").focus();
			alert("하위 카테고리 URL을 조회하세요.");
			return false;
		}
		...
		$("input[name='categoryFlag'][value='" + $("input[name='categoryLowFlag']:checked").val() + "']").prop('checked', true);
...
//			    	"max_depth": 4, 
		...
		$("#jstree_wrapper").on("select_node.jstree",function(e, data){
			if (!data.event) {
				$('#jstree_wrapper').jstree("deselect_all");	// 선택해제
				$("#jstree_wrapper").jstree("close_all");	// 노드 모두 닫기
				$('#jstree_wrapper').jstree(true).refresh();	// 초기화
				data.node = '';
			}
			if (data.node) {
...
// 	var message = Message.get("M01598") + "\n" + Message.get("M00196");	// 하위 카테고리까지 모두 삭제되며 복구하실 수 없습니다. 삭제하시겠습니까?
	...
	if(categoryLevel > 1) message = Message.get("M00196");
	if (confirm(message)) {
