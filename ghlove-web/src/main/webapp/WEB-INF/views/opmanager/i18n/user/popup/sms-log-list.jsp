...
<input type="hidden" name="page" value="${fn:escapeXml(searchParam.page)}" />
<input type="hidden" name="userName" value="${fn:escapeXml(user.userName)}">	
	<table class="board_write_table" summary="${op:message('M00269')} ">
		...
		<tbody>
			 <tr>
			 	<td class="label"><c:out value="${op:message('M01668')}"/></td> <!-- 발송유형 --> 
			 	<td>
			 		<div>
						<select name="sendType" class="form-control03">
							<option value="" label="전체">전체</option>
							<c:forEach items="${smsTemplateCodeList}" var="list">
								<option value="${fn:escapeXml(list.key)}" label="${fn:escapeXml(list.value)}"><c:out value="${fn:escapeXml(list.value)}"/></option>
							</c:forEach>
						</select>
					</div>
				</td>
			 	...
			 	<td>
					<div class="search-date">
						...
						<span class="day_btns"> 
							...
							<a href="#" class="btn_date week-1"><c:out value="${op:message('M00222')}"/></a> 
							<a href="#" class="btn_date month-1"><c:out value="${op:message('M00029')}"/></a> 
							<a href="#" class="btn_date month-2"><c:out value="${op:message('M00223')}"/></a> 
							<a href="#" class="btn_date clear"><c:out value="${op:message('M00039')}"/></a> 
						</span>
					</div>							
				</td>	
			 </tr>		 
	...
	<div class="btn_all">
		<div class="btn_left">
			<button type="button" class="btn btn-dark-gray btn-sm" onclick="SendSmsLogReset(${fn:escapeXml(serId)});"><span class="glyphicon glyphicon-repeat"></span> <c:out value="${op:message('M00047')}"/></button> <!-- 초기화 -->
		</div>
		<div class="btn_right">
