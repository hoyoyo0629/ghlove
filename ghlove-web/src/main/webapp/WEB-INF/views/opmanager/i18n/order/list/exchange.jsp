...
<form:form modelAttribute="claimApplyParam" method="post">
	...
	<div class="board_write">
		...
		<div class="btn_all btn_right">
			<div class="flex_box gap-08">
				<button type="button" class="btn btn-dark-gray btn-mini" onclick="location.href='<c:out value="${requestContext.sellerPage ? '/seller' : '/opmanager'}"/>/order/exchange/list'"> <c:out value="${op:message('M00047')}"/><!-- 초기화 --></button>
			...
			<tbody>
				...
				<c:forEach items="${ list }" var="apply" varStatus="index">
					...
					<tr style="background:${fn:escapeXml(tdColor)};">
						...
						<c:if test="${requestContext.sellerPage == false}">
							<c:if test="${(op:hasRole('ROLE_ADMIN_1') || op:hasRole('ROLE_ADMIN_2') || op:hasRole('ROLE_ADMIN_3') || op:hasRole('ROLE_ADMIN_4'))}">
								<td><c:out value="${apply.locgovNm}"/></td>
							</c:if>
						</c:if>
						<td><c:out value="${op:datetime(orderItem.createdDate)}"/></td>
						<td><c:out value="${op:datetime(apply.createdDate)}"/></td>
						<td>
							<a href="<c:out value="${fn:escapeXml(requestContext.sellerPage) ? '/seller' : '/opmanager'}"/>/order/exchange/order-detail/${fn:escapeXml(orderItem.orderSequence)}/${fn:escapeXml(orderItem.orderCode)}?url=${fn:escapeXml(requestContext.currentUrl)}"><c:out value="${fn:escapeXml(orderItem.orderCode)}"/></a>
						</td>
