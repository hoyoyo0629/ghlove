...
<%@ taglib prefix="shop"	uri="/WEB-INF/tlds/shop" %>
	...
	<form:form modelAttribute="messageParam" method="post" enctype="multipart/form-data">
			...
			<table class="board_write_table" summary="${op:message('MENU_1402')}"> <!-- 메세지 관리 -->
				...
				<tbody>
					 <tr>
					 	...
					 	<td>
					 		<div>
								<form:select path="where" title="${op:message('M00468')}" class="wd-150"> <!-- 키워드 선택 -->
		...
		<div class="count_title mt20">
			<h5>
				${op:message('M00039')} : ${fn:escapeXml(messageCount)} ${op:message('M00743')}
			</h5>	 <!-- 전체 -->   <!-- 건 조회 -->
			<span>
				${op:message('M00052')} :
				...
				<tbody>
					<c:forEach items="${messageList}" var="list" varStatus="i">
						<tr>
							...
							<td>${fn:escapeXml(list.id)}</td>
							<td>
								<div class="tex_l">
									<a href="/opmanager/message/edit/${fn:escapeXml(list.id)}">${fn:escapeXml(list.kMessage)}</a>
								</div></td>
							<td>
								<div class="tex_l">
									<a href="/opmanager/message/edit/${fn:escapeXml(list.id)}">${fn:escapeXml(list.jMessage)}</a>
								</div>
							</td>
						</tr>
					</c:forEach>
