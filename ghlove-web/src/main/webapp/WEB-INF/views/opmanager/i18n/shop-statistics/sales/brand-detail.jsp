...
<h1 class="popup_title"><c:out value="${brandName}"/></h1>
		...
		<table class="board_list_table">
			...
			<tbody>
				...
				<c:forEach items="${ brandDetailList }" var="base">
					...
					<tr>
						...
						<c:forEach items="${ base.list }" var="list" varStatus="i">
							...
							<c:if test="${ list.orderType eq 'PAY' }">
								...
								<td rowspan="${fn:escapeXml(rowspan)}"><c:out value="${ osType }"/></td>
								<c:forEach items="${ list.items }" var="item" varStatus="itemIndex">

									<c:if test="${ itemIndex.first == false }"></tr><tr></c:if>

									<td style="text-align:left;"><c:out value="${ item.itemName }"/></td>
									...
									<td class="border_left number"><c:out value="${ op:numberFormat(item.itemPay) }"/></td>

									<c:if test="${ itemIndex.first == true }">
										<td class="border_left number" rowspan="${ fn:length(list.items) }"><c:out value="${ op:numberFormat(list.orderPayAmount) }"/></td>
									</c:if>
								...
								<c:set var="payTotalAmount"><c:out value="${ payTotalAmount + list.orderPayAmount }"/></c:set>
							</c:if>
						</c:forEach>
						...
						<c:forEach items="${ base.list }" var="list" varStatus="i">
							...
							<c:if test="${ list.orderType eq 'CANCEL' }">
								...
								<c:set var="cancelTotalAmount"><c:out value="${ cancelTotalAmount + list.orderPayAmount }"/></c:set>
							</c:if>
						</c:forEach>
					...
					<tr>
						<td class="total-price-title" colspan="8" style="text-align:right; padding:5px;"><c:out value="${ op:date(base.key) }"/>소계</td>
						<td class="total-price-title number"><c:out value="${ op:numberFormat(payTotalAmount + cancelTotalAmount) }"/></td>
					</tr>

					<c:set var="totalAmount"><c:out value="${ totalAmount + (payTotalAmount + cancelTotalAmount) }"/></c:set>

				</c:forEach>
				<tr>
					<td class="total-price-title" colspan="9" style="text-align:right; padding:5px;">합계</td>
					<td class="total-price-title number"><c:out value="${ op:numberFormat(totalAmount) }"/></td>
				</tr>
			</tbody>
		</table>
	</div>
