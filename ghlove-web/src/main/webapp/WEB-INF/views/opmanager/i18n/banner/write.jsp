		...
		<tr>
			...
			<td>
				 <div class="file_attach">
				 	<a href="javascript:File.open('banner', '${fn:escapeXml(requestContext.token)}', 0, 1, 5)" class="btn_write" title="새창">사진첨부</a>
				 	<ul id="temp_image_files" class="files">
							<c:if test="${file.uploadType == '0'}">
								<li id="file_id_${fn:escapeXml(file.fileId)}">
									<p class="name"><img src="/content/images/common/icon_file.gif" alt=""><c:out value='${file.fileName}'/>
									<a href="javascript:deleteFile(${fn:escapeXml(file.fileId)})">삭제</a></p>
									<p class="placeholder_wrap">
										<!-- <span class="required_mark">*</span> -->
										<input type="hidden" name="fileIds" value="${fn:escapeXml(file.fileId)}" />
										<input type="hidden" name="fileDescriptions" value="${fn:escapeXml(file.fileDescription)}" class="full required _filter" title="사진에 대한 설명" />
									</p>
								</li>
							</c:if>
					</ul>
...
<form id="deleteForm" name="deleteForm" method="post" action="/opmanager/banner/delete">
	<input type="hidden" name="bannerId" id="bannerId" value="${fn:escapeXml(banner.bannerId) }" />
</form>
