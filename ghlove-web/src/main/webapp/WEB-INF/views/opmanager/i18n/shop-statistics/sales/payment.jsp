...
<div class="statistics_web">
			...
			<table class="board_write_table" summary="${op:message('M01376')}">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<div>
								<form:checkboxes items="${op:nullToEmptyArr(approvalTypes)}" path="approvalTypes" itemLabel="label" itemValue="key.id" />
							</div>
						</td>
					</tr>
				</tbody>
	...
	<div class="board_list" style="overflow-x:auto">
		...
		<c:forEach items="${list}" var="item">
			<c:set var="totalPayCount"><c:out value="${totalPayCount + item.payCount}"/></c:set>
			<c:set var="totalCancelCount"><c:out value="${totalCancelCount + item.cancelCount}"/></c:set>
			<c:set var="totalPayAmount"><c:out value="${totalPayAmount + item.payAmount}"/></c:set>
			<c:set var="totalCancelAmount"><c:out value="${totalCancelAmount + item.cancelAmount}"/></c:set>
		</c:forEach>
		...
		<table class="board_list_table">
			...
			<thead>
				<tr>
					<th>총 매출금액</th>
					<c:forEach items="${approvalTypes}" var="type">
						<th class="border_left"><c:out value="${type.label}"/></th>
					</c:forEach>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td><c:out value="${op:numberFormat(totalPayAmount - totalCancelAmount)}"/>원</td>
					<c:forEach items="${approvalTypes}" var="type">
						<td>
							<c:set var="value">0</c:set>
							<c:forEach items="${list}" var="item">
								<c:if test="${item.approvalType == type.key.id}">
									<c:set var="value"><c:out value="${value + (item.payAmount - item.cancelAmount)}"/></c:set>
								</c:if>
							</c:forEach>
							<c:out value="${op:numberFormat(value)}"/>원
						</td>
					</c:forEach>
				</tr>
			</tbody>
		...
		<table class="board_list_table" >
			...
			<thead>
				<tr>
					...
					<c:forEach items="${approvalTypes}" var="type">
						<th colspan="3" class="border_left"><c:out value="${type.label}"/></th>
					</c:forEach>
				</tr>
				<tr>
					<c:forEach items="${approvalTypes}" var="type">
				...
				<c:forEach items="${days}" var="day">
					...
					<c:forEach items="${list}" var="item">
						<c:if test="${day == item.payDate}">
							<c:set var="dayTotal"><c:out value="${dayTotal + (item.payAmount - item.cancelAmount)}"/></c:set>
						</c:if>
					</c:forEach>

					<tr>
						<td>
							<c:out value="${op:date(day)}"/>
						</td>
						<td class="text-right">
							<c:choose>
								...
								<c:when test="${dayTotal < 0 }">
									<strong style="color:red"><c:out value="${op:numberFormat(dayTotal)}"/></strong>원
								</c:when>
								<c:otherwise>
									<strong style="color:blue"><c:out value="${op:numberFormat(dayTotal)}"/></strong>원
								</c:otherwise>
							</c:choose>
						</td>
						...
						<c:forEach items="${approvalTypes}" var="type">
							<td class="text-right">
								...
								<c:choose>
									...
									<c:otherwise>
										<strong><c:out value="${op:numberFormat(value)}"/></strong>원
									</c:otherwise>
								</c:choose>
							</td>
							<td class="text-right">
		...
		<sec:authorize access="hasRole('ROLE_EXCEL')">
			<div class="btn_all">
				<div class="right">
					<a href="javascript:downloadExcel();"  class="btn btn-success btn-sm"><span class="glyphicon glyphicon-save"></span><c:out value="${op:message('M00254')}"/></a> <!-- 엑셀 다운로드 -->
				</div>
			</div>
		</sec:authorize>
...
<script type="text/javascript">
	...
	function downloadExcel() {
		Shop.downloadExcelOrder("/opmanager/shop-statistics/sales/payment/excel-download", $('#statisticsParam').serialize(), false);
	}
</script>