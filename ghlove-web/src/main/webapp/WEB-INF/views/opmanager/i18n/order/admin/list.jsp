						...
						<tr>
							<td>
								...
								<c:if test="${order.error != true}">
									<input type="checkbox" name="id" value="${fn:escapeXml(key)}" 
										...
										data-user-id="${fn:escapeXml(order.buyer.userId)}" />
										
									<input type="hidden" name="orderAdminMap[${fn:escapeXml(key)}].userId" value="${fn:escapeXml(order.buyer.userId)}" />
								</c:if>
								</div>
							</td>
							<td>
							...
							<td>
								<div style="width:100px;">
									${op:date(order.workDate)} / ${fn:escapeXml(order.workSequence)}
								</div>
							</td>
							<c:choose>
								<c:when test="${order.error == true}">
									<td colspan="17" class="text-left">
										${fn:escapeXml(order.errorMessage)}
									</td>
								</c:when>
								<c:otherwise>
									<td>
										<div style="width:100px;">
											<input type="text" name="orderAdminMap[${fn:escapeXml(key)}].orderGroupCode" class="required-optional" title="주문 그룹번호" maxlength="10" value="${fn:escapeXml(order.orderGroupCode)}" />
										</div>
									</td>
									<td>
										<div>
											${fn:escapeXml(order.item.itemUserCode)}
											<input type="hidden" name="orderAdminMap[${fn:escapeXml(key)}].itemId" value="${fn:escapeXml(order.item.itemId)}" />
										</div>
									</td>
									<td>
										<div style="width:50px;">
											<img src="${fn:escapeXml(order.item.imageSrc)}" alt="${fn:escapeXml(order.item.itemName)}" width="100%"/>
										</div>
									</td>
									<td>
										<div style="width:200px;" class="text-left">
											${fn:escapeXml(order.item.itemName)}
										</div>
									</td>
									<td>
												...
												<c:when test="${order.item.itemOptionType == 'S'}">
													...
													<ul>
														<c:forEach items="${order.item.optionGroups}" var="group">
															...
															${fn:escapeXml(group.title)} :
																<select name="orderAdminMap[${fn:escapeXml(key)}].optionIds" class="required-optional" title="옵션 - ${fn:escapeXml(group.title)}">
																	<option value="">-선택-</option>
																	<c:forEach items="${order.item.itemOptions}" var="option">
																		<c:if test="${option.optionType != 'T' && group.title == option.optionName1}">
																			<option value="${fn:escapeXml(option.itemOptionId)}">
													...
													<select name="orderAdminMap[${fn:escapeXml(key)}].optionIds" class="required-optional" title="옵션 - ${fn:escapeXml(group.title)}">
															...
															<c:forEach items="${order.item.itemOptions}" var="option">
																<c:if test="${option.optionType != 'T'}">
																	<option value="${fn:escapeXml(option.itemOptionId)}">
																		<c:choose>
																			<c:when test="${option.optionType == 'S2'}">${fn:escapeXml(option.optionName1)} / ${fn:escapeXml(option.optionName2)}</c:when>
																			<c:when test="${option.optionType == 'S3'}">${fn:escapeXml(option.optionName1)} / ${fn:escapeXml(option.optionName2)} / ${fn:escapeXml(option.optionName3)}</c:when>
																		</c:choose>
									...
									<td>
										<div style="width:100px;">
											<input type="text" class="_number_comma text-right required-optional" name="orderAdminMap[${fn:escapeXml(key)}].salePrice" title="판매가" maxlength="9" value="${op:numberFormat(order.item.presentPrice)}" />
										</div>
									</td>
									<td>
										<div style="width:100px;">
											<input type="text" class="_number_comma text-right" name="orderAdminMap[${fn:escapeXml(key)}].quantity" title="수량" maxlength="9" value="${fn:escapeXml(order.quantity)}" />
										</div>
									</td>
									<td>
										<div>
											<c:choose>
												...
												<c:otherwise>[${fn:escapeXml(order.buyer.loginId)}]</c:otherwise>
											</c:choose>
											<input type="text" name="orderAdminMap[${fn:escapeXml(key)}].userName" class="required-optional" title="구매자 이름" maxlength="20" value="${fn:escapeXml(order.buyer.userName)}" /> 
										</div>
									</td>
									<td>
										<div>
											<input type="text" name="orderAdminMap[${fn:escapeXml(key)}].phone" title="구매자 전화번호" maxlength="20" value="${fn:escapeXml(order.buyer.phone)}" />
										</div>
									</td>
									<td>
										<div>
											<input type="text" name="orderAdminMap[${fn:escapeXml(key)}].mobile" class="required-optional" title="구매자 휴대폰" maxlength="20" value="${fn:escapeXml(order.buyer.mobile)}" />
										</div>
									</td>
									<td>
										<div>
											<input type="text" name="orderAdminMap[${fn:escapeXml(key)}].receiveName" class="required-optional" title="받는사람 이름" maxlength="20" value="${fn:escapeXml(order.receiver.receiveName)}" />
										</div>
									</td>
									<td>
										<div>
											<input type="text" name="orderAdminMap[${fn:escapeXml(key)}].receiveZipcode" class="required-optional" title="받는사람 우편번호" maxlength="7" value="${fn:escapeXml(order.receiver.receiveZipcode)}" />
										</div>
									</td>
									<td>
										<div style="width:250px;">
											<input type="text" name="orderAdminMap[${fn:escapeXml(key)}].receiveAddress" class="required-optional" title="받는사람 주소" maxlength="100" value="${fn:escapeXml(order.receiver.receiveAddress)}" />
										</div>
									</td>
									<td>
										<div style="width:250px;">
											<input type="text" name="orderAdminMap[${fn:escapeXml(key)}].receiveAddressDetail" class="required-optional" title="받는사람 상세 주소" maxlength="255" value="${fn:escapeXml(order.receiver.receiveAddressDetail)}" />
										</div>
									</td>
									<td>
										<div>
											<input type="text" name="orderAdminMap[${fn:escapeXml(key)}].receivePhone" title="받는사람 전화번호" maxlength="14" value="${fn:escapeXml(order.receiver.receivePhone)}" />
										</div>
									</td>
									<td>
										<div>
											<input type="text" name="orderAdminMap[${fn:escapeXml(key)}].receiveMobile" title="받는사람 휴대폰" maxlength="14" value="${fn:escapeXml(order.receiver.receiveMobile)}" />
										</div>
									</td>
									<td>
										<div>
											<input type="text" name="orderAdminMap[${fn:escapeXml(key)}].content" title="배송 요구사항" maxlength="50" value="${fn:escapeXml(order.receiver.content)}" />
										</div>
									</td>
								</c:otherwise>
							</c:choose>
