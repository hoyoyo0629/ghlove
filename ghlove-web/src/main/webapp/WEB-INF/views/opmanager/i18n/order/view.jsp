...
<%@ taglib prefix="op" 		uri="/WEB-INF/tlds/functions" %>
		...
		<div class="popup_wrap" style="min-width:1200px;">
			...
			<div class="popup_contents">			
				<form:form modelAttribute="order" action="/opmanager/order/edit/${ fn:escapeXml(order.orderId) }" method="post">
							...
							<c:if test="${ not empty orderStatusLogMap }">
								<div class="cont">
									...
									<c:forEach items="${ orderStatusLogMap }" var="map">
										...
										<p><span class="blet"><img src="/content/opmanager/images/common/popup_bl.png" alt="" /></span> ${fn:escapeXml(title)} </p><!-- 주문 변경 상태 -->
										<ul>
											<c:forEach items="${logs}" var="item">
												<li>
													• ${ fn:escapeXml(item.orderStatusLabel) } (${ op:datetime(item.createdDate) }<c:if test="${ not empty item.userName }"> [ ${fn:escapeXml(item.userName)} ]</c:if>)
												</li>
											</c:forEach>
										</ul>
									</c:forEach>
								...
								<div class="cont">
									...
									<ul>
										<c:forEach items="${sendMailLogList}" var="item">
											<li>• ${ fn:escapeXml(item.orderStatusLabel) } (${ op:datetime(item.createdDate) } [ ${ fn:escapeXml(item.sendLoginId) } ]) <a href="/opmanager/order/send-mail-view?id=${ fn:escapeXml(item.sendMailLogId) }" class="table_btn"  onclick="Common.popup(this.href, 'mail-view', 600, 562, 1); return false;">메일보기</a></li>
										</c:forEach>
									</ul>
								</div>
							</c:if>	
								...
								<div class="cont">
									...
									<ul>
										<c:forEach items="${orderPrintLogList}" var="item">
											<li>• ${ fn:escapeXml(item.title) } (${ op:datetime(item.createdDate) }<c:if test="${ not empty item.userName }"> [ ${fn:escapeXml(item.userName)} ]</c:if>)</li>
										</c:forEach>
									</ul>
								</div>
							</c:if>
					...
					<h3><span>${op:message('M00284')} <!-- 주문관리 --></span>
						<div class="right">
							<button type="button" class="btn gray02" onclick="Common.popup('/opmanager/order/order-print?id=${fn:escapeXml(order.orderId)}', 'print-view', 672, 900, 1); return false;">${op:message('M00343')} <!-- 주문서 인쇄 --></button>
						</div>
					</h3>
						...
						<table class="board_write_table" summary="${op:message('M00284')}">
							...
							<tbody>
								<tr>
									...
									<td>
										<div>
											<strong>${ fn:escapeXml(order.orderCode) }</strong>
									 	</div>
									</td>
									...
									<td>
										<div>
											...
											<a href="/opmanager/order/order-mail?id=${ fn:escapeXml(order.orderId) }" class="table_btn" onclick="Common.popup(this.href, 'order-mail', 1000, 600, 1); return false;">${op:message('M00345')} <!-- 수동 메일발송 --></a>
									 	</div>
									</td>
								</tr>						 
							</tbody>
						...
						<div class="left">
							...
							<c:if test="${not empty changeOrderButtons}">
								<c:forEach items="${changeOrderButtons}" var="button">
									<button type="button" class="btn btn-dark-gray btn-sm" onclick="editItem('${fn:escapeXml(order.orderId)}', '${fn:escapeXml(button.key)}')">${fn:escapeXml(button.value)}</button>
								</c:forEach>
							</c:if>
							
							<c:if test="${order.orderChange == true}">
								<button type="button" class="btn btn-dark-gray btn-sm" onclick="editItem('${fn:escapeXml(order.orderId)}', 'change')">주문서 수정</button>
							</c:if>
						</div>
								...
								<c:forEach items="${ order.orderItemGroup }" var="group" varStatus="orderItemGroupIndex">
									<c:forEach items="${ group.value }" var="item" varStatus="i">
										<tr>
											 <td>
											 	<div>
											 		<div class="item_img"><img src="${ fn:escapeXml(item.item.imageSrc) }" style="width:50px;" /></div>
											 		<div class="item_name" style="padding: 0 10px;">
															...
															<ul class="item-list">
								 								<c:forEach items="${ item.requiredOptionsList }" var="option">
								 									<c:if test="${option.optionHideFlag == 'Y'}">
																		<li>
																			<span>${ fn:escapeXml(option.optionName1) }</span>
																			${fn:escapeXml(option.optionName2)} 
																			...
																			(상품번호 : ${fn:escapeXml(option.optionCode)})
																			</c:if>
																		</li>
																	</c:if>
																</c:forEach>
														...
														<c:if test="${ not empty item.openMarketOption }">
															<br /><br /> 
															<span>${fn:escapeXml(item.openMarketOption)}</span>
														</c:if>
											 		</div> 
											 	</div>
											 </td>
												...
												<td rowspan="${ fn:length(group.value) }">
													...
													<p>
														<c:choose>
															...
															<c:otherwise>
																<select id="groupOrderStatus-${fn:escapeXml(order.orderId)}-${fn:escapeXml(groupIndex)}" class="baseGroupOrderStatus">
																	<c:choose>
													...
													<input type="hidden" name="orderItemStatusParams[${fn:escapeXml(itemIndex)}].orgOrderStatus" value="${fn:escapeXml(item.orderStatus)}" class="groupOrgOrderStatus-${fn:escapeXml(order.orderId)}-${fn:escapeXml(groupIndex)}" id="orgOrderStatus-${fn:escapeXml(order.orderId)}-${fn:escapeXml( item.orderItemId )}" />
													<input type="hidden" name="orderItemStatusParams[${fn:escapeXml(itemIndex)}].orderItemId" value="${fn:escapeXml(item.orderItemId)}" />
													<input type="hidden" name="orderItemStatusParams[${fn:escapeXml(itemIndex)}].orderStatus" value="${fn:escapeXml(item.orderStatus)}" class="changeOrderStatus changeOrderStatus-${fn:escapeXml(order.orderId)} groupOrderStatus-${fn:escapeXml(order.orderId)}" id="orderStatus-${fn:escapeXml(order.orderId)}-${fn:escapeXml( item.orderItemId )}" />
													
													<c:choose>
														<c:when test="${item.orderStatus == '50' && order.approvalType == 'bank'}">
															<br/> <button type="button" class="btn btn-dark-gray btn-sm" onclick="changeOrderStatus('${fn:escapeXml(order.orderId)}', 0, '해당 주문의 상태를 입금 대기 상태로 변경 하시겠습니까?')">입금 확인</button>
														</c:when>
														<c:when test="${item.orderStatus == '1' || item.orderStatus == '33'}">
															<br/>
															<select name="orderItemStatusParams[${fn:escapeXml(itemIndex)}].deliveryCompanyId" id="groupDeliveryCompanyId-${fn:escapeXml(order.orderId)}" class="full baseGroupDeliveryCompanyId">
																<option value="0">-선택-</option>
																<c:forEach items="${activeDeliveryCompanyList}" var="deliveryCompany">
																	<option value="${fn:escapeXml(deliveryCompany.deliveryCompanyId)}" ${op:selected(item.deliveryCompanyId, deliveryCompany.deliveryCompanyId)}>${fn:escapeXml(deliveryCompany.deliveryCompanyName)}</option>
																</c:forEach>
															</select> <br />
															<input type="text" name="orderItemStatusParams[${fn:escapeXml(itemIndex)}].deliveryNumber" id="groupDeliveryNumber-${fn:escapeXml(order.orderId)}" class="full baseGroupDeliveryNumber" value="${fn:escapeXml(item.deliveryNumber)}" />
														</c:when>
														<c:when test="${item.orderStatus == '2' || item.orderStatus == '34'}">
															<p class="mt10">송장번호 : ${ fn:escapeXml(item.deliveryNumber) }</p>
															<input type="hidden" name="orderItemStatusParams[${fn:escapeXml(itemIndex)}].deliveryCompanyId" value="${fn:escapeXml(item.deliveryCompanyId)}" />
															<input type="hidden" name="orderItemStatusParams[${fn:escapeXml(itemIndex)}].deliveryNumber" value="${fn:escapeXml(item.deliveryNumber)}" />
														</c:when>
														<c:when test="${item.orderStatus == '3'}">
													...
													<c:forEach items="${ group.value }" var="groupItem" varStatus="groupItemIndex">
														<c:if test="${groupItemIndex.index > 0}">
															...
															<input type="hidden" name="orderItemStatusParams[${fn:escapeXml(itemIndex)}].orgOrderStatus" value="${fn:escapeXml(groupItem.orderStatus)}" class="groupOrgOrderStatus-${fn:escapeXml(order.orderId)}-${fn:escapeXml(groupIndex)}" id="orgOrderStatus-${fn:escapeXml(order.orderId)}-${fn:escapeXml( groupItem.orderItemId )}" />
															<input type="hidden" name="orderItemStatusParams[${fn:escapeXml(itemIndex)}].orderItemId" value="${fn:escapeXml(groupItem.orderItemId)}" />
															<input type="hidden" name="orderItemStatusParams[${fn:escapeXml(itemIndex)}].orderStatus" class="changeOrderStatus changeOrderStatus-${fn:escapeXml(order.orderId)} groupOrderStatus-${fn:escapeXml(order.orderId)}" id="orderStatus-${fn:escapeXml(order.orderId)}-${fn:escapeXml( groupItem.orderItemId )}" value="${fn:escapeXml( groupItem.orderStatus )}" />
															<c:if test="${groupItem.orderStatus == '1' || groupItem.orderStatus == '2' || groupItem.orderStatus == '33' || groupItem.orderStatus == '34'}">
																<input type="hidden" name="orderItemStatusParams[${fn:escapeXml(itemIndex)}].deliveryCompanyId" class="groupDeliveryCompanyId-${fn:escapeXml(fn:escapeXml(order.orderId)}" value="${fn:escapeXml(groupItem.deliveryCompanyId)}" />
																<input type="hidden" name="orderItemStatusParams[${fn:escapeXml(itemIndex)}].deliveryNumber" class="groupDeliveryNumber-${fn:escapeXml(order.orderId)}" value="${fn:escapeXml(groupItem.deliveryNumber)}" />
															</c:if>
					...
					<div class="btn_center">
						...
						<a href="javascript:;" onclick="Common.popup('/opmanager/order/order-print?id=${fn:escapeXml(order.orderId)}', 'print-view', 672, 900, 1);" class="btn btn-default">${op:message('M00343')} <!-- 주문서 인쇄 --></a>
					</div>
					...
					<div class="total_wrap">
				 		<table cellpadding="0" cellspacing="0" summary="상품금액, 배송비, 할인금액, 최종 결제 금액" class="total_price">
							...
							<colgroup>
								...
								<col style="width:${ fn:escapeXml(width) };">
								<col style="width:2%;">
								<col style="width:${ fn:escapeXml(width) };">
								<c:if test="${ order.taxType eq '2' }">
									<col style="width:2%;">
						...
						<table class="board_write_table" summary="${op:message('M00318')}">
							...
							<tbody>
								<tr>
									...
									<td>
										<div>
											${ fn:escapeXml(order.orderPayment.approvalTypeLabel) } 
									 	</div>
									</td>
								</tr>
								...
								<c:if test="${ order.orderPayment.approvalType eq 'bank' }">
					 				<tr>
					 					...
					 					<td><div>${ fn:escapeXml(order.orderPayment.bankVirtualNo) }</div></td>
					 				</tr>
					 				<tr>
					 					<td class="label">${op:message('M00542')} <!-- 입금명의 --></td>
					 					<td><div>${ fn:escapeXml(order.orderPayment.bankInName) }</div></td>
					 				</tr>
					 				<tr>
								...
								<c:if test="${ order.orderPayment.approvalType eq 'conv' }">
									<tr>
					 					...
					 					<td><div>${ fn:escapeXml(order.orderPayment.cvsName) }</div></td>
					 				</tr>
					 				
					 				<tr>
					 					<td class="label last">支払番号 <!-- 지불번호 --> </td>
					 					<td class="last"><div>${ fn:escapeXml(order.orderPayment.receiptNo) }</div></td>
					 				</tr>
					 				<tr>
								...
								<tbody>
									<c:forEach items="${cashReceiptList}" var="item">
										<tr>
											...
											<td>
												<div>
													${fn:escapeXml(item.cashReceiptName)}
											 	</div>
											</td>
										...
										<tr>
											...
											<td>
												<div>
													${fn:escapeXml(item.cashReceiptType)}
											 	</div>
											</td>
											<td class="label">신청 정보</td>
											<td>
												<div>
													${fn:escapeXml(item.cashReceiptCode)}
											 	</div>
											</td>
										</tr>
									</c:forEach>
							...
							<table class="board_write_table" summary="세금계산서 신청 정보">
								...
								<tbody>
										<tr>
											...
											<td>
												<div>
													${fn:escapeXml(order.taxInvoiceBusinessNumber)}
											 	</div>
											</td>
											<td colspan="2"/>
										</tr>
										<tr>
											...
											<td>
												<div>
													${fn:escapeXml(order.taxInvoiceBusinessCompanyName)}
											 	</div>
											</td>
											<td colspan="2"/>
										</tr>
										<tr>
											...
											<td>
												<div>
													${fn:escapeXml(order.taxInvoiceBusinessBossName)}
											 	</div>
											</td>
											<td colspan="2"/>
										</tr>
										<tr>
											...
											<td>
												<div>
													${fn:escapeXml(order.taxInvoiceBusinessStatus)}
											 	</div>
											</td>
											<td class="label">종목</td>
											<td>
												<div>
													${fn:escapeXml(order.taxInvoiceBusinessStatusType)}
											 	</div>
											</td>
										</tr>
										<tr>
											...
											<td>
												<div>
													${fn:escapeXml(order.taxInvoiceBusinessLocation)}
											 	</div>
											</td>
											<td colspan="2"/>
										</tr>
										<tr>
											...
											<td>
												<div>
													${fn:escapeXml(order.taxInvoiceBusinessEmail)}
											 	</div>
											</td>
											<td colspan="2"/>
										</tr>
									...
									<c:forEach items="${ order.addInfoToHashMap }" var="item">
										<tr>
											...
											<td>
												<div>
													- ${ op:nl2br(item.value) }
													<form:hidden path="content" value="${fn:escapeXml(comment)}" />
											 	</div>
											</td>
										</tr>
									</c:forEach>
								...
								<table class="board_write_table" summary="환불 정보">
									...
									<tbody>
										<tr>
											...
											<td>
												<div>
													${fn:escapeXml(order.orderPayment.returnBankInName)} <br/>
													<c:choose>
														<c:when test="${fn:trim(order.orderPayment.returnVirtualNo) == ''}">
															계좌 정보 없음
														</c:when>
														<c:otherwise>
															계좌 정보 : ${fn:escapeXml(order.orderPayment.returnVirtualNo)} (${fn:escapeXml(order.orderPayment.returnBankName)})
														</c:otherwise>
													</c:choose>
											 	</div>
											</td>
						...
						<table class="board_write_table" summary="${op:message('M00322')}">
							...
							<tbody>
								<tr>
									...
									<td>
										<div>
										 	<textarea cols="30" rows="6" name="adminMessage" title="${op:message('M00323')}">${ fn:escapeXml(order.adminMessage) }</textarea>
									 	</div>
									</td>
								</tr>
								...
								<tr>
									...
									<td>
										<div>
										 	<textarea cols="30" rows="6" name="adminMessageEtc" title="${op:message('M00323')} (${op:message('M00324')})">${ fn:escapeXml(order.adminMessageEtc) }</textarea>
									 	</div>
									</td>
								</tr>
...
<script type="text/javascript">
	...
	function refuseYourApplication(orderItemId) {
		...
		if (confirm('해당 주문 상품의 사용자 요청을 거절 하시겠습니까?\n거절후에는 사용자가 해당 요청을 신청하기 전상태로 복구됩니다.')) {
			var param = {
				'orderId'		: '${fn:escapeXml(order.orderId)}',
	...
	$(function(){
		...
		$('#update_delivery_prearranged_date').on('click', function() {
			var orderId = '${fn:escapeXml(order.orderId)}';
