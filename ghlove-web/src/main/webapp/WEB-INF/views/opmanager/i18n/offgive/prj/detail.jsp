...
<%-- 	            </c:if> --%>
	            ...
	            <button type="button" class="btn btn-dark-gray btn-small" onclick="window.open('/opmanager/offgive/prj/popup/form-apply/${fn:escapeXml(offgive.cntrSn)}','pop','width=900,height=1200');">납부신청서 출력</button>
	            <c:if test="${today == fn:replace(fn:substring(offgive.frstRegistPnttm, 0, 10), '-','')}">
...
function cancel() {
	Common.confirm("${op:message('M01700')}", function() {
		$.post(url("/opmanager/offgive/prj/delete/${fn:escapeXml(offgive.cntrSn)}"), {}, function(response) {
...
function goList() {
	...
	if (document.referrer.indexOf("create") != -1) {
		location.href = "/opmanager/offgive/prj/list"
	} else {
		history.back()
	}
	*/
...
function offgiveCancel(){
	...
	if("${fn:escapeXml(offgive.cntrUsePoint)}" > 0){
		...
		if(confirm("해당 기부 건을 취소를 요청하시겠습니까?")){
			...
			$.post("/opmanager/offgive/prj/offgiveCancel", param, function(response) {
...
function cancelReq(reqId){
	...
	if(confirm("요청된 기부취소신청을 취소하시겠습니까?")){
		var param = {
	 	...
	 	$.post('opmanager/offgive/prj/offgiveCancelReq',param, function(response) {
	 		if(response.isSuccess == true) {
