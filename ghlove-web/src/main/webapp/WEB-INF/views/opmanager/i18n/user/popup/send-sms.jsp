...
<div class="popup_wrap">
	...
	<form:form modelAttribute="umsDetail">
		<div class="popup_contents pb0">
					
			<c:choose>
				<c:when test="${empty umsDetail}">
					...
					<table cellpadding="0" cellspacing="0" summary="" class="board_write_table">
						...
						<tr>
							...
							<td>
								<div>
									<c:choose>
										<c:when test="${umsDetail.sendType eq 'sms'}">SMS</c:when>
										<c:when test="${umsDetail.sendType eq 'lms'}">LMS</c:when>
										<c:when test="${umsDetail.sendType eq 'mms'}">MMS</c:when>
						...
						<%--<c:if test="${userSms.sendType eq 'lms' and umsDetail.sendType eq 'mms'}">
							<tr>
						...
						<tr>
							...
							<td>
								<div>
									<form:textarea path="message" cssClass="required _filter content" title="내용" style="height:150px;"/>
								</div>
							</td>
						</tr>
						</tbody>
...
<script type="text/javascript">
	$(function() {
		...
		$("#umsDetail").validator(function(){
			
			/*var sendType = $('input[name="sendType"]:checked').val();
			if ('mms' == sendType || 'lms' == sendType) {
