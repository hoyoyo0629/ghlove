...
<!--대그룹 타이틀-->
	<div class="subMain_title">
		<img src="/content/images/common/${fn:escapeXml(categoryTeamCode)}_title.gif" alt="hair" /> 
	</div><!--//subMain_title-->
	
	<div class="breadcrumbs">
		<a href="/categories/index/${fn:escapeXml(categoryTeamCode)}" class="home">${fn:escapeXml(teamName)}用品通販HOME</a>
	</div>
	...
	<div class="main_banner">
		<c:forEach items="${promotionList}" var="list">
			<c:if test="${i == 1 && list.editImage != ''  }">
				<a href="${fn:escapeXml(list.editUrl) }"><img src="/upload/category-edit/${fn:escapeXml(list.code)}/defult/${fn:escapeXml(list.editImage)}" alt="${fn:escapeXml(list.editContent)}" width="670" height="320" /></a>
				<c:set var="i">2</c:set>
			</c:if>
		</c:forEach>
	</div><!--//main_banner E-->
	...
	<ul class="min-banner">
		<c:forEach items="${promotionList}" var="list" varStatus="i">
			<c:if test="${list.editImage != ''}">
				<li><a href="${fn:escapeXml(list.editUrl)}"><img src="/upload/category-edit/${fn:escapeXml(list.code)}/thumb/${fn:escapeXml(list.editImage)}" alt="${fn:escapeXml(list.editContent)}" width="167" height="80" /></a></li>
			</c:if>
		</c:forEach>
	</ul> 
	...
	<c:if test="${categoryTeamCode != 'sale_outlets'}">
		<c:set var="i">1</c:set>
		<h4>${fn:escapeXml(teamName)}の新着情報 <span><a href="/board/index/${fn:escapeXml(categoryTeamCode)}">${fn:escapeXml(teamName)}の新着情報一覧</a></span></h4>
		<div class="tab_info">					 	 
							 	...
							 	<li>
							 		...
							 		<c:forEach items="${subCategoryCode}" var="list2">
						 				<c:if test="${list2.key.id == list.subCategory }">
						 					<img src="/content/images/btn/images/btn_notice_${fn:escapeXml(list2.key.id)}.gif" alt="${fn:escapeXml(list2.label)}" />
						 				</c:if>
						 			</c:forEach>
							 		<a href="${fn:escapeXml(list.url)}" ${fn:escapeXml(target)} ${fn:escapeXml(rel)}>${fn:escapeXml(list.subject) }</a>
							 	</li>
	...
	<!-- 신상품 시작-->
	<h4>新着商品<span><a href="/products/searchResult?cate00team=${fn:escapeXml(categoryTeamCode)}">新着商品情報一覧</a></span></h4>		
			...
			<ul>
				<c:forEach items="${newItemList}" var="list" varStatus="i">
					 <li> 
				 		...
				 		<div><a href="${fn:escapeXml(list.link)}" ${fn:escapeXml(list.noFollow)}><img src="${fn:escapeXml(list.imageSrc)}" class="item_list_image_min" alt="${fn:escapeXml(list.itemName)}" /></a></div>
				 		<p><a href="${fn:escapeXml(list.link)}" ${fn:escapeXml(list.noFollow)} style="display:block;">${fn:escapeXml(list.itemName)}</a></p>
				 	</li>
				</c:forEach>
	...
	<c:if test="${!empty categoryEdit.event}">
		<div class="free-space-content">
			${fn:escapeXml(categoryEdit.event)}
		</div>
	</c:if>
	...
	<!--// 신상품 끝-->
		...
		<!-- 히트상품랭킹1 시작 -->
		<h4>${teamName}売れ筋ランキング<span><a href="/ranking/cate00team_${fn:escapeXml(categoryTeamCode)}">もっと見る</a></span></h4>
	...
	<!-- 리뷰 시작 -->
	<h4>${fn:escapeXml(teamName)}新着レビュー<span><a href="/product_reviews/recent">${fn:escapeXml(teamName)}レビュー一覧</a></span></h4>
	<div class="selling_rating">
		<ul style="position: relative;">
			<c:forEach items="${reviewList}" var="itemReview" varStatus="i">
		    	<li>
		        	<a href="${fn:escapeXml(itemReview.item.link)}" ${fn:escapeXml(itemReview.item.noFollow)}><img src="${fn:escapeXml(itemReview.item.imageSrc)}" class="item_list_image_min" alt="${itemReview.item.itemName)}" /></a>	
	...
	<c:if test="${!empty categoryEdit.footer1}">
		<div class="free-space-content">
			${fn:escapeXml(categoryEdit.footer1)}
		</div>
	</c:if>
	...
	<c:if test="${!empty itemList}">
		<h4>${fn:escapeXml(teamName)}のおすすめ商品</h4>		
