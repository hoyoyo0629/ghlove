...
public class UmsTemplate implements OpMessage {
    ...
    public UmsTemplate(Cryptor cryptor) {
        this.cryptor = cryptor;
        initCommonCodeMap();
    }

    public UmsTemplate(Ums ums, Cryptor cryptor) {
        this.cryptor = cryptor;
        init(ums);
    }

    public UmsTemplate(Ums ums, String phoneNumber, Cryptor cryptor) {
        this(ums, cryptor);
        this.receivers[0] = cryptor.decrypt(phoneNumber);
    }

    public UmsTemplate(Ums ums, String phoneNumber, Cryptor cryptor, DataMasking dataMasking) {
        this(ums, cryptor);
        this.dataMasking = dataMasking;
        this.receivers[0] = cryptor.decrypt(phoneNumber);
    }

    public UmsTemplate(Ums ums, String phoneNumber, String callbackNumber, Cryptor cryptor) {
        this(ums, phoneNumber, cryptor);
        this.callbackNumber = callbackNumber;
    }

    public UmsTemplate(Ums ums, String phoneNumber, UmsExpansionInfo umsExpansionInfo, Cryptor cryptor) {
        this(ums, phoneNumber, cryptor);
        this.umsExpansionInfo = umsExpansionInfo;
    }

    public UmsTemplate(Ums ums, String phoneNumber, String callbackNumber, UmsExpansionInfo umsExpansionInfo, Cryptor cryptor) {
        this(ums, phoneNumber, callbackNumber, cryptor);
        this.umsExpansionInfo = umsExpansionInfo;
    }
                ...
                if (detail.isUsedFlag()) {
                    ...
                    if (!ObjectUtils.isEmpty(message)) {
                        for (String key : codeMap.keySet()) {
                            ...
                            if (ObjectUtils.isEmpty(fieldValue) || "null".equals(fieldValue)) {
                                continue;
                            }
    ...
    public void initialize(long userId, ApplicationInfo applicationInfo) {
        ...
        try {
            ...
            expansionInfo.setTargetUserId(userId);
    ...
    public boolean isValidUms(Ums ums) {
        ...
        // 야간발송 허용 or 불가인데 21시 ~ 6시가 아니면 OK
        boolean isNightSendFlag = ums.isNightSendFlag() || (!ums.isNightSendFlag() && !(LocalTime.now().isAfter(startTime) || LocalTime.now().isBefore(endTime)));
