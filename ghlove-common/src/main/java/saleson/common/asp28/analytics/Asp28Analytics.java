...
public class Asp28Analytics {
	...
	public Asp28Analytics(List<BuyItem> list, String deviceType, String type) {
		...
		for(BuyItem buyItem : list) {
			...
			if (item != null && itemPrice != null && "cart".equals(type.toLowerCase())) {
				this.cartItems += CommonUtils.dataNvl(item.getItemUserCode())  + "_" + CommonUtils.dataNvl(itemPrice.getQuantity());
			} else {
				this.products += CommonUtils.dataNvl(item.getItemUserCode()) + "_" + CommonUtils.dataNvl(itemPrice.getSaleAmount()) + "_" + CommonUtils.dataNvl(itemPrice.getQuantity());
			}
