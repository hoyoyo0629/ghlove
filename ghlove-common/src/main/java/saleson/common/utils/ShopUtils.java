...
public class ShopUtils {
	...
	private static final Logger log = LoggerFactory.getLogger(ShopUtils.class);
	 ...
	 * @return
	 */
	public static String image(String position, String itemUserCode, String fileName, String detailsType) {
		if (!StringUtils.hasText(fileName) || fileName.indexOf("/") > -1) {
			return fileName;
		}

		StringBuilder imageUrl = new StringBuilder();
		imageUrl.append(SalesonProperty.getSalesonUrlCdn());
		imageUrl.append(SalesonProperty.getUploadBaseFolder());
		imageUrl.append("/item/").append(itemUserCode).append("/").append(fileName);

		// old code source
		/*imageUrl.append("/item/").append(itemUserCode).append("/").append(position).append("/");
		if (StringUtils.hasText(detailsType)) imageUrl.append(detailsType).append("/");
		imageUrl.append(fileName);*/

		return imageUrl.toString();
	}

	public static String image(String position, String itemUserCode, String fileName) {
		return image(position, itemUserCode, fileName, "");
	}
	 ...
	 * @return
	 */
	public static String catalogImage(String itemUserCode, String fileName) {
		return image("catalog", itemUserCode, fileName);
	}
	 ...
	 * @return
	 */
	public static String listImage(String itemUserCode, String fileName) {
		return image("list", itemUserCode, fileName);
	}
	 ...
	 * @return
	 */
	public static String detailsImage(String itemUserCode, String fileName) {
		return image("details", itemUserCode, fileName);
	}
	 ...
	 * @return
	 */
	public static String detailsThumbnail(String itemUserCode, String fileName) {
		return image("details", itemUserCode, fileName, "thumb");
	}
	 ...
	 * @return
	 */
	public static String detailsBigImage(String itemUserCode, String fileName) {
		return image("details", itemUserCode, fileName, "big");
	}
	...
	public static void getItemOptionStockMap(HashMap<String, HashMap<String, Integer>> stockMap, ItemOption option, int maxQuantity, int buyQuantity, long sellerId) {
		...
		if (!"".equals(stockKey)) {
			...
			int totalBuyQuantity = info.get("totalBuyQuantity");
	...
	public static HashMap<String, HashMap<String, Integer>> makeStockMap(List<BuyItem> buyItems) {
		HashMap<String, HashMap<String, Integer>> stockMap = new HashMap<>();
		...
		for (BuyItem buyItem : buyItems) {
			...
			if (buyItem.getItemPrice() != null && buyItem.getItemPrice().getQuantity() > 0) {
	...
	/**
	 * 세트 구매 상품의 재고량을 최대 구매 가능 수량과 연동하여 결정함
	 * @param buyItem
	 * @param buyItemSets
	 * @return
	 */
	public static HashMap<String, HashMap<String, Integer>> makeStockSetMap(BuyItem buyItem, List<BuyItem> buyItemSets) {
		HashMap<String, HashMap<String, Integer>> stockSetMap = new HashMap<>();

		if (buyItemSets == null || buyItemSets.isEmpty()) {
			return null;
		}
		...
		for (BuyItem buyItemSet : buyItemSets) {
			...
			if (buyItem.getItemPrice() != null && buyItem.getItemPrice().getQuantity() > 0) {
				int buyQuantity = buyItem.getItemPrice().getQuantity() * buyItemSet.getItemPrice().getQuantity();
				int maxQuantity = -1;

				if ("N".equals(item.getItemOptionFlag())) {
					getItemStockMap(stockSetMap, item, maxQuantity, buyQuantity);
				} else {
					List<ItemOption> itemOptions = buyItemSet.getOptionList();
					if (ValidationUtils.isNotNull(itemOptions)) {
						for (ItemOption itemOption : itemOptions) {
							getItemOptionStockMap(stockSetMap, itemOption, maxQuantity, buyQuantity, item.getSellerId());
						}
					}
				}
			}
	 ...
	 * @return
	 */
	public static int getMaxQuantity(int maxQuantity, int buyQuantity, HashMap<String, Integer> stockInfo) {
		...
		if (stockInfo == null) {
			...
			int stockQuantity = stockInfo.get("stockQuantity");
			int totalBuyQuantity = stockInfo.get("totalBuyQuantity");
	 ...
	 */
	public static void setBuyQuantityMap(BuyItem buyItem, HashMap<String, HashMap<String, Integer>> stockMap, HashMap<String, Integer> buyQuantityMap) {
		...
		if (buyItem.getItemPrice() != null && buyItem.getItemPrice().getQuantity() > 0) {
			...
			if ("N".equals(item.getItemOptionFlag())) {
				...
				maxQuantity = getMaxQuantity(maxQuantity, buyQuantity, stockInfo);

				if (buyQuantityMap != null) {
					if (buyQuantityMap.get(stockKey) != null) {
						int totalBuyQuantity = buyQuantityMap.get(stockKey);
				...
				if (ValidationUtils.isNotNull(itemOptions)) {
					for(ItemOption itemOption : itemOptions) {
						if ("T".equals(itemOption.getOptionType())) {
							...
							if (maxQuantity == 0 || "Y".equals(itemOption.getOptionSoldOutFlag())) {
	 ...
	 * @param buyItemSet
	 * @param stockSetMap
	 * @param buySetQuantityMap
	 */
	public static void setBuySetQuantityMap(BuyItem buyItem, BuyItem buyItemSet, HashMap<String,
			HashMap<String, Integer>> stockSetMap, HashMap<String, Integer> buySetQuantityMap) {
		OrderQuantity orderQuantity = new OrderQuantity();
		...
		if (buyItem.getItemPrice() != null && buyItem.getItemPrice().getQuantity() > 0) {
			...
			int buyQuantity = buyItem.getItemPrice().getQuantity() * buyItemSet.getItemPrice().getQuantity();
			int maxQuantity = !"0".equals(item.getSoldOut()) ? 0 : -1;	// 세트상품은 최대 구매수량 고려 X
			...
			// 품절?
			if (maxQuantity == 0) {
				buyItemSet.setOrderQuantity(orderQuantity);
				return;
			}

			if ("N".equals(item.getItemOptionFlag())) {
				String stockKey = getItemStockKey(item, item.getStockQuantity());
				HashMap<String, Integer> stockInfo = !"".equals(stockKey) ? stockSetMap.get(stockKey) : null;

				maxQuantity = getMaxQuantity(maxQuantity, buyQuantity, stockInfo);

				if (buySetQuantityMap != null) {
					if (buySetQuantityMap.get(stockKey) != null) {
						int totalBuyQuantity = buySetQuantityMap.get(stockKey);
						buySetQuantityMap.put(stockKey, totalBuyQuantity + buyQuantity);
					} else {
						buySetQuantityMap.put(stockKey, buyQuantity);
					}
				}
	...
	public static List<ItemOption> getRequiredItemOptions(Item item, String options) {
		...
		// 세트상품이거나, 옵션이 없는 상품일 경우 패스
		if ("3".equals(item.getItemType()) || "N".equals(item.getItemOptionFlag())) {
			return null;
		}
	...
	public static void buyVerification(List<BuyItem> list, int inventoryVerificationCount) {
		...
		for(BuyItem buyItem : list) {
			...
			// 일반상품 1, 세트상품 3 - 세트상품 추가로 타입검사 주석
						...
						if ("Y".equals(item.getStockFlag())) {
							...
							for (BuyItem checkBuyItem : list) {
								...
								if ("N".equals(checkItem.getItemOptionFlag())) {
									if (buyItem.getItemId() == checkBuyItem.getItemId() || checkBuyItem.getItemPrice().getQuantity() != 0) {
							...
							for (BuyItem checkBuyItem : list) {
								...
								if ("Y".equals(option.getOptionStockFlag()) && !ObjectUtils.isEmpty(checkOptionList)) {
									for (ItemOption checkOption : checkOptionList) {
										if (checkOption.getItemOptionId() == option.getItemOptionId() || checkBuyItem.getItemPrice().getQuantity() != 0) {
	 ...
	 * @return
	 */
	public static void buySetVerification(BuyItem buyItem) {
		List<BuyItem> list = buyItem.getItemSets();
		if (list.size() > 0) {
			int inventoryVerificationCount = list.size();
			...
			// 재고량을 검사
			int successCount = 0;
			for (BuyItem buyItemSet : list) {
				if(buyItemSet != null) {
					Item item = buyItemSet.getItem();
					OrderQuantity orderQuantity = buyItemSet.getOrderQuantity();

					// 판매 불가, 세트 구성이나 주문 오류사항으로 인한 에러
					if ("N".equals(buyItemSet.getAvailableForSaleFlag())) {
						throw new OrderException(buyItemSet.getSystemComment());
					}

					// 최대 구매가능 수량이 0이면 재고가 없거나 품절된 상품임
					if (orderQuantity.getMaxQuantity() == 0) {
						throw new OrderException(item.getItemName() + " " + MessageUtils.getMessage("M00483")); // 이쪽의 상품은 재고가 없습니다.
					}

					// 상품에 옵션이 있고 상품 옵션 구성이 null인경우 상품의 구성이 변경됨..
					if ("Y".equals(item.getItemOptionFlag())) {
						if (item.getItemOptions() != null && buyItemSet.getOptionList() == null) {
							throw new OrderException(item.getItemName() + "상품의 옵션 구성이 변경되었습니다.");
						}
					}
					...
					if ("N".equals(item.getItemOptionFlag())) {
						if ("Y".equals(item.getStockFlag())) {
							...
							for (BuyItem checkBuyItemSet : list) {
								Item checkItem = checkBuyItemSet.getItem();
								if ("N".equals(checkItem.getItemOptionFlag())) {
									if (buyItemSet.getItemId() == checkBuyItemSet.getItemId()) {
										sumBuyQuantity += buyItem.getItemPrice().getQuantity() * checkBuyItemSet.getItemPrice().getQuantity();
									}
								}
							}
						...
						// 옵션이 있는 상품 검사 - 옵션 ID가 같은 상품들의 재고량을 더하여 해당 상품의 주문 가능 상태를 채크
						for (ItemOption option : buyItemSet.getOptionList()) {
							if ("Y".equals(option.getOptionStockFlag())) {
								...
								for (BuyItem checkBuyItemSet : list) {
									...
									if ("Y".equals(checkItem.getItemOptionFlag())) {
										for (ItemOption checkOption : checkBuyItemSet.getOptionList()) {
											if (checkOption.getItemOptionId() == option.getItemOptionId() || buyItem != null) {
												sumBuyQuantity = sumBuyQuantity + (buyItem.getItemPrice().getQuantity() * checkBuyItemSet.getItemPrice().getQuantity());
											}
										}
									}
								}
	...
	public static int getCouponDiscountAmountForCartCoupon(OrderCoupon orderCouponUser, int totalPayAmount) {
		...
		if (!ObjectUtils.isEmpty(orderCouponUser.getCouponPayRestriction())) {
	 ...
	 * @param response
	 */
	public static void setCookieWhenServerLoad(HttpServletRequest request, HttpServletResponse response) {
		...
		if (r != null) {
			if(r.matches("[0-9]+"))
			{
				...
				if (sessionID != null) {
					...
					{
						Cookie c = new Cookie("sessionID", sessionID.replaceAll("\\n", "").replaceAll("\\r", ""));
	...
	public static boolean isOpmanagerPage() {
		if (RequestContextUtils.getRequestUri() == null) return false;
				...
				for(String s : temp) {
					...
					if (temp.length == index + 1) {
						...
						if (addPrice != 0) {
							t += " (";
							t += addPrice > 0 ? "+" : "-";
							t += NumberUtils.formatNumber(addPrice, "#,###") + "원)";
						}
					} else {
						if (index % 2 == 0) {
							t += "<span>" + s + "</span>";
	...
	public static List<Map<String, Object>> getOptions(String optionText) {
		...
		try {
			...
			log.error("ShopUtils getOptions error", ignore);
		}
	...
	public static String getBankName(String pgType, String key) {
		...
		try {
			...
			log.error("ShopUtils getBankName error", e);
		}
	...
	public static boolean checkOrderStatusChange(String mode, String orderStatus) {
		...
		if ("payment-verification-cancel".equals(mode)) {
		...
		} else if ("shpping-complete".equals(mode)) {
			 ...
			 * 55 : 교환배송시작
			 */
			String[] checkArray = new String[]{"30", "55"};
			if (ArrayUtils.contains(checkArray, orderStatus)) {
				return true;
			}
			 ...
			 * 69 : 반품거절
			 */
			String[] checkArray = new String[]{"30", "35", "55", "58", "59", "69"};
			if (ArrayUtils.contains(checkArray, orderStatus)) {
				return true;
			}
		}
	...
	public static String getSearchStartDate(String key) {
		...
		try {
			...
			if (ObjectUtils.isEmpty(key) == false) {
...
//		int imageSize = 0;
	...
	public static String loadImage(String itemUserCode, String fileName, String sizeName) {
		...
		imageSrc.append(getImageFilePath("item", itemUserCode, fileName, sizeName));
	...
	public static String loadImage(String imageSrc, String sizeName) {
		...
		//상품에 등록된 이미지가 없을경우
		if (!StringUtils.hasText(imageSrc) || !imageSrc.contains(".")) {
			return ShopUtils.getNoImagePath();
		}

		// 상품 이미지명에 http, https 가 포함된 경우
		if (imageSrc.indexOf("http") > -1 || imageSrc.indexOf("https") > -1) {
			return imageSrc;
		}
		...
		if ("1".equals(shopConfig.getThumbnailType())) {	// 관리자 설정대로 리사이즈 (사이즈별 썸네일 저장)
			targetImageSrc.append(imageSrc, 0, imageSrc.lastIndexOf("/"));	// ex: https://saleson.onlinepowers

			if (imageSrc.contains("_")) {
				targetImageSrc.append(imageSrc, imageSrc.lastIndexOf("/"), imageSrc.lastIndexOf("_") + 1);	// ex: /picture123_
				targetImageSrc.append(sizeName);
			} else {
				targetImageSrc.append(imageSrc, imageSrc.lastIndexOf("/"), imageSrc.lastIndexOf("."));	// ex: /picture123
			}

			targetImageSrc.append(imageSrc, imageSrc.lastIndexOf("."), imageSrc.length());	// ex: .png
		} else {	// 호출시 썸네일 리사이즈
			targetImageSrc.append("/thumbnail?src=");
			targetImageSrc.append(imageSrc).append("&size=").append(getImageSize(sizeName));
		}

		return targetImageSrc.toString();
	}
	 ...
	 * @return
	 */
	public static String loadSellerImage(long sellerId, String sellerImage) {
		if (!org.springframework.util.StringUtils.hasText(sellerImage)) {
			return ShopUtils.getNoImagePath();
		}
		...
		sb.append("/seller/").append(sellerId).append("/").append(sellerImage);
	...
	public static String getString(Cell cell) {
		...
		return ShopUtils.stripTags(formatter.formatCellValue(cell));
	}
	...
	public static int getInt(Cell cell) {
		...
		if (!StringUtils.hasText(cellValue.trim())) {
			return 0;
		}
	...
	public static String makeGiftItemText(List<GiftItem> giftItems) {
		...
		try {
			...
			log.error("ShopUtils makeGiftItemText error", e);
		}
	...
	public static String makeOrderGiftItemText(List<OrderGiftItem> giftItems)  {
		...
		try {
			...
			log.error("ShopUtils makeOrderGiftItemText error", e);
		}
	...
	/**
	 * 사용자 화면에 보여줄 세트상품 또는 옵션을 구성함
	 * @param setItemFlag
	 * @param optionText
	 * @return
	 */
	public static String viewItemOptions(String setItemFlag, String optionText) {
		if ("Y".equals(setItemFlag)) {
			StringBuffer sb = new StringBuffer();

			try {
				List<Map<String, String>> itemSets = (List<Map<String, String>>) JsonViewUtils.jsonToObject(optionText, new TypeReference<List<Map<String, String>>>() {});
				for (Map<String, String> itemSet : itemSets) {
					sb.append("- ");
					sb.append(itemSet.get("itemName"));

					if (!ObjectUtils.isEmpty(itemSet.get("options"))) {
						sb.append(" &lt;");
						sb.append(viewOptionTextNoUl(itemSet.get("options")));
						sb.append("&gt;");
					}

					sb.append(" (");
					sb.append(itemSet.get("quantity"));
					sb.append("개)");
			...
			return viewOptionTextNoUl(optionText);
		}
	}
	...
	public static String appendUri(String uri, String appendQuery) {
		...
		try {
			...
			//e.printStackTrace();
			log.error("ShopUtils appendUri error", ignore);
		}

		return uri;
	}

	public static boolean needMasking() {

		if (SecurityUtils.hasRole("ROLE_ISMS")) {
			return false;
		}
		...
		if (SecurityUtils.hasRole("ROLE_ADMIN_6")) {
			return false;
		}
	...
	public static boolean needMasking(long userId) {

		if (UserUtils.isUserLogin() && UserUtils.getUserId() == userId) {
			return false;
		}

		return ShopUtils.needMasking();
	}

	/**
	 * HttpRequest에서 인코딩 된 param을 decode한다.
	 * @param request
	 * @return
	 */

	public static String getStringParamDecodeUtf8(HttpServletRequest request, String targetParamKey) {
		String result = "";
		Enumeration<String> enumeration =  request.getParameterNames();
		while (enumeration.hasMoreElements()) {
			String key = (String)enumeration.nextElement();
			String value = request.getParameter(key);
			if(targetParamKey.equals(key)) {
				try {
	...
	public static String removeNewlineCharaters(String input) {
		if(input == null) {
			return "";
		}
	...
	/**
	 * 이미지로드용 코드 재사용(loadImage)
	 * @author
	 * @param imageSrc
	 * @param sizeName
	 * @return
	 */
	public static String loadImagePreview(String itemUserCode, String fileName, String sizeName) {
	 ...
	 * @return
	 */
	public static String getViewItemTextOption(String textOption) {
		StringBuffer sb = new StringBuffer();

		if (ObjectUtils.isEmpty(textOption)) {
			return "";
		}

		try {
			String[] options = StringUtils.delimitedListToStringArray(textOption, "||");
			...
			log.error("ShopUtils getViewItemTextOption error", ignore);
			return "";
		}
