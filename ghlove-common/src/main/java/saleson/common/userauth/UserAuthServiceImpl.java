...
public class UserAuthServiceImpl implements UserAuthService {
	...
	// 사용자 생년월일 호출
	@Override
	public String getBirth(Long userId) {
		// 엑셀 다운로드 시, 엑셀 암호화를 위한 비밀번호 조회를 위한 생년월일 조회
		String encryptedBirth = userAuthMapper.getBirth(userId);

		// 생년월일이 없는 경우, null을 return
		if (encryptedBirth == null) return null;

		// 암호화되어있는 생년월일 복호화 및 6자리로 변경
		String parsedBirth;
		try {
			String decryptedBirth = pCrypto.Decrypt("normal", encryptedBirth, "", 0);
			parsedBirth = decryptedBirth.substring(2);
