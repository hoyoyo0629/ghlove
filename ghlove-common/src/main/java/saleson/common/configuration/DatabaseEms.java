...
//		basePackages = {"saleson.shop.email.ems"}
//)
public class DatabaseEms extends DatabaseConfig {

	private final String DATABASE_PREFIX = "email";
	private final String[] BASE_PACKAGES = new String[] {"saleson.shop.email.ems"};



	@Bean(name = DATABASE_PREFIX + "DataSource")
	@ConfigurationProperties(prefix = "email.datasource")
	public DataSource dataSource() {
		String profile = System.getProperty("spring.profiles.active");
		if (!ObjectUtils.isEmpty(profile) && !"local".equals(profile)) {
			String jndiName = getYamlProperty("email.datasource.jndi-name");
			String flag = getYamlProperty("email.datasource.jndi-flag");
	...
	public MapperScanConfigurer mapperConfigurer() {
		MapperScanConfigurer mapper = new MapperScanConfigurer(MapperEms.class, DATABASE_PREFIX + "SqlSessionFactory");
