...
@Getter @Setter
@Configuration
public class DatabaseAnalysis extends DatabaseConfig {

	private final String DATABASE_PREFIX = "analysis";
	private final String[] BASE_PACKAGES = new String[] {"saleson.shop.analysis"};



	@Bean(name = DATABASE_PREFIX + "DataSource")
	@ConfigurationProperties(prefix = "analysis.datasource")
	public DataSource dataSource() {
		String profile = System.getProperty("spring.profiles.active");
		if (!ObjectUtils.isEmpty(profile) && !"local".equals(profile)) {
			String jndiName = getYamlProperty("analysis.datasource.jndi-name");
			String flag = getYamlProperty("analysis.datasource.jndi-flag");
	...
	public MapperScanConfigurer mapperConfigurer() {
		MapperScanConfigurer mapper = new MapperScanConfigurer(MapperAnalisys.class, DATABASE_PREFIX + "SqlSessionFactory");
