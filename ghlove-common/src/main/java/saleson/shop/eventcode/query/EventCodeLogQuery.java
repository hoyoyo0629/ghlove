...
public class EventCodeLogQuery {
    ...
    public static final String GET_BASE_EVENT_STATISTICS = "SELECT\n" +
            ...
            "    SELECT\n" +
            "        L.EVENT_CODE,\n" +
            "        COUNT(L.EVENT_UID) AS VISIT_COUNT,\n" +
            "        0 AS JOIN_COUNT,\n" +
            "        0 AS ORDER_COUNT\n" +
            "    FROM (\n" +
            "        SELECT\n" +
            "            DISTINCT EVENT_UID, EVENT_CODE\n" +
    ...
    public static final String GET_BASE_EVENT_ITEM_STATISTICS = "SELECT\n" +
            ...
            "           IF (SUM(BASE.LIST_VIEW_COUNT) = 0,0,ROUND(COALESCE((SUM(BASE.ITEM_VIEW_COUNT) / SUM(BASE.LIST_VIEW_COUNT)) * 100, 0), 2)) AS CLICKED_RATE,\n" +
            "           SUM(BASE.ORDER_COUNT)                                                                AS ORDER_COUNT,\n" +
            "           IF (SUM(BASE.ITEM_VIEW_COUNT) = 0,0,ROUND(COALESCE((SUM(BASE.ORDER_COUNT) / SUM(BASE.ITEM_VIEW_COUNT)) * 100, 0), 2))     AS ORDER_RATE\n" +
            "    FROM (\n" +
