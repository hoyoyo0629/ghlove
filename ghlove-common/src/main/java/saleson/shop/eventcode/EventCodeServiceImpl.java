...
public class EventCodeServiceImpl extends EgovAbstractServiceImpl implements EventCodeService{
    ...
    @Override
    public EventCode getEventCode(String code) throws IllegalArgumentException {
    ...
    public String saveEventCode(Featured featured, String utmScource, String utmMedium) {
        ...
        try {
            ...
            if (featured != null) {
                ...
                // 2. 외부 링크가 있을경우 변경
                if (!ObjectUtils.isEmpty(featured.getLink())) {
...
//          log.error("saveEventCode Featured {}", ignore.getMessage(), ignore );
          log.error("saveEventCode Featured {}", getClass().getName() + " :: saveEventCode UnsupportedEncodingException ============");
        }
...
//            log.error("getCampaignRedirect error {}", code, ignore);
            log.error("getCampaignRedirect error {}", code);
        }
        ...
        try {
            ...
            if (eventCode != null) {
                ...
                if (!ObjectUtils.isEmpty(url)) {
                    ...
                    if (!ObjectUtils.isEmpty(utmQueryString)) {
...
//            log.error("event code make redirect error", ignore);
            log.error("event code make redirect error", getClass().getName() + " :: getRedirect RuntimeException =========");
        }
    ...
    public String getItemRedirect(String channel, String itemUserCode, long sourceUserId) {
        ...
        } catch (UnsupportedEncodingException igroe) {
//            log.error("getItemRedirect error [{}][{}]", channel,itemUserCode, igroe);
            log.error("getItemRedirect error [{}][{}]", channel,itemUserCode);
        }
    ...
    @Override
    public String getUtmQueryString(String utmSource, String utmMedium, String utmCampaign, String utmItem, String utmContent) throws UnsupportedEncodingException {

        StringBuffer sb = new StringBuffer();
        String enc = "UTF-8";

        if (!ObjectUtils.isEmpty(utmSource)) {
            sb.append("&utm_source="+ URLEncoder.encode(utmSource, enc));
        }

        if (!ObjectUtils.isEmpty(utmMedium)) {
            sb.append("&utm_medium="+URLEncoder.encode(utmMedium, enc));
        }

        if (!ObjectUtils.isEmpty(utmCampaign)) {
            sb.append("&utm_campaign="+URLEncoder.encode(utmCampaign, enc));
        }

        if (!ObjectUtils.isEmpty(utmItem)) {
            sb.append("&utm_item="+URLEncoder.encode(utmItem, enc));
        }

        if (!ObjectUtils.isEmpty(utmContent)) {
    ...
    public void insertLog(EventCodeLog eventCodeLog) {
        ...
        try {
            ...
            if (eventCodeLog != null && !ObjectUtils.isEmpty(eventCodeLog.getEventCode())) {

                uid = eventCodeLog.getEventUid();
                ...
                dto.setUid(eventCodeLog.getEventUid());
...
//            log.error("insertLog Error {} / {}", uid, userId, ignore);
            log.error("insertLog Error {} / {}", uid, userId);
        }
                ...
                if (!ObjectUtils.isEmpty(uid)) {
                    ...
                    if (logs != null && !logs.isEmpty()) {
                        ...
                        for (EventCodeLog log : logs) {
                            ...
                            if (!ObjectUtils.isEmpty(log.getItemUserCode())) {
...
//            log.error("insertLog Error {} / {}", uid, userId, ignore);
    ...
    public void updateLogForUserId(String uid, long userId) {
        ...
        try {
            ...
            if (!ObjectUtils.isEmpty(uid) && userId > 0) {
...
//            log.error("updateLogForUserId Error {} / {}", uid, userId, ignore);
            log.error("updateLogForUserId Error {} / {}", uid, userId);
        }
