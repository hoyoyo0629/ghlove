...
public class PointServiceImpl extends EgovAbstractServiceImpl implements PointService {
	...
	public void expirationPointSendMessage() {
		...
		for(Integer userId : userIds) {
			...
			try {
				...
				if ("0".equals(userDetail.getReceiveEmail()) && ObjectUtils.isEmpty(user.getEmail()) == false) {
					...
					if (!pointMap.isEmpty()) {
						...
						MemberExpirationPointMail mail = new MemberExpirationPointMail(mailConfigService.getMailConfigByTemplateId(templateId), user, DateUtils.getToday("yyyy년MM월dd일"), DateUtils.getToday("yyyy년MM월dd일"), pointMap, shopConfig, cryptor, dataMasking);
	...
	private void earnPoint(String mode, Point point, Config shopConfig) {
		...
		if (point.getPoint() != 0) {
			...
			// 포인트 만료일 설정
			if (ObjectUtils.isEmpty(point.getExpirationDate()) || point.getExpirationDate().length() != 8) {
	...
	private void processReturnPoint(Point point) {
		...
		for (PointUsed pu : pointUsedList) {
			...
			Point originPoint = pointMapper.getPointByPointId(pu.getPointId());

			point.setSavedPoint(insertPoint);
			point.setPoint(insertPoint);
			point.setExpirationDate(originPoint.getExpirationDate());
			point.setPointManagerId(originPoint.getPointManagerId());
			point.setManagerUserId(originPoint.getManagerUserId());
			point.setGroupCode(originPoint.getGroupCode());
			pointMapper.insertPoint(point);

			returnPoint = returnPoint - insertPoint;

			// 만료일이 오늘날짜 이전이면 자동 소멸 처리
			if (DateUtils.getDaysDiff(originPoint.getExpirationDate()) > 0) {
				...
				pointUsed.setPointId(point.getPointId());
				pointUsed.setPoint(point.getPoint());
				pointUsed.setDetails("사용기한 만료로 자동 소멸");
				pointUsed.setCreatedDate(DateUtils.getToday(Const.DATETIME_FORMAT));
				pointUsed.setRemainingPoint(point.getPoint());
...
//		    		throw new Exception("Column is not exist. (会員番号)");
		    		throw new RuntimeException("ID 컬럼이 없습니다.");
		    	}
...
//		    		log.warn(e.getMessage(), e);
		    		log.warn(getClass().getName() + " :: insertPointByExcel RuntimeException1 =============", e);
		    		continue;
		    	}
			} // row
...
//			throw new UserException(MessageUtils.getMessage("M01534") + "(" + e.getMessage() + ")"); // 엑셀 파일 로드 시 오류가 발생하였습니다.
			log.error("ERROR: {}", getClass().getName() + " :: insertPointByExcel IOException =============", e);
...
//			log.error("ERROR: {}", e.getMessage(), e);
//			throw new UserException(MessageUtils.getMessage("M01534") + "(" + e.getMessage() + ")"); // 엑셀 파일 로드 시 오류가 발생하였습니다.
			log.error("ERROR: {}", getClass().getName() + " :: insertPointByExcel RuntimeException2 =============", e);
//			throw new UserException(MessageUtils.getMessage("M01534")); // 엑셀 파일 로드 시 오류가 발생하였습니다.
			throw new UserException("엑셀 파일 로드 시 오류가 발생하였습니다." + "(" + e.getErrorMessage() + ")"); // 엑셀 파일 로드 시 오류가 발생하였습니다.
		}
	}
...
//				log.error("ERROR: {}", e.getMessage(), e);
				log.error("ERROR: {}", getClass().getName() + " :: userLevelShippingCoupon RuntimeException =============", e);
	...
	public void insertPointPay(Point point) {
		try{
			...
			log.error("ERROR: {}","insertPointPay OpRuntimeException", e);
			//throw new UserException(MessageUtils.getMessage("실패했습니다."));
			throw new UserException("실패 했습니다.");
		}
	}
