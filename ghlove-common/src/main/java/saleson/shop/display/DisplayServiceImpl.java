...
public class DisplayServiceImpl extends EgovAbstractServiceImpl implements DisplayService {
	...
	public List<DisplayItem> getDisplayItemListByParam(DisplayItemParam param){
		...
		List<DisplayItem> items = displayMapper.getDisplayItemListByParam(param);

		if (UserUtils.isUserLogin()) {
			wishlistService.setWishlistFlagByDisplayItem(items);
		}
		...
		for (int index=0 ; index<settingValues.size(); index++) {
			...
			try{
				...
				if (settingValue.getType() == 1) {
					...
					// 에디터
					List<String> displayEditorContents = sendData.getDisplayEditorContents();
					if (ValidationUtils.isNotNull(displayEditorContents) && !displayEditorContents.isEmpty()) {
						updateDisplayEditorByGroupCode(groupCode, String.valueOf(index), viewTarget, displayEditorContents);
					}
...
//				log.warn("[Exception] updateDisplayDataByGroupCode : {}", e.getMessage(), e);
				log.warn("[Exception] updateDisplayDataByGroupCode : {}", getClass().getName() + " :: updateDisplayDataByGroupCode RuntimeException ===========", e);
...
//				throw new Exception(e.getMessage());
	...
	private void updateDisplayImageByGroupCode(String groupCode, String subCode, String viewTarget,
											   DisplaySendData sendData, int dataIndex) {
		...
		if (!ObjectUtils.isEmpty(displayImages)
				&& !ObjectUtils.isEmpty(displayUrls) && !ObjectUtils.isEmpty(displayContents) ) {
			...
			for (MultipartFile file : displayImages) {
				...
				if (ObjectUtils.isEmpty(fileName)) {
					...
					if (!ObjectUtils.isEmpty(tempFileName)) {
				...
				if (!ObjectUtils.isEmpty(displayUrl) || !ObjectUtils.isEmpty(displayImageName) || !ObjectUtils.isEmpty(displayContent) || !ObjectUtils.isEmpty(displayColor)) {
	...
	private void updateDisplayItemByGroupCode(String groupCode, String subCode, String viewTarget,
		...
		if (ValidationUtils.isNotNull(displayItemIds) && !displayItemIds.isEmpty()) {
			...
			for (String temp : displayItemIds) {
				if (!ObjectUtils.isEmpty(temp) && !"undefined".equals(temp)) {
					ids.add(temp);
				}
			}
	...
	private void updateDisplayEditorByGroupCode(String groupCode, String subCode, String viewTarget,
		...
		if (displayEditorContents != null && !displayEditorContents.isEmpty()) {
			AtomicInteger ordering = new AtomicInteger();
			displayEditorContents.forEach(s -> {
				ordering.getAndIncrement();

				DisplayEditor displayEditor = new DisplayEditor();
				displayEditor.setOrdering(ordering.get());
				displayEditor.setDisplayGroupCode(groupCode);
				displayEditor.setDisplaySubCode(subCode);
				displayEditor.setDisplayEditorContent(s);
	...
	private String getUploadPath(String groupCode, String subCode, String type) {
	...
	private String imageSave(String groupCode, String subCode, MultipartFile multipartFile){
		...
		if(!multipartFile.isEmpty()){
			...
			// 1. 업로드 경로설정
			String uploadPath = getUploadPath(groupCode, subCode,"default");
			...
			try {
				...
				String uploadPathThumb = getUploadPath(groupCode, subCode,"thumb");
				fileService.makeUploadPath(uploadPathThumb);
				
				File thumbnailFile = new File(uploadPathThumb + File.separator + newFileName);
				fileStorage.upload(multipartFile.getBytes(), thumbnailFile);

	        } catch (IOException e) {
//				log.error("ERROR: {}", e.getMessage(), e);
				log.error("ERROR: {}", getClass().getName() + " :: imageSave IOException ===========");
	            throw new RuntimeException(e);
	        }
	...
	private  void deleteImage (String groupCode, String subCode, String fileName) {
		try {
			fileStorage.delete(getUploadPath(groupCode, subCode,"default"), ShopUtils.unescapeHtml(fileName));
			fileStorage.delete(getUploadPath(groupCode, subCode,"thumb"), ShopUtils.unescapeHtml(fileName));

		} catch (RuntimeException e) {
//			log.error("fileStorage.delete(..  : {}", e.getMessage(), e);
			log.error("fileStorage.delete(..  : {}", getClass().getName() + " :: deleteImage RuntimeException ===========");
		}
	}
	...
	public void deleteDisplayImageFile(DisplayImage displayImage) {
		...
		if (ObjectUtils.isEmpty(tempDisplayImage.getDisplayUrl()) && ObjectUtils.isEmpty(tempDisplayImage.getDisplayContent()) && ObjectUtils.isEmpty(tempDisplayImage.getDisplayColor())) {
			displayMapper.deleteDisplayImageByParam(param);
		}
