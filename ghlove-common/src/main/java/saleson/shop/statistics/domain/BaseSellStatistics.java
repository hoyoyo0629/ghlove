...
public class BaseSellStatistics {
	...
	// 권세희 [2021-07-26 추가] 세트 할인 추가
	private double setDiscountPrice;
	private double cancelSetDiscountPrice;
	...
	public double getTotalDiscountAmount() {
		return ShopUtils.statsNegativeNnumber(new BigDecimal(""+this.itemCouponDiscountAmount).add(new BigDecimal(""+this.sellerDiscountPrice)).add(new BigDecimal(""+this.spotDiscountPrice)).add(new BigDecimal(""+this.userLevelDiscountPrice)).add(new BigDecimal(""+this.setDiscountPrice)).doubleValue());
	}
	//취소 할인 총 합
	public double getCancelTotalDiscountAmount() {
		return new BigDecimal(""+this.cancelItemCouponDiscountAmount).add(new BigDecimal(""+this.cancelSellerDiscountPrice)).add(new BigDecimal(""+this.cancelSpotDiscountPrice)).add(new BigDecimal(""+this.cancelUserLevelDiscountPrice)).add(new BigDecimal(""+this.cancelSetDiscountPrice)).doubleValue();
	}
	...
	public double getSetDiscountPrice() {
		return setDiscountPrice;
	}

	public void setSetDiscountPrice(double setDiscountPrice) {
		this.setDiscountPrice = setDiscountPrice;
	}

	public double getCancelSetDiscountPrice() {
		return cancelSetDiscountPrice;
	}

	public void setCancelSetDiscountPrice(double cancelSetDiscountPrice) {
		this.cancelSetDiscountPrice = cancelSetDiscountPrice;
	}
}
