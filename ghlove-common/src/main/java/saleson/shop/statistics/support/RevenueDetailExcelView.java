		...
		for(RevenueDetail detail : base.getList()) {
			if (orderType.equals(detail.getOrderType())) {
				...
				for(RevenueDetailItem item : detail.getItems()) {
					...
					String optionString = "";
					if (!ObjectUtils.isEmpty(item.getOpenMarketOption())) {
						optionString = ShopUtils.viewItemOptions(item.getSetItemFlag(), item.getOpenMarketOption()).replaceAll("<br/>", "\n");
					} else {
					...
					setTextRight(sheet,			rowIndex, itemCellIndex, StringUtils.numberFormat(String.valueOf(item.getQuantity())));
					setTextRight(sheet,			rowIndex, itemCellIndex, StringUtils.numberFormat(String.valueOf(item.getItemPrice() + detail.getSumDeliveryPrice())));
					setTextRight(sheet,			rowIndex, itemCellIndex, StringUtils.numberFormat(String.valueOf(item.getItemCouponDiscountAmount())));
					setTextRight(sheet,			rowIndex, itemCellIndex, StringUtils.numberFormat(String.valueOf(item.getVendorAddDiscountAmount())));
					setTextRight(sheet,			rowIndex, itemCellIndex, StringUtils.numberFormat(String.valueOf(item.getTotalItemPrice())));

					if (sellerIdForParam == null) {
						setTextRight(sheet,		rowIndex, itemCellIndex, StringUtils.numberFormat(String.valueOf(item.getOrderShipping()))); //배송비
					}

					setTextRight(sheet,			rowIndex, itemCellIndex, StringUtils.numberFormat(String.valueOf(item.getSubTotal()))); //소계
