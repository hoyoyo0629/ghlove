...
//			throw new UserException(MessageUtils.getMessage("M01534") + "(" + e.getMessage() + ")"); // 엑셀 파일 로드 시 오류가 발생하였습니다.
			log.error("insertExcelData : {}", getClass().getName() + " :: insertExcelData IOException ==========", e);
...
//			log.error("insertExcelData : {}", e.getMessage(), e);
//			throw new UserException(MessageUtils.getMessage("M01534") + "(" + e.getMessage() + ")"); // 엑셀 파일 로드 시 오류가 발생하였습니다. 
			log.error("insertExcelData : {}", getClass().getName() + " :: insertExcelData Exception ==========", e);
			throw new UserException("엑셀 파일 로드 시 오류가 발생하였습니다."); // 엑셀 파일 로드 시 오류가 발생하였습니다.
		}
			...
			if (!insertCustomer.isEmpty()) {
				...
				for(Customer data : insertCustomer) {
					...
					try {
						...
						if (!ObjectUtils.isEmpty(param.getEmail()) || !ObjectUtils.isEmpty(param.getTelNumber())) {
...
//						log.error("ERROR: {}", e.getMessage(), e);
						log.error("ERROR: {}", getClass().getName() + " :: processExcelSheet RuntimeException ============", e);
						insertErrorCount++;
					}
...
//				log.warn("CustomerServiceImpl isUpdate() : {}", e.getMessage(), e);
				log.warn("CustomerServiceImpl isUpdate() : {}", getClass().getName() + " :: isUpdate Exception ===========", e);
			}
	...
	public void newCustomerForOrder(OrderShippingInfo order, String customerType) {
		...
		if (ObjectUtils.isEmpty(customerCode)) {
