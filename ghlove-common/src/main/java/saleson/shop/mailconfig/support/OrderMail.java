...
public class OrderMail extends MailTemplate {
	 ...
	 * @param config
	 */
	public OrderMail(Order order, long userId, MailConfig mailConfig, Config config, Cryptor cryptor, DataMasking dataMasking) {
		...
		this.orderName = getCryptor().decrypt(order.getUserName());

		this.mobile = getCryptor().decrypt(order.getMobile());
		this.email = getCryptor().decrypt(order.getEmail());
	...
	public OrderMail(Buy buy, long userId, MailConfig mailConfig, Config config, Cryptor cryptor, DataMasking dataMasking) {
		super(mailConfig, cryptor, dataMasking);
		if (buy== null) {
			return;
		}
		...
		this.orderName = getCryptor().decrypt(buyer.getUserName());
...
//		this.siteUrl = SalesonProperty.getSalesonUrlShoppingmall();
		...
		for (BuyPayment orderPayment : buy.getPayments()) {
			...
			/*else if ("point".equals(orderPayment.getApprovalType())) {
				...
				this.bankInName = getCryptor().decrypt(orderPayment.getBankInName());
				this.bankDate = DateUtils.formatDate(orderPayment.getBankExpirationDate(), "-");
				this.bankAmount = StringUtils.numberFormat(orderPayment.getAmount());
				this.bankVirtualNo = getCryptor().decrypt(orderPayment.getBankVirtualNo());
				this.orderTotalPrice = StringUtils.numberFormat(orderPayment.getAmount());
			}
		}
		...
		this.mobile = getCryptor().decrypt(buyer.getMobile());
		this.email = getCryptor().decrypt(buyer.getEmail());
	...
	private String getOrderDetailListByReceiver(List<Receiver> receivers) {
		...
		for (Receiver receiver : receivers) {
			...
			if (ObjectUtils.isEmpty(receiver.getReceiveNewZipcode())) {
				address = "(" + receiver.getReceiveZipcode() + ") ";
			}
			
			address += receiver.getReceiveAddress() + " " + getDataMasking().mask(receiver.getReceiveAddressDetail(), Masking.ADDRESS_DETAIL);
			...
			receiverInfo += "				<p style='margin:0; font-weight:bold;'>" + getDataMasking().mask(receiver.getReceiveName(), Masking.NAME) + "</p>";
			receiverInfo += "				<p style='margin:0;'>" + address + "</p>";



			if ( !receiver.getReceivePhone().isEmpty() && receiver.getReceivePhone().length() > 6 ) {
				receiverInfo += "				<p style='margin:0;'>" + getDataMasking().mask(receiver.getReceivePhone(), Masking.PHONE_NUMBER) + " / " + getDecryptAndDataMasking(receiver.getReceiveMobile(), Masking.PHONE_NUMBER) + "</p>";
			} else {
				receiverInfo += "				<p style='margin:0;'>" + getDataMasking().mask(receiver.getReceiveMobile(), Masking.PHONE_NUMBER) + "</p>";
			}
			receiverInfo += "			</td>";
			receiverInfo += "		</tr>";

			if (!ObjectUtils.isEmpty(receiver.getContent())) {
				receiverInfo += "		<tr>";
	...
	private String getOrderDetailList(List<OrderShippingInfo> shippingInfoList, MailConfig mailConfig) {
		...
		for (OrderShippingInfo shippingInfo : shippingInfoList) {
			...
			if (ObjectUtils.isEmpty(shippingInfo.getReceiveNewZipcode())) {
				address = "(" + shippingInfo.getReceiveZipcode() + ") ";
			}

			address += shippingInfo.getReceiveAddress() + " " + getDecryptAndDataMasking(shippingInfo.getReceiveAddressDetail() ,Masking.ADDRESS_DETAIL);
			...
			for (OrderItem item : shippingInfo.getOrderItems()) {
				...
				if (!ObjectUtils.isEmpty(item.getOptions())) {
					subText += "<p style='margin:0 0 8px; font-size:12px; color:#333'>" + ShopUtils.viewItemOptions(item.getSetItemFlag(), item.getOptions()) + "</p>";
				}
				...
				if (subText != null && !subText.isEmpty()) {
					deliverInfo += "				<p style='margin:0; font-family:'Dotum''>" + subText + "</p>";
				}

				if (!ObjectUtils.isEmpty(giftItemText)) {
				...
				//주문금액
				itemsPay += (item.getPrice() + item.getOptionPrice()) * item.getQuantity();
			}
			...
			deliverInfo += "				<p style='margin:0; font-weight:bold;'>" + getDecryptAndDataMasking(shippingInfo.getReceiveName(), Masking.NAME) + "</p>";
			deliverInfo += "				<p style='margin:0;'>" + address + "</p>";

			if ( !shippingInfo.getReceivePhone().isEmpty() && shippingInfo.getReceivePhone().length() > 6 ) {
				deliverInfo += "				<p style='margin:0;'>" + getDecryptAndDataMasking(shippingInfo.getReceivePhone(), Masking.PHONE_NUMBER) + " / " + getDecryptAndDataMasking(shippingInfo.getReceiveMobile(), Masking.PHONE_NUMBER) + "</p>";
			} else {
				deliverInfo += "				<p style='margin:0;'>" + getDecryptAndDataMasking(shippingInfo.getReceiveMobile(), Masking.PHONE_NUMBER) + "</p>";
			}

			deliverInfo += "			</td>";
			deliverInfo += "		</tr>";

			if (!ObjectUtils.isEmpty(shippingInfo.getMemo())) {
				deliverInfo += "		<tr>";
...
//			this.usePoint = StringUtils.numberFormat(discountPay);
			...
			if (ObjectUtils.isEmpty(orderPayment.getPayDate())) {
				payment = "입금예정";
	...
	private String OrderItemAndReceiverInfoTable(BuyItem item, String receiverInfo, int shippingAmount) {
		...
		if (!ObjectUtils.isEmpty(item.getFreeGiftItemText())) {
		...
		if (!ObjectUtils.isEmpty(subText)) {
			receiverInfo += "				<p style='margin:0; font-family:'Dotum'>" + subText  +"</p>";
		}
		...
		if (item.getItemPrice() == null) {
			throw new OrderException("가격 정보가 없습니다.");
		}
		...
		receiverInfo +=
				new StringBuilder("			<td style='padding:42px 0; border-right:1px solid #e1e1e1; border-bottom:1px solid #d9d9d9; font-size:13px; text-align:center'>")
						.append(StringUtils.numberFormat(item.getItemPrice().getPrice() + item.getItemPrice().getOptionPrice()))
						.append("원")
		...
		receiverInfo += new StringBuilder("			<td style='padding:42px 0; border-right:1px solid #e1e1e1; border-bottom:1px solid #d9d9d9; font-size:13px; font-weight:bold; color:#000; text-align:center'>")
				.append(StringUtils.numberFormat(item.getItemPrice().getSaleAmount()+shippingAmount))
		...
		receiverInfo += new StringBuilder("			<td style='padding:42px 0; border-bottom:1px solid #d9d9d9; font-size:13px; text-align:center'>")
				.append(shippingAmount == 0 ? "무료" : StringUtils.numberFormat(shippingAmount) + "원")
				.append("</td>").toString();
