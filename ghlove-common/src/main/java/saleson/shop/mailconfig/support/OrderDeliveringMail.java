...
public class OrderDeliveringMail extends MailTemplate {
	...
	public OrderDeliveringMail(Order order, long userId, MailConfig mailConfig, Config config, Cryptor cryptor) {
		super(mailConfig, cryptor);
		this.init(order, userId, config, cryptor);
	}
	
	private void init(Order order, long userId, Config config, Cryptor cryptor) {
		if (order== null) {
			return;
		}
