...
public class QnaCompleteMail extends MailTemplate {
	...
	public QnaCompleteMail(Qna qna, MailConfig mailConfig, Cryptor cryptor, DataMasking dataMasking) {

		super(mailConfig, cryptor, dataMasking);

		HashMap<String, String> map = new HashMap<>();
		map.put("userName", getCryptor().decrypt(qna.getUserName()));
		map.put("email", getCryptor().decrypt(qna.getEmail()));
