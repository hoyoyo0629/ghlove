...
import saleson.shop.mailconfig.support.*;

import com.onlinepowers.framework.sequence.service.SequenceService;


@Service("mailConfigService")
public class MailConfigServiceImpl extends EgovAbstractServiceImpl implements MailConfigService {
	...
	public HashMap<String, String> getMailChangeCodes(String templateId) {
		...
		} else if ("qna_complete".equals(templateId)) {
			codeMap = new QnaCompleteMail().getCodes();
		} else if ("manager_request_approval".equals(templateId)) {
			codeMap = new ManagerRequestApprovalMail().getCodes();
		} else if("manager_request_reject".equals(templateId)) {
			codeMap = new ManagerRequestRejectMail().getCodes();
		}
