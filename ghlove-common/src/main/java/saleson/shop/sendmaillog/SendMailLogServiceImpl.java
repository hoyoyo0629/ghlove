...
import saleson.shop.sendmaillog.domain.SendMailLogEncryptor;
...
public class SendMailLogServiceImpl extends EgovAbstractServiceImpl implements SendMailLogService{
	...
	@Autowired
	private SendMailLogEncryptor sendMailLogEncryptor;
	...
	public List<SendMailLog> getSendMailLogList(SendMailLogParam sendMailLogParam) {
		...
		list.forEach(s -> {
			// 복호화 & 마스킹
			s.decrypt(sendMailLogEncryptor, ShopUtils.needMasking());
		});
	...
	public void sendMail(MailConfig mailConfig, SendMailLog sendMailLog, String email, String name, Config config) {
		...
		String adminEmail = mailConfig.getAdminEmail();
		...
		if ("Y".equals(mailConfig.getBuyerSendFlag())) {
			...
			String buyerSubjectMasking = isHpMail ? mailConfig.getMobileBuyerSubject() : mailConfig.getBuyerSubjectMasking();
			String buyerContentMasking = isHpMail ? mailConfig.getMobileBuyerContent() : mailConfig.getBuyerContentMasking();
			...
			try {
				...
				ShopMailMessage mail = new ShopMailMessage(cryptor.decrypt(email), buyerSubject, buyerContent, adminEmail);
				mail.setHtml("Y".equals(mailConfig.getBuyerTagUse()) ? true : false);
				if (mail.isHTML() == false) {
					buyerContent = buyerContent.replaceAll("<!--(?:.|\\s)*?-->", "");
					mail = new ShopMailMessage(cryptor.decrypt(email), buyerSubject, buyerContent, adminEmail);
				}
				...
				sendMailLog.setContent(buyerContentMasking);
				sendMailLog.setSubject(buyerSubjectMasking);
