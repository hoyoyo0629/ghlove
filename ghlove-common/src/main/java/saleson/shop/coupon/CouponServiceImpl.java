...
public class CouponServiceImpl extends EgovAbstractServiceImpl implements CouponService {
	...
	@Autowired
	private CouponUserEncryptor couponUserEncryptor;

	@Autowired
	private CouponEncryptor couponEncryptor;
	...
	public int getCouponAppliesItemCountParamForCouponUser(CouponParam couponParam) {
		if (!UserUtils.isManagerLogin()) {
			// 전용상품 조회
			couponParam.setPrivateTypes(ItemUtils.getPrivateTypes());
		}
	...
	public List<User> getCouponTargetUserListByCoupon(Coupon coupon) {
		coupon.encrypt(couponEncryptor);
		List<User> users = couponMapper.getCouponTargetUserListByCoupon(coupon);
		coupon.decrypt(couponEncryptor);

		users.forEach(u -> {
			u.decrypt(userEncryptor, ShopUtils.needMasking());
		});
	...
	public List<CouponUser> getCouponUserListByParamForManager(CouponParam couponParam) {
		...
		List<CouponUser> list = couponMapper.getCouponUserListByParamForManager(couponParam);

		// 복호화
		if (!list.isEmpty()) {
			list.forEach(cu -> {
				cu.decrypt(couponUserEncryptor, ShopUtils.needMasking());
			});
	...
	public void updateCouponPublish(Coupon coupon) {
		...
		if ("3".equals(coupon.getCouponTargetTimeType())) {
			...
			if (umsService.isValidUms(ums)) {
				...
				couponUser.stream()
						.filter(list -> !ObjectUtils.isEmpty(list.getPhoneNumber()))
						.forEach(user -> {
							...
							unifiedMessagingService.sendMessage(new BirthdayCoupon(ums, user, user.getUserId(), user.getPhoneNumber(), config, applicationInfo, cryptor, dataMasking));
						});
			}
		}
	}
...
//		userCouponParam.setCouponUserId(sequenceService.getId("OP_COUPON_USER"));
	...
	@Override
	public List<Item> insertItemExcelData(MultipartFile multipartFile) {
...
//			throw new UserException(MessageUtils.getMessage("M01534") + "(" + e.getMessage() + ")"); // 엑셀 파일 로드 시 오류가 발생하였습니다.
			log.error("ERROR: {}", getClass().getName() + " :: insertItemExcelData IOException ============");
...
//			log.error("ERROR: {}", e.getMessage(), e);
//			throw new UserException(MessageUtils.getMessage("M01534") + "(" + e.getMessage() + ")"); // 엑셀 파일 로드 시 오류가 발생하였습니다.
			log.error("ERROR: {}", getClass().getName() + " :: insertItemExcelData Exception ============");
//			throw new UserException(MessageUtils.getMessage("M01534")); // 엑셀 파일 로드 시 오류가 발생하였습니다.
			throw new UserException("엑셀 파일 로드 시 오류가 발생하였습니다."); // 엑셀 파일 로드 시 오류가 발생하였습니다.
		}
    ...
    private List<Item> processCouponItemExcelSheet(XSSFSheet sheet) {

        if (sheet == null) {
            return new ArrayList<>();
        }

        List<Item> couponItems = new ArrayList<>();
        Coupon coupon = new Coupon();
	...
	public String getOfflineCode(String offlineCode) {
    	...
    	if (ObjectUtils.isEmpty(offlineCode)) {
    		return "";
		}
