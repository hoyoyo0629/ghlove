...
public class AuthServiceImpl extends EgovAbstractServiceImpl implements AuthService {
	...
	public String getSmsAuthNumber(String loginId, String phoneNumber, boolean isAdmin) {
		...
		if (isAdmin) {
			List<UserRole> userRoles = userMapper.getManagerRoleListByLoginId(loginId);

			boolean hasManagerRole = false;
			for (UserRole userRole : userRoles) {
				if ("ROLE_OPMANAGER".equals(userRole.getAuthority())
						|| "ROLE_SUPERVISOR".equals(userRole.getAuthority())) {
			...
			String deLoginStr = "";
			try {
				deLoginStr = pCrypto.Decrypt("normal", loginId, "", 0);
			} catch (UnsupportedEncodingException e) {
				log.error("■■■ERROR■■■ getSmsAuthNumber Exception {}",e.getStackTrace()[0]);
			}
...
//			log.error("getEmailAuthNumber - 메일 템틀릿 파일 오픈 오류 : {}", e.getMessage(), e);
			log.error("getEmailAuthNumber - 메일 템틀릿 파일 오픈 오류 : {}", getClass().getName() + " :: getEmailAuthNumber IOException ===========");
