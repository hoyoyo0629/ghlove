...
//			   int MAX_AGE = 86400; // 1일 (60*60*24);
			   ...
			   if (r != null) {
				   if(r.matches("[0-9]+"))
				   {
					   ...
					   if(request != null && (sessionID != null || sessionID.matches("[A-Z=0-9a-z]+")))
					   {
...
//				log.warn("통계 저장 중 오류 발생: {}", e.getMessage(), e);
				log.warn("통계 저장 중 오류 발생: {}", getClass().getName() + " :: saveVisitData RuntimeException ============");
	...
	public void saveVisitDataForApi(Visit visit) {
		...
		try {
			...
			visit.setVisitId(statsMapper.selectOpVisitIdNextValue());
			visit.setVisitDate(DateUtils.getToday(Const.DATE_FORMAT));
			visit.setVisitTime(DateUtils.getToday("HHmmss"));
			visit.setLanguage(CommonUtils.getLanguage());
			visit.setRemoteAddr(saleson.common.utils.CommonUtils.getClientIp());
...
//			log.warn("통계 저장 중 오류 발생: {}", e.getMessage(), e);
			log.warn("통계 저장 중 오류 발생: {}", getClass().getName() + " :: saveVisitDataForApi RuntimeException ===========");
			throw new SystemException("통계 저장 중 오류 발생");
		}
