...
public class Shipping {
	...
	public List<Shipping> getShippingGroups(List<BuyItem> list, String islandType) {
		...
		for(BuyItem buyItem : list) {
			...
			boolean isSingleShipping = false;
			if ("1".equals(item.getShippingType()) || "6".equals(item.getShippingType())) {
			...
			if (isSingleShipping == false) {
				if (!groups.isEmpty()) {
					for (Shipping shipping : groups) {
						// [2020-10-22 KSH] Oracle 이슈때문에 null 방지 로직 추가 (CommonUtils.dataNvl)
						// 무료배송 타입 선택시 SHIPPING_GROUP_CODE가 MySQL은 ''로 저장, Oracle은 null로 저장되어 체크 과정에서 null exception을 일으킴
						// null을 회피할 경우 itemGroups 분기처리 되므로 MySQL과 동일하게 진행 (둘다 '' or null 일때 통과)
						if (CommonUtils.dataNvl(shipping.getShippingGroupCode()).equals(CommonUtils.dataNvl(item.getShippingGroupCode()))) {
							isNew = false;
							break;
						}
			...
			if (isNew) {
				...
				shipping.setShippingPaymentType(buyItem.getShippingPaymentType());
		...
		for (Shipping shipping : groups) {
			...
			if ("1".equals(shipping.getShippingType())) { // 무료 배송
                ...
                if (shipping.getBuyItems() != null) {
                    ...
                    if (shipping.getShippingFreeAmount() != 0 || itemPrice != null) {
