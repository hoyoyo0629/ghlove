...
public class Buy {
	...
	// 세트 재고 차감 목록
	private HashMap<String, Integer> stockSetMap;
	...
	public String getUserData() throws Exception {
		...
		if (receivers == null || this.receivers.isEmpty()) {
			return "";
		}
		...
		for(Receiver receiver : this.receivers) {
			...
			receiverMap.put("zipcode", receiver.getReceiveZipcode());
				...
				if (shipping != null) {
					...
					if (shipping.isSingleShipping()) {
						...
						if (buyItem != null) {
							...
							item.put("optionText", ShopUtils.viewItemOptions(buyItem.getSetItemFlag(), buyItem.getOptions()));
							item.put("shipmentGroupCode", shipping.getShipmentGroupCode());

							if (buyItem.getItem() != null) {
		...
		buy.put("totalSetDiscountAmount", this.orderPrice.getTotalSetDiscountAmount());
		...
		if (items != null) {
			for(BuyItem buyItem : items) {
				if(buyItem.getItemPrice() != null) {
					count += CommonUtils.intNvl(buyItem.getItemPrice().getQuantity());
				}
			}
		}
	...
	public HashMap<String, Integer> getStockSetMap() {
		return stockSetMap;
	}

	public void setStockSetMap(HashMap<String, Integer> stockSetMap) {
		this.stockSetMap = stockSetMap;
	}
