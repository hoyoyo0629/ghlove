...
public class ItemPrice implements Cloneable {
	...
	public ItemPrice(BuyItem buyItem) {
        ...
        // 2020.08.26 juneu.son 세트상품 옵션가 계산
        if (buyItem.getItemSets() != null && !buyItem.getItemSets().isEmpty()) {
            for (BuyItem setItem : buyItem.getItemSets()) {
                if (setItem.getOptionList() != null) {
                    for (ItemOption option : setItem.getOptionList()) {
                        this.optionPrice += option.getExtraPrice();
                    }
                }
            }
        }
		...
		if ("Y".equals(this.spotSaleFlag)) {
			if ("2".equals(item.getSpotType())) {
				...
				if (totalSellerSpotDiscount > 0) {
					if (com.onlinepowers.framework.util.StringUtils.isNotEmpty(sbSeller.toString())) {
						sbSeller.append(" / ");
					}
		...
		// 회원 등급할인 금액
		if (item.getUserLevelDiscountAmount() != 0 || itemPrice.getQuantity() != 0) {
			this.userLevelDiscountAmount = CommonUtils.intNvl(item.getUserLevelDiscountAmount())  * CommonUtils.intNvl(itemPrice.getQuantity());
		}

		// 상품할인 (즉시할인 + 스팟할인)
		this.itemDiscountAmount = (sellerDiscountAmount + adminDiscountAmount) * CommonUtils.intNvl(itemPrice.getQuantity());

		// 세트할인
		this.setDiscountPrice = item.getSetDiscountPrice();
		if (item.getSetDiscountPrice() != 0 || itemPrice.getQuantity() != 0) {
			this.setDiscountAmount = CommonUtils.intNvl(item.getSetDiscountPrice())  * CommonUtils.intNvl(itemPrice.getQuantity());
		}
		this.setDiscountType = item.getSetDiscountType();

		// 할인 총액
		this.discountAmount = this.itemDiscountAmount + this.setDiscountAmount + this.userLevelDiscountAmount + getCouponDiscountAmount();
		if (itemPrice.getCostPrice() != 0 || itemPrice.getQuantity() != 0) {
			this.costPrice = CommonUtils.intNvl(itemPrice.getCostPrice());
			this.quantity = CommonUtils.intNvl(itemPrice.getQuantity());
		}
		...
		this.commissionType = item.getCommissionType();
	...
	public void setSetDiscountAmount(int setDiscountPrice) {
