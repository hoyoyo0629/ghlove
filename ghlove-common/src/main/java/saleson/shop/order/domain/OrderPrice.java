...
public class OrderPrice {
	...
	private int totalSetDiscountAmount; 		// 세트 할인 금액
	...
	public void setItems(List<Receiver> receivers, int bankPayAmount, Config shopConfig) {
		...
		this.totalSetDiscountAmount = 0; 	//  세트 할인 금액
			...
			for (Shipping shipping : receiver.getItemGroups()) {
				...
				if (shipping.isSingleShipping()) {
					...
					if (ValidationUtils.isNull(itemPrice) == false) {
						...
						this.totalSetDiscountAmount += itemPrice.getSetDiscountAmount();
...
//		}
	...
	public int getTotalSetDiscountAmount() {
		return totalSetDiscountAmount;
	}

	public void setTotalSetDiscountAmount(int totalSetDiscountAmount) {
		this.totalSetDiscountAmount = totalSetDiscountAmount;
	}
}
