...
public class ShippingDetailExcelView extends AbstractSXSSFExcelView {
	...
	public void buildExcelDocument(Map<String, Object> model,
	    ...
	    // 데이터 생성(시트생성)
	    buildItemSheet(workbook, (List<OrderList>)model.get("ShippingOrderDetailList"));
	}
	
	private void buildItemSheet(SXSSFWorkbook workbook, List<OrderList> list) {
		...
		for(OrderList orderItem : list){
			...
			setText(sheet, 			rowIndex, cellIndex, ShopUtils.viewItemOptions(orderItem.getSetItemFlag(), orderItem.getOptions()).replaceAll("<br/>", "\n")); // 상품옵션
			...
			setText(sheet, 			rowIndex, cellIndex, ObjectUtils.isEmpty(orderItem.getDeliveryCompanyName()) ? "" : String.valueOf(orderItem.getDeliveryCompanyName())); // 배송업체
			setText(sheet, 			rowIndex, cellIndex, ObjectUtils.isEmpty(orderItem.getDeliveryNumber()) ? "" : String.valueOf(orderItem.getDeliveryNumber())); // 운송장번호
