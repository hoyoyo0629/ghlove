			...
			for(OrderShippingInfo orderShippingInfo : order.getOrderShippingInfos()){
				...
				for(OrderItem orderItem :  orderShippingInfo.getOrderItems()){
					...
					cellIndex = new CellIndex(-1);
					 ...
					 setText(sheet, 			rowIndex, cellIndex, orderShippingInfo.getReceiveMobile()); // 수령인 전화번호
					 setText(sheet, 			rowIndex, cellIndex, orderShippingInfo.getReceiveZipcode()); // 수령인 우편번호
					 ...
					 setText(sheet, 			rowIndex, cellIndex, StringUtils.numberFormat(String.valueOf(orderItem.getRealShipping()))); // 개당 배송비
					 setText(sheet, 			rowIndex, cellIndex, "  "); // 배송업체(입력받을 값)
					 setText(sheet, 			rowIndex, cellIndex, "  "); // 운송장번호(입력받을 값)
					 setText(sheet, 			rowIndex, cellIndex, StringUtils.numberFormat(TotalAmount)); // 총 주문 금액
