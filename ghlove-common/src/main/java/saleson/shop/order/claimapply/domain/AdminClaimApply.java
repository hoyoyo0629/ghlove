...
public class AdminClaimApply {
	...
	public void changeAdminClaimApplyKey(OrderCancelApply apply){
		String[] adminClaimApplyKeys = CommonUtils.dataAryNvl(getAdminClaimApplyKey());
		List<String> adminClaimApplyKeyList = new ArrayList<>();
		for(String key : adminClaimApplyKeys){
			String[] temp = StringUtils.delimitedListToStringArray(key, "-");
			String newTemp = temp[0] + "-" + temp[1] + "-" + apply.getItemSequence();
			adminClaimApplyKeyList.add(newTemp);
		}
		String[] newAdminClaimApplyKeys = adminClaimApplyKeyList.toArray(new String[adminClaimApplyKeyList.size()]);
		setAdminClaimApplyKey(newAdminClaimApplyKeys);
	}
}
