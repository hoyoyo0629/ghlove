...
import saleson.shop.order.OrderMessageService;
...
@Service("kcpService")
public class KcpServiceImpl extends EgovAbstractServiceImpl implements PgService{
...
//			log.error("ERROR: {}", e.getMessage(), e);
			log.error("ERROR: {}", getClass().getName() + " :: init RuntimeException ===========", e);
...
//								log.error("ERROR: {}", e.getMessage(), e);
								log.error("ERROR: {}", getClass().getName() + " :: pay UnsupportedEncodingException1 ===========", e);
							}
...
//								log.error("ERROR: {}", e.getMessage(), e);
								log.error("ERROR: {}", getClass().getName() + " :: pay UnsupportedEncodingException2 ===========", e);
							}
	    ...
	    if (req_tx.equals("pay")) {
	        if (res_cd.equals("0000")) {
	        	if ( bSucc.equals("false")) {
    				...
    				orderPgData.setPartCancelFlag(ObjectUtils.isEmpty(partcanc_yn) ? "N" : partcanc_yn);
	...
	public String confirmationOfPayment(PgData pgData) {
	    ...
	    if ( tx_cd.equals("TX00") )
	    {
	    	OrderPayment orderPayment = pgData.getOrderPayment();
...
//							log.error("BufferedWriter Close Error : {}", e.getMessage(), e);
							log.error("ERROR: {}", getClass().getName() + " :: confirmationOfPayment IOException1 ===========", e);
						}
					}
					if (fw != null) {
						try {
...
//							log.error("FileWriter Close Error : {}", e.getMessage(), e);
							log.error("ERROR: {}", getClass().getName() + " :: confirmationOfPayment IOException2 ===========", e);
						}
					}
...
//			log.error("ERROR: {}", e.getMessage(), e);
			log.error("ERROR: {}", getClass().getName() + " :: getServerIp UnknownHostException ============", e);
		}
		if (serverIp == null) {
			return null;
		}
