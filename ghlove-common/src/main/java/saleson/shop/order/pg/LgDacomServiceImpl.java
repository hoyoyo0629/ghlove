...
public class LgDacomServiceImpl extends EgovAbstractServiceImpl implements PgService {
	...
	public HashMap<String, Object> init(Object data, HttpSession session) {
		 ...
		 */
		String configPath = environment.getProperty("pg.lgdacom.home"); //토스페이먼츠에서 제공한 환경파일("/conf/lgdacom.conf,/conf/mall.conf") 위치 지정.
		 ...
		 */
		String CST_PLATFORM			= environment.getProperty("pg.lgdacom.serviceType");			//토스페이먼츠 결제서비스 선택(test:테스트, service:서비스)
		String CST_MID				= environment.getProperty("pg.lgdacom.mid");					//토스페이먼츠로 부터 발급받으신 상점아이디를 입력하세요.
		...
		String LGD_TIMESTAMP		= DateUtils.getToday(Const.DATETIME_FORMAT);					//타임스탬프
		...
		try {
			...
			throw new OrderException("토스페이먼츠 제공 API를 사용할 수 없습니다. 환경파일 설정을 확인해 주시기 바랍니다. ");
		}
		...
		payReqMap.put("CST_PLATFORM"				, CST_PLATFORM);				// 토스페이먼츠 결제서비스 선택(test:테스트, service:서비스)
		payReqMap.put("CST_MID"						, CST_MID);						// 토스페이먼츠로 부터 발급받으신 상점아이디를 입력하세요.
		payReqMap.put("CST_WINDOW_TYPE"				, CST_WINDOW_TYPE);				// 모바일 결제창 호출 방식
		payReqMap.put("LGD_WINDOW_TYPE"				, CST_WINDOW_TYPE);				// 결제창 호출 방식 (수정불가)
		...
		payReqMap.put("LGD_CUSTOM_SWITCHINGTYPE"	, "SUBMIT");					// 신용카드 카드사 인증 페이지 연동 방식 (수정불가)
		payReqMap.put("LGD_VERSION"					, "JSP_Non-ActiveX_Standard");	// 사용 모듈 정보
		payReqMap.put("LGD_DOMAIN_URL"				, "xpayvvip");					// 모듈 연결 URL 정보

		if ("MOBILE".equals(((PgData) data).getDeviceType())) {
			payReqMap.put("CST_WINDOW_TYPE"			, "submit");					// 수정불가
			payReqMap.put("LGD_WINDOW_TYPE"			, "submit");					// 수정불가
		...
		payReqMap.put("LGD_OSTYPE_CHECK"			, LGD_OSTYPE_CHECK);			// XPay 실행(결제 모듈): PC용과 모바일용
		...
		// 가상계좌 입금 만료일 설정
		if ("vbank".equals(((PgData) data).getApprovalType())) {
			payReqMap.put("LGD_CLOSEDATE", getBankCloseDate());
		}
		...
		*/
		...
		if ( xpay.TX() ) {
			...
			if( "0000".equals( xpay.m_szResCode ) ) {
				...
				// 신용카드 결제의 경우 부분취소 가능여부를 조회 (카드에 따라 부분취소가 불가한 경우가 있음)
				if ("card".equals(((PgData) data).getApprovalType()) || "SC0010".equals(((PgData) data).getApprovalType())) {
					orderPgData.setPartCancelFlag("1".equals(xpay.Response("LGD_PCANCELFLAG", 0)) ? "Y" : "N");

				// 실시간 계좌이체 부분취소 가능여부 조회
				} else if ("realtimebank".equals(((PgData) data).getApprovalType()) || "SC0030".equals(((PgData) data).getApprovalType())) {
					orderPgData.setPartCancelFlag(environment.getProperty("pg.lgdacom.realtimePartcancelFlag"));

				// 가상계좌일 경우 입금 계좌 정보 조회
				} else if ("vbank".equals(((PgData) data).getApprovalType()) || "SC0040".equals(((PgData) data).getApprovalType())) {
					orderPgData.setPartCancelFlag("Y");

					orderPgData.setBankCode(xpay.Response("LGD_FINANCECODE", 0));
					orderPgData.setBankName(xpay.Response("LGD_FINANCENAME", 0));
					orderPgData.setBankVirtualNo(xpay.Response("LGD_ACCOUNTNUM", 0));
					orderPgData.setBankInName(xpay.Response("LGD_PAYER", 0));
					orderPgData.setBankDate(getBankCloseDate());

				// 휴대폰 결제 부분취소 가능여부 조회
				} else if ("hp".equals(((PgData) data).getApprovalType()) || "SC0060".equals(((PgData) data).getApprovalType())) {
					orderPgData.setPartCancelFlag(environment.getProperty("pg.lgdacom.hpPartcancelFlag"));
				}

				orderPgData.setPartCancelDetail("");

				// 에스크로 적용
				if ("Y".equals(xpay.Response("LGD_ESCROWYN", 0))) {
					orderPgData.setEscrowStatus("Y");
				}
	...
	public boolean cancel(OrderPgData orderPgData) {
		 ...
		 */
	    String CST_PLATFORM         = environment.getProperty("pg.lgdacom.serviceType");		//토스페이먼츠 결제서비스 선택(test:테스트, service:서비스)
	    String CST_MID              = environment.getProperty("pg.lgdacom.mid");				//토스페이먼츠으로 부터 발급받으신 상점아이디를 입력하세요.
	    String LGD_MID              = ("test".equals(CST_PLATFORM.trim())?"t":"")+CST_MID;		//테스트 아이디는 't'를 제외하고 입력하세요.
		//상점아이디(자동생성)
		String LGD_TID = orderPgData.getPgKey();												//토스페이먼츠으로 부터 내려받은 거래번호(LGD_TID)
		 ...
		 * 예) [Window 계열] C:\inetpub\wwwroot\lgdacom ==> 절대불가(웹 디렉토리)
		 */
	    String configPath 			= environment.getProperty("pg.lgdacom.home");  										//토스페이먼츠에서 제공한 환경파일("/conf/lgdacom.conf") 위치 지정.
		...
		// 가상계좌일 경우
		String txname = "Cancel";
		if ("SC0040".equals(orderPgData.getPgPaymentType())) {

			// 입금 후 취소일 경우 환불은행 정보 입력
			if (orderPgData.getReturnBankName() != null) {
				setCancelVbankParam(xpay, orderPgData);
			} else {
				// 환불은행 정보가 없으면 입금 대기중으로 판단. 입금 대기 상태에서는 정산 요청 API 호출
				txname = "Settlement";
			}
		}

		xpay.Set("LGD_TXNAME", txname);
...
//			log.error("confirmationOfPayment() :  {}", e.getMessage(), e);
			log.error("confirmationOfPayment() :  {}", getClass().getName() + " :: confirmationOfPayment NoSuchAlgorithmException ===========", e);
		}
		return resultMSG;
	}
	...
	public OrderPgData partCancel(OrderPgData orderPgData) {
		...
		xpay.Set("LGD_REMAINAMOUNT", Integer.toString(orderPgData.getRemainAmount() + orderPgData.getCancelAmount())); //취소전 남은금액
		...
		// 가상계좌 취소일 경우 환불계좌 정보 입력
		if ("SC0040".equals(orderPgData.getPgPaymentType()) && orderPgData.getReturnBankName() != null) {
			setCancelVbankParam(xpay, orderPgData);
		}
...
//        String LGD_TID				= cashReceipt.getPgTid(); //토스페이먼츠 거래번호 (취소시만 사용)
//
//        /* ※ 중요
//         * 환경설정 파일의 경우 반드시 외부에서 접근이 가능한 경로에 두시면 안됩니다.
//         * 해당 환경파일이 외부에 노출이 되는 경우 해킹의 위험이 존재하므로 반드시 외부에서 접근이 불가능한 경로에 두시기 바랍니다.
...
//        return cashReceipt;
	...
	private String getBankCloseDate() {
		Config shopConfig = ShopUtils.getConfig();
		...
		return DateUtils.addDay(DateUtils.getToday(Const.DATE_FORMAT), bankDepositDueDay);
	}

	private void setCancelVbankParam(XPayClient xpay, OrderPgData orderPgData) {
		xpay.Set("LGD_RFACCOUNTNUM", orderPgData.getReturnAccountNo().replace("-", "")); //환불계좌 번호(가상계좌 환불인경우만 필수)
		xpay.Set("LGD_RFBANKCODE", orderPgData.getReturnBankName());						//환불계좌 은행코드(가상계좌 환불인경우만 필수)
		xpay.Set("LGD_RFCUSTOMERNAME", orderPgData.getReturnName());						//환불계좌 예금주(가상계좌 환불인경우만 필수)
