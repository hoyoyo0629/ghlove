...
public class InicisServiceImpl extends EgovAbstractServiceImpl implements PgService {
	...
	public HashMap<String, Object> init(Object data, HttpSession session) {
		...
		if (!"webStandard".equals(environment.getProperty("pg.inipay.web.type"))) {
			...
			// 신용카드 할부 설정
			if ("card".equals(((PgData) data).getApprovalType()) && !ObjectUtils.isEmpty(((PgData) data).getInstalment())) {
				if ("MOBILE".equals(((PgData) data).getDeviceType())) {
					map.put("P_QUOTABASE", ((PgData) data).getInstalment());
				} else {
					map.put("quotabase", ((PgData) data).getInstalment());
				}
			}
			...
			try {
				...
				log.error(" SignatureUtil.hash(...) :  {}", getClass().getName() + " :: init RuntimeException ==========");
				throw new OrderException();
			} catch (Exception e) {
//				log.error(" SignatureUtil.hash(...) :  {}", e.getMessage());
				log.error(" SignatureUtil.hash(...) :  {}", getClass().getName() + " :: init Exception ==========");
				throw new OrderException();
			}
				...
				if ("realtimebank".equals(((PgData) data).getApprovalType()) && ((PgData) data).isMobilePage()) {
					...
					if (((PgData) data).isMobilePage()) {
						...
						try {
							...
							log.error("InicisServiceImpl : {}", getClass().getName() + " :: pay RuntimeException ============");
						} catch (Exception e) {
//							log.error("InicisServiceImpl : {}", e.getMessage());
							log.error("InicisServiceImpl : {}", getClass().getName() + " :: pay Exception ============");
						}
...
//									log.error("ERROR: {}", ex.getMessage(), ex);
									log.error("ERROR: {}", getClass().getName() + " :: pay IOException ============", ex);
									throw new NotificationException("검색 결과 파싱 오류", ex);

								} catch (RuntimeException ex) {
//									log.error("ERROR: {}", ex.getMessage(), ex);
									log.error("ERROR: {}", getClass().getName() + " :: pay RuntimeException ============", ex);
									httpPost.abort();
...
//						log.error("InicisServiceImpl : {}", e.getMessage());
						log.error("ERROR: {}", getClass().getName() + " :: pay UnsupportedEncodingException ============", e);
						throw new NotificationException("결제 요청 실패", e);

					} catch (Exception e) {
//						log.error("InicisServiceImpl : {}", e.getMessage());
						log.error("ERROR: {}", getClass().getName() + " :: pay Exception ============", e);
						throw new NotificationException("결제 요청 실패", e);
					}
...
//			log.error(" inipay.startAction(); :  {}", e.getMessage());
			log.error(" inipay.startAction(); :  {}", getClass().getName() + " :: cancel RuntimeException ===========");
			return false;
		}
...
//				log.error("InicisServiceImpl : {}", e.getMessage());
				log.error("InicisServiceImpl : {}", getClass().getName() + " :: makeWebPgLog RuntimeException ===========");
			}
		}
...
//				log.error("InicisServiceImpl : {}", e.getMessage());
				log.error("InicisServiceImpl : {}", getClass().getName() + " :: makeMobilePgLog RuntimeException ===========");
			}
		}
...
//			log.error(" inipay.startAction(); :  {}", e.getMessage());
			log.error(" inipay.startAction(); :  {}", getClass().getName() + " :: partCancel RuntimeException ===========");
			orderPgData.setSuccess(false);
			return orderPgData;
		}
	...
	public CashbillResponse cashReceiptIssued(CashbillParam param) {
	  ...
	  //***********************************************************************************************************
		...
		inipay.SetField("buyertel", "0".equals(useopt) ? param.getCashbillCode().replaceAll("-", "") : param.getCashbillCode());     // 구매자 전화번호
