		...
		try {
			...
			if (httpConn != null) {
				...
				try (
					...
					pout = new PrintStream (os,false,"euc-kr");
					os.close();
				} catch (IOException e) {
...
//			log.error(ERROR_MESSAGE, e.getMessage(), e);
			log.error(ERROR_MESSAGE, getClass().getName() + " :: send_url IOException1 ===========", e);
...
//				log.error(ERROR_MESSAGE, e.getMessage(), e);
				log.error(ERROR_MESSAGE, getClass().getName() + " :: send_url RuntimeException1 ===========", e);
			} pout     = null;
			try{if (brd      != null) brd      .close(); }
			catch(IOException e){
//				log.error(ERROR_MESSAGE, e.getMessage(), e);
				log.error(ERROR_MESSAGE, getClass().getName() + " :: send_url IOException2 ===========", e);
			}
			brd      = null;
			httpConn = null;
        }
	...
	public static void write_log(Object pstr)
	{
		...
		try{
			if(openFile.exists()){
				try (FileOutputStream fos = new FileOutputStream(openFile, true);) {
					pout = new PrintStream(fos, true);
				} catch (IOException e) {
					throw new IOException(e);
				}
			}else{
				try (FileOutputStream fos = new FileOutputStream(openFile);) {
					pout = new PrintStream(fos, true);
				} catch (IOException e) {
					throw new IOException(e);
				}
		...
		{
//			log.error(ERROR_MESSAGE, e.getMessage(), e);
			log.error(ERROR_MESSAGE, "KSPayWebHostBean :: write_log IOException ==============", e);
		}finally
		{
			try{
...
//				log.error(ERROR_MESSAGE, e.getMessage(), e);
				log.error(ERROR_MESSAGE, "KSPayWebHostBean :: write_log RuntimeException ==============", e);
			}
			pout     = null;
		}
	}
...
//			log.error(ERROR_MESSAGE, tw.getMessage(), tw);
			log.error(ERROR_MESSAGE, "KSPayWebHostBean :: write_log Error ==============", tw);
			pout.println();
		}else
		{
			sb.append(pstr);
