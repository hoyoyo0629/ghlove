...
public class NicepayServiceImpl extends EgovAbstractServiceImpl implements PgService {
...
//            throw new NotificationException(MessageUtils.getMessage("실패했습니다."), e);			// 실패했습니다.
            throw new NotificationException("실패했습니다.", e);			// 실패했습니다.
        }
        ...
        try {
            ...
            int amount = Integer.parseInt(((PgData) data).getAmount());
            ...
            payReqMap.put("SelectQuota", amount >= 50000 ? ((PgData) data).getInstalment() : null);
            payReqMap.put("SelectCardCode", ((PgData) data).getCardCode());
...
//            throw new NotificationException(ne.getMessage(), ne);
            throw new NotificationException("(Init) PG 정보가 존재하지 않습니다.", ne);
    ...
    public OrderPgData pay(Object data, HttpSession session) {
        ...
        String ccPartCl             = "";  String clickpayCl           = "";  String bankCode             = "";
        String bankName             = "";  String rcptType             = "";  String rcptAuthCode         = "";
        String rcptTID              = "";  String carrier              = "";  String dstAddr              = "";
        String vbankBankCode        = "";  String vbankBankName        = "";  String vbankNum             = "";
        String vbankExpDate         = "";  String vbankInputName       = "";
        ...
        if ("0000".equals(authResultCode)) {
            try {
                ...
                clickpayCl           = responseDTO.getParameter("ClickpayCl");       // 간편결제 종류
                ...
                if ("CARD".equals(payMethod) && "3001".equals(resultCode)) {
                    if (!ObjectUtils.isEmpty(clickpayCl)) {
                        orderPgData.setCardEasyType(getEasyPayType(clickpayCl));
                    }
                ...
                if ("BANK".equals(payMethod) && "4000".equals(resultCode)) {
                    ...
                    orderPgData.setCashReceiptIssued(!ObjectUtils.isEmpty(rcptTID));
                    orderPgData.setSuccess(true);
                }
    ...
    public String confirmationOfPayment(PgData pgData) {
        ...
        if ("VBANK".equals(payMethod) && "4110".equals(resultCode)) {
            ...
            if ("0".equals(orderPayment.getOrderStatus())) {
                ...
                // 현금영수증 발행 :: PG 모듈에서 현금영수증 발행신청하지 않은 경우만 실행 (미발행이면 rcptTID = "" 으로 들어옴)
                if (ObjectUtils.isEmpty(rcptTID)) {
                    ...
                    for (CashbillIssue cashbillIssue : cashbillIssues) {
                        ...
                        if (response == null) {
                        	throw new OrderException("결제 진행 중 통신에 실패했습니다.");
                        }
				...
				try (FileWriter fw = new FileWriter(logPath + fileName, true)) {
					...
					try (BufferedWriter bf = new BufferedWriter(fw)){
						...
						log.error("[FILE] ERROR >> 로그파일 저장 오류 :  {}");
						return "FAIL";
					}
    ...
    public boolean vbankCancel(OrderPgData orderPgData, boolean partCancelFlag) throws IOException {
        ...
        String cancelReason = ObjectUtils.isEmpty(orderPgData.getCancelReason())? "주문 취소(사유 미입력)" : orderPgData.getCancelReason();
    ...
    public String getEasyPayType(String easyPayType) {
        if ("6".equals(easyPayType)) {
            return "skpay";
        } else if ("8".equals(easyPayType)) {
            return "samsungpay";
        } else if ("15".equals(easyPayType)) {
            return "payco";
        } else if ("16".equals(easyPayType)) {
            return "kakaopay";
        } else if ("20".equals(easyPayType)) {
            return "naverpay";
        } else if ("21".equals(easyPayType)) {
            return "samsungpay";
        }
    ...
    private String makePgLog(WebMessageDTO responseDTO, String approvalType, String authResultCode, String authResultMsg) {
        ...
        if (approvalType.equals("VBANK")) {
            ...
            keys = new String[]{
                    "TID", "AuthResultCode", "AuthResultMsg", "ResultCode", "ResultMsg", "PayMethod", "Moid",
                    "Amt", "AuthDate", "AuthCode", "CardName", "CardQuota", "ClickpayCl"
            };
        ...
        for(String key : keys) {
            try {
                ...
                log.error("NicepayServiceImpl Error: {}");
            }
        }
    ...
    public String encrypt(String strData) {
        ...
        try {
            ...
            throw new NotificationException("암호화 에러", e);
        }
