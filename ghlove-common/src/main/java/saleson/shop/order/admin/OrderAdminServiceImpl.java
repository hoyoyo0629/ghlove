...
import saleson.shop.order.infra.BuyAdminReceiverEncryptor;
...
public class OrderAdminServiceImpl extends EgovAbstractServiceImpl implements OrderAdminService {
	...
	@Autowired
	private BuyAdminReceiverEncryptor buyAdminReceiverEncryptor;
			...
			for(OrderAdminDetail detail : list) {
				...
				if ("v01".equals(detail.getTemplateVersion())) {
					...
					try {
						...
						if (ObjectUtils.isEmpty(template.getLoginId()) == false) {
							String deLoginStr = "";
							try {
								deLoginStr = pCrypto.Encrypt("normal", template.getLoginId(), "");
						...
						log.error("ERROR: {}","getOrderAdminListByParam OpRuntimeException" , e);
		            	detail.setError(true);
		            	//detail.setErrorMessage(e.getErrorMessage());
		            	detail.setErrorMessage("실패했습니다.");
		            }
				} else {
					detail.setError(true);
			...
			for (Row row : sheet) {
		    	...
		    	if ("v01".equals(templateVersion)) {
		    		...
		    		try {
						...
						log.error("OrderAdminServiceImpl.insertExcelData getItemByItemUserCode ERROR: {}");
		    			HashMap<String, String> cellReference = cellReferences.get(12);
    					cellReference.put("rowIndex", Integer.toString(rowIndex));
    					executionLog.append(PoiUtils.log(cellReference, "상품코드가 존재하지 않거나 판매중인 상품이 아닙니다. (" + template.getItemUserCode() + ")"));
			...
			if (!insertRows.isEmpty()) {
				...
				for (Row row : insertRows) {
			    	...
			    	try {
						...
						log.error("OrderAdminServiceImpl.insertExcelData insertOrderAdminDetail ERROR: {}");
			    		executionLog.append("관리자 대량주문 업로드 상품 정보 등록 중 오류가 발생했습니다.");
			    		rowErrorCount++;
			    	}
				}
			}
...
//			throw new UserException(MessageUtils.getMessage("M01534") + "(" + e.getMessage() + ")"); // 엑셀 파일 로드 시 오류가 발생하였습니다.
			log.error("ERROR: {}", getClass().getName() + " :: insertExcelData IOException =========");
...
//			log.error("ERROR: {}", e.getMessage(), e);
//			throw new UserException(MessageUtils.getMessage("M01534") + "(" + e.getMessage() + ")"); // 엑셀 파일 로드 시 오류가 발생하였습니다.
			log.error("ERROR: {}", getClass().getName() + " :: insertExcelData RuntimeException =========");
//			throw new UserException(MessageUtils.getMessage("M01534")); // 엑셀 파일 로드 시 오류가 발생하였습니다.
			throw new UserException("엑셀 파일 로드 시 오류가 발생하였습니다."); // 엑셀 파일 로드 시 오류가 발생하였습니다.
		}
	...
	public void insertOrderAdmin(OrderAdminParam orderAdminParam, HttpServletRequest request) {
		...
		for(BuyAdmin buy : orders) {
			...
			for(BuyAdminReceiver receiver : buy.getReceivers()) {
				...
				// 배송지 정보 등록
				receiver.encrypt(buyAdminReceiverEncryptor);
			...
			for(String key : newKeys) {
				...
				if ("Y".equals(item.getItemOptionFlag())) {
					...
					for(String optionId : data.getOptionIds()) {
						...
						if (ObjectUtils.isEmpty(optionId)) {
							throw new OrderAdminException("옵션을 선택해 주세요.");
						}
