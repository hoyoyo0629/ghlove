...
public class CategoriesEditServiceImpl implements CategoriesEditService {
	...
	public void updateCategoryEditFile(CategoriesEdit categoriesEdit) {
		try {
			...
			fileStorage.delete( categoriesEdit.getUploadPath(), categoriesEdit.getEditImage());
			fileStorage.delete( categoriesEdit.getThumbUploadPath(), categoriesEdit.getEditImage());
...
//			log.warn(" updateCategoryEditFile(CategoriesEdit categoriesEdit) : {}", e.getMessage(), e);
			log.warn(" updateCategoryEditFile(CategoriesEdit categoriesEdit) : {}", getClass().getName() + " :: updateCategoryEditFile RuntimeException ==========");
		}
		...
		if (categoriesEdit.getEditImages() != null) {
				...
				//if (categoriesEdit.getEditImages().get(i) != null) {
					...
					if(!multipartFile.isEmpty()){
						...
						String uploadPathDefault = categoriesEdit.getUploadPath();
						...
						try {
							...
							//if(categoriesEdit.getCode().equals("featured")) {
								// thumbnail 생성
								//ThumbUtils.create(multipartFile, saveFile, 840, 150);
							//} else {
								fileStorage.upload(multipartFile.getBytes(), saveFile);
								//ThumbUtils.create(multipartFile, saveFile, 670, 320);
							//}

							String uploadPathThumb = categoriesEdit.getThumbUploadPath();
					        ...
					        //ThumbUtils.create(multipartFile, thumbnailFile, 167, 80);

						 	/*} else {

								ThumbUtils.create(multipartFile, saveFile, 840, 320);
								// thumbnail 생성
					            File thumbnailFile = new File(uploadPath + File.separator + "thumb" + File.separator + newFileName);
					            ThumbUtils.create(multipartFile, thumbnailFile, 210, 80);
		...
		CategoriesEdit secordCategoriesEdit = categoriesEditMapper.getCountCategoryEdit(categoriesEdit.getCategoryEditId());
			...
			if (secordCategoriesEdit.getEditImage() != null) {
				...
				if (secordCategoriesEdit.getEditImage().indexOf("/") > -1) {
					fileStorage.delete((FileUtils.getWebRootPath() + ShopUtils.unescapeHtml(secordCategoriesEdit.getEditImage())).replaceAll("/",  File.separator));
				} else {
	...
	public String createLoginBannerImage(CategoriesEdit categoriesEdit, MultipartFile multipartFile, String bannerType, int index) {
		...
		String defaultFileName = DateUtils.getToday(Const.DATETIME_FORMAT) + "." + FileUtils.getExtension(multipartFile.getOriginalFilename());
			...
			// 1. 업로드 경로설정
			String uploadPath = categoriesEdit.getLoginBannerPath(bannerType);
