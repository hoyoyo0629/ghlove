...
public class MoblieCategoriesEditServiceImpl implements MobileCategoriesEditService {
	...
	public void updateCategoryEditFile(CategoriesEdit categoriesEdit) {
		try {
			...
			fileStorage.delete(categoriesEdit.getMobileUploadPath(), ShopUtils.unescapeHtml(categoriesEdit.getEditImage()));
			//fileStorage.delete( "/"+ "mobile-category-edit" + "/" + categoriesEdit.getCode()+File.separator + "thumb" , categoriesEdit.getEditImage());

			mobileCategoriesMainMapper.updateCategoryEditFile(categoriesEdit);

		} catch (RuntimeException e) {
//			log.error("ERROR: {}", e.getMessage(), e);
			log.error("ERROR: {}", getClass().getName() + " :: updateCategoryEditFile RuntimeException ===========");
		}
		...
		if (categoriesEdit.getEditImages() != null) {
				...
				//if (categoriesEdit.getEditImages().get(i) != null) {
					...
					if(!multipartFile.isEmpty()){
						// 1. 업로드 경로설정
						String uploadPath = SalesonProperty.getUploadSaveFolder() + File.separator + "mobile-category-edit" + File.separator + categoriesEdit.getCode() + File.separator + categoriesEdit.getEditPosition();
						...
						try {
							...
							//if( categoriesEdit.getEditKind().equals("1") ){
								// 원본 이미지
							//ThumbUtils.create(multipartFile, saveFile, 320, 188);
