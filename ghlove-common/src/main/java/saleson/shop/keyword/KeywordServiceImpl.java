...
public class KeywordServiceImpl extends EgovAbstractServiceImpl implements KeywordService {
	...
	@Override
	public List<Keyword> getBestKeyword(int limit) {
		return keywordMapper.getBestKeyword(limit);
	}
...
//				log.warn(e1.getMessage());
				log.warn(getClass().getName() + " :: setKeywordDaily JsonProcessingException =============");
			}
			
			try (PrintWriter writer = new PrintWriter(environment.getProperty("upload.root")+"/auto-complete.json","utf-8");){
				// utf-8로 upload.root에 json파일 생성
			    writer.println(jsonStr);
			    writer.close();
			} catch (IOException e) {
//				log.error("[Error] file 생성 오류 -  {}", e.getMessage());
				log.warn(getClass().getName() + " :: setKeywordDaily IOException =============");
			}
		}
