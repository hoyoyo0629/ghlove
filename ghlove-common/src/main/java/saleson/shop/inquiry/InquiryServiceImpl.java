...
//    @Qualifier("freemelSmsService")
    ...
    public void insertInquiry(Inquiry inquiry) {
        ...
        if (inquiry.getInquiryImg() != null) {
            String uploadPath = inquiry.getUploadPath();
            fileService.makeUploadPath(uploadPath);

            // 2. 파일명
            String defaultFileName = fileStorage.getNewFileName(inquiry.getInquiryImg().getOriginalFilename());    //새로운 이미지명 지정.
            ...
            try {
                fileStorage.upload(inquiry.getInquiryImg().getBytes(), saveFile);
