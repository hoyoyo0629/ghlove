...
public class UmsSendLogDto extends Param {
    ...
    public Predicate getPredicate(SendSmsLogParam param) {
        ...
        // 작성일 검색조건(끝)
        if (!ObjectUtils.isEmpty(param.getSearchEndDate())) {
    ...
    public Predicate getPredicateForInit(SendSmsLogParam param) {
        ...
        QUmsSendLog umsSendLog = QUmsSendLog.umsSendLog;

        if (!ObjectUtils.isEmpty(param.getUserId())) {
            builder.andAnyOf(
                    umsSendLog.createdBy.eq(param.getUserId()),
                    umsSendLog.targetUserId.eq(param.getUserId())
            );
        }
