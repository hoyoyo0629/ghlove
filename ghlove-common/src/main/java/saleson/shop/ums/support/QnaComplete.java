...
public class QnaComplete extends UmsTemplate {
    ...
    public QnaComplete(Cryptor cryptor) {
        super(cryptor);
        intiCodeMapView();
    }

    public QnaComplete(Ums ums, String phoneNumber, Qna qna, ApplicationInfo applicationInfo, Cryptor cryptor, DataMasking dataMasking) {
    ...
    private void intiCodeMap() {
        ...
        map.put("user_name", getDataMasking().mask(getCryptor().decrypt(qna.getUserName()), Masking.NAME));
        map.put("email", getDataMasking().mask(getCryptor().decrypt(qna.getEmail()), Masking.EMAIL));
