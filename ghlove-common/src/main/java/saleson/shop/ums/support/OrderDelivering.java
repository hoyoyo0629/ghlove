...
public class OrderDelivering extends UmsTemplate {
    ...
    public OrderDelivering(Cryptor cryptor) {
        super(cryptor);
        intiCodeMapView();
    }

    public OrderDelivering(Order order, long userId, Config config, Ums ums, String phoneNumber, ApplicationInfo applicationInfo, Cryptor cryptor, DataMasking dataMasking) {
        super(ums, phoneNumber, cryptor, dataMasking);

        this.init(order, userId, config, applicationInfo, cryptor);
    }

    private void init(Order order, long userId, Config config, ApplicationInfo applicationInfo, Cryptor cryptor) {
        if (order== null) {
            return;
        }
    ...
    private void intiCodeMap() {
        ...
        map.put("order_name", getDataMasking().mask(getCryptor().decrypt(this.orderName), Masking.NAME));
