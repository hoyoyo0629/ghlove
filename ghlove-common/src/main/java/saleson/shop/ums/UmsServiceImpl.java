...
public class UmsServiceImpl extends EgovAbstractServiceImpl implements UmsService {
    ...
    public HashMap<String, String> getUmsChangeCodes(String templateCode) {
        ...
        if ("order_deposit_wait".equals(templateCode)) {
            codeMap = new OrderBank(cryptor).getCodeViewMap();
        } else if ("order_cready_payment".equals(templateCode)) {
            codeMap = new OrderNew(cryptor).getCodeViewMap();
        } else if ("order_delivering".equals(templateCode)) {
            codeMap = new OrderDelivering(cryptor).getCodeViewMap();
        } else if ("pwsearch".equals(templateCode)) {
            codeMap = new Pwsearch(cryptor).getCodeViewMap();
        } else if ("member_join".equals(templateCode)) {
            codeMap = new MemberJoin(cryptor).getCodeViewMap();
        } else if ("qna_complete".equals(templateCode)) {
            codeMap = new QnaComplete(cryptor).getCodeViewMap();
        } else if ("item_restock".equals(templateCode)) {
            codeMap = new ItemRestock(cryptor).getCodeViewMap();
        } else if ("user_sms".equals(templateCode)) {
            codeMap = new UserSms(cryptor).getCodeViewMap();
        } else if ("confirm_purchase".equals(templateCode)) {
            codeMap = new ConfirmPurchase(cryptor).getCodeViewMap();
        } else if ("confirm_purchase_request".equals(templateCode)) {
            codeMap = new ConfirmPurchaseRequest(cryptor).getCodeViewMap();
        } else if ("expiration_coupon".equals(templateCode)) {
            codeMap = new ExpirationCoupon(cryptor).getCodeViewMap();
        } else if ("birthday_coupon".equals(templateCode)) {
            codeMap = new BirthdayCoupon(cryptor).getCodeViewMap();
        } else if ("order_refund".equals(templateCode)) {
            codeMap = new OrderRefundApproval(cryptor).getCodeViewMap();
        } else if ("expiration_point".equals(templateCode)) {
            codeMap = new ExpirationPoint(cryptor).getCodeViewMap();
        }
