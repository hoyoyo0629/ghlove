    ...
    public void sendRestockNotice(int itemId) {
        ...
        if (umsService.isValidUms(ums)) {
            ...
            restockNotices.stream()
                    .filter(rn -> !ObjectUtils.isEmpty(rn.getPhoneNumber()))
                    .forEach(rn -> {
                        ...
                        unifiedMessagingService.sendMessage(new ItemRestock(ums, rn, rn.getPhoneNumber(), applicationInfo, item, user, cryptor, dataMasking));
                    });
