...
public class CategoriesServiceImpl extends EgovAbstractServiceImpl implements CategoriesService {
...
//	private final Logger log = LoggerFactory.getLogger(CategoriesServiceImpl.class);
	...
	public void updateCategoriesByParam(CategoriesSearchParam categoriesSearchParam) {
		if ("REMOVE".equals(categoriesSearchParam.getConditionType())) {	// REMOVE
			categoriesMapper.removeCategoryGroupIdByParam(categoriesSearchParam);
		} else if (categoriesSearchParam.getCategoryCodes() != null) {		// SAVE
			categoriesMapper.saveCategoryGroupIdByParam(categoriesSearchParam);
		}
		updateCategoryUpdatedDate();
	}

	@Override
	public List<Categories> getChildCategoriesFromParentCategoryClass(
			String categoryClass) {
		return categoriesMapper.getChildCategoriesFromParentCategoryClass(categoryClass);
	}
	...
	public void deleteCategoryByClass(Categories categories) {
		...
		List<Categories> orgChildCategories = categoriesMapper.getChildCategoriesFromParentCategoryClass(categoryCodeBase);
	...
	public void updateCategoryPosition(
		...
		for (Categories category : orgChildCategories) {
			...
			if (!ObjectUtils.isEmpty(newCategory.getCategoryGroupId())) {
...
//			log.error("saveCategoriesFilter error {}", e.getMessage(), e);
			log.error("saveCategoriesFilter error {}", getClass().getName() + " :: saveCategoriesFilter RuntimeException ===========", e);
		}
...
//			log.error("saveReviewFilter error {}", e.getMessage(), e);
			log.error("saveReviewFilter error {}", getClass().getName() + " :: saveReviewFilter RuntimeException ===========", e);
		}
	}
	...
	private int getScaleValue(int value) {
		...
		try {
			...
			log.error(getClass().getName() + " getScaleValue error", ignore);
		}
	...
	private int getMedian(int value1, int value2) {
		...
		try {
			...
			log.error(getClass().getName() + " getMedian error", ignore);
		}
		return 0;
	}

	@Override
	public String getCategoryPathByBreadcrumbs(List<Breadcrumb> breadcrumbs, boolean displayGroupFlag) {
		...
		if (breadcrumbs != null && !breadcrumbs.isEmpty()) {
			...
			if (displayGroupFlag) {
				sb.append(breadcrumb.getGroupName());
				sb.append(seperate);
			}
	...
	private void updateCategoryUpdatedDate() {
		configService.updateCategoryUpdatedDate();
		shopCacheService.sendFrontReload(ReloadType.FRONT_CATEGORIES);
	}

	@Override
	public List<Item> getItemListByCategoryParam(CategoryParam categoryParam) {
		List<Item> searchList = itemMapper.getItemListByCategoryParam(categoryParam);
		return searchList;
	}

	@Override
	public int getItemListCountByCategoryParam(CategoryParam categoryParam) {
		return itemMapper.getItemListCountByCategoryParam(categoryParam);
	}

	@Override
	public Categories getCategoryLevelClassByCategoryParam(CategoryParam categoryParam) {
//		return categoriesMapper.getCategoryLevelClassByCategoryParam(categoryParam);
		Categories categories = categoriesMapper.getCategoryLevelClassByCategoryParam(categoryParam);
		transactionManager.commit(transactionManager.getTransaction(new DefaultTransactionDefinition()));
		return categories;
	}

	@Override
	public List<CategoryPath> getCategoryPath(CategoryParam categoryParam){
		return categoriesMapper.getCategoryPath(categoryParam);
	}
