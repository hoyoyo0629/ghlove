...
//	private int deliveryChargeCount = 0;
	...
	public void buildExcelDocument(Map<String, Object> model,
		...
		cookie.setMaxAge(60*60);				// 쿠키 유지 기간 - 1일 => 1시간으로 변경(관리자 세션 1시간) 시큐어코딩 조치
...
//		buildItemSheet(workbook, list, itemPointConfigList);
		buildItemSheet(workbook, list, null);
...
//			new HeaderCell(1024, 	"상품No.(*)"/*,	"새로운 상품등록시 반드시 하나 이상의 문자를 포함해야합니다. ex)g12345, 12345g"*/),
			new HeaderCell(1024, 	"상품코드"),

			// 기본정보
//			new HeaderCell(8192, 	"상품명(*)"),
			new HeaderCell(8192, 	"상품명"),
//			new HeaderCell(1524, 	"공급사(*)", getSellerList(), 3, sellerList.size() + 2),
//			new HeaderCell(1024, 	"제조사"),
//			new HeaderCell(1024, 	"브랜드", getBrandList(), 3, brandList.size() + 2),
			new HeaderCell(3072, 	"판매자"),
			new HeaderCell(2048, 	"원산지"),
//			new HeaderCell(512, 	"무게"),
			new HeaderCell(512, 	"과세구분"/*, "1:과세, 2:면세"*/),
//			new HeaderCell(1524, 	"사은품 사용유무", "Y:사용, N:사용안함"),
//			new HeaderCell(8192, 	"사은품 정보"),
			new HeaderCell(8192, 	"상품간략설명"),
//			new HeaderCell(5192, 	"검색어"),

			// 판매설정
//			new HeaderCell(512, 	"판매가격(*)", ""),
			new HeaderCell(512, 	"판매가격"),
//			new HeaderCell(1024, 	"공급가 설정(*)", "1:입점업체 수수료로 설정, 2:상품별 수수료로 설정, 3:공급가 설정"),
//			new HeaderCell(512, 	"공급가", "공급가 설정인 경우 적용됩니다."),
//			new HeaderCell(512, 	"수수료율", "입점업체/상품별 수수료로 설정인 경우에 적용됩니다."),
//			new HeaderCell(512, 	"정가", ""),
			/* new HeaderCell(512, 	"원가(*)", 			""),*/
			new HeaderCell(2048, 	"재고연동"/*, "옵션이 등록된 경우 옵션재고 수량으로 판단됩니다., Y:연동함, N:연동안함(무제한)"*/),
			new HeaderCell(512, 	"상품재고"),
//			new HeaderCell(1024, 	"관리코드"),
			new HeaderCell(512, 	"품절여부"/*, "1일시 무조건 품절로 표시됩니다., 0:품절안함, 1:품절"*/),
			new HeaderCell(1024, 	"최소주문수량"),
			new HeaderCell(1024, 	"최대주문수량"),
//			new HeaderCell(1024, 	"쿠폰사용여부", "Y:사용, N:사용안함"),

			// 할인/포인트 설정
			/*new HeaderCell(512, 	"즉시할인", "Y:사용, N:사용안함"),
			new HeaderCell(1024, 	"즉시할인 금액"),
			...
			new HeaderCell(1024, 	"적립구분", "1:%, 2:P"),*/

			// 관리항목
//			new HeaderCell(1024, 	"상품공개유무(*)", "Y:공개, N:비공개"),
			new HeaderCell(1024, 	"상품공개유무"),

			// 배송정보 설정
//			new HeaderCell(1024, 	"배송구분(*)", "1:본사배송, 2:업체배송"),
			new HeaderCell(1024, 	"택배사"/*, getDeliveryCompanyList(), 3, deliveryCompanyList.size() + 2*/),
			new HeaderCell(1024, 	"필수추가정보1"),//20260325 필수 추가정보 isb
			new HeaderCell(1024, 	"필수추가정보2"),//20260325 필수 추가정보 isb
			new HeaderCell(1024, 	"필수추가정보3"),//20260325 필수 추가정보 isb
//			new HeaderCell(1024, 	"출고지 주소(*)", getShipmentList(), 6, shipmentList.size() + 10),
			new HeaderCell(10240, 	"출고지 주소"),
			/*new HeaderCell(2048, 	"배송비 종류(*)", "1:무료배송, 2:판매자조건부, 3:출고지조건부, 4:상품조건부, 5:개당배송비, 6:고정배송비"),
			new HeaderCell(1024, 	"배송비"),
			new HeaderCell(2648, 	"배송비 무료 기준금액"),
			new HeaderCell(2648, 	"배송비 추가 기준수량"),
			new HeaderCell(2648, 	"제주 추가배송비"),
			new HeaderCell(2648, 	"도서산간 추가배송비"),*/
			new HeaderCell(2648, 	"반품신청 가능여부"/*, "Y:가능, N:불가능"*/),
//			new HeaderCell(2648, 	"반품/교환 구분(*)", "1:본사반품, 2:업체반품"),
//			new HeaderCell(2648,	"반품/교환 주소(*)", getShipmentReturnList(), 5, shipmentReturnList.size() + 10),
			new HeaderCell(10240,	"반품/교환 주소"),
//			new HeaderCell(2648,	"반품/교환 배송비(*)"),
//			new HeaderCell(8148,	"상품 상세 설명"),
//			new HeaderCell(8148,	"상품 상세 설명(모바일)"),

//			new HeaderCell(512, 	"조회수(x)"),
//			new HeaderCell(2048, 	"등록일(x)")
			new HeaderCell(2048, 	"등록일"),
			new HeaderCell(2048, 	"수정일")
		};
...
//			setTextLeft(sheet, 		rowIndex, cellIndex, item.getManufacturer());	// 제조사
			setTextLeft(sheet, 		rowIndex, cellIndex, item.getSeller().getCompanyName());
...
//			setTextRight(sheet, 	rowIndex, cellIndex, item.getWeight());			// 무게
			setText(sheet, 			rowIndex, cellIndex, getTaxType(item.getTaxType()));		// 과세구분
...
//			setTextLeft(sheet, 		rowIndex, cellIndex, item.getItemKeyword());	// 검색어
			...
			// 판매설정
			setTextRight(sheet, 	rowIndex, cellIndex, StringUtils.numberFormat(item.getSalePrice()));		// 판매가격
//			setText(sheet, 			rowIndex, cellIndex, item.getCommissionType());	// 공급가 설정
//			setNumberRight(sheet, 	rowIndex, cellIndex, item.getDisplaySupplyPrice());		// 공급가
//			setFloatFormat(sheet, 	rowIndex, cellIndex, item.getDisplayCommissionRate()); 	// 수수료율

//			setTextRight(sheet, 	rowIndex, cellIndex, item.getItemPrice());				// 정가
			/*setNumberRight(sheet, 	rowIndex, cellIndex, item.getCostPrice());	*/		// 원가
			setText(sheet, 	rowIndex, cellIndex, getStockFlag(item.getStockFlag()));					// 재고연동
			setTextRight(sheet, 	rowIndex, cellIndex, item.getStockQuantity() == -1 ? "" : StringUtils.numberFormat(Integer.toString(item.getStockQuantity())));		// 상품재고
//			setTextLeft(sheet, 	rowIndex, cellIndex, item.getStockCode());				// 관리코드
			setText(sheet, 	rowIndex, cellIndex, getSoldOut(item.getSoldOut()));					// 품절여부
			setTextRight(sheet, 	rowIndex, cellIndex, item.getOrderMinQuantity() == -1 ? "" : StringUtils.numberFormat(Integer.toString(item.getOrderMinQuantity())));	// 최소 주문 수량
			setTextRight(sheet, 	rowIndex, cellIndex, item.getOrderMaxQuantity() == -1 ? "" : StringUtils.numberFormat(Integer.toString(item.getOrderMaxQuantity())));	// 최대 주문 수량
...
//			}
			...
			// 관리항목
			setText(sheet, 			rowIndex, cellIndex, getDisplayFlag(item.getDisplayFlag()));			// 공개여부
...
//			setNumber(sheet, 		rowIndex, cellIndex, item.getDeliveryCompanyId());		// 택배사
			setText(sheet, 		rowIndex, cellIndex, item.getDeliveryCompanyName());		// 택배사
			setText(sheet, 		rowIndex, cellIndex, item.getItemTextOptionTitle1());		// 필수추가정보1 20260325 필수 추가정보 isb 수정
			setText(sheet, 		rowIndex, cellIndex, item.getItemTextOptionTitle2());		// 필수추가정보2 20260325 필수 추가정보 isb 수정
			setText(sheet, 		rowIndex, cellIndex, item.getItemTextOptionTitle3());		// 필수추가정보3 20260325 필수 추가정보 isb 수정
//			setNumber(sheet, 		rowIndex, cellIndex, item.getShipmentId());				// 출고지 주소
			setText(sheet, 		rowIndex, cellIndex, item.getShipmentAddress());				// 출고지 주소
...
//			setNumberRight(sheet, 	rowIndex, cellIndex, item.getShippingExtraCharge2());	// 도서산간 추가배송비
			setText(sheet, 			rowIndex, cellIndex, getItemReturnFlag(item.getItemReturnFlag()));			// 반품신청 가능여부
...
//			setNumber(sheet, 		rowIndex, cellIndex, item.getShipmentReturnId());		// 반품/교환 주소
			setText(sheet, 		rowIndex, cellIndex, item.getShipmentReturnAddress());		// 반품/교환 주소
...
//			setNumberRight(sheet, 	rowIndex, cellIndex, item.getHits());			// 조회 수
			setText(sheet, 			rowIndex, cellIndex, DateUtils.datetime(item.getCreatedDate()));	// 등록일
			setText(sheet, 			rowIndex, cellIndex, DateUtils.datetime(item.getUpdatedDate()));	// 수정일
...
//		String title = MessageUtils.getMessage("상품 정보고시 (테이블)");	// 상품 정보고시 (테이블)
		String title = MessageUtils.getMessage("상품 정보고시");	// 상품 정보고시 (테이블)
...
//				setText(sheet, 		rowIndex, cellIndex, itemInfo.getItemNoticeCode());		// 상품의 상품군
				setText(sheet, 		rowIndex, cellIndex, itemInfo.getItemNoticeTitle());		// 상품의 상품군
...
//			new HeaderCell(8192, 	"상품명(x)"),									// 상품명
			new HeaderCell(8192, 	"상품명"),									// 상품명
//			new HeaderCell(2048, 	"상품옵션 형태(*)",	"S:선택형, S2:2조합형, S3:3조합형, T:텍스트형"),		// 상품옵션 형태
			new HeaderCell(2048, 	"상품옵션 형태"),		// 상품옵션 형태
...
//				int itemOptionGroupCount = 0;
				for (ItemOption itemOption : itemOptionGroup.getItemOptions()) {
					...
					setText(sheet, 			rowIndex, cellIndex, item.getItemUserCode());				// 상품코드
					setTextLeft(sheet, 		rowIndex, cellIndex, item.getItemName());					// 상품명
					setText(sheet, 			rowIndex, cellIndex, getOptionType(itemOption.getOptionType()));			// 상품옵션 형태
					...
					setTextRight(sheet, 	rowIndex, cellIndex, StringUtils.numberFormat(itemOption.getOptionPrice()));			// 추가금액
					/*setNumberRight(sheet, 	rowIndex, cellIndex, itemOption.getOptionCostPrice());*/		// 원가
					setText(sheet, 			rowIndex, cellIndex, getStockFlag(ObjectUtils.isEmpty(itemOption.getOptionStockFlag()) ? "N" : itemOption.getOptionStockFlag()));		// 재고연동
					setTextRight(sheet, 	rowIndex, cellIndex, itemOption.getOptionStockQuantity() == -1 ? "" : StringUtils.numberFormat(Integer.toString(itemOption.getOptionStockQuantity())));	// 재고수량
					setText(sheet, 			rowIndex, cellIndex, getSoldOut(ObjectUtils.isEmpty(itemOption.getOptionSoldOutFlag()) ? "N" : itemOption.getOptionSoldOutFlag()));	// 판매상태
					setText(sheet, 			rowIndex, cellIndex, getDisplayFlag(itemOption.getOptionDisplayFlag()));	// 노출여부
...
//					itemCount++;
	...
	/**
	 * 2-3. 상품 이미지 Sheet (OP_ITEM_IMAGE)
	 * @param workbook
	 * @param list
	 */
...
//			int itemCount = 0;
			...
			for (ItemImage itemImage : item.getItemImages()) {
				...
				*/
				...
				setText(sheet, 		rowIndex, cellIndex, StringUtils.numberFormat(itemImage.getOrdering()));		// 이미지노출 순서
...
//				itemCount++;
	...
	/**
	 * 2-5. 상품 관련상품 Sheet (OP_ITEM_RELATION)
	 * @param workbook
	 * @param list
	 */
...
//	}
	...
	/**
	 * 2-4. 상품 카테고리 Sheet (OP_ITEM_CATEGORY)
	 * @param workbook
	 * @param list
	 */
...
//			new HeaderCell(2048, 	"카테고리 코드(*)", "ex), /categories/index/14100000000000 => 14100000000000, /categories/index/36300000000000 => 36300000000000"),			// 카테고리코드
			new HeaderCell(2048, 	"카테고리 코드"),			// 카테고리코드
//			new HeaderCell(4096, 	"카테고리 이름(x)"),				// 카테고리명
			new HeaderCell(4096, 	"카테고리 이름"),				// 카테고리명
//			new HeaderCell(1024, 	"출력순서(*)", "숫자입력")			// 출력순서
			new HeaderCell(1024, 	"순번")			// 출력순서
		};
...
//			oldItemUserCode = itemCategory.getItemUserCode();
	...
	/*
	/**
	 * 2-6. 상품 포인터 설정 Sheet (OP_POINT_CONFIG)
	 * @param workbook
...
//	}
	...
	private String getTaxType(String taxType) {
		switch (taxType) {
		case "1":
			return "과세";
		case "2":
	...
	private String getStockFlag(String stockFlag) {
		switch (stockFlag) {
		case "Y":
		case "y":
			return "연동";
	...
	private String getDisplayFlag(String displayFlag) {
		switch (displayFlag) {
		case "Y":
		case "y":
			return "공개";
	...
	private String getItemReturnFlag(String itemReturnFlag) {
		switch (itemReturnFlag) {
		case "Y":
		case "y":
			return "가능";
		case "N":
	...
	private String getSoldOut(String soldOut) {
		switch (soldOut) {
		case "0":
		case "N":
		case "n":
	...
	private String getOptionType(String optionType) {
		switch (optionType) {
		case "S":
		case "s":
			return "선택형";
