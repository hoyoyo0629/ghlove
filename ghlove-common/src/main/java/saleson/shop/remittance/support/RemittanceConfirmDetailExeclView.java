	...
	private void buildItemSheet(SXSSFWorkbook workbook, List<RemittanceConfirmDetail> list) {
		...
		for (RemittanceConfirmDetail remittanceConfirmDetail : list) {
			...
			if ("3".equals(remittanceConfirmDetail.getCommissionType())) {
				sortation = "공급가";
			} else if ("1".equals(remittanceConfirmDetail.getCommissionType()) || "2".equals(remittanceConfirmDetail.getCommissionType())) {
				sortation = "수수료";
			}
			...
			setText(sheet, 					rowIndex, cellIndex, ShopUtils.viewItemOptions(remittanceConfirmDetail.getSetItemFlag(), remittanceConfirmDetail.getOptions()).replaceAll("<br/>", "\n")); // 옵션
			setTextRight(sheet, 			rowIndex, cellIndex, StringUtils.numberFormat(String.valueOf(remittanceConfirmDetail.getCommissionBasePrice() * remittanceConfirmDetail.getQuantity()))); // 판매가
			setTextRight(sheet, 			rowIndex, cellIndex, StringUtils.numberFormat(String.valueOf(remittanceConfirmDetail.getCommissionPrice() * remittanceConfirmDetail.getQuantity()))); // 수수료
			...
			setTextRight(sheet, 			rowIndex, cellIndex, StringUtils.numberFormat(String.valueOf(remittanceConfirmDetail.getSupplyPrice() * remittanceConfirmDetail.getQuantity()))); // 공급가
			setTextRight(sheet, 			rowIndex, cellIndex, StringUtils.numberFormat(String.valueOf((remittanceConfirmDetail.getSellerDiscountPrice() + remittanceConfirmDetail.getSetDiscountPrice()) * remittanceConfirmDetail.getQuantity()))); // 판매자할인
			setTextRight(sheet, 			rowIndex, cellIndex, StringUtils.numberFormat(String.valueOf(remittanceConfirmDetail.getSellerPoint() * remittanceConfirmDetail.getQuantity()))); // 판매자포인트
			setTextRight(sheet, 			rowIndex, cellIndex, StringUtils.numberFormat(String.valueOf(remittanceConfirmDetail.getQuantity()))); // 주문수량
			setTextRight(sheet, 			rowIndex, cellIndex, StringUtils.numberFormat(String.valueOf(remittanceConfirmDetail.getRemittancePrice() * remittanceConfirmDetail.getQuantity()))); // 정산금액
