	...
	private void buildItemSheet(SXSSFWorkbook workbook, List<RemittanceDetail> list) {
		...
		for(RemittanceDetail remittanceDetail : list){
			...
			if ("3".equals(remittanceDetail.getCommissionType())) {
				sortation = "공급가";
			} else if ("1".equals(remittanceDetail.getCommissionType()) || "2".equals(remittanceDetail.getCommissionType())) {
				sortation = "수수료";
			}
			...
			setText(sheet, 					rowIndex, cellIndex, ShopUtils.viewItemOptions(remittanceDetail.getSetItemFlag(), remittanceDetail.getOptions()).replaceAll("<br/>", "\n")); // 옵션
			setTextRight(sheet, 			rowIndex, cellIndex, StringUtils.numberFormat(String.valueOf(remittanceDetail.getCommissionBasePrice() * remittanceDetail.getQuantity()))); // 판매가
			setTextRight(sheet, 			rowIndex, cellIndex, StringUtils.numberFormat(String.valueOf(remittanceDetail.getCommissionPrice() * remittanceDetail.getQuantity()))); // 수수료
			setText(sheet, 					rowIndex, cellIndex, remittanceDetail.getCommissionRate() + "%"); // 수수료율
			setText(sheet, 			        rowIndex, cellIndex, sortation); // 구분
			setTextRight(sheet, 			rowIndex, cellIndex, StringUtils.numberFormat(String.valueOf(remittanceDetail.getSupplyPrice() * remittanceDetail.getQuantity()))); // 공급가
			setTextRight(sheet, 			rowIndex, cellIndex, StringUtils.numberFormat(String.valueOf((remittanceDetail.getSellerDiscountPrice() + remittanceDetail.getSetDiscountPrice()) * remittanceDetail.getQuantity()))); // 판매자할인
			setTextRight(sheet, 			rowIndex, cellIndex, StringUtils.numberFormat(String.valueOf(remittanceDetail.getSellerPoint() * remittanceDetail.getQuantity()))); // 판매자포인트
			setTextRight(sheet, 			rowIndex, cellIndex, StringUtils.numberFormat(String.valueOf(remittanceDetail.getQuantity()))); // 주문수량
			setTextRight(sheet, 			rowIndex, cellIndex, StringUtils.numberFormat(String.valueOf(remittanceDetail.getRemittancePrice() * remittanceDetail.getQuantity()))); // 정산금액
