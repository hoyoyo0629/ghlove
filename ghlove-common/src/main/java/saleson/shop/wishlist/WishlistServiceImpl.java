...
public class WishlistServiceImpl extends EgovAbstractServiceImpl implements WishlistService {
	...
	public int getWishlistCountByParam(WishlistParam wishlistParam) {
		if (!UserUtils.isManagerLogin()) {
			// 전용상품 조회
			wishlistParam.setPrivateTypes(ItemUtils.getPrivateTypes());
		}
	...
	@Override
	public boolean includedByItem(int itemId, long userId) {

		if (itemId > 0 && userId > 0) {
			Wishlist wishlist = new Wishlist();
			wishlist.setUserId(userId);
			wishlist.setItemId(itemId);
			return wishlistMapper.getWishlistDuplicateCount(wishlist) > 0;
		}
	...
	@Override
	public void setWishlistFlagByItem(List<Item> list) {
		if (UserUtils.isUserLogin()) {

			List<Wishlist> wishlists = getWishlistListByUserId(UserUtils.getUserId());

			transactionManager.commit(transactionManager.getTransaction(new DefaultTransactionDefinition()));
			
			if (!ObjectUtils.isEmpty(list) && !ObjectUtils.isEmpty(wishlists)) {
				for (Wishlist wishlist : wishlists) {
					list.stream()
							.filter(item -> item.getItemId() == wishlist.getItemId())
							.forEach(item -> item.setWishlistFlag(true));
				}
			}
		}
	}
	...
	@Override
	public void setWishlistFlagByFeaturedItem(List<FeaturedItem> list) {
		if (UserUtils.isUserLogin()) {
	...
	@Override
	public void setWishlistFlagByDisplayItem(List<DisplayItem> list) {
		if (UserUtils.isUserLogin()) {
	...
	public int removeWishlist(Wishlist wishlist) {
		...
		return wishlistMapper.removeWishlist(wishlist);
	}
}
