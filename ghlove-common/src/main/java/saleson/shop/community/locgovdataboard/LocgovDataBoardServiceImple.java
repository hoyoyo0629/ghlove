...
import saleson.shop.databoard.DataboardMapper;
import saleson.shop.databoard.DataboardServiceImpl;
...
@Service("locgovDataBoardService")
public class LocgovDataBoardServiceImple implements LocgovDataBoardService {

	private static final Logger log = LoggerFactory.getLogger(LocgovDataBoardServiceImple.class);

	@Autowired
	private LocgovDataBoardMapper locgovDataBoardMapper;
	...
	public int getDataboardCount(DataboardParam databoardParam) {
		...
		return locgovDataBoardMapper.getDataboardCount(databoardParam);
	}

	@Override
	public List<Databoard> getDataboardList(DataboardParam databoardParam) {

		return locgovDataBoardMapper.getDataboardList(databoardParam);
	}

	@Override
	public Databoard getDataboard(int dataId) {
		Databoard databoard = locgovDataBoardMapper.getDataboard(dataId);
		databoard.setDataboardFiles(locgovDataBoardMapper.getFrontDataboardFileList(dataId));
	...
	public void insertDataboard(Databoard item) {
		...
		int dataId = sequenceService.getId("OP_COMMUNITY_DATABOARD");
		item.setDataId(dataId);

		locgovDataBoardMapper.insertDataboard(item);
	...
	//						message = "업로드 가능한 최대 용량 : 20MB 입니다";
	...
	//					message = "유효하지 않은 파일입니다.";
		//				return ViewUtils.getView("/store-inquiry/inquiry", message);
	                ...
	                itemImage.setDataFileId(String.valueOf(dataId));
	                ...
	                locgovDataBoardMapper.insertDataboardFile(itemImage);
				}
			}
		}
	...
	public void updateDataboard(Databoard item) {
		...
		locgovDataBoardMapper.updateDataboard(item);
	...
	//					message = "유효하지 않은 파일입니다.";
		//				return ViewUtils.getView("/store-inquiry/inquiry", message);
					...
					DataboardFile itemImage = new DataboardFile();
	                itemImage.setDataFileId(String.valueOf(sequenceService.getId("OP_COMMUNITY_DATABOARD_FILE")));
	                itemImage.setDataId(item.getDataId());
	                itemImage.setFileName(saveFileName.toString());
	                itemImage.setFileTy(extension);
	                itemImage.setOrgFileName(multipartFile.getOriginalFilename());
	...
	public void deleteDataboard(int dataId) {
		locgovDataBoardMapper.deleteDataboard(dataId);

	}

	@Override
	public List<Databoard> getFrontDataboardList(DataboardParam databoardParam) {
		return locgovDataBoardMapper.getFrontDataboardList(databoardParam);
	}

	@Override
	public int getFrontDataboardListCount(DataboardParam databoardParam) {
		return locgovDataBoardMapper.getFrontDataboardListCount(databoardParam);
	}

	@Override
	public void addHitCount(int dataId) {
		locgovDataBoardMapper.addHitCount(dataId);

	}

	@Override
	public List<Databoard> getFrontDataboardListByTeamCodes(DataboardParam databoardParam) {
		return locgovDataBoardMapper.getFrontDataboardListByTeamCodes(databoardParam);
	}
	...
	public String databoardListDelete(DataboardParam databoardParam) throws UserException {
		...
		if(databoardParam.getDataboardList() != null && databoardParam.getDataboardList().size() > 0) {
			...
			for (String databoardListId : databoardParam.getDataboardList()) {
				...
				nResult += locgovDataBoardMapper.databoardListDelete(dataId);
	...
	public Databoard getFrontDataboardDetail(Integer dataId) {
		locgovDataBoardMapper.addHitCount(dataId);
		return locgovDataBoardMapper.getDataboard(dataId);
	}

	@Override
	public List<DataboardFile> getFrontDataboardFileList(Integer dataId) {
		return locgovDataBoardMapper.getFrontDataboardFileList(dataId);
	}

	@Override
	public DataboardFile getFrontDataboardFileDetail(Integer dataFileId) {
		return locgovDataBoardMapper.getFrontDataboardFileDetail(dataFileId);
	}

	@Override
	public void deleteItemImageByItemId(Integer dataFileId) {
		DataboardFile databoardFile = locgovDataBoardMapper.getFrontDataboardFileDetail(dataFileId);
        ...
        // 데이터 삭제
        locgovDataBoardMapper.deleteDataboardFile(dataFileId);
