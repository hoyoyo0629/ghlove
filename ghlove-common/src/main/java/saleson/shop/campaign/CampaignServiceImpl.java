...
import saleson.shop.campaign.infra.CampaignUserParamEncryptor;
...
public class CampaignServiceImpl extends EgovAbstractServiceImpl implements CampaignService {
    ...
    @Autowired
    private CampaignUserParamEncryptor campaignUserParamEncryptor;
    ...
    public Page<CampaignBatch> getCampaignBatchList(Predicate predicate, Pageable pageable) {
        Page<CampaignBatch> batchList = campaignBatchRepository.findAll(predicate, pageable);
        batchList.forEach(this::decryptBatchList);
        return batchList;
    }
    ...
    public void insertCampaignUser(Long campaignId, CampaignUserParam campaignUserParam, String sendType) {
        ...
        // 암호화
        campaignUserParam.encrypt(campaignUserParamEncryptor);
    ...
    public void insertCampaignBatch() {
        ...
        campaignBatchRepository.updateCampaignBatchForPointByMysql();
    ...
    private void sendCampaignMessage(Campaign campaign, List<CampaignUser> campaignUsers, String callbackNumber) {
        ...
        // 복호화
        campaignUsers.forEach(this::decryptCampaignUsers);
...
//            logger.error("sendCampaignMessage [{}] {}", campaign, e.getMessage(), e);
            logger.error("sendCampaignMessage [{}] {}", campaign, getClass().getName() + " :: sendCampaignMessage RuntimeException =============");
        }
    }
...
//                    logger.error("setEventCodes [{}] {}", campaignRegulars, e.getMessage(), e);
                    logger.error("setEventCodes [{}] {}", campaignRegulars, getClass().getName() + " :: insertCampaignMessageBatch RuntimeException =============");
                }
...
//                logger.error("insertCampaignMessageBatch not CampaignId [{}] {}", campaign.getId(), e.getMessage(), e);
                logger.error("insertCampaignMessageBatch not CampaignId [{}] {}", campaign.getId(), getClass().getName() + " :: insertCampaignMessageBatch RuntimeException =============");
            }
    ...
    @Override
    public void publishCouponByCampaign(Campaign campaign) {
    ...
    @Override
    public List<Map<String, Object>> getUserApplicationInfo(CampaignUserParam campaignUserParam) {
    ...
    private void setEventCodesForRegular(CampaignRegular campaignRegular) {
    ...
    private void decryptCampaignUsers(CampaignUser c) {
        c.setUserName(decryptor.decryptName(c.getUserName(), false));
        c.setPhoneNumber(decryptor.decryptPhone(c.getPhoneNumber(), false));
    }

    private void decryptBatchList(CampaignBatch c) {
