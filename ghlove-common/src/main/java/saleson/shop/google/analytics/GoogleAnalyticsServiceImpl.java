...
public class GoogleAnalyticsServiceImpl extends EgovAbstractServiceImpl implements GoogleAnalyticsService {
...
//            log.error("sendGoogleAnalytics Error {}", e.getMessage(), e);
            log.error("sendGoogleAnalytics Error {}", getClass().getName() + " :: sendGoogleAnalytics RestClientException ============");
        }
    }
    ...
    public void saveConfig(ConfigGoogleAnalytics configGoogleAnalytics) {
        ...
        try {
            ...
            if (!ObjectUtils.isEmpty(authFileString)) {
                storeConfigGoogleAnalytics.setAuthFile(CipherUtils.encrypt(authFileString));
            }
        } catch (RuntimeException ignore) {
          log.error("setAuthFile Error {}", getClass().getName() + " :: saveConfig RuntimeException ============");
        } catch (Exception ignore) {
//            log.error("setAuthFile Error [{}]", ignore.getMessage(), ignore);
            log.error("setAuthFile Error {}", getClass().getName() + " :: saveConfig Exception ============");
        }
    ...
    private String getAuthFileString(MultipartFile authFile) {
        ...
        if (authFile != null) {
            try (
            		InputStream is = authFile.getInputStream();
...
//                log.error("getAuthFileString error [{}]", e.getMessage(), e);
                log.error("getAuthFileString error [{}]", getClass().getName() + " :: getAuthFileString IOException ===========");
            }
        }
    ...
    private CommonTrackingScript getCommonScript(ConfigGoogleAnalytics configGoogleAnalytics, String gtag) {
        ...
        if (configGoogleAnalytics != null) {
            ...
            if (configGoogleAnalytics.isCommonTrackingFlag() && !ObjectUtils.isEmpty(measurementId)) {
    ...
    public void purchase(HttpServletRequest request, String cid, Order order, String page) {
        ...
        if (validEcommerce(config) && !ObjectUtils.isEmpty(cid) && order != null) {
    ...
    public void addToCart(HttpServletRequest request, String cid, List<Product> products) {
        ...
        if (validEcommerce(config) && !ObjectUtils.isEmpty(cid) && products != null && !products.isEmpty()) {
    ...
    public void checkout(HttpServletRequest request, String cid, Buy buy, String page) {
        ...
        if (validEcommerce(config) && !ObjectUtils.isEmpty(cid) && buy != null) {
    ...
    public void allRefund(HttpServletRequest request, String orderCode, int orderSequence) {
        ...
        if (validEcommerce(config) && !ObjectUtils.isEmpty(cid) && !ObjectUtils.isEmpty(orderCode)) {
    ...
    public void partRefund(HttpServletRequest request, Order order, OrderRefund orderRefund) {
        ...
        if (validEcommerce(config) && !ObjectUtils.isEmpty(cid) && order != null && orderRefund != null) {
    ...
    public void adminRefund(HttpServletRequest request, OrderRefund orderRefund, List<OrderItem> orderItems) {
        ...
        if (validEcommerce(config) && !ObjectUtils.isEmpty(cid)
    ...
    private boolean validEcommerce(ConfigGoogleAnalytics config) {
        ...
        if (ObjectUtils.isEmpty(measurementId) || !config.isEcommerceTrackingFlag()) {
            return false;
        }
    ...
    public String getAccessToken(HttpSession session) {
        ...
        if (config != null && config.isStatisticsFlag() && !ObjectUtils.isEmpty(config.getAuthFile())) {
            ...
            try {
                ...
                log.error("getAuthToken error [{}]", getClass().getName() + " :: getAccessToken IOException ===========");
            } catch (Exception ignore) {
//              log.error("getAuthToken error [{}]", ignore.getMessage(), ignore);
              log.error("getAuthToken error [{}]", getClass().getName() + " :: getAccessToken Exception ============");
          }
