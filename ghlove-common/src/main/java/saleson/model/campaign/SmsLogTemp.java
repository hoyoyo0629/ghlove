...
public class SmsLogTemp extends BaseEntity {
    ...
    public SmsLogTemp(CampaignStatistics statistics) {
        if (statistics != null) {
            setUserId(ObjectUtils.isEmpty(statistics.getUserId()) ? null : statistics.getUserId());
            setMsgkey(statistics.getMsgkey());
            setCampaignId(!ObjectUtils.isEmpty(statistics.getKey()) && statistics.getKey() != null ? Long.valueOf(statistics.getKey()) : null);
